...
CollisionMath::OverlapType CollisionMath::Overlap_Test(const AABoxClass &box, const SphereClass &sphere) {
  ...
  //
  if (WWMath::Fabs(dist.X) > extent.X)
    return OUTSIDE;
  if (WWMath::Fabs(dist.Y) > extent.Y)
    return OUTSIDE;
  if (WWMath::Fabs(dist.Z) > extent.Z)
    return OUTSIDE;
...
CollisionMath::OverlapType CollisionMath::Overlap_Test(const AABoxClass &box, const LineSegClass &line) {
  ...
  if (inside_point_count == 2) {
    ...
    if (p0_proj > 0) {
      ...
      if (-p0_proj > box_proj + WWMath::Max(dp_proj, 0.0f))
        return OUTSIDE;
    }
    ...
    if (WWMath::Fabs(p0_proj) > box_proj)
      return OUTSIDE;
 ...
 *   11/19/99   gth : Created.                                                                 *
 *=============================================================================================*/
bool CollisionMath::Collide(const AABoxClass &box, const Vector3 &move, const AABoxClass &box2,
                            CastResultStruct *result) {
  /*
  **	Test the X-axis
...
CollisionMath::OverlapType CollisionMath::Overlap_Test(const AABoxClass &box, const Vector3 &point) {
  if (WWMath::Fabs(point.X - box.Center.X) > box.Extent.X)
    return POS;
  if (WWMath::Fabs(point.Y - box.Center.Y) > box.Extent.Y)
    return POS;
  if (WWMath::Fabs(point.Z - box.Center.Z) > box.Extent.Z)
    return POS;
...
CollisionMath::OverlapType CollisionMath::Overlap_Test(const AABoxClass &box, const AABoxClass &box2) {
  ...
  if ((dc.X + box2.Extent.X <= box.Extent.X) && (dc.Y + box2.Extent.Y <= box.Extent.Y) &&
      (dc.Z + box2.Extent.Z <= box.Extent.Z) && (dc.X - box2.Extent.X >= -box.Extent.X) &&
      (dc.Y - box2.Extent.Y >= -box.Extent.Y) && (dc.Z - box2.Extent.Z >= -box.Extent.Z)) {
    return NEG; // inside;
  }
