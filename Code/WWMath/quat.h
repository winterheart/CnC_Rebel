...
public:
  ...
  WWINLINE explicit Quaternion(float a, float b, float c, float d) {
    X = a;
    Y = b;
    Z = c;
    W = d;
...
WWINLINE Quaternion Inverse(const Quaternion &a) { return Quaternion(-a[0], -a[1], -a[2], a[3]); }

// Conjugate of the quaternion
WWINLINE Quaternion Conjugate(const Quaternion &a) { return Quaternion(-a[0], -a[1], -a[2], a[3]); }
...
// Multiply a quaternion by a scalar
WWINLINE Quaternion operator*(const Quaternion &a, float scl) { return scl * a; }

// Multiply two quaternions
WWINLINE Quaternion operator*(const Quaternion &a, const Quaternion &b) {
  return Quaternion(a.W * b.X + b.W * a.X + (a.Y * b.Z - b.Y * a.Z), a.W * b.Y + b.W * a.Y - (a.X * b.Z - b.X * a.Z),
                    a.W * b.Z + b.W * a.Z + (a.X * b.Y - b.X * a.Y), a.W * b.W - (a.X * b.X + a.Y * b.Y + a.Z * b.Z));
}

// Divide two quaternions
WWINLINE Quaternion operator/(const Quaternion &a, const Quaternion &b) { return a * Inverse(b); }
...
WWINLINE Vector3 Quaternion::Rotate_Vector(const Vector3 &v) const {
  ...
  return Vector3(w * (-X) + W * x + (y * (-Z) - (-Y) * z), w * (-Y) + W * y - (x * (-Z) - (-X) * z),
                 w * (-Z) + W * z + (x * (-Y) - (-X) * y));
}
...
WWINLINE bool Equal_Within_Epsilon(const Quaternion &a, const Quaternion &b, float epsilon) {
  return ((WWMath::Fabs(a.X - b.X) < epsilon) && (WWMath::Fabs(a.Y - b.Y) < epsilon) &&
          (WWMath::Fabs(a.Z - b.Z) < epsilon) && (WWMath::Fabs(a.W - b.W) < epsilon));
}
