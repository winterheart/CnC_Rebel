 ...
 ** AABoxClass::Init -- initialize from a min-max form of a box                               * AABoxClass::Init_Min_Max
 *-- init the box from a min and max vector                        * AABoxClass::Add_Point -- expand the box to contain
 *the given point                        * AABoxClass::Project_To_Axis -- compute projection onto the given axis *
 ...
 *   2/24/98    GTH : Created.                                                                 *
 *=============================================================================================*/
WWINLINE void AABoxClass::Translate(const Vector3 &trans) { Center += trans; }
...
WWINLINE bool AABoxClass::Contains(const MinMaxAABoxClass &other_box) const {
  ...
  if (other_box.MinCorner.X < bmin.X)
    return false;
  if (other_box.MinCorner.Y < bmin.Y)
    return false;
  if (other_box.MinCorner.Z < bmin.Z)
    return false;

  if (other_box.MaxCorner.X > bmax.X)
    return false;
  if (other_box.MaxCorner.Y > bmax.Y)
    return false;
  if (other_box.MaxCorner.Z > bmax.Z)
    return false;
