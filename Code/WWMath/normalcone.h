...
public:
  ...
  NormalCone(const Vector3 &normal, float angle = 1.0f) : Vector3(normal), Angle(angle) {}
  ...
  void Set(const NormalCone &src) { Set(src, src.Angle); }

  // return true if this object has degenerated into a sphere.
  inline bool Complete_Sphere() { return (Angle - WWMATH_EPSILON) <= -1.0f; }
  ...
  // find the two vectors on the edge of the cone residing on the same plane as the input vector and their dot products
  // with the input.
  inline float Get_Coplanar_Normals_And_Dots(const Vector3 &Input, Vector3 &Output1, Vector3 &Output2, float &dot1,
                                             float &dot2) const;
...
inline float NormalCone::Get_Coplanar_Normals_And_Dots(const Vector3 &Input, Vector3 &Output1, Vector3 &Output2,
                                                       float &Dot1, float &Dot2) const {
