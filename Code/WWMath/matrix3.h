...
public:
  ...
  */
  ...
  WWINLINE explicit Matrix3(float m11, float m12, float m13, float m21, float m22, float m23, float m31, float m32,
                            float m33);
  ...
  */
  WWINLINE Matrix3 Transpose() const;
  WWINLINE Matrix3 Inverse() const;
  ...
  */
  ...
  static WWINLINE void Add(const Matrix3 &a, const Matrix3 &b, Matrix3 *c);

  /*
  ** matrix subtraction
  */
  WWINLINE friend Matrix3 operator-(const Matrix3 &a, const Matrix3 &b);
  static WWINLINE void Subtract(const Matrix3 &a, const Matrix3 &b, Matrix3 *c);
...
WWINLINE void Matrix3::Make_Identity() {
  Row[0].Set(1.0f, 0.0f, 0.0f);
  Row[1].Set(0.0f, 1.0f, 0.0f);
  Row[2].Set(0.0f, 0.0f, 1.0f);
}

WWINLINE Matrix3::Matrix3(float m11, float m12, float m13, float m21, float m22, float m23, float m31, float m32,
                          float m33) {
...
WWINLINE void Matrix3::Set(float m11, float m12, float m13, float m21, float m22, float m23, float m31, float m32,
                           float m33) {
  Row[0].Set(m11, m12, m13);
  Row[1].Set(m21, m22, m23);
  Row[2].Set(m31, m32, m33);
...
WWINLINE Matrix3::Matrix3(const Vector3 &axis, float angle) { Set(axis, angle); }

WWINLINE Matrix3::Matrix3(const Vector3 &axis, float s_angle, float c_angle) { Set(axis, s_angle, c_angle); }

WWINLINE void Matrix3::Set(const Vector3 &axis, float angle) { Set(axis, sinf(angle), cosf(angle)); }
...
WWINLINE Matrix3::Matrix3(const Quaternion &q) { this->Set(q); }
...
WWINLINE Matrix3 Matrix3::Transpose() const {
  return Matrix3(Vector3(Row[0][0], Row[1][0], Row[2][0]), Vector3(Row[0][1], Row[1][1], Row[2][1]),
 ...
 *   1/7/20     DRM : Created.                                                                 *
 *=============================================================================================*/
WWINLINE float Matrix3::Determinant() const {
 ...
 *=============================================================================================*/
WWINLINE float Matrix3::Get_X_Rotation() const {
 ...
 *   08/11/1997 GH  : Created.                                                                 *
 *=============================================================================================*/
WWINLINE float Matrix3::Get_Y_Rotation() const {
 ...
 *   08/11/1997 GH  : Created.                                                                 *
 *=============================================================================================*/
WWINLINE float Matrix3::Get_Z_Rotation() const {
  Vector3 v = (*this) * Vector3(1.0, 0.0, 0.0);
  return WWMath::Atan2(v[1], v[0]);
}

WWINLINE Vector3 Matrix3::Get_X_Vector() const { return {Row[0][0], Row[1][0], Row[2][0]}; }

WWINLINE Vector3 Matrix3::Get_Y_Vector() const { return {Row[0][1], Row[1][1], Row[2][1]}; }

WWINLINE Vector3 Matrix3::Get_Z_Vector() const { return {Row[0][2], Row[1][2], Row[2][2]}; }

WWINLINE void Matrix3::Get_X_Vector(Vector3 *set) const { set->Set(Row[0][0], Row[1][0], Row[2][0]); }

WWINLINE void Matrix3::Get_Y_Vector(Vector3 *set) const { set->Set(Row[0][1], Row[1][1], Row[2][1]); }

WWINLINE void Matrix3::Get_Z_Vector(Vector3 *set) const { set->Set(Row[0][2], Row[1][2], Row[2][2]); }

WWINLINE Matrix3 operator-(const Matrix3 &a) { return Matrix3(-a.Row[0], -a.Row[1], -a.Row[2]); }

WWINLINE Matrix3 operator*(const Matrix3 &a, float d) { return Matrix3(a.Row[0] * d, a.Row[1] * d, a.Row[2] * d); }

WWINLINE Matrix3 operator*(float d, const Matrix3 &a) { return a * d; }
...
#define ROWCOL(i, j) a[i][0] * b[0][j] + a[i][1] * b[1][j] + a[i][2] * b[2][j]
  ...
  return Matrix3(Vector3(ROWCOL(0, 0), ROWCOL(0, 1), ROWCOL(0, 2)), Vector3(ROWCOL(1, 0), ROWCOL(1, 1), ROWCOL(1, 2)),
...
WWINLINE int operator!=(const Matrix3 &a, const Matrix3 &b) { return (!(a == b)); }
 ...
 *=============================================================================================*/
WWINLINE void Matrix3::Rotate_X(float theta) { Rotate_X(sinf(theta), cosf(theta)); }
 ...
 *   08/26/1997 GH  : Created.                                                                 *
 *=============================================================================================*/
WWINLINE void Matrix3::Rotate_Y(float theta) { Rotate_Y(sinf(theta), cosf(theta)); }
 ...
 *   08/26/1997 GH  : Created.                                                                 *
 *=============================================================================================*/
WWINLINE void Matrix3::Rotate_Z(float theta) { Rotate_Z(sinf(theta), cosf(theta)); }
...
WWINLINE Matrix3 Create_X_Rotation_Matrix3(float rad) { return Create_X_Rotation_Matrix3(sinf(rad), cosf(rad)); }
...
WWINLINE Matrix3 Create_Y_Rotation_Matrix3(float rad) { return Create_Y_Rotation_Matrix3(sinf(rad), cosf(rad)); }
...
WWINLINE Matrix3 Create_Z_Rotation_Matrix3(float rad) { return Create_Z_Rotation_Matrix3(sinf(rad), cosf(rad)); }
