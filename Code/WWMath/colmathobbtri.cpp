 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obbtri_collision_separation_test(BTCollisionStruct &context, float lp, float leb0, float leb1) {
  /*
 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obbtri_check_collision_cross_axis(BTCollisionStruct &context, float dp, int dpi, float leb0) {
 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obbtri_check_collision_basis_axis(BTCollisionStruct &context, float leb0, float dp1, float dp2) {
  float dist;     // separation along the axis
  float axismove; // size of the move along the axis.
  float leb1;     // final coordinate of the leading edge of the box
  float lp;       // leading edge of the polygon.
...
static inline bool obbtri_check_collision_normal_axis(BTCollisionStruct &context) {
  ...
  leb0 = context.Box.Extent.X * WWMath::Fabs(context.AN[0]) + context.Box.Extent.Y * WWMath::Fabs(context.AN[1]) +
 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline void obbtri_compute_contact_normal(const BTCollisionStruct &context, Vector3 *set_normal) {
 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline void obbtri_compute_contact_point(BTCollisionStruct &context, CastResultStruct *result) {
  int i;
  float x[3];
  ...
  */
  result->ContactPoint.X = context.Box.Center.X + x[0] * context.A[0].X + x[1] * context.A[1].X + x[2] * context.A[2].X;

  result->ContactPoint.Y = context.Box.Center.Y + x[0] * context.A[0].Y + x[1] * context.A[1].Y + x[2] * context.A[2].Y;

  result->ContactPoint.Z = context.Box.Center.Z + x[0] * context.A[0].Z + x[1] * context.A[1].Z + x[2] * context.A[2].Z;
 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
bool CollisionMath::Collide(const OBBoxClass &box, const Vector3 &move, const TriClass &tri, const Vector3 &trimove,
...
struct BTIntersectStruct {
  BTIntersectStruct(const OBBoxClass &box, const TriClass &tri) : Box(box), Tri(tri) {
    Vector3::Subtract(*tri.V[0], box.Center, &D); // vector from center of box to vertex 0
    Vector3::Subtract(*tri.V[1], *tri.V[0], &E[0]);
    Vector3::Subtract(*tri.V[2], *tri.V[0], &E[1]);
    Vector3::Subtract(E[1], E[0], &E[2]);
 ...
 *=============================================================================================*/
static inline bool obbtri_intersection_separation_test(BTIntersectStruct &context, float lp, float leb0) {
  /*
 ...
 *   5/4/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obbtri_check_intersection_cross_axis(BTIntersectStruct &context, float dp, float leb0) {
  float p0; // distance from box center to vertex 0
  float lp; // leading edge of the polygon.
 ...
 *   5/4/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obbtri_check_intersection_basis_axis(BTIntersectStruct &context, float leb0, float dp1, float dp2) {
  float dist; // separation along the axis
  float lp;   // leading edge of the polygon.
 ...
 *   5/4/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obbtri_check_intersection_normal_axis(BTIntersectStruct &context) {
  float dist; // separation along the axis
  float leb0; // initial coordinate of the leading edge of the box
  float lp;   // leading edge of the polygon.
