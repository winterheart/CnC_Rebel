...
public:
  MatPassTaskClass(MaterialPassClass *pass, MeshClass *mesh) : MaterialPass(pass), Mesh(mesh), NextVisible(NULL) {
...
DX8TextureCategoryClass::DX8TextureCategoryClass(DX8FVFCategoryContainer *container_, TextureClass **texs,
                                                 ShaderClass shd, VertexMaterialClass *mat, int pass_)
    : pass(pass_), shader(shd), render_task_head(NULL), material(mat), container(container_) {
  WWASSERT(pass >= 0);
...
void DX8FVFCategoryContainer::Remove_Texture_Category(DX8TextureCategoryClass *tex_category) {
  ...
  for (unsigned pass = 0; pass < passes; pass++) {
    ...
    if (texture_category_list[pass].Peek_Head() != NULL)
      return;
  }
...
void DX8TextureCategoryClass::Log(bool only_visible) {
  ...
  work.Format("	%8s %8s %6s %6s %6s %5s %s\n", "idx_cnt", "poly_cnt", "i_offs", "min_vi", "vi_rng", "ident", "name");
  WWDEBUG_SAY((work));
  ...
  while (!it.Is_Done()) {
    ...
    while (prtc) {
      if (prtc->Peek_Polygon_Renderer() == p_renderer)
        break;
...
DX8FVFCategoryContainer::DX8FVFCategoryContainer(unsigned FVF_, bool sorting_)
    : FVF(FVF_), sorting(sorting_), visible_matpass_head(NULL), visible_matpass_tail(NULL), index_buffer(0),
      used_indices(0), passes(MAX_PASSES), uv_coordinate_channels(0), AnythingToRender(false),
      AnyDelayedPassesToRender(false) {
  if ((FVF & D3DFVF_TEX1) == D3DFVF_TEX1)
    uv_coordinate_channels = 1;
  if ((FVF & D3DFVF_TEX2) == D3DFVF_TEX2)
    uv_coordinate_channels = 2;
  if ((FVF & D3DFVF_TEX3) == D3DFVF_TEX3)
    uv_coordinate_channels = 3;
  if ((FVF & D3DFVF_TEX4) == D3DFVF_TEX4)
    uv_coordinate_channels = 4;
  if ((FVF & D3DFVF_TEX5) == D3DFVF_TEX5)
    uv_coordinate_channels = 5;
  if ((FVF & D3DFVF_TEX6) == D3DFVF_TEX6)
    uv_coordinate_channels = 6;
  if ((FVF & D3DFVF_TEX7) == D3DFVF_TEX7)
    uv_coordinate_channels = 7;
  if ((FVF & D3DFVF_TEX8) == D3DFVF_TEX8)
    uv_coordinate_channels = 8;
}
...
DX8FVFCategoryContainer::Find_Matching_Texture_Category(TextureClass *texture, unsigned pass, unsigned stage,
  ...
  while (!dest_it.Is_Done()) {
    if (dest_it.Peek_Obj()->Peek_Texture(stage) == texture) {
      ...
      for (unsigned int s = 0; s < MeshMatDescClass::MAX_TEX_STAGES; s++) {
        if (stage != s) {
          all_textures_same =
              all_textures_same && (dest_tex_category->Peek_Texture(s) == ref_category->Peek_Texture(s));
        }
      }
      if (all_textures_same && Equal_Material(dest_tex_category->Peek_Material(), ref_category->Peek_Material()) &&
...
DX8TextureCategoryClass *
DX8FVFCategoryContainer::Find_Matching_Texture_Category(VertexMaterialClass *vmat, unsigned pass,
                                                        DX8TextureCategoryClass *ref_category) {
  ...
  while (!dest_it.Is_Done()) {
    if (Equal_Material(dest_it.Peek_Obj()->Peek_Material(), vmat)) {
      ...
      if (all_textures_same && dest_tex_category->Get_Shader() == ref_category->Get_Shader()) {
        return dest_tex_category;
      }
    }
    dest_it.Next();
      ...
      while (!poly_it.Is_Done()) {
        ...
        if (prc == src_tex_category) {
          DX8TextureCategoryClass *dest_tex_category =
              Find_Matching_Texture_Category(new_texture, pass, stage, src_tex_category);
          ...
          if (!dest_tex_category) {
            ...
            DX8TextureCategoryClass *new_tex_category =
                new DX8TextureCategoryClass(this, tmp_textures, src_tex_category->Get_Shader(),
        ...
        if (prc == src_tex_category) {
          ...
          if (!dest_tex_category) {
            ...
            DX8TextureCategoryClass *new_tex_category = new DX8TextureCategoryClass(
                this, tmp_textures, src_tex_category->Get_Shader(), const_cast<VertexMaterialClass *>(new_vmat), pass);
...
DX8RigidFVFCategoryContainer::DX8RigidFVFCategoryContainer(unsigned FVF, bool sorting_)
    : DX8FVFCategoryContainer(FVF, sorting_), vertex_buffer(0), used_vertices(0), delayed_matpass_head(NULL),
      delayed_matpass_tail(NULL) {}

// ----------------------------------------------------------------------------

DX8RigidFVFCategoryContainer::~DX8RigidFVFCategoryContainer() { REF_PTR_RELEASE(vertex_buffer); }
...
bool DX8RigidFVFCategoryContainer::Check_If_Mesh_Fits(MeshModelClass *mmc) {
  if (!vertex_buffer)
    return true; // No VB created - mesh will fit as a new vb will be created when inserting
  ...
  if (required_vertices <= available_vertices && (required_indices) <= available_indices) {
    return true;
  }
  return false;
}
...
public:
  Vertex_Split_Table(MeshClass *mesh_) : mesh(mesh_), mmc(NULL), npatch_enable(false), allocated_polygon_array(false) {
    ...
    if (gap_filler)
      polygon_count += gap_filler->Get_Polygon_Count();
    //		if (mmc->Get_Gap_Filler_Polygon_Count()) {
    allocated_polygon_array = true;
    polygon_array = new TriIndex[polygon_count];
    memcpy(polygon_array, mmc->Get_Polygon_Array(), mmc->Get_Polygon_Count() * sizeof(TriIndex));
    if (gap_filler) {
      memcpy(polygon_array + mmc->Get_Polygon_Count(), gap_filler->Get_Polygon_Array(),
  ...
  const Vector3 *Get_Vertex_Normal_Array() const { return mmc->Get_Vertex_Normal_Array(); }
  ...
  const Vector2 *Get_UV_Array(unsigned uv_array_index) const { return mmc->Get_UV_Array_By_Index(uv_array_index); }
  ...
  unsigned short *Get_Polygon_Array(unsigned pass) { return (unsigned short *)polygon_array; }
};
...
void DX8RigidFVFCategoryContainer::Add_Mesh(MeshClass *mesh_) {
  ...
  */
  if (!vertex_buffer) {
    int vb_size = 4000;
    if (vb_size < needed_vertices)
      vb_size = needed_vertices;
    if (sorting) {
      ...
      vertex_buffer = NEW_REF(DX8VertexBufferClass,
                              (FVF, vb_size,
                               (DX8Wrapper::Get_Current_Caps()->Support_NPatches() && WW3D::Get_NPatches_Level() > 1)
...
void DX8FVFCategoryContainer::Insert_To_Texture_Category(Vertex_Split_Table &split_table, TextureClass **texs,
  ...
  */
  ...
  while (!it.Is_Done()) {
    ...
    if (all_textures_same && Equal_Material(tex_category->Peek_Material(), mat) &&
  ...
  if (!fit_in_existing_category) {
    ...
    */
    bool found_similar_category = false;
    TextureCategoryListIterator tcl_it(&texture_category_list[pass]);
    while (!tcl_it.Is_Done()) {
      // Categorize according to first stage's texture for now
      if (tcl_it.Peek_Obj()->Peek_Texture(0) == texs[0]) {
        texture_category_list[pass].Add_After(new_tex_category, tcl_it.Peek_Obj());
        found_similar_category = true;
        break;
      }
      tcl_it.Next();
  ...
  */
    ...
    if (sorting) {
      ...
      index_buffer =
          NEW_REF(DX8IndexBufferClass,
                  (ib_size, (DX8Wrapper::Get_Current_Caps()->Support_NPatches() && WW3D::Get_NPatches_Level() > 1)
  ...
  for (unsigned pass = 0; pass < split_table.Get_Pass_Count(); ++pass) {
    ...
    for (int i = 0; i < polygon_count; ++i) {
      ...
      if (!textures_material_and_shader_booking.Add_Textures_Material_And_Shader(textures, mat, shader))
        continue;
...
DX8SkinFVFCategoryContainer::DX8SkinFVFCategoryContainer(bool sorting)
    : DX8FVFCategoryContainer(DX8_FVF_XYZNUV1, sorting), VisibleVertexCount(0), VisibleSkinHead(NULL) {}
...
void DX8SkinFVFCategoryContainer::Render(void) {
  ...
  DynamicVBAccessClass vb(sorting ? BUFFER_TYPE_DYNAMIC_SORTING : BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type,
                          VisibleVertexCount);
...
bool DX8SkinFVFCategoryContainer::Check_If_Mesh_Fits(MeshModelClass *mmc) {
  if (!index_buffer)
    return true; // No IB created - mesh will fit as a new ib will be created when inserting
  ...
  if (polygons) {
    ...
    if (stripify) {
      ...
      if (index_count < unsigned(strip[0])) {
        ...
        DX8PolygonRendererClass *p_renderer = new DX8PolygonRendererClass(index_count, split_table.Get_Mesh_Class(),
                                                                          this, vertex_offset, index_offset, true);
    ...
    if (!stripify) {
      DX8PolygonRendererClass *p_renderer = new DX8PolygonRendererClass(index_count, split_table.Get_Mesh_Class(), this,
                                                                        vertex_offset, index_offset, false);
      PolygonRendererList.Add_Tail(p_renderer);
...
DX8MeshRendererClass::DX8MeshRendererClass()
    : camera(NULL), enable_lighting(true), texture_category_container_list_skin(NULL), visible_decal_meshes(NULL) {}
...
void DX8MeshRendererClass::Init(void) { texture_category_container_list_skin = new FVFCategoryList; }
...
void DX8MeshRendererClass::Register_Mesh_Type(MeshClass *mesh) {
  ...
  WWDEBUG_SAY(("Registering mesh: %s (%d polys, %d verts + %d gap polygons)\n", mmc->Get_Name(),
               mmc->Get_Polygon_Count(), mmc->Get_Vertex_Count(), mmc->Get_Gap_Filler_Polygon_Count()));
#endif
...
void DX8MeshRendererClass::Request_Log_Statistics() { statistics_requested = WW3D::Get_Frame_Count(); }
...
void DX8MeshRendererClass::Log_Statistics_String(bool only_visible) {
  if (statistics_requested != WW3D::Get_Frame_Count())
    return;
