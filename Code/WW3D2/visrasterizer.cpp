...
VisRasterizerClass::VisRasterizerClass(void) : ModelTransform(1), Camera(NULL), MVTransform(1) {}

VisRasterizerClass::~VisRasterizerClass(void) { REF_PTR_RELEASE(Camera); }

void VisRasterizerClass::Set_Resolution(int width, int height) { IDBuffer.Set_Resolution(width, height); }
...
bool VisRasterizerClass::Render_Triangles(const Vector3 *verts, int vcount, const TriIndex *tris, int tcount,
...
bool VisRasterizerClass::Render_Triangles_No_Clip(const Vector3 *verts, int vcount, const TriIndex *tris, int tcount) {
...
bool VisRasterizerClass::Render_Triangles_Clip(const Vector3 *verts, int vcount, const TriIndex *tris, int tcount) {
  bool pixel_passed = false;
...
IDBufferClass::IDBufferClass(void)
    : BackfaceID(0), FrontfaceID(0), CurID(0), RenderMode(OCCLUDER_MODE), TwoSidedRenderingEnabled(false),
      PixelCounter(0), ResWidth(0), ResHeight(0), IDBuffer(NULL), ZBuffer(NULL) {}
...
struct GradientsStruct {
  GradientsStruct(const Vector3 *verts) {
    ...
    DOOZ_DX =
        oodx * (((OOZ[1] - OOZ[2]) * (verts[0].Y - verts[2].Y)) - ((OOZ[0] - OOZ[2]) * (verts[1].Y - verts[2].Y)));

    DOOZ_DY =
        oody * (((OOZ[1] - OOZ[2]) * (verts[0].X - verts[2].X)) - ((OOZ[0] - OOZ[2]) * (verts[1].X - verts[2].X)));
  }
