...
public:
  ...
  enum INTERSECTION_TYPE { NONE = 0, GENERIC, POLYGON } IntersectionType;
};
...
public:
  enum {
    MAX_POLY_INTERSECTION_COUNT = 1000, // arbitrary size of stack array used for storing intersection results within
                                        // Intersect_Mesh(). Non-recursive function.
  ...
  Vector3 *IntersectionNormal; // if non-zero then Process_Intersection_Results() will place the (perhaps vertex
                               // interpolated) surface normal here.
  ...
  inline void Set(Vector3 *location, Vector3 *direction, Vector3 *intersection_normal, bool interpolate_normal,
                  float max_distance, bool convex_test = false) {
  ...
  inline IntersectionClass() : ConvexTest(false) {
  ...
  inline IntersectionClass(Vector3 *location, Vector3 *direction, Vector3 *intersection_normal,
                           bool interpolate_normal = false, float max_distance = WWMATH_FLOAT_MAX,
  ...
  inline void Copy_Results(IntersectionResultClass *Source) { Copy_Results(&Result, Source); }

  // this is called only for the nearest intersection. If the request passes a Interpolated_Normal pointer then it will
  // be calculated. otherwise the results are copied into the request structure. This does not copy the matrix or
  // location members; it is intended to be used during poly testing where these values are identical between results,
  ...
  inline IntersectionClass(IntersectionClass *source) { *this = source; }

  inline IntersectionClass *operator=(IntersectionClass *source) {
    Set(source->RayLocation, source->RayDirection, source->IntersectionNormal, source->InterpolateNormal,
        source->MaxDistance, source->ConvexTest);
  ...
  // this will only set the result's range if intersection occurs; it is intended to be used as a first pass
  // intersection test before intersecting the mesh polygons itself. Note: Does NOT do Max_Distance testing
  inline bool Intersect_Sphere_Quick(SphereClass &Sphere, IntersectionResultClass *FinalResult) {
    ...
    FinalResult->Beta = Sphere.Radius * Sphere.Radius -
                        (Vector3::Dot_Product(sphere_vector, sphere_vector) - FinalResult->Alpha * FinalResult->Alpha);
  ...
  inline bool Intersect_Polygon(IntersectionResultClass *Result, Vector3 &PolygonNormal, Vector3 &v1, Vector3 &v2,
                                Vector3 &v3);
  inline bool Intersect_Polygon(IntersectionResultClass *Result, Vector3 &v1, Vector3 &v2, Vector3 &v3);
  inline bool Intersect_Polygon_Z(IntersectionResultClass *Result, Vector3 &PolygonNormal, Vector3 &v1, Vector3 &v2,
                                  Vector3 &v3);
  ...
  */
  static inline int _Intersect_Triangles_Z(Vector3 ClipPoints[3], Vector3 TrianglePoints[3], Vector2 UV[3],
                                           Vector3 ClippedPoints[6], Vector2 ClippedUV[6]);
  ...
  */
  ...
  void Append_Hierarchy_Objects(int MaxCount, int &CurrentCount, RenderObjClass **ObjectArray,
                                RenderObjClass *Heirarchy, bool Test_Bounding_Spheres, bool Convex);
  ...
  bool Intersect_Object_Array(int ObjectCount, RenderObjClass **ObjectArray, IntersectionResultClass *FinalResult,
  ...
  bool Intersect_Screen_Point_RenderObject(float screen_x, float screen_y, const LayerClass &Layer,
                                           RenderObjClass *RObj, IntersectionResultClass *FinalResult);

  bool Intersect_Screen_Point_Layer_Range(float ScreenX, float ScreenY, const LayerClass &TopLayer,
  ...
  bool Intersect_Hierarchy(RenderObjClass *Hierarchy, IntersectionResultClass *FinalResult,
                           bool Test_Bounding_Sphere = true, bool Convex = false);
  ...
  */
  RenderObjClass *Intersect_Sub_Object(float screenx, float screeny, LayerClass &layer, RenderObjClass *robj,
                                       IntersectionResultClass *result);
  ...
  */
  ...
  static inline bool _Point_In_Polygon(IntersectionResultClass *FinalResult, Vector3 &Normal, Vector3 &loc1,
                                       Vector3 &loc2, Vector3 &loc3);
  static inline bool _Point_In_Polygon(IntersectionResultClass *FinalResult, Vector3 &loc1, Vector3 &loc2,
                                       Vector3 &loc3, int axis_1, int axis_2);
  static inline bool _Point_In_Polygon(Vector3 &Point, Vector3 &loc1, Vector3 &loc2, Vector3 &loc3, int axis_1,
                                       int axis_2, float &Alpha, float &Beta);
...
protected:
  ...
  */
  // static inline void _Intersect_Lines_Z(Vector3 &A, Vector3 &B, Vector2 &UVStart, Vector2 &UVEnd, Vector3 &C, Vector3
  // &D, Vector3 ClippedPoints[6], Vector2 ClippedUV[6], int &DestIndex);
  static inline bool In_Front_Of_Line(const Vector3 &p,  // point to test
                                      const Vector3 &e0, // point on edge
                                      const Vector3 &de  // direction of edge
  );

  static inline float Intersect_Lines(const Vector3 &p0, // start of line segment
                                      const Vector3 &p1, // end of line segment
                                      const Vector3 &e0, // point on clipping edge
                                      const Vector3 &de  // direction of clipping edge
  );

  static inline int Clip_Triangle_To_LineXY(int incount, Vector3 *InPoints, Vector2 *InUVs, Vector3 *OutPoints,
                                            Vector2 *OutUVs, const Vector3 &edge_point0, const Vector3 &edge_point1);
