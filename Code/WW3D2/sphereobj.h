...
struct AlphaVectorStruct {
  ...
  bool operator==(const AlphaVectorStruct &src) {
    return (angle.X == src.angle.X) && (angle.Y == src.angle.Y) && (angle.Z == src.angle.Z) &&
...
public:
  AlphaVectorStruct Evaluate(float time) {
    ...
    if (time < m_Data[key_count - 1].Get_Time()) {
      ...
      value.intensity =
          (key1->Get_Value().intensity + (key2->Get_Value().intensity - key1->Get_Value().intensity) * percent);
...
inline void SphereMeshClass::Set_DCG(bool is_additive, int index, float value) {
  if (is_additive) {
    dcg[index].X = value;
    dcg[index].Y = value;
    dcg[index].Z = value;
