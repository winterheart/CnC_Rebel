...
inline void IntersectionClass::Get_Screen_Ray(float screen_x, float screen_y, const LayerClass &Layer) {
  ...
  *RayLocation = camera_location;
  ...
  // float x_offset = width / (float)Scene->width; // render width in pixels divided by display width in pixels = ratio
  // of displayed area float y_offset = height / (float)Scene->height;
  //	float zmod = Scene->perspective;
  //	float xmod = ((ScreenX / x_offset) * width  - xmid - Scene->xstart) * zmod  * 16384.0f/ (Scene->axratio *
  //128.0f); 	float ymod = ((ScreenY / y_offset) * height - ymid - Scene->ystart) * zmod  * 16384.0f/ (Scene->ayratio *
  //128.0f);
...
*/
inline bool IntersectionClass::_Point_In_Polygon_Z(Vector3 &Point, Vector3 &Corner1, Vector3 &Corner2,
                                                   Vector3 &Corner3) {
...
**
*/
inline bool IntersectionClass::_Point_In_Polygon_Z(Vector3 &Point, Vector3 Corners[3]) {
...
*/
inline bool IntersectionClass::_Point_In_Polygon(Vector3 &Point, Vector3 &loc1, Vector3 &loc2, Vector3 &loc3,
                                                 int axis_1, int axis_2, float &Alpha, float &Beta) {
...
*/
inline bool IntersectionClass::_Point_In_Polygon(IntersectionResultClass *FinalResult, Vector3 &loc1, Vector3 &loc2,
                                                 Vector3 &loc3, int axis_1, int axis_2) {
  return (FinalResult->Intersects = _Point_In_Polygon(FinalResult->Intersection, loc1, loc2, loc3, axis_1, axis_2,
                                                      FinalResult->Alpha, FinalResult->Beta));
}
...
*/
inline bool IntersectionClass::_Point_In_Polygon(IntersectionResultClass *FinalResult, Vector3 &Normal, Vector3 &loc1,
...
inline float IntersectionClass::Plane_Z_Distance(Vector3 &PlaneNormal, Vector3 &PlanePoint) {
  // do a parallel check
  float divisor =
      (PlaneNormal[0] * (*RayDirection)[0] + PlaneNormal[1] * (*RayDirection)[1] + PlaneNormal[2] * (*RayDirection)[2]);
  if (divisor == 0)
  ...
  float value = -(d + PlaneNormal[0] * (*RayLocation)[0] + PlaneNormal[1] * (*RayLocation)[1] +
                  PlaneNormal[2] * (*RayLocation)[2]) /
                divisor;
...
*/
inline float IntersectionClass::_Get_Z_Elevation(Vector3 &Point, Vector3 &PlanePoint, Vector3 &PlaneNormal) {
...
*/
inline bool IntersectionClass::Intersect_Polygon_Z(IntersectionResultClass *Result, Vector3 &PolygonNormal, Vector3 &v1,
...
*/
inline bool IntersectionClass::Intersect_Plane_Quick(IntersectionResultClass *Result, Vector3 &PlaneNormal,
                                                     Vector3 &PlanePoint) {
  // do a parallel check
  float divisor =
      (PlaneNormal[0] * (*RayDirection)[0] + PlaneNormal[1] * (*RayDirection)[1] + PlaneNormal[2] * (*RayDirection)[2]);
  ...
  Result->Range = -(d + PlaneNormal[0] * (*RayLocation)[0] + PlaneNormal[1] * (*RayLocation)[1] +
                    PlaneNormal[2] * (*RayLocation)[2]) /
                  divisor;
...
*/
inline bool IntersectionClass::Intersect_Plane(IntersectionResultClass *Result, Vector3 &PlaneNormal,
                                               Vector3 &PlanePoint) {
  ...
  if (!Intersect_Plane_Quick(Result, PlaneNormal, PlanePoint))
    return false;
...
*/
inline bool IntersectionClass::Intersect_Polygon(IntersectionResultClass *Result, Vector3 &PolygonNormal, Vector3 &v1,
                                                 Vector3 &v2, Vector3 &v3) {
...
*/
inline bool IntersectionClass::Intersect_Polygon(IntersectionResultClass *Result, Vector3 &v1, Vector3 &v2,
                                                 Vector3 &v3) {
  Vector3 vec1 = v2 - v1;
  Vector3 vec2 = v3 - v1;
...
bool IntersectionClass::Intersect_Screen_Object(IntersectionResultClass *Final_Result, Vector4 &Area,
                                                RenderObjClass *obj) {
  if (Final_Result->Intersects =
          ((ScreenX >= Area[0]) && (ScreenX <= Area[2]) && (ScreenY >= Area[1]) && (ScreenY <= Area[3]))) {
        ...
        int &DestIndex)
{
        /*
                ...
                If the intersection point of the 2 lines are needed (lines in this context mean infinite lines)
regardless whether the two line segments intersect, then
        ...
        * /
        ...
        float numerator =   (A[AXIS_2] - C[AXIS_2]) * (D[AXIS_1] - C[AXIS_1]) - (A[AXIS_1] - C[AXIS_1]) * (D[AXIS_2] -
C[AXIS_2]);

        // if the denominator is zero, then the segments are parallel.
        float denominator = (B[AXIS_1] - A[AXIS_1]) * (D[AXIS_2] - C[AXIS_2]) - (B[AXIS_2] - A[AXIS_2]) * (D[AXIS_1] -
C[AXIS_1]);
        ...
        numerator = (A[AXIS_2] - C[AXIS_2]) * (B[AXIS_1] - A[AXIS_1]) - (A[AXIS_1] - C[AXIS_1]) * (B[AXIS_2] -
A[AXIS_2]); s = numerator * denominator;
...
inline bool IntersectionClass::In_Front_Of_Line(const Vector3 &p,  // point to test
...
inline float IntersectionClass::Intersect_Lines(const Vector3 &p0, // start of line segment
...
inline int IntersectionClass::Clip_Triangle_To_LineXY(int incount, Vector3 *InPoints, Vector2 *InUVs,
                                                      Vector3 *OutPoints, Vector2 *OutUVs, const Vector3 &edge_point0,
...
inline int IntersectionClass::_Intersect_Triangles_Z(Vector3 ClipPoints[3], Vector3 TrianglePoints[3], Vector2 UV[3],
...
{ ClippedPoints[points] = ClipPoints[0]; Vector2 uv1 = UV[1] - UV[0]; Vector2 uv2 = UV[2] - UV[0]; ClippedUV[points++] =
UV[0] + alpha * uv1 + beta * uv2;
        }

        if(_Point_In_Polygon(ClipPoints[1], TrianglePoints[0], TrianglePoints[1], TrianglePoints[2], 0, 1, alpha, beta))
{ ClippedPoints[points] = ClipPoints[1]; Vector2 uv1 = UV[1] - UV[0]; Vector2 uv2 = UV[2] - UV[0]; ClippedUV[points++] =
UV[0] + alpha * uv1 + beta * uv2;
        }

        if(_Point_In_Polygon(ClipPoints[2], TrianglePoints[0], TrianglePoints[1], TrianglePoints[2], 0, 1, alpha, beta))
{ ClippedPoints[points] = ClipPoints[2]; Vector2 uv1 = UV[1] - UV[0]; Vector2 uv2 = UV[2] - UV[0]; ClippedUV[points++] =
UV[0] + alpha * uv1 + beta * uv2;
        }
