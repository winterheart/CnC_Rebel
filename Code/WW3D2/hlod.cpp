 ...
 * HISTORY:                                                                                    *
 *=============================================================================================*/
HLodDefClass::HLodDefClass(void) : Name(NULL), HierarchyTreeName(NULL), LodCount(0), Lod(NULL), ProxyArray(NULL) {}
...
HLodDefClass::HLodDefClass(HLodClass &src_lod)
    : Name(NULL), HierarchyTreeName(NULL), LodCount(0), Lod(NULL), ProxyArray(NULL) {
  Initialize(src_lod);
  return;
}
...
WW3DErrorType HLodDefClass::Save(ChunkSaveClass &csave) {
  ...
  if (csave.Begin_Chunk(W3D_CHUNK_HLOD) == TRUE) {
    ...
    if ((Save_Header(csave) == WW3D_ERROR_OK) && (Save_Lod_Array(csave) == WW3D_ERROR_OK)) {
...
HLodDefClass::SubObjectArrayClass::SubObjectArrayClass(void)
    : MaxScreenSize(NO_MAX_SCREEN_SIZE), ModelCount(0), ModelName(NULL), BoneIndex(NULL) {}
...
HLodClass::HLodClass(void)
    : Animatable3DObjClass(NULL), LodCount(0), CurLod(0), Lod(NULL), BoundingBoxIndex(-1), Cost(NULL), Value(NULL),
      AdditionalModels(), SnapPoints(NULL), ProxyArray(NULL), LODBias(1.0f) {}
...
HLodClass::HLodClass(const HLodClass &src)
    : Animatable3DObjClass(src), LodCount(0), CurLod(0), Lod(NULL), BoundingBoxIndex(-1), Cost(NULL), Value(NULL),
      AdditionalModels(), SnapPoints(NULL), ProxyArray(NULL), LODBias(1.0f) {
  *this = src;
}
...
HLodClass::HLodClass(const char *name, RenderObjClass **lods, int count)
  ...
  for (int lod_index = 0; lod_index < LodCount; lod_index++) {
    ...
    if ((lod_obj->Class_ID() == RenderObjClass::CLASSID_HMODEL) ||
        (lod_obj->Class_ID() == RenderObjClass::CLASSID_HLOD) || (lod_obj->Get_Num_Sub_Objects() > 1)) {
...
HLodClass::HLodClass(const HLodDefClass &def)
    : Animatable3DObjClass(def.HierarchyTreeName), LodCount(0), CurLod(0), Lod(NULL), BoundingBoxIndex(-1), Cost(NULL),
      Value(NULL), AdditionalModels(), SnapPoints(NULL), ProxyArray(NULL), LODBias(1.0f) {
  // Set the name
...
HLodClass::HLodClass(const HModelDefClass &def)
    : Animatable3DObjClass(def.BasePoseName), LodCount(0), CurLod(0), Lod(NULL), BoundingBoxIndex(-1), Cost(NULL),
      Value(NULL), AdditionalModels(), SnapPoints(NULL), ProxyArray(NULL), LODBias(1.0f) {
  // Set the name
  Set_Name(def.Get_Name());
 ...
 *   1/26/00    gth : Created.                                                                 *
 *=============================================================================================*/
RenderObjClass *HLodClass::Clone(void) const { return new HLodClass(*this); }
...
void HLodClass::Get_Obj_Space_Bounding_Box(AABoxClass &box) const {
  ...
  if (BoundingBoxIndex >= 0 && BoundingBoxIndex < count) {
    ...
    if (mesh != NULL && mesh->Class_ID() == RenderObjClass::CLASSID_OBBOX) {
      ...
      box_to_hlod_tm.Transform_Center_Extent_AABox(obbox_mesh->Get_Local_Center(), obbox_mesh->Get_Local_Extent(),
...
const AABoxClass &HLodClass::Get_Bounding_Box(void) const {
  if (BoundingBoxIndex >= 0) {
    ...
    //
    Get_Transform().Transform_Center_Extent_AABox(box.Center, box.Extent, &CachedBoundingBox.Center,
...
RenderObjClass *HLodClass::Peek_Lod_Model(int lod_index, int model_index) const {
  ...
  WWASSERT(lod_index >= 0);
  WWASSERT(lod_index < LodCount);
  if ((lod_index >= 0) && (lod_index < LodCount) && (model_index < Lod[lod_index].Count())) {
 ...
 *   8/23/00    NH : Created.                                                                  *
 *=============================================================================================*/
int HLodClass::Get_Additional_Model_Count(void) const { return AdditionalModels.Count(); }
 ...
 *   1/26/00    gth : Created.                                                                 *
 *=============================================================================================*/
int HLodClass::Add_Sub_Object(RenderObjClass *subobj) { return Add_Sub_Object_To_Bone(subobj, 0); }
 ...
 *   1/26/00    gth : Created.                                                                 *
 *=============================================================================================*/
void HLodClass::Set_Animation(HAnimClass *motion0, float frame0, HAnimClass *motion1, float frame1, float percentage) {
 ...
 *   1/26/00    gth : Created.                                                                 *
 *=============================================================================================*/
float HLodClass::Get_Post_Increment_Value(void) const { return (Value[CurLod + 1]); }
 ...
 *   1/26/00    gth : Created.                                                                 *
 *=============================================================================================*/
void HLodClass::Set_HTree(HTreeClass *htree) { Animatable3DObjClass::Set_HTree(htree); }
