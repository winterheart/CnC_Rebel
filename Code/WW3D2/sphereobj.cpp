...
SphereRenderObjClass::SphereRenderObjClass(void)
    : anim_time(0.0F), IsAnimating(true), AnimDuration(0.0F), SphereMaterial(NULL), SphereTexture(NULL),
      Color(0.75F, 0.75F, 0.75F), Scale(1, 1, 1), Alpha(1.0F), Flags(USE_ALPHA_VECTOR), ObjSpaceCenter(0, 0, 0),
      ObjSpaceExtent(1, 1, 1), CurrentLOD(SPHERE_NUM_LOD - 1) {
...
SphereRenderObjClass::~SphereRenderObjClass() { REF_PTR_RELEASE(SphereMaterial); } // destructor
 ...
 *   1/19/00    gth : Created.                                                                 *
 *=============================================================================================*/
int SphereRenderObjClass::Get_Num_Polys(void) const { return SphereMeshArray[CurrentLOD].Get_Num_Polys(); }
 ...
 *   1/19/00    gth : Created.                                                                 *
 *=============================================================================================*/
void SphereRenderObjClass::Set_Texture(TextureClass *tf) { REF_PTR_SET(SphereTexture, tf); }
 ...
 *   1/19/00    gth : Created.                                                                 *
 *=============================================================================================*/
void SphereRenderObjClass::vis_render_sphere(SpecialRenderInfoClass &rinfo, const Vector3 &center,
                                             const Vector3 &extent) {} // vis_render_sphere
 ...
 *   1/19/00    gth : Created.                                                                 *
 *=============================================================================================*/
RenderObjClass *SphereRenderObjClass::Clone(void) const { return new SphereRenderObjClass(*this); }
 ...
 *   1/19/00    gth : Created.                                                                 *
 *=============================================================================================*/
int SphereRenderObjClass::Class_ID(void) const { return RenderObjClass::CLASSID_SPHERE; }
...
void SphereRenderObjClass::Render(RenderInfoClass &rinfo) {
  ...
  if (WW3D::Are_Static_Sort_Lists_Enabled() && sort_level != SORT_LEVEL_NONE) {
    ...
    if (lod_int < 0)
      lod_int = 0;
    if (lod_int >= SPHERE_NUM_LOD)
      lod_int = SPHERE_NUM_LOD - 1;
    ...
    if (Flags & USE_CAMERA_ALIGN) {
      ...
      Matrix3D tm(0.0f, 1.0f, 0.0f, cpos.X, 0.0f, 0.0f, 1.0f, cpos.Y, 1.0f, 0.0f, 0.0f, cpos.Z);
...
void SphereRenderObjClass::animate(void) {
  if (Is_Animating()) {
    ...
    if (ColorChannel.Get_Key_Count() > 0 || AlphaChannel.Get_Key_Count() > 0 || ScaleChannel.Get_Key_Count() > 0 ||
...
const char *SpherePrototypeClass::Get_Name(void) const { return Definition.Name; }

int SpherePrototypeClass::Get_Class_ID(void) const { return RenderObjClass::CLASSID_SPHERE; }
...
SphereMeshClass::SphereMeshClass(float radius, int slices, int stacks)
    : Radius(radius), Slices(slices), Stacks(stacks), Vertex_ct(0), vtx(NULL), vtx_normal(NULL), vtx_uv(NULL),
      strip_ct(0), strip_size(0), strips(NULL), fan_ct(0), fan_size(0), fans(NULL), face_ct(0), tri_poly(NULL),
      inverse_alpha(false) {
  // compute # of vertices
...
SphereMeshClass::SphereMeshClass(void)
    : Radius(0.0f), Slices(0), Stacks(0), Vertex_ct(0), vtx(NULL), vtx_normal(NULL), vtx_uv(NULL), strip_ct(0),
      strip_size(0), strips(NULL), fan_ct(0), fan_size(0), fans(NULL), face_ct(0), tri_poly(NULL),
...
void SphereMeshClass::Set_Alpha_Vector(const AlphaVectorStruct &v, bool inverse, bool is_additive, bool force) {
  ...
  //
  if (alpha_vector == v && inverse == inverse_alpha && is_additive == IsAdditive && !force) {
    return;
  }
