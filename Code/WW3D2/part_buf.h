...
struct NewParticleStruct {
  ...
  bool operator!=(const NewParticleStruct &p) { return (p.TimeStamp != TimeStamp) || (p.Position != Position); }
  bool operator==(const NewParticleStruct &p) { return (p.TimeStamp == TimeStamp) && (p.Position == Position); }
};
...
public:
  ParticleBufferClass(ParticleEmitterClass *emitter, unsigned int buffer_size, ParticlePropertyStruct<Vector3> &color,
                      ParticlePropertyStruct<float> &opacity, ParticlePropertyStruct<float> &size,
                      ParticlePropertyStruct<float> &rotation, float orient_rnd, ParticlePropertyStruct<float> &frame,
                      ParticlePropertyStruct<float> &blurtime, Vector3 accel, float max_age, TextureClass *tex,
                      ShaderClass shader, bool pingpong, int render_mode, int frame_mode,
  ...
  */
  ...
  void Set_Acceleration(const Vector3 &acceleration) {
    Accel = acceleration;
    HasAccel = ((Accel.X != 0) || (Accel.Y != 0) || (Accel.Z != 0));
  }
  ...
  Vector3 Get_End_Color(void) const {
    return (NumColorKeyFrames > 1) ? ColorKeyFrameValues[NumColorKeyFrames - 1] : ColorKeyFrameValues[0];
  }
  float Get_End_Opacity(void) const {
    return (NumAlphaKeyFrames > 1) ? AlphaKeyFrameValues[NumAlphaKeyFrames - 1] : AlphaKeyFrameValues[0];
  }
  TextureClass *Get_Texture(void) const;
  void Set_Texture(TextureClass *tex);
  float Get_Fade_Time(void) const {
    return (NumColorKeyFrames > 1) ? (((float)ColorKeyFrameTimes[1]) / 1000.0f) : 0.0f;
  }
