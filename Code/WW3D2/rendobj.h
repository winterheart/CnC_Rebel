...
public:
  ...
  virtual void Set_Has_User_Lighting(bool onoff) {
    if (onoff) {
      Bits |= HAS_USER_LIGHTING;
    } else {
      Bits &= ~HAS_USER_LIGHTING;
    }
  }
  ...
  virtual void Set_Collision_Type(int type) {
    Bits &= ~COLLISION_TYPE_MASK;
    Bits |= (type & COLLISION_TYPE_MASK) | COLLISION_TYPE_ALL;
  }
...
protected:
  ...
  enum {
    ...
    IS_REALLY_VISIBLE = static_cast<uint32_t>(IS_VISIBLE) | static_cast<uint32_t>(IS_NOT_HIDDEN) |
                        static_cast<uint32_t>(IS_NOT_ANIMATION_HIDDEN),
    IS_NOT_HIDDEN_AT_ALL = static_cast<uint32_t>(IS_NOT_HIDDEN) | static_cast<uint32_t>(IS_NOT_ANIMATION_HIDDEN),
    DEFAULT_BITS = static_cast<uint32_t>(COLLISION_TYPE_ALL) | static_cast<uint32_t>(IS_NOT_HIDDEN) |
                   static_cast<uint32_t>(IS_NOT_ANIMATION_HIDDEN),
  };
...
WWINLINE const Matrix3D &RenderObjClass::Get_Transform_No_Validity_Check(void) const { return Transform; }
...
// Warning: Be sure to call this function only if the transform is known to be valid!
WWINLINE bool RenderObjClass::Is_Transform_Identity_No_Validity_Check() const { return IsTransformIdentity; }
