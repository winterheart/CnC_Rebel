 ...
 *   1/26/00    gth : Created.                                                                 *
 *=============================================================================================*/
DecalMeshClass::DecalMeshClass(MeshClass *parent, DecalSystemClass *system) : Parent(parent), DecalSystem(system) {
...
void RigidDecalMeshClass::Render(void) {
  if ((Decals.Count() == 0) || (WW3D::Are_Decals_Enabled() == false))
    return;
  ...
  */
  ...
  while (next_poly_index < Polys.Count()) {
    ...
    DX8Wrapper::Draw_Triangles(3 * cur_poly_index,
                               ...
                               Polys[cur_poly_index].I, 1 + Polys[next_poly_index - 1].K - Polys[cur_poly_index].I);
...
int RigidDecalMeshClass::Process_Material_Run(int start_index) {
  ...
  while ((next_index < Polys.Count()) && (Textures[next_index] == Textures[start_index]) &&
 ...
 *   1/26/00    gth : Created.                                                                 *
 *=============================================================================================*/
bool RigidDecalMeshClass::Create_Decal(DecalGeneratorClass *generator, const OBBoxClass &localbox,
 ...
 *   1/31/00    NH : Created.                                                                  *
 *=============================================================================================*/
SkinDecalMeshClass::SkinDecalMeshClass(MeshClass *parent, DecalSystemClass *system) : DecalMeshClass(parent, system) {}
 ...
 *   1/31/00    NH : Created.                                                                  *
 *=============================================================================================*/
bool SkinDecalMeshClass::Create_Decal(DecalGeneratorClass *generator, const OBBoxClass &localbox,
                                      SimpleDynVecClass<uint32> &apt,
                                      const DynamicVectorClass<Vector3> *world_vertex_locs) {
  int i;
  WWASSERT(generator->Peek_Decal_System() == DecalSystem);
  ...
  */
  ...
  for (i = 0; i < apt.Count(); i++) {
    ...
    for (int vi = 0; vi < 3; vi++) {
      ...
      */
      Vector3 stq;
      generator->Compute_Texture_Coordinate((*world_vertex_locs)[ParentVertexIndices[ParentVertexIndices.Count() - 1]],
                                            &stq);
      TexCoords.Add(Vector2(stq.X, stq.Y));
    }
  }
