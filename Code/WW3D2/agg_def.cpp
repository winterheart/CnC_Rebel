...
//	AggregateDefClass
//
AggregateDefClass::AggregateDefClass(const AggregateDefClass &src) : m_pName(NULL) {
  // Set our member data to default settings
  ::memset(&m_Info, 0, sizeof(m_Info));
  ::memset(&m_MiscInfo, 0, sizeof(m_MiscInfo));
  m_MiscInfo.OriginalClassID = RenderObjClass::CLASSID_HLOD;
...
//	AggregateDefClass
//
AggregateDefClass::AggregateDefClass(RenderObjClass &base_model) : m_pName(NULL) {
  // Set our member data to default settings
  ::memset(&m_Info, 0, sizeof(m_Info));
  ::memset(&m_MiscInfo, 0, sizeof(m_MiscInfo));
  m_MiscInfo.OriginalClassID = RenderObjClass::CLASSID_HLOD;
...
RenderObjClass *AggregateDefClass::Create(void) {
  ...
  if (pmodel != NULL) {
    ...
    pmodel->Set_Sub_Objects_Match_LOD((m_MiscInfo.Flags & W3D_AGGREGATE_FORCE_SUB_OBJ_LOD) ==
...
RenderObjClass *AggregateDefClass::Find_Subobject(RenderObjClass &model,
  ...
  for (int index = 1; (mesh_path[index][0] != 0) && (parent_model != NULL); index++) {
...
//
void AggregateDefClass::Build_Subobject_List(RenderObjClass &original_model, RenderObjClass &model) {
  int index;
  ...
  for (int bone_index = 0; bone_index < bone_count; bone_index++) {
    ...
    for (index = 0; index < original_model.Get_Num_Sub_Objects_On_Bone(bone_index); index++) {
    ...
    for (index = 0; index < model.Get_Num_Sub_Objects_On_Bone(bone_index); index++) {
    ...
    if (node_count > 0) {
      ...
      for (int node_index = 0; node_index < node_count; node_index++) {
        ...
        if (psubobject != NULL && (Is_Object_In_List(prototype_name, orig_node_list) == false)) {
...
//	Is_Object_In_List
//
bool AggregateDefClass::Is_Object_In_List(const char *passet_name, DynamicVectorClass<RenderObjClass *> &node_list) {
  // Assume failure
  bool retval = false;
...
WW3DErrorType AggregateDefClass::Read_Info(ChunkLoadClass &chunk_load) {
  ...
  if (chunk_load.Read(&m_Info, sizeof(m_Info)) == sizeof(m_Info)) {
    ...
    for (UINT isubobject = 0; (isubobject < m_Info.SubobjectCount) && (ret_val == WW3D_ERROR_OK); isubobject++) {
...
WW3DErrorType AggregateDefClass::Save_W3D(ChunkSaveClass &chunk_save) {
  ...
  if (chunk_save.Begin_Chunk(W3D_CHUNK_AGGREGATE) == TRUE) {
    ...
    if ((Save_Header(chunk_save) == WW3D_ERROR_OK) && (Save_Info(chunk_save) == WW3D_ERROR_OK) &&
...
WW3DErrorType AggregateDefClass::Save_Info(ChunkSaveClass &chunk_save) {
  ...
  if (chunk_save.Begin_Chunk(W3D_CHUNK_AGGREGATE_INFO) == TRUE) {
    ...
    if (chunk_save.Write(&m_Info, sizeof(m_Info)) == sizeof(m_Info)) {
      ...
      for (int isubobject = 0; (isubobject < m_SubobjectList.Count()) && (ret_val == WW3D_ERROR_OK); isubobject++) {
...
//	Save_Subobject
//
WW3DErrorType AggregateDefClass::Save_Subobject(ChunkSaveClass &chunk_save, W3dAggregateSubobjectStruct *psubobject) {
  // Assume error
  WW3DErrorType ret_val = WW3D_ERROR_SAVE_FAILED;
