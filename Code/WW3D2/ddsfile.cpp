...
void DDSFileClass::Copy_Level_To_Surface(unsigned level, WW3DFormat dest_format, unsigned dest_width,
                                         unsigned dest_height, unsigned char *dest_surface, unsigned dest_pitch) {
  WWASSERT(DDSMemory);
...
unsigned DDSFileClass::Get_Pixel(unsigned level, unsigned x, unsigned y) const {
  ...
  case WW3D_FORMAT_DXT1: {
    const unsigned char *block_memory =
        Get_Memory_Pointer(level) + (x / 4) * 8 + ((y / 4) * (Get_Width(level) / 4)) * 8;
  ...
  case WW3D_FORMAT_DXT5: {
    const unsigned char *alpha_block =
        Get_Memory_Pointer(level) + (x / 4) * 16 + ((y / 4) * (Get_Width(level) / 4)) * 16;
...
bool DDSFileClass::Get_4x4_Block(unsigned char *dest_ptr, // Destination surface pointer
                                 ...
                                 unsigned source_y) const // DDS y offset to copy from, must be aligned by 4!
{
  ...
  case WW3D_FORMAT_DXT1: {
    const unsigned char *block_memory =
        Get_Memory_Pointer(level) + (source_x / 4) * 8 + ((source_y / 4) * (Get_Width(level) / 4)) * 8;
  ...
  case WW3D_FORMAT_DXT5: {
    // Init alphas
    const unsigned char *alpha_block =
        Get_Memory_Pointer(level) + (source_x / 4) * 16 + ((source_y / 4) * (Get_Width(level) / 4)) * 16;
