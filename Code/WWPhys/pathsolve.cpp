...
//
///////////////////////////////////////////////////////////////////////////
static inline void Shrink_Box(AABoxClass &box, float width) {
...
//
///////////////////////////////////////////////////////////////////////////
static inline void Clip_Point(Vector3 *point, const AABoxClass &box, float edge_dist) {
  Vector3 temp_point = *point;
...
PathSolveClass::PathSolveClass(void)
    : m_StartPos(0, 0, 0), m_DestPos(0, 0, 0), m_StartSector(NULL), m_DestSector(NULL), m_CompletedNode(NULL),
      m_State(ERROR_INVALID_START_POS), m_BinaryHeap(10000), m_Priority(0.5F), m_BirthTime(0) {
  //
...
//
///////////////////////////////////////////////////////////////////////////
PathSolveClass::STATE_DESC PathSolveClass::Reset(const Vector3 &start, const Vector3 &dest, float sector_fudge) {
...
//
///////////////////////////////////////////////////////////////////////////
void PathSolveClass::Unlink_Pathfind_Hooks(void) {
  for (int index = 0; index < m_NodeList.Count(); index++) {
...
//
///////////////////////////////////////////////////////////////////////////
void PathSolveClass::Resolve_Path(unsigned int milliseconds) {
  WWMEMLOG(MEM_PATHFIND);
  ...
  do {
    ...
    if (node == NULL) {
      ...
      //
      for (PathNodeClass *path_node = m_CompletedNode; path_node != NULL; path_node = path_node->Peek_Parent_Node()) {
  ...
  // () - start_time) / (_TicksPerMilliSec/100)), iterations, (int)(m_State == TRAVERSING_PATH));
  return;
}
...
//
///////////////////////////////////////////////////////////////////////////
PathSolveClass::STATE_DESC PathSolveClass::Timestep(unsigned int milliseconds) {
  ...
  while (index--) {
    ...
    if (portal != NULL) {
      ...
      if (Does_Object_Have_Access_To_Portal(portal)) {
        Submit_Node(0, NULL, portal, portal->Peek_Dest_Sector(m_StartSector), Matrix3D(m_StartPos), ending_tm);
      }
    }
  }
...
//
///////////////////////////////////////////////////////////////////////////
float Find_Tangent_Angle(const Vector2 &center, float radius, const Vector2 &point, bool clockwise) {
  float angle = 0;
...
//
///////////////////////////////////////////////////////////////////////////
void Get_Turn_Arc_Center(float wheel_offset, float turn_radius, bool left_turn, Vector2 *arc_center
...
//
///////////////////////////////////////////////////////////////////////////
void Find_Adjacent_Portals(PathfindSectorClass *sector, PathfindPortalClass *portal, PathfindSectorClass *dest_sector,
                           PathfindPortalClass **portal1, PathfindPortalClass **portal2) {
  AABoxClass portal_box;
  portal->Get_Bounding_Box(portal_box);
...
//
///////////////////////////////////////////////////////////////////////////
bool Does_Object_Fit_Through_Portal(const PathObjectClass &object, PathfindSectorClass *sector,
...
//
///////////////////////////////////////////////////////////////////////////
bool PathSolveClass::Does_Object_Have_Access_To_Portal(PathfindPortalClass *portal) {
  bool retval = true;
...
//
///////////////////////////////////////////////////////////////////////////
bool PathSolveClass::Can_Object_Go_Through_Portal(const Matrix3D &current_tm, PathfindSectorClass *sector,
  ...
  //	Can we fit through this portal?
  //
  if (portal->Does_Size_Matter() == false || ::Does_Object_Fit_Through_Portal(m_PathObject, sector, portal)) {
    //
    //	Find where we should enter the portal...
    //
...
//
///////////////////////////////////////////////////////////////////////////
bool Can_Object_Enter_Portal(const AABoxClass &portal_box, const PathObjectClass &object,
...
//
///////////////////////////////////////////////////////////////////////////
void PathSolveClass::Process_Portals(PathNodeClass *node) {
    ...
    if (sector->Can_Access_Portal(last_portal, portal)) {
      ...
      if (dest_sector != NULL) {
        ...
        if (Does_Object_Have_Access_To_Portal(portal) &&
          ...
          //
          Submit_Node(node->Get_Traversal_Cost(), node, portal, dest_sector, current_tm, ending_tm);
        }
      }
    }
  }
...
//
///////////////////////////////////////////////////////////////////////////
float Calculate_Heuristic_Cost(const PathObjectClass &object, const PathfindPortalClass &portal,
                               const PathfindSectorClass &sector, const Vector3 &curr_pos, const Vector3 &dest_pos) {
...
//
///////////////////////////////////////////////////////////////////////////
void PathSolveClass::Submit_Node(float traversal_cost, PathNodeClass *current_node, PathfindPortalClass *portal,
                                 PathfindSectorClass *dest_sector, const Matrix3D &current_tm,
  ...
  //
  float heuristic_cost = ::Calculate_Heuristic_Cost(m_PathObject, *portal, *dest_sector, dest_pos, m_DestPos);

  //
  //	Try to keep vehicles from taking one-way portals
  //
  if (m_PathObject.Is_Flag_Set(PathObjectClass::IS_VEHICLE) && portal->Is_Two_Way_Portal() == false) {
...
//
///////////////////////////////////////////////////////////////////////////
void PathSolveClass::Reset_Lists(void) {
  //
...
//
/////////////////////////////////////////////////////////////////////////////////
bool PathSolveClass::Does_Line_Go_Through_Portal(const Vector3 &start, const Vector3 &end, const AABoxClass &box) {
  ...
  //
  bool retval = true;
  if ((box.Center.X + box.Extent.X) < dest_point.X)
    retval = false;
  if ((box.Center.Y + box.Extent.Y) < dest_point.Y)
    retval = false;
  if ((box.Center.X - box.Extent.X) > dest_point.X)
    retval = false;
  if ((box.Center.Y - box.Extent.Y) > dest_point.Y)
...
//
/////////////////////////////////////////////////////////////////////////////////
Vector3 PathSolveClass::Relax_Line(const Vector3 &start, const Vector3 &end, const AABoxClass &box) {
...
//
/////////////////////////////////////////////////////////////////////////////////
bool Check_Line(const Vector3 &p0, const Vector3 &p1, const AABoxClass &sector, const AABoxClass &portal, float width,
                Vector3 *result) {
  bool retval = false;
  ...
  if (WWMath::Fabs(portal.Center.X - sector.Center.X + sector.Extent.X) < WWMATH_EPSILON ||
    ...
    if ((p0.Y < y_val0 && p1.Y > y_val0) || (p0.Y > y_val0 && p1.Y < y_val0)) {
      do_it = true;
      y_val = y_val0;

    } else if ((p0.Y < y_val1 && p1.Y > y_val1) || (p0.Y > y_val1 && p1.Y < y_val1)) {
      do_it = true;
      y_val = y_val1;
    }
    ...
    if ((p0.X < x_val0 && p1.X > x_val0) || (p0.X > x_val0 && p1.X < x_val0)) {
      do_it = true;
      x_val = x_val0;

    } else if ((p0.X < x_val1 && p1.X > x_val1) || (p0.X > x_val1 && p1.X < x_val1)) {
      do_it = true;
      x_val = x_val1;
    }
...
void PathSolveClass::Post_Process_Path(void) {
  m_Path.Delete_All();
...
//
/////////////////////////////////////////////////////////////////////////////////
void PathSolveClass::Keep_Unit_Inside_Sectors(void) {
  if (m_Path.Count() == 0) {
    return;
  }
  ...
  for (int index = 0; index < m_Path.Count() - 1; index++) {
    ...
    //
    if ((m_Path[index].m_SectorExtent.X > 0) || (m_Path[index].m_SectorExtent.Y > 0)) {
      //
...
//
/////////////////////////////////////////////////////////////////////////////////
void PathSolveClass::Set_Path_Object(PathObjectClass &path_object) {
...
//
/////////////////////////////////////////////////////////////////////////////////
void PathSolveClass::Get_Path_Object(PathObjectClass &path_object) const {
...
//
/////////////////////////////////////////////////////////////////////////////////
void PathSolveClass::Get_Volumes(BOX_LIST &sector_list, BOX_LIST &portal_list) {
  //
  //	Build a list of the nodes (in order) the path passes through.
  //
  for (PathNodeClass *node = m_CompletedNode; node != NULL; node = node->Peek_Parent_Node()) {
...
//
////////////////////////////////////////////////////////////////////////////////////////////
void PathSolveClass::Save(ChunkSaveClass &csave) {
...
//
////////////////////////////////////////////////////////////////////////////////////////////
void PathSolveClass::Load(ChunkLoadClass &cload) {
...
//
///////////////////////////////////////////////////////////////////////
void PathSolveClass::On_Post_Load(void) {
  //
