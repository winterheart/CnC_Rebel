...
VTOLVehicleClass::VTOLVehicleClass(void)
    : EngineAngleBones(MAX_CAPTURED_BONE_COUNT), RotorAngleBones(MAX_CAPTURED_BONE_COUNT),
      NormalizedEngineRotation(0.0f), NormalizedEngineThrust(0.0f), RotorAngle(0.0f), RotorAngularVelocity(0.0f) {
...
void VTOLVehicleClass::Init(const VTOLVehicleDefClass &def) { VehiclePhysClass::Init(def); }
    ...
    */
    ...
    if (IsEngineOn) {
      ...
      *torque += yaw_torque;
      ...
      Vector3 pitch_torque =
          (def->PitchControllerGain * (target_pitch - pitch) + def->PitchControllerDamping * (0.0f - dpitch)) *
          IBody[1][1] * yvec;
      Vector3 roll_torque =
          (def->RollControllerGain * (target_roll - roll) + def->RollControllerDamping * (0.0f - droll)) * IBody[0][0] *
          xvec;
      ...
      *force += left_force * yvec2d;
      ...
      NormalizedEngineThrust =
          0.25f + (WWMath::Fabs(pitch) / def->MaxFuselagePitch) + (WWMath::Fabs(roll) / def->MaxFuselageRoll) + move_up;
...
SuspensionElementClass *VTOLVehicleClass::Alloc_Suspension_Element(void) { return new VTOLWheelClass; }

float VTOLVehicleClass::Get_Normalized_Engine_Flame(void) { return NormalizedEngineThrust; }
...
** Save-Load System
*/
const PersistFactoryClass &VTOLVehicleClass::Get_Factory(void) const { return _VTOLVehicleFactory; }
...
VTOLVehicleDefClass::VTOLVehicleDefClass(void)
    : MaxVerticalAcceleration(0.0f), MaxHorizontalAcceleration(0.0f), MaxFuselagePitch(DEG_TO_RADF(15.0f)),
      MaxFuselageRoll(DEG_TO_RADF(20.0f)), PitchControllerGain(45.5), PitchControllerDamping(12.75),
      RollControllerGain(45.5), RollControllerDamping(12.75), MaxYawVelocity(DEG_TO_RADF(180.0f)),
      YawControllerGain(5.0f), MaxEngineRotation(DEG_TO_RADF(25.0f)), RotorSpeed(DEG_TO_RADF(360.0f)),
      RotorAcceleration(DEG_TO_RADF(180.0f)), RotorDeceleration(DEG_TO_RADF(180.0f)) {
...
uint32 VTOLVehicleDefClass::Get_Class_ID(void) const { return CLASSID_VTOLVEHICLEDEF; }

const PersistFactoryClass &VTOLVehicleDefClass::Get_Factory(void) const { return _VTOLVehicleDefFactory; }
