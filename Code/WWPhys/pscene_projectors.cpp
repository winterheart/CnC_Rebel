...
public:
  ...
  //	void					Reset(); // Jani: Disabling reset. Re-allocating render targets if the
  //device is out of
...
**
************************************************************************************/
StaticShadowTexMgrClass::ShadowTexClass::ShadowTexClass(void) : ObjectTypeID(0), ObjectOrientation(1), Texture(NULL) {}

StaticShadowTexMgrClass::ShadowTexClass::ShadowTexClass(uint32 obj_type_id, const Quaternion &orientation,
                                                        TextureClass *tex)
    : ObjectTypeID(obj_type_id), ObjectOrientation(orientation), Texture(NULL) {
  REF_PTR_SET(Texture, tex);
}

StaticShadowTexMgrClass::ShadowTexClass::ShadowTexClass(const ShadowTexClass &that)
    : ObjectTypeID(0), ObjectOrientation(1), Texture(NULL) {
  *this = that;
}
...
StaticShadowTexMgrClass::ShadowTexClass::~ShadowTexClass(void) { REF_PTR_RELEASE(Texture); }
...
void StaticShadowTexMgrClass::Reset(void) { ShadowTextures.Delete_All(); }
...
void StaticShadowTexMgrClass::Remove_Shadow_Texture(TextureClass *tex) {
  for (int i = 0; i < ShadowTextures.Count(); i++) {
...
DynamicShadowTexMgrClass::DynamicShadowTexMgrClass(void)
    : CurShadow(0), TextureResolution(DEFAULT_DYNAMIC_SHADOW_RESOLUTION) {
...
void DynamicShadowTexMgrClass::Set_Max_Simultaneous_Shadows(unsigned int max) {
  ...
  if (curlen > ShadowTextures.Length())
    curlen = ShadowTextures.Length();
  int i;
  for (i = 0; i < curlen; i++) {
    if (!ShadowTextures[i]) {
...
unsigned int DynamicShadowTexMgrClass::Get_Max_Simultaneous_Shadows(void) { return ShadowTextures.Length(); }
...
unsigned int DynamicShadowTexMgrClass::Get_Shadow_Resolution(void) { return TextureResolution; }

void DynamicShadowTexMgrClass::Per_Frame_Reset(void) { CurShadow = 0; }
...
void PhysicsSceneClass::Set_Shadow_Resolution(unsigned int res) { _DynamicShadowTexMgr.Set_Shadow_Resolution(res); }

unsigned int PhysicsSceneClass::Get_Shadow_Resolution(void) { return _DynamicShadowTexMgr.Get_Shadow_Resolution(); }
...
SpecialRenderInfoClass *PhysicsSceneClass::Get_Shadow_Render_Context(int width, int height) {
  if (ShadowRenderContext == NULL) {
    /*
...
void PhysicsSceneClass::Enable_Static_Projectors(bool onoff) { StaticProjectorsEnabled = onoff; }

bool PhysicsSceneClass::Are_Static_Projectors_Enabled(void) { return StaticProjectorsEnabled; }

void PhysicsSceneClass::Enable_Dynamic_Projectors(bool onoff) { DynamicProjectorsEnabled = onoff; }

bool PhysicsSceneClass::Are_Dynamic_Projectors_Enabled(void) { return DynamicProjectorsEnabled; }
...
PhysicsSceneClass::ShadowEnum PhysicsSceneClass::Get_Shadow_Mode(void) { return ShadowMode; }
...
float PhysicsSceneClass::Get_Shadow_Normal_Intensity(void) { return ShadowNormalIntensity; }
...
float PhysicsSceneClass::Compute_Projector_Attenuation(TexProjectClass *dynamic_projector, const Vector3 &view_pos,
...
void PhysicsSceneClass::Apply_Projectors(const CameraClass &camera) {
...
void PhysicsSceneClass::Apply_Projector_To_Objects(TexProjectClass *tex_proj, const CameraClass &camera) {
  ...
  */
  ...
  DynamicVBAccessClass vb_access(BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type, vertex_count);
  {
    ...
    Vector3 vertices[vertex_count] = {
        ...
        Vector3(-1.0f, -1.0f, 0.0f), Vector3(1.0f, -1.0f, 0.0f),  Vector3(1.0f, 0.0f, 0.0f),

        Vector3(1.0f, -1.0f, 0.0f),  Vector3(1.0f, 1.0f, 0.0f),   Vector3(0.0f, 1.0f, 0.0f)};

    unsigned colors[vertex_count] = {0xff0000, 0xff0000, 0xff0000, 0x00ff00, 0x00ff00,
                                     0x00ff00, 0x0000ff, 0x0000ff, 0x0000ff};
...
static bool Test_Render_Target_Surface(TextureClass *render_target) {
  ...
  BitmapHandlerClass::Read_B8G8R8A8(color1, tmpbits, desc.Format, 1 * desc.Width / 4, 1 * desc.Height / 4, desc.Width,
                                    desc.Height, NULL, 0);

  BitmapHandlerClass::Read_B8G8R8A8(color2, tmpbits, desc.Format, 3 * desc.Width / 4, 1 * desc.Height / 4, desc.Width,
                                    desc.Height, NULL, 0);

  BitmapHandlerClass::Read_B8G8R8A8(color3, tmpbits, desc.Format, 3 * desc.Width / 4, 3 * desc.Height / 4, desc.Width,
                                    desc.Height, NULL, 0);

  BitmapHandlerClass::Read_B8G8R8A8(color4, tmpbits, desc.Format, 1 * desc.Width / 4, 3 * desc.Height / 4, desc.Width,
                                    desc.Height, NULL, 0);

  new_surf->Unlock();
  REF_PTR_RELEASE(new_surf);

  WWDEBUG_SAY(("Render target test: 0x%8.8x, 0x%8.8x, 0x%8.8x, 0x%8.8x\n", color1 & 0x00ffffff, color2 & 0x00ffffff,
               color3 & 0x00ffffff, color4 & 0x00ffffff));
  // color1 is 0x00000000
  if (color1 & 0x00ffffff)
    return false;
  // color2 is 0x000000ff
  if (color2 & 0x00ffff00)
    return false;
  if (!(color2 & 0x000000ff))
    return false;
  // color3 is 0x0000ff00
  if (color3 & 0x00ff00ff)
    return false;
  if (!(color3 & 0x0000ff00))
    return false;
  // color4 is 0x00ff0000
  if (color4 & 0x0000ffff)
    return false;
  if (!(color4 & 0x00ff0000))
    return false;
  return true;
}
...
void PhysicsSceneClass::Generate_Static_Shadow_Projectors(void) {
  ...
  */
  TextureClass *render_target =
      Create_Projector_Render_Target(STATIC_PROJECTOR_RESOLUTION, STATIC_PROJECTOR_RESOLUTION);
  //	TextureClass * render_target =
  //DX8Wrapper::Create_Render_Target(STATIC_PROJECTOR_RESOLUTION,STATIC_PROJECTOR_RESOLUTION);
...
void PhysicsSceneClass::Setup_Static_Directional_Shadow(StaticAnimPhysClass &obj, const Vector3 &light_dir,
                                                        TextureClass *render_target) {
  /*
