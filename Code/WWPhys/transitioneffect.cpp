...
static float STEALTH_ZNEAR_FADE_DISTANCE =
    15.0f;                                 // distance at which the intensity starts to fade due to camera proximity
...
TransitionEffectClass::TransitionEffectClass(void)
    : CurrentParameter(0.0f), TargetParameter(1.0f), ParameterVelocity(1.0f), MinUVRate(0.5f, -0.5f),
      MaxUVRate(2.75f, -3.0f), UVRate(0.0f, 0.0f), StartDelay(0.0f), RemoveOnComplete(false), RenderBaseMaterial(false),
      RenderTransitionMaterial(true), IntensityScale(1.0f), UVOffset(0.0f, 0.0f), MaxIntensity(1.0f), Mapper(NULL),
      MaterialPass(NULL) {
