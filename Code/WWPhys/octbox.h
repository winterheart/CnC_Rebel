...
public:
  ...
  void Get_Inner_Box(OBBoxClass *set_box) {
    *set_box = Get_World_Inner_Box();
    set_box->Extent += Vector3(Thickness, Thickness, Thickness);
  }
...
inline void OctBoxClass::Set_Transform(const Matrix3D &tm) { Transform_To_World_Space(tm); }

inline void OctBoxClass::Set_Transform(const Quaternion &q, const Vector3 &p) { Transform_To_World_Space(q, p); }

inline int OctBoxClass::Get_Contact_Count(void) { return ContactCount; }
...
inline void OctBoxClass::Reset_Contacts(void) { ContactCount = 0; }
