...
void PhysicsSceneClass::Export_Vis_Data(ChunkSaveClass &csave) { VisTableManager.Save(csave); }
  ...
  while (cload.Open_Chunk()) {
    ...
    case PSCENE_SD_CHUNK_VARIABLES:
      while (cload.Open_Micro_Chunk()) {
        switch (cload.Cur_Micro_Chunk_ID()) { READ_MICRO_CHUNK(cload, PSCENE_SD_VARIABLE_AMBIENT, SceneAmbientLight); }
...
void PhysicsSceneClass::Load_LDD_Variables(ChunkLoadClass &cload) {
  while (cload.Open_Micro_Chunk()) {
    ...
    default:
      WWDEBUG_SAY(
          ("Unhandled Micro Chunk: 0x%x in file &s, line %d\n", cload.Cur_Micro_Chunk_ID(), __FILE__, __LINE__));
    }
    cload.Close_Micro_Chunk();
  }
}
