...
#define VERBOSE_LOG(x)                                                                                                 \
  if (/*WWDEBUG_TRIGGER(WWDEBUG_TRIGGER_GENERIC0) &&*/ (strstr(Model->Get_Name(), "lt"))) {                            \
    WWDEBUG_SAY(x);                                                                                                    \
  }
#else
#define VERBOSE_LOG(x)
...
private:
  ...
  public:
    ...
    StateSnapshotClass &operator=(const StateSnapshotClass &that) {
...
Phys3HistoryClass::Phys3HistoryClass(void) : Initialized(false), SnapshotArray(NULL), HeadIndex(0) {
...
void Phys3HistoryClass::StateSnapshotClass::Lerp(const StateSnapshotClass &a, const StateSnapshotClass &b,
                                                 float fraction) {
 ...
 *   9/16/2000  gth : Created.                                                                 *
 *=============================================================================================*/
const Vector3 &Phys3Class::Get_Position(void) const { return State.Position; }
 ...
 *   9/16/2000  gth : Created.                                                                 *
 *=============================================================================================*/
float Phys3Class::Get_Heading(void) const { return Heading; }
 ...
 *   9/16/2000  gth : Created.                                                                 *
 *=============================================================================================*/
float Phys3Class::Get_Slide_Angle(void) const { return SlideAngle; }
...
Phys3Class::GroundStateStruct &Phys3Class::Get_Ground_State(void) {
  ...
  VERBOSE_LOG((" GS: OnGround = %d Normal = %f %f %f\r\n", GroundState.OnGround, GroundState.Normal.X,
               GroundState.Normal.Y, GroundState.Normal.Z));
 ...
 *   9/16/2000  gth : Created.                                                                 *
 *=============================================================================================*/
void Phys3Class::Invalidate_Ground_State(void) { GroundState.IsDirty = true; }
...
void Phys3Class::Check_Ground(const AABoxClass &box, GroundStateStruct *gs, float check_dist) {
  ...
  PhysAABoxCollisionTestClass test(box, Vector3(0, 0, -check_dist), &result, Get_Collision_Group(),
 ...
 *   9/16/2000  gth : Created.                                                                 *
 *=============================================================================================*/
bool Phys3Class::Apply_Move(const Vector3 &input_move, float dt, bool allow_sliding, bool allow_stepping,
  ...
  */
  ...
  while (!done) {
    ...
    */
    ...
    CastResultStruct result;
    PhysAABoxCollisionTestClass test(box, move, &result, Get_Collision_Group(), COLLISION_TYPE_PHYSICAL);
    ...
    if (result.StartBad) {
      ...
      if (result.Fraction == 1.0f) {
        ...
        */
        ...
        VERBOSE_LOG(("Collided with %s. normal=(%f,%f,%f)\r\n", test.CollidedRenderObj->Get_Name(), result.Normal.X,
        ...
        */
        move_len = result.Fraction * move.Length();
        if (move_len > epsilon) {
          move_len -= epsilon;
        ...
        */
        if ((dt < WWMATH_EPSILON) || (move.Length2() < WWMATH_EPSILON) || (allow_sliding == false)) {
          ...
          if (reaction & COLLISION_REACTION_NO_BOUNCE) {
            ...
            */
            ...
            if ((test.CollidedRenderObj != NULL) &&
                (test.CollidedRenderObj->Class_ID() == RenderObjClass::CLASSID_MESH)) {
            ...
            if ((mesh != NULL) && (mesh->Get_W3D_Flags() & W3D_MESH_FLAG_SHATTERABLE) &&
              ...
              PhysicsSceneClass::Get_Instance()->Shatter_Mesh(mesh, State.Position, result.Normal, State.Velocity);
              if (Observer != NULL) {
            ...
            */
            if (clip_move) {
              ...
              if (contact_count < MAX_CONTACTS) {
                ...
                if ((normal.Z < SlideNormalZ) && (normal.Z > 0.0f) && (GroundState.OnGround) &&
                    (GroundState.Normal.Z > SlideNormalZ) && (MoveMode == BALLISTIC_MOVE)) {
                  normal.Z = 0.0f;
                  normal.Normalize();
                }
...
bool Phys3Class::Can_Teleport(const Matrix3D &test_tm, bool check_dyn_only, NonRefPhysListClass *result_list) {
  ...
  //
  PhysAABoxIntersectionTestClass test(collision_box, Get_Collision_Group(), COLLISION_TYPE_PHYSICAL, result_list);
...
bool Phys3Class::Can_Teleport_And_Stand(const Matrix3D &test_tm, Matrix3D *new_tm) {
  ...
  PhysAABoxCollisionTestClass test(collision_box, move_vector, &result, 0, COLLISION_TYPE_PHYSICAL);
 ...
 *   9/16/2000  gth : Created.                                                                 *
 *=============================================================================================*/
bool Phys3Class::Find_Teleport_Location(const Vector3 &start, float radius, Vector3 *out) {
  bool retval = false;
...
bool Phys3Class::Can_Move_To(const Matrix3D &new_tm) {
  ...
  Inc_Ignore_Counter();
  CastResultStruct result;
  PhysAABoxCollisionTestClass test(box, move, &result, 0, COLLISION_TYPE_PHYSICAL);
  PhysicsSceneClass::Get_Instance()->Cast_AABox(test);
  Dec_Ignore_Counter();
...
void Phys3Class::Assert_State_Valid(void) {
  ...
  CastResultStruct result;
  PhysAABoxCollisionTestClass test(box, Vector3(0, 0, 0), &result, 0, COLLISION_TYPE_PHYSICAL);
...
const PersistFactoryClass &Phys3Class::Get_Factory(void) const { return _Phys3Factory; }
...
Phys3Class::GroundStateStruct::GroundStateStruct(void)
    : IsDirty(true), OnGround(false), OnDynamicObj(false), SurfaceType(SURFACE_TYPE_DEFAULT), Height(0.0f),
      Normal(0, 0, 1), Down(1, 0, 0), GroundObject(NULL), GroundRenderObject(NULL) {
  Reset();
}
...
Phys3DefClass::Phys3DefClass(void)
    : NormSpeed(DEFAULT_NORMALIZED_SPEED), SlideAngle(DEFAULT_SLIDE_ANGLE), StepHeight(DEFAULT_STEP_HEIGHT) {
...
const PersistFactoryClass &Phys3DefClass::Get_Factory(void) const { return _Phys3DefFactory; }

uint32 Phys3DefClass::Get_Class_ID(void) const { return CLASSID_PHYS3DEF; }
