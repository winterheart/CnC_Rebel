...
const char *RadarManager::Get_Blip_Shape_Type_Name(int index) {
  static const char *names[NUM_BLIP_SHAPE_TYPES] = {
      "None", "Human", "Vehicle", "Stationary", "Objective",
  };
...
void RadarManager::Init() {
  ...
  if (HUDGlobalSettingsDef::Get_Instance() != NULL) {
    ...
    BlipColors[BLIP_COLOR_TYPE_PRIMARY_OBJECTIVE] =
        HUDGlobalSettingsDef::Get_Instance()->Get_Primary_Objective_Color().Convert_To_ARGB();
    BlipColors[BLIP_COLOR_TYPE_SECONDARY_OBJECTIVE] =
        HUDGlobalSettingsDef::Get_Instance()->Get_Secondary_Objective_Color().Convert_To_ARGB();
    BlipColors[BLIP_COLOR_TYPE_TERTIARY_OBJECTIVE] =
        HUDGlobalSettingsDef::Get_Instance()->Get_Tertiary_Objective_Color().Convert_To_ARGB();
  }
...
float RadarManager::Add_Blip(const Vector3 &pos, int shape_type, int color_type, float intensity, bool bracket,
                             bool altitude_fade) {
...
void RadarManager::Update(const Matrix3D &player_tm, const Vector2 &center) {
  ...
  Renderer->Add_Quad_Backfaced(draw.Lower_Right(), draw.Lower_Left(), draw.Upper_Right(), draw.Upper_Left(), uv,
                               RadarColor);
...
//	compass += Render2DClass::Get_Screen_Resolution().Lower_Left() + settings->RadarCompassOffset;
  ...
  if (!CompassRenderers[CurrentCompassRendererIndex]) {
    ...
    Vector2 text_size =
        CompassRenderers[CurrentCompassRendererIndex]->Get_Text_Extents(TRANSLATE(dir[CurrentCompassRendererIndex]));
...

  {
    ...
    for (objnode = GameObjManager::Get_Game_Obj_List()->Head(); objnode; objnode = objnode->Next()) {
      ...
      if (obj) {
        if (obj == COMBAT_STAR)
          continue; // Don't draw the star
        if (obj->Peek_Physical_Object() == NULL)
          continue;

        // Don't draw dead soldiers
        if (obj->As_SoldierGameObj() && obj->Get_Defense_Object() && obj->Get_Defense_Object()->Get_Health() <= 0.0f) {
          continue;
        }
        ...
        if (COMBAT_STAR != NULL && RadarMode != RADAR_ALL) {
          if (RadarMode == RADAR_NOBODY) {
            ...
            if (my_pt != other_pt || (my_pt != PLAYERTYPE_NOD && my_pt != PLAYERTYPE_GDI)) {
              continue;
            }
          }
        }
        ...
        //			if ( (WWMath::Fabs( z_diff ) < RADAR_Z_RANGE) || (obj->Get_Radar_Blip_Shape_Type() ==
        //BLIP_SHAPE_TYPE_OBJECTIVE) ) {
        float intensity = obj->Get_Radar_Blip_Intensity();
        bool bracket = obj == BracketObj;
        intensity = Add_Blip(objpos, obj->Get_Radar_Blip_Shape_Type(), obj->Get_Radar_Blip_Color_Type(), intensity,
                             bracket, altitude_fade);
#if 0
...

  {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (objective->DrawBlip && objective->Status == ObjectiveManager::STATUS_IS_PENDING) {
        ...
        intensity = Add_Blip(objective->Position, BLIP_SHAPE_TYPE_OBJECTIVE, objective->Radar_Blip_Color_Type(),
                             intensity, false);
#if 0
			intensity -= RadarSweepMove / DEG_TO_RAD( 360 ) * 0.666f;				// Drop Blip intensity
#endif
