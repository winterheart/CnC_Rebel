...
void Register_Action_Code_Deletion(ActionCodeClass *action) { ActionDeleteList.Add(action); }
...
SimplePersistFactoryClass<FollowInputActionCodeClass, CHUNKID_ACTION_CODE_FOLLOW_INPUT>
    _FollowInputActionCodeClassFactory;
  ...
  */
  ...
  virtual ActResult Act(void) {
    ...
    if ((obj->As_SoldierGameObj() != NULL) && (obj->As_SoldierGameObj()->Get_Vehicle() != NULL)) {
      amount =
          Input::Get_Amount(INPUT_FUNCTION_VEHICLE_TURN_LEFT) - Input::Get_Amount(INPUT_FUNCTION_VEHICLE_TURN_RIGHT);
    } else {
    ...
    float forward_amount =
        Input::Get_Amount(INPUT_FUNCTION_MOVE_FORWARD) - Input::Get_Amount(INPUT_FUNCTION_MOVE_BACKWARD);
    ...
    obj->Set_Boolean_Control(ControlClass::BOOLEAN_VEHICLE_TOGGLE_GUNNER,
                             Input::Get_State(INPUT_FUNCTION_VEHICLE_TOGGLE_GUNNER));
    ...
    obj->Set_Boolean_Control(ControlClass::BOOLEAN_WEAPON_FIRE_PRIMARY,
                             Input::Get_State(INPUT_FUNCTION_FIRE_WEAPON_PRIMARY));
    obj->Set_Boolean_Control(ControlClass::BOOLEAN_WEAPON_FIRE_SECONDARY,
                             Input::Get_State(INPUT_FUNCTION_FIRE_WEAPON_SECONDARY));
...
SimplePersistFactoryClass<PlayAnimationActionCodeClass, CHUNKID_ACTION_CODE_PLAY_ANIMATION>
    _PlayAnimationActionCodeClassFactory;
...
public:
  ...
  */
  ...
  SmartGameObj *Peek_Transport_Object(SmartGameObj *game_obj) {
    ...
    if (game_obj->As_SoldierGameObj() != NULL && game_obj->As_SoldierGameObj()->Get_Profile_Vehicle() != NULL) {
  ...
  void Human_Move_To_Relative(Vector3 &orig_rel_pos) {
    ...
    if (soldier && soldier->Is_On_Ladder()) {
      obj->Set_Analog_Control(ControlClass::ANALOG_MOVE_FORWARD,
                              (move[2] >= 0) ? 1 : -1); // Could we make this move up?
    }
  }
  ...
  bool Move_To_Absolute(const Vector3 &abs_pos, bool ignore_arrived_dist = false) {
    ...
    //
    if (!IsAttacking && !Action->Get_Parameters().ForceFacing && !Action->Get_Parameters().IgnoreFacing && soldier &&
...
public:
  ...
  GotoActionCodeClass(void) : UseMoveObject(false), FirstCall(true), PathSolver(NULL), Path(NULL) {}

  ~GotoActionCodeClass(void) { return; }
  ...
  */
  ...
  virtual bool Is_Busy(void) { return (PathAction.Get_State() != PathActionClass::STATE_FINISHED); }
  ...
  bool Clip_Point(Vector3 *point, const AABoxClass &box) {
  ...
  void Update_Move_Location(void) {
    ...
    if (target == NULL) { // If the target is gone
                          ...
                          //			Move_Target_Gone();
                          //			Action->Reset( Action->Get_Parameters().Priority );		// Stop current Command
                          //(this may get this deleted!)
      // Do we need to notify observers?
      //			return ACTION_DONE;
      return;
    }

    //		Action->Get_Parameters().MoveLocation = target->Get_Transform() *
    //Action->Get_Parameters().MoveObjectOffset;
    ...
    if (FirstCall == false) {
      ...
      if (to_target_dist > 0 && (soldier == NULL || (soldier->Is_Airborne() == false &&
  ...
  */
  ...
  virtual ActResult Act(void) {
    ...
    if (WWMath::Fabs(rel_pos.X) > WWMath::Fabs(rel_pos.Y)) {
      obj->Set_Boolean_Control(rel_pos.X > 0 ? ControlClass::BOOLEAN_DIVE_FORWARD
                                             : ControlClass::BOOLEAN_DIVE_BACKWARD);
    } else {
...
public:
  ...
  AttackActionCodeClass(void)
      : UseAttackObject(false), WanderPos(0, 0, 0), WanderTimer(0), HasArrived(false), HoldPrimaryTriggerTimer(0) {
  ...
  */
  ...
  void Attack_Absolute(Vector3 &abs_pos) {
    WWPROFILENAMED(attack_absolute, "Attack Absolute");
    IsAttacking = true;
    ...
    float error_angle = WWMath::Fabs(Action->Get_Parameters().AttackError) + WWMath::Fabs(obj->Get_Weapon_Error());
...
SimplePersistFactoryClass<FaceLocationActionCodeClass, CHUNKID_ACTION_CODE_FACE_LOCATION>
    _FaceLocationActionCodeClassFactory;
...
public:
  ...
  FaceLocationActionCodeClass(void) : EndTime(0) {}
  ...
  */
  ...
  virtual void Init(ActionClass *action) {
    ...
    EndTime =
        TimeManager::Get_Seconds() + MAX(action->Get_Parameters().FaceDuration, Action->Get_Parameters().LookDuration);
  }
...
SimplePersistFactoryClass<ConversationActionCodeClass, CHUNKID_ACTION_CODE_CONVERSATION>
    _ConversationActionCodeClassFactory;
...
public:
  ...
  enum { STATE_WALKING_TO_START_POS = 0, STATE_HAVING_CONVERSATION, STATE_FINISHED };

  ConversationActionCodeClass(void) : Conversation(NULL), State(STATE_WALKING_TO_START_POS), OriginalPos(0, 0, 0) {}

  ~ConversationActionCodeClass(void) { return; }
    ...
    if (obj != NULL) {
      ...
      if (soldier != NULL) {
        ...
        if (action->Get_Parameters().ActiveConversationID != 0) {
          ...
          //
          Conversation =
              ConversationMgrClass::Start_Conversation(soldier, action->Get_Parameters().SafeConversationName);
        }
...
public:
  ...
  enum { STATE_DRIVING_TO_ENTRANCE = 0, STATE_DRIVING_TO_SIDE, STATE_BACKING_INTO_BAY };

  DockActionCodeClass(void) : State(STATE_DRIVING_TO_ENTRANCE) {}

  ~DockActionCodeClass(void) { return; }
  ...
  */
  ...
  virtual ActResult Arrived(void) {
    ...
    if (State == STATE_DRIVING_TO_ENTRANCE) {
      ...
      test_waypath->Add_Point(params.DockLocation + (params.DockEntrance - params.DockLocation) * 0.7F -
                              (side_vector * turn_radius * 0.6F));
...
**
*/
ActionClass::ActionClass(SmartGameObj *obj) : ActionCode(NULL), ActCount(0), IsPaused(false) {
  WWASSERT(obj != NULL);
  ActionObj = obj;
}
...
bool ActionClass::Is_Acting(void) { return (ActionCode != NULL); }
...
bool ActionClass::Is_Active(void) { return (ActionCode != NULL); }
