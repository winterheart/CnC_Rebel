...
WeaponDefinitionClass::WeaponDefinitionClass(void)
    : Style(1), SwitchTime(0), ReloadTime(0), KeyNumber(0), CanSnipe(false), CanReceiveGenericCnCAmmo(true),
      //	LegacyFireSoundDefID( 0 ),
      EjectPhysDefID(0), MuzzleFlashPhysDefID(0), Rating(0.1f), FirstPersonOffset(0, 0, 0), RecoilImpulse(0.0f),
      ReloadSoundDefID(0), EmptySoundDefID(0), PrimaryAmmoDefID(0), SecondaryAmmoDefID(0), ClipSize(0),
      RecoilTime(0.1f), RecoilScale(1.0f), AGiveWeaponsWeapon(false), MaxInventoryRounds(100), IconNameID(0),
      IconTextureUV(0, 0, 0, 0), IconOffset(0, 0) {
    ...
    case CHUNKID_WEAPON_DEF_VARIABLES:
      while (cload.Open_Micro_Chunk()) {
        switch (cload.Cur_Micro_Chunk_ID()) {
          ...
          //						READ_MICRO_CHUNK(				cload,
          //LEGACY_MICROCHUNKID_WEAPON_DEF_FIRE_SOUND_DEFID,	LegacyFireSoundDefID );
...
AmmoDefinitionClass::AmmoDefinitionClass(void)
    : Warhead(0), Damage(1), Range(10), EffectiveRange(10), Velocity(1), Gravity(0), Elasticity(1.0f), RateOfFire(1),
      SprayAngle(0), SprayCount(1), AquireTime(0), BurstDelayTime(0), BurstMax(0), SoftPierceLimit(0), TurnRate(0),
      TimeActivated(false), TerrainActivated(false), IsTracking(false),
      //	WeaponDefID( 0 ),
      ExplosionDefID(0), RandomTrackingScale(0), DisplayLaser(false), ChargeTime(0), ContinuousSoundDefID(0),
      FireSoundDefID(0), MaxBounces(0), SprayBulletCost(1), AmmoType(AMMO_TYPE_NORMAL), BeaconDefID(0),
      C4TriggerTime1(0), C4TriggerTime2(0), C4TriggerTime3(0), C4TriggerRange1(0), C4TriggerRange2(0),
      C4TriggerRange3(0), C4TimingSound1ID(0), C4TimingSound2ID(0), C4TimingSound3ID(0), AliasedSpeed(0),
      HitterType(SurfaceEffectsManager::HITTER_TYPE_BULLET), BeamEnabled(false), BeamColor(1, 1, 1), BeamTime(0.3f),
      BeamWidth(0.25f), BeamEndCaps(true), BeamSubdivisionEnabled(false), BeamSubdivisionScale(1.0f),
      BeamSubdivisionFrozen(false), IconNameID(0), IconTextureUV(0, 0, 0, 0), IconOffset(0, 0), GrenadeSafetyTime(0) {
#ifdef PARAM_EDITING_ON
  int i;
  EnumParameterClass *param;
...
bool AmmoDefinitionClass::Save(ChunkSaveClass &csave) {
  ...
  //		WRITE_MICRO_CHUNK(			 csave, 	MICROCHUNKID_AMMO_DEF_WEAPON_DEF_ID,
  //WeaponDefID );
    ...
    case CHUNKID_AMMO_DEF_VARIABLES:
      while (cload.Open_Micro_Chunk()) {
        switch (cload.Cur_Micro_Chunk_ID()) {
          ...
          //						READ_MICRO_CHUNK(				cload,
          //MICROCHUNKID_AMMO_DEF_WEAPON_DEF_ID,			WeaponDefID );
