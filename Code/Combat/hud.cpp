...
static void Powerup_Add(const WCHAR *name, int number, const char *texture_name, const RectClass &uv,
                        const Vector2 &offset, bool right_list = true) {
...
void HUDClass::Force_Weapon_Chart_Update(void) { ForceChartRebuild = true; }

void HUDClass::Force_Weapon_Chart_Display(void) { WeaponChartTimer = WEAPON_CHART_TIME; }
  ...
  for (int column = 1; column <= 10; column++) {
    ...
    for (i = 0; i < weapon_bag->Get_Count(); i++) {
      ...
      if (weapon != NULL) {
        ...
        //				if ( weapon->Is_Reloading() || weapon->Is_Switching() ||
        //weapon->Get_Total_Rounds() == 0 ) { 				if ( weapon->Is_Switching() || weapon->Get_Total_Rounds() == 0 ) {
...
static void Damage_Add_Indicator(int index, float start_x, float start_y, float end_x, float end_y) {
  ...
  case 3:
  case 4:
    DamageRenderer->Add_Quad(vert[1], vert[3], vert[0], vert[2], uv, color);
    break;
  case 5:
  case 6:
    DamageRenderer->Add_Quad(vert[0], vert[1], vert[2], vert[3], uv, color);
    break;
  case 7:
  case 0:
    DamageRenderer->Add_Quad(vert[2], vert[0], vert[3], vert[1], uv, color);
    break;
  case 1:
  case 2:
    DamageRenderer->Add_Quad(vert[3], vert[2], vert[1], vert[0], uv, color);
    break;
  }
}
...
static void Damage_Update(void) {
  ...
  if (!DamageIndicatorIntensityChanging && DamageIndicatorOrientation == CombatManager::Is_First_Person()) {
    return;
  }
  ...
  Damage_Add_Indicator(0, -HORIZ_WIDTH / 2, -OFFSET_Y - HORIZ_HEIGHT / 2, HORIZ_WIDTH / 2,
                       -OFFSET_Y + HORIZ_HEIGHT / 2);
  ...
  Damage_Add_Indicator(1, DIAG_OFFSET_X - DIAG_WIDTH / 2, -DIAG_OFFSET_Y - DIAG_HEIGHT / 2,
                       DIAG_OFFSET_X + DIAG_WIDTH / 2, -DIAG_OFFSET_Y + DIAG_HEIGHT / 2);
  Damage_Add_Indicator(3, DIAG_OFFSET_X - DIAG_WIDTH / 2, DIAG_OFFSET_Y - DIAG_HEIGHT / 2,
                       DIAG_OFFSET_X + DIAG_WIDTH / 2, DIAG_OFFSET_Y + DIAG_HEIGHT / 2);
  Damage_Add_Indicator(5, -DIAG_OFFSET_X - DIAG_WIDTH / 2, DIAG_OFFSET_Y - DIAG_HEIGHT / 2,
                       -DIAG_OFFSET_X + DIAG_WIDTH / 2, DIAG_OFFSET_Y + DIAG_HEIGHT / 2);
  Damage_Add_Indicator(7, -DIAG_OFFSET_X - DIAG_WIDTH / 2, -DIAG_OFFSET_Y - DIAG_HEIGHT / 2,
                       -DIAG_OFFSET_X + DIAG_WIDTH / 2, -DIAG_OFFSET_Y + DIAG_HEIGHT / 2);
}

void HUDClass::Damage_Render(void) { DamageRenderer->Render(); }
...
static void Target_Update(void) {
  ...
  if (d_obj != NULL) {
    ...
    if (obj && COMBAT_STAR) {
      ...
      if (obj->As_SmartGameObj() != NULL && obj->As_SmartGameObj()->As_VehicleGameObj() != NULL &&
      ...
      if (draw_chevrons) {
        ...
        enterable_box += Vector2(box.Center().X - enterable_box.Center().X, box.Top - enterable_box.Bottom);
...
static RectClass Get_Target_Box(PhysicalGameObj *obj) {
  ...
  if (po != NULL) {
    ...
    obj_to_boxview_tm.Transform_Center_Extent_AABox(obj_box.Center, obj_box.Extent, &boxview_box.Center,
                                                    &boxview_box.Extent);
    ...
    boxview_to_camera_tm.Transform_Center_Extent_AABox(boxview_box.Center, boxview_box.Extent, &camera_box.Center,
                                                       &camera_box.Extent);
  ...
  info_box.Set(top.X * screen.Right, top.Y * screen.Bottom, bottom.X * screen.Right,
               bottom.Y * screen.Bottom); // convention 0,0 - 1,1
  ...
  if (rebuild || ObjectiveManager::Are_HUD_Objectives_Changed()) {
    ...
    for (int i = count - 1; i >= 0; i--) {
      ...
      if (renderer) {
        ...
        if (age < POG_FLY_TIME) {
          Vector2 offset =
              Render2DClass::Get_Screen_Resolution().Center() - Render2DClass::Get_Screen_Resolution().Upper_Right();
          offset.X *= 0.75f;
          ...
          if (renderer) {
            ...
            offset = star_fly_start - star_fly_end;
            offset *= fly;
            star_box += offset;
...
static void Info_Editor_Update(void) {
  ...
  if (changed) {
    Debug_Say(("Vector2 %s( %d, %d );\n", InfoEditorFieldList[index].Name, (int)InfoEditorFieldList[index].Value->U,
               (int)InfoEditorFieldList[index].Value->V));
  }
}

static void Info_Editor_Shutdown(void) {
  while (InfoEditorFieldList.Count() > 0) {
    Debug_Say(("Vector2 %s( %d, %d );\n", InfoEditorFieldList[0].Name, (int)InfoEditorFieldList[0].Value->U,
               (int)InfoEditorFieldList[0].Value->V));
...
void HUDClass::Init(bool render_available) {
  ...
  RenderImages[DEMO_HUD_IMAGE]->Add_Quad(RectClass(float(8), float(-8), float(256 + 8), float(128 - 8)),
                                         RectClass(0.0f, 0.0f, 1.0f, 0.5f), 0xffffffff);
  ...
  RenderImages[DEMO_HUD_IMAGE2]->Add_Quad(RectClass(rect.Right + float(-256 - 8), rect.Bottom + float(-128 + 8),
                                                    rect.Right + float(-8), rect.Bottom + float(8)),
                                          RectClass(0.0f, 0.5f, 1.0f, 1.0f), 0xffffffff);
#endif
...
static bool Is_HUD_Displayed(void) {
  return (_HUDEnabled && COMBAT_STAR && !COMBAT_STAR->Is_Dead() && !COMBAT_STAR->Is_Destroyed());
}
...
void HUDClass::Think() {
  ...
  if (HUDInfo::Display_Action_Status_Bar()) {
    ...
    RenderImages[ACTION_STATUSBAR_RENDERER]->Add_Line(status_bar_rect.Upper_Left(), status_bar_rect.Upper_Right(), 1,
                                                      0xFFFFFFFF);
    RenderImages[ACTION_STATUSBAR_RENDERER]->Add_Line(status_bar_rect.Upper_Right(), status_bar_rect.Lower_Right(), 1,
                                                      0xFFFFFFFF);
    RenderImages[ACTION_STATUSBAR_RENDERER]->Add_Line(status_bar_rect.Lower_Right(), status_bar_rect.Lower_Left(), 1,
                                                      0xFFFFFFFF);
    RenderImages[ACTION_STATUSBAR_RENDERER]->Add_Line(status_bar_rect.Lower_Left(), status_bar_rect.Upper_Left(), 1,
                                                      0xFFFFFFFF);
  ...
  RenderImages[RETICLE]->Add_Quad(RectClass(reticle_offset.X - RETICLE_WIDTH / 2, reticle_offset.Y - RETICLE_HEIGHT / 2,
                                            reticle_offset.X + RETICLE_WIDTH / 2,
                                            reticle_offset.Y + RETICLE_HEIGHT / 2),
                                  reticle_color);
  ...
  if (CombatManager::Is_Gameplay_Permitted() && (weapon != NULL) && CombatManager::Is_Hit_Reticle_Enabled()) {
    ...
    RenderImages[RETICLE_HIT]->Add_Quad(
        RectClass(reticle_hit_offset.X - RETICLE_WIDTH / 2, reticle_hit_offset.Y - RETICLE_HEIGHT / 2,
                  reticle_hit_offset.X + RETICLE_WIDTH / 2, reticle_hit_offset.Y + RETICLE_HEIGHT / 2),
        reticle_color);
...
bool HUDClass::Is_Enabled(void) { return _HUDEnabled; }

void HUDClass::Enable(bool enable) { _HUDEnabled = enable; }
...
void HUDClass::Add_Powerup_Weapon(int id, int rounds) {
  ...
  if (def) {
    if (!def->IconTextureName.Is_Empty()) {
      Powerup_Add(TranslateDBClass::Get_String(def->IconNameID), rounds, def->IconTextureName, def->IconTextureUV,
                  def->IconOffset);
    }
  }
}
...
void HUDClass::Add_Shield_Grant(float strength) {
  ...
  Powerup_Add(TranslateDBClass::Get_String(IDS_Power_up_Armor_00), (int)strength, texture_name, RectClass(0, 0, 64, 64),
              Vector2(10, 40));
}
...
void HUDClass::Add_Health_Grant(float amount) {
  ...
  Powerup_Add(TranslateDBClass::Get_String(IDS_Power_up_Health_00), (int)amount, texture_name, RectClass(0, 0, 64, 64),
              Vector2(10, 40));
}

void HUDClass::Add_Shield_Upgrade_Grant(float strength) {
  Powerup_Add(TranslateDBClass::Get_String(IDS_Power_up_Armor_Upgrade), (int)strength, "hud_armedal.tga",
              RectClass(0, 0, 64, 64), Vector2(10, 40), false);
}

void HUDClass::Add_Health_Upgrade_Grant(float amount) {
  Powerup_Add(TranslateDBClass::Get_String(IDS_Power_up_Health_Upgrade), (int)amount, "hud_hemedal.tga",
              RectClass(0, 0, 64, 64), Vector2(10, 40), false);
}
...
void HUDClass::Add_Key_Grant(int key) {
  ...
  Powerup_Add(TranslateDBClass::Get_String(IDS_Power_up_SecurityCard), 0, texture_name, RectClass(0, 0, 64, 64),
              Vector2(10, 40), false);
}

void HUDClass::Add_Objective(int type) {
  if (type == ObjectiveManager::TYPE_PRIMARY) {
    Powerup_Add(TranslateDBClass::Get_String(IDS_Enc_Obj_Priority_0_Primary), 0, "p_eva1.tga", RectClass(0, 0, 64, 64),
                Vector2(10, 40), false);
  } else if (type == ObjectiveManager::TYPE_SECONDARY) {
    Powerup_Add(TranslateDBClass::Get_String(IDS_Enc_Obj_Priority_0_Secondary), 0, "p_eva2.tga",
                RectClass(0, 0, 64, 64), Vector2(10, 40), false);
  }
}
...
void HUDClass::Add_Data_Link(void) {
  ...
  last = cur;
  Powerup_Add(TranslateDBClass::Get_String(IDS_Power_up_DataDisc_01), 0, "hud_cd_rom.tga", RectClass(0, 0, 64, 64),
              Vector2(10, 40), false);
}
...
void HUDClass::Add_Map_Reveal(void) {
  Add_Data_Link();
  //	Powerup_Add( TranslateDBClass::Get_String(IDS_Power_up_DataDisc_01),  0, "hud_cd_rom.tga", RectClass( 0,0,64,64
  //),	Vector2( 10,40 ), false ); 	Powerup_Add( TranslateDBClass::Get_String(IDS_Enc_Pow_Bonus_Map_Name),  0,
  //"hud_cd_rom.tga", RectClass( 0,0,64,64 ),	Vector2( 10,40 ), false );
}
