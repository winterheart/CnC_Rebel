...
char Button_State_Table[4] = {0, DirectInput::DI_BUTTON_HIT | DirectInput::DI_BUTTON_HELD,
                              DirectInput::DI_BUTTON_RELEASED, DirectInput::DI_BUTTON_HELD};
...
typedef HRESULT(WINAPI *DirectInput8CreateType)(HINSTANCE hinst, DWORD dwVersion, REFIID riidltf, LPVOID *ppvOut,
                                                LPUNKNOWN punkOuter);
...
void DirectInput::Init(void) {
  ...
  if (DirectInputLibrary != NULL) {
    ...
    if (DirectInput8CreatePtr) {
      ...
      if FAILED (hr) {
        Debug_Say(("DirectInput %x not available, trying version %x\n", DIRECTINPUT_VERSION / 0x100,
                   MINIMUM_DIRECTINPUT_VERSION / 0x100));
        hr = DirectInput8CreatePtr(ProgramInstance, MINIMUM_DIRECTINPUT_VERSION, IID_IDirectInput8, (void **)&DIObject,
                                   NULL);
        if FAILED (hr) {
  ...
  if (DIMouseDevice != NULL) {
    ...
    hr = DIMouseDevice->SetCooperativeLevel(MainWindow, DISCL_EXCLUSIVE | DISCL_FOREGROUND);
    WWASSERT(!FAILED(hr));
    /**/
...
int PASCAL InitJoystick(LPCDIDEVICEINSTANCE pdinst, LPVOID pvRef) {
  ...
  if (DIJoystickDevice == NULL) {
    ...
    if (!FAILED(hr)) {
      hr = temp->QueryInterface(IID_IDirectInputDevice2, (LPVOID *)&DIJoystickDevice);
...
void DirectInput::ReadKeyboard(void) {
  ...
  while (!done) {
    ...
    hr = DIKeyboardDevice->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), &input_buffer, &buffer_size, 0);
...
void DirectInput::ReadMouse(void) {
  ...
  while (!done) {
    ...
    hr = DIMouseDevice->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), &input_buffer, &buffer_size, 0);
...
retry_joystick:
  ...
  DIJoystickButtons[0] =
      Button_State_Table[((DIJoystickButtons[0] & 1) << 1) + ((DIJoystickState.rgbButtons[0] & 0x80) ? 1 : 0)];
  DIJoystickButtons[1] =
      Button_State_Table[((DIJoystickButtons[1] & 1) << 1) + ((DIJoystickState.rgbButtons[1] & 0x80) ? 1 : 0)];
}
...
**
*/
long DirectInput::Get_Joystick_Axis_State(JoystickAxis axis) { return ((long *)&DIJoystickState.lX)[axis]; }
