...
WeaponClass::WeaponClass(const WeaponDefinitionClass *def)
    : Definition(NULL), PrimaryAmmoDefinition(NULL), SecondaryAmmoDefinition(NULL), Model(NULL), State(STATE_IDLE),
      StateTimer(0.0f), UpdateModel(WEAPON_MODEL_UPDATE_IS_NEEDED), NextAnimState(WEAPON_ANIM_NOT_FIRING),
      CurrentAnimState(WEAPON_ANIM_NOT_FIRING), LastFrameIsPrimaryTriggered(false),
      LastFrameIsSecondaryTriggered(false), IsPrimaryTriggered(false), IsSecondaryTriggered(false), TotalRoundsFired(0),
      ClipRounds(0), InventoryRounds(0), BurstDelayTimer(0), BurstCount(0), BulletBumpTime(0), DidFire(false),
      ContinuousEmitters(NULL), ContinuousSound(NULL), C4DetonationMode(1), Target(0, 0, 0), FiringSound(NULL),
      FiringSoundDefID(0), WeaponExists(true), SafetySet(false), LockTriggers(false), EmptySoundTimer(0) {
  if (def != NULL) {
    Init(def);
  }
}
...
void WeaponClass::Set_Owner(ArmedGameObj *owner) { Owner = owner; }

void WeaponClass::Set_Target_Object(PhysicalGameObj *obj) { TargetObject = obj; }
...
bool WeaponClass::Is_Ammo_Maxed(void) {
  if (InventoryRounds == -1)
    return true;

  // Special case for C4
  if (((int)Definition->MaxInventoryRounds == 0) && (ClipRounds == 0))
    return false;
...
void WeaponClass::Do_Fire(bool primary) {
  ...
  // Stats
  if (Get_Owner() && Get_Owner()->As_SoldierGameObj() && Get_Owner()->As_SoldierGameObj()->Get_Player_Data()) {
    Get_Owner()->As_SoldierGameObj()->Get_Player_Data()->Stats_Add_Shot_Fired();

    Get_Owner()->As_SoldierGameObj()->Get_Player_Data()->Stats_Add_Weapon_Fired(this->Get_Definition()->Get_ID());
  }
...
void WeaponClass::Fire_Bullet(const AmmoDefinitionClass *ammo_def, bool primary) {
  ...
  if (Get_Owner() == COMBAT_STAR) {
    ...
    DIAG_LOG(("WFRD", "%1.2f; %1.2f; %1.2f; %s; %d; %d", pos.X, pos.Y, pos.Z, Get_Definition()->Get_Name(), !primary,
              Get_Total_Rounds()));
  }
...
#define READY_TIME 25.0f
  ...
  if (State == STATE_START_SWITCH)
    StateTimer = SWITCH_TIME / 2.0f;
  if (State == STATE_END_SWITCH)
    StateTimer = SWITCH_TIME / 2.0f;
  if (State == STATE_RELOAD)
    StateTimer = (float)RELOAD_TIME;
  if (State == STATE_READY)
    StateTimer = READY_TIME; // delay til IDLE
  if (State == STATE_FIRE_PRIMARY)
    StateTimer = 1.0f / PrimaryAmmoDefinition->RateOfFire;
  if (State == STATE_FIRE_SECONDARY)
    StateTimer = 1.0f / SecondaryAmmoDefinition->RateOfFire;
  if (State == STATE_CHARGE)
    StateTimer = PrimaryAmmoDefinition->ChargeTime; // Charge Time
...
void WeaponClass::Update_State(float pending_time) {
  ...
  while (pending_time > 0.0f) {
    ...
    if (Get_Style() == WEAPON_HOLD_STYLE_BEACON && Get_Owner() && Get_Owner()->As_SoldierGameObj() &&
    ...
    if ((State == STATE_READY) || (State == STATE_IDLE)) {
      if ((IsPrimaryTriggered || IsSecondaryTriggered) && (BurstCount != 0) && CombatManager::Is_Gameplay_Permitted() &&
          trigger_ok) {
        if (SafetySet) {
          LockTriggers = true;
        } else {
            ...
            if (Definition->EmptySoundDefID != 0) {
              ...
              if (EmptySoundTimer <= 0) {
                ...
                AudibleSoundClass *sound =
                    WWAudioClass::Get_Instance()->Create_Sound(Definition->EmptySoundDefID, owner_ref);
                if (sound != NULL) {
...
void WeaponClass::Update(void) {
  ...
  if (Get_Owner() && Get_Owner()->As_SoldierGameObj() && Get_Owner()->As_SoldierGameObj()->Is_In_Vehicle()) {
    IsPrimaryTriggered = false;
    IsSecondaryTriggered = false;
  }

  if (Get_Style() == WEAPON_HOLD_STYLE_C4 || Get_Style() == WEAPON_HOLD_STYLE_BEACON || Get_Can_Snipe()) {
    IsSecondaryTriggered = false;
  }
  ...
  if ((IsPrimaryTriggered == false && LastFrameIsPrimaryTriggered == true) ||
    ...
    if (State == STATE_START_SWITCH || State == STATE_END_SWITCH || State == STATE_RELOAD || Get_Clip_Rounds() == 0) {
      emitters_on = false;
    }
...
*/
bool WeaponClass::Is_Reload_OK(void) {
...
MuzzleFlashClass::MuzzleFlashClass(void)
    : MuzzleA0Bone(0), MuzzleA1Bone(0), Rotation(0), Model(0), LastFlashA0(true), LastFlashA1(true) {}

MuzzleFlashClass::~MuzzleFlashClass(void) { REF_PTR_RELEASE(Model); }
