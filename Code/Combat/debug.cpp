...
void wwdebug_message_handler(DebugType type, const char *message) {
  ...
  */
  if (!DebugManager::Is_Type_Enabled((DebugManager::DebugType)(1 << type)))
    return;
...
bool wwdebug_trigger_handler(int trigger_num) {
  ...
  switch (trigger_num) {
  case ' ':
    return Input::Get_State(INPUT_FUNCTION_DEBUG_SINGLE_STEP_STEP);
  case 'S':
    return Input::Get_State(INPUT_FUNCTION_DEBUG_SINGLE_STEP);

  case WWDEBUG_TRIGGER_GENERIC0:
    return Input::Get_State(INPUT_FUNCTION_DEBUG_GENERIC0);
  case WWDEBUG_TRIGGER_GENERIC1:
    return Input::Get_State(INPUT_FUNCTION_DEBUG_GENERIC1);
  case WWPHYS_TRIGGER_COLLISION_DEBUGGING:
    return DebugManager::Option_Is_Enabled(
        DebugManager::DEBUG_COLLISION_MESSAGES); // Input::Get_State( INPUT_FUNCTION_DEBUG_COLLISION_MESSAGES );
  case WWPHYS_TRIGGER_COLLISION_DISPLAY:
    return DebugManager::Option_Is_Enabled(
        DebugManager::DEBUG_COLLISION_DISPLAY); // Input::Get_State( INPUT_FUNCTION_DEBUG_SHOW_COLLISIONS );
  case WWPHYS_TRIGGER_INVERT_VIS:
    return DebugManager::Option_Is_Enabled(
        DebugManager::DEBUG_INVERT_VIS); // Input::Get_State( INPUT_FUNCTION_DEBUG_VIS_INVERT );
  case WWPHYS_TRIGGER_DISABLE_VIS:
    return DebugManager::Option_Is_Enabled(
        DebugManager::DEBUG_DISABLE_VIS); // Input::Get_State( INPUT_FUNCTION_DEBUG_VIS_DISABLE );
  case WW3D_TRIGGER_PROCESS_STATS:
    return DebugManager::Option_Is_Enabled(
        DebugManager::DEBUG_PROCESS_STATS); // Input::Get_State( INPUT_FUNCTION_DEBUG_RENDER_STATS );
  case WW3D_TRIGGER_RENDER_STATS:
    return false; // Input::Get_State( INPUT_FUNCTION_DEBUG_RENDER_STATS );
  case WW3D_TRIGGER_SURFACE_CACHE_STATS:
    return DebugManager::Option_Is_Enabled(
        DebugManager::DEBUG_SURFACE_CACHE); // Input::Get_State( INPUT_FUNCTION_DEBUG_SURFACE_CACHE );
  default:
...
/*
** List of addresses to 'watch'. You can stuff them in here and compile or manually poke them in with the debugger at
*run time.
*/
#define NUM_WATCH_ADDRESSES 8
unsigned long WatchAddresses[NUM_WATCH_ADDRESSES] = {0x00000000, 0x00000000, 0x00000000, 0x00000000,
...
static char const *ImagehelpFunctionNames[] = {
    ...
    "SymUnloadModule", "StackWalk",         "SymFunctionTableAccess", "SymGetModuleBaseType", NULL};
...
here:
  ...
  */
  for (int i = 0; i < num_addresses + 1; i++) {
    if (_StackWalk(IMAGE_FILE_MACHINE_I386, GetCurrentProcess(), GetCurrentThread(), &stack_frame, NULL, NULL,
                   _SymFunctionTableAccess, _SymGetModuleBase, NULL)) {
