...
void ScriptableGameObj::Post_Think(void) {
  ...
  for (i = ObserverTimerList.Count() - 1; i >= 0; i--) {
    if (ObserverTimerList[i]->Update()) {
      ...
      const GameObjObserverList &observer_list1 = Get_Observers();
      for (int index = 0; index < observer_list1.Count(); index++) {
        if (observer_list1[index]->Get_ID() == ObserverTimerList[i]->ObserverID) {
          observer_list1[index]->Timer_Expired(this, ObserverTimerList[i]->TimerID);
          found = true;
        }
      }
      ...
      if (!found) {
        ...
        const GameObjObserverList &observer_list2 = Get_Observers();
        for (int index = 0; index < observer_list2.Count(); index++) {
          Debug_Say(("have %d\n", observer_list2[index]->Get_ID()));
        }
      }
