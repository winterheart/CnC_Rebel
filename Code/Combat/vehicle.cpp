...
** Defines
*/
#define NORMALIZE_RADIANS(v)                                                                                           \
  while (v > (float)DEG_TO_RAD(180.0f))                                                                                \
    v -= (float)DEG_TO_RAD(360.0f);                                                                                    \
  while (v < (float)DEG_TO_RAD(-180.0f))                                                                               \
    v += (float)DEG_TO_RAD(360.0f);
...
bool VehicleGameObj::Toggle_Target_Steering(void) { return _Use_Target_Steering = !_Use_Target_Steering; }
...
bool VehicleGameObj::Is_Target_Steering(void) { return _Use_Target_Steering; }
...
VehicleGameObjDef::VehicleGameObjDef(void)
    : Type(VEHICLE_TYPE_CAR), TurnRadius(10.0f), OccupantsVisible(true), EngineSoundMaxPitchFactor(2.0F),
      SightDownMuzzle(false), Aim2D(true), SquishVelocity(1.5f), VehicleNameID(0), NumSeats(2), GDIDamageReportID(0),
      NodDamageReportID(0), GDIDestroyReportID(0), NodDestroyReportID(0) {
  ...
  NAMED_EDITABLE_PARAM(VehicleGameObjDef, ParameterClass::TYPE_SOUNDDEFINITIONID,
                       EngineSound[ENGINE_SOUND_STATE_STARTING], "Engine Start Sound");
  NAMED_EDITABLE_PARAM(VehicleGameObjDef, ParameterClass::TYPE_SOUNDDEFINITIONID,
                       EngineSound[ENGINE_SOUND_STATE_RUNNING], "Engine Running Loop");
  NAMED_EDITABLE_PARAM(VehicleGameObjDef, ParameterClass::TYPE_SOUNDDEFINITIONID,
                       EngineSound[ENGINE_SOUND_STATE_STOPPING], "Engine Stop Sound");
  //	NAMED_EDITABLE_PARAM(VehicleGameObjDef, ParameterClass::TYPE_SOUNDDEFINITIONID,
  //EngineSound[ENGINE_SOUND_STATE_OFF],"Engine Off Sound");
...
VehicleGameObjDef::~VehicleGameObjDef(void) { Free_Transition_List(); }

uint32 VehicleGameObjDef::Get_Class_ID(void) const { return CLASSID_GAME_OBJECT_DEF_VEHICLE; }
...
const PersistFactoryClass &VehicleGameObjDef::Get_Factory(void) const { return _VehicleGameObjDefPersistFactory; }
...
const PersistFactoryClass &VehicleGameObj::Get_Factory(void) const { return _VehicleGameObjPersistFactory; }

VehicleGameObj::VehicleGameObj()
    : TurretBone(0), BarrelBone(0), TurretTurn(0), BarrelTilt(0), BarrelOffset(0), Sound(NULL),
      EngineSoundState(ENGINE_SOUND_STATE_OFF), CachedEngineSound(NULL), WheelSurfaceSound(NULL),
      TransitionsEnabled(true), OccupiedSeats(0), HasEnterTransitions(false), HasExitTransitions(false),
      VehicleDelivered(false), LockTimer(0) {
  ...
  if (TurretBone == 0) {
    ...
    if (TurretBone != 0) {
      ...
      if (!Peek_Model()->Is_Bone_Captured(TurretBone)) {
        Debug_Say(
            ("VehicleGameObj::Aquire_Bones() : Turret Bone not captured for model %s\n", Peek_Model()->Get_Name()));
        TurretBone = 0;
      }
    }
  }
  ...
  if (BarrelBone == 0) {
    ...
    if (BarrelBone != 0) {
      ...
      if (!Peek_Model()->Is_Bone_Captured(BarrelBone)) {
        BarrelBone = 0;
        Debug_Say(
            ("VehicleGameObj::Aquire_Bones() : Barrel Bone not captured for model %s\n", Peek_Model()->Get_Name()));
      }
    }
  ...
  case VEHICLE_TYPE_FLYING: {
    ...
    if (p_obj != NULL) {
      ...
      if (COMBAT_STAR && (COMBAT_STAR->Get_Vehicle() == this)) {
        p_obj->Network_Latency_State_Update(sc_position, q, vel, ang_vel);
      } else {
        p_obj->Network_Interpolate_State_Update(sc_position, q, vel, ang_vel, 0.1f);
      }
...
//-----------------------------------------------------------------------------
void VehicleGameObj::Import_State_Cs(BitStreamClass &packet) { SmartGameObj::Import_State_Cs(packet); }

void VehicleGameObj::Export_State_Cs(BitStreamClass &packet) { SmartGameObj::Export_State_Cs(packet); }
...
bool VehicleGameObj::Set_Targeting(const Vector3 &target_pos, bool do_tilt) {
  ...
  if (TurretBone != 0) {
    ...
    if (BarrelOffset) {
      ...
      //			Debug_Say(( "barrel angle %f %f\n", RAD_TO_DEG( barrel_offset_angle ), RAD_TO_DEG(
...
void VehicleGameObj::Apply_Control( void )
{
        ...
        if ( gunner && (gunner != driver) && !gunner->In_Transition() ) {
                Control.Set_Boolean( ControlClass::BOOLEAN_WEAPON_FIRE_PRIMARY,
gunner->Get_Control().Get_Boolean( ControlClass::BOOLEAN_WEAPON_FIRE_PRIMARY ) ); Control.Set_Boolean(
ControlClass::BOOLEAN_WEAPON_FIRE_SECONDARY,		gunner->Get_Control().Get_Boolean(
ControlClass::BOOLEAN_WEAPON_FIRE_SECONDARY ) ); Control.Set_Boolean( ControlClass::BOOLEAN_WEAPON_NEXT,
gunner->Get_Control().Get_Boolean( ControlClass::BOOLEAN_WEAPON_NEXT ) ); Control.Set_Boolean(
ControlClass::BOOLEAN_WEAPON_PREV,		gunner->Get_Control().Get_Boolean( ControlClass::BOOLEAN_WEAPON_PREV )
); Control.Set_Boolean( ControlClass::BOOLEAN_WEAPON_RELOAD,	gunner->Get_Control().Get_Boolean(
ControlClass::BOOLEAN_WEAPON_RELOAD ) ); Control.Set_Boolean( ControlClass::BOOLEAN_WEAPON_USE,
gunner->Get_Control().Get_Boolean( ControlClass::BOOLEAN_WEAPON_USE ) );
        }
...
void VehicleGameObj::Apply_Control(void) {
  ...
  if (gunner && (gunner != driver) && !gunner->In_Transition() && !Get_Driver_Is_Gunner()) {
    Control.Set_Boolean(ControlClass::BOOLEAN_WEAPON_FIRE_PRIMARY,
                        gunner->Get_Control().Get_Boolean(ControlClass::BOOLEAN_WEAPON_FIRE_PRIMARY));
    Control.Set_Boolean(ControlClass::BOOLEAN_WEAPON_FIRE_SECONDARY,
                        gunner->Get_Control().Get_Boolean(ControlClass::BOOLEAN_WEAPON_FIRE_SECONDARY));
    Control.Set_Boolean(ControlClass::BOOLEAN_WEAPON_RELOAD,
                        gunner->Get_Control().Get_Boolean(ControlClass::BOOLEAN_WEAPON_RELOAD));
  }
...
void VehicleGameObj::Init_Wheel_Effects(void) {
  ...
  if ((Peek_Vehicle_Phys() != NULL) && (Peek_Vehicle_Phys()->Get_VehiclePhysDef()->Is_Fake() == false)) {
...
void VehicleGameObj::Update_Wheel_Effects(void) {
  if ((Peek_Vehicle_Phys() == NULL) || (Peek_Vehicle_Phys()->Get_VehiclePhysDef()->Is_Fake())) {
    return;
  }
    ...
    if (Peek_Vehicle_Phys() != NULL) {
      ...
      for (int i = 0; i < wheel_count; i++) {
        ...
        if (wheel->Get_Flag(SuspensionElementClass::FAKE) == false) {
          ...
          SurfaceEffectsManager::Update_Persistant_Emitter(WheelSurfaceEmitters[emitter_index], surface_type,
                                                           hitter_type, wheel->Get_Wheel_Transform());
          ...
          if (!sound_done && wheel->Get_Flag(SuspensionElementClass::ENGINE)) {
            SurfaceEffectsManager::Update_Persistant_Sound(WheelSurfaceSound, surface_type, hitter_type,
                                                           Get_Transform());
...
void VehicleGameObj::Update_Sound_Effects(void) {
  ...
  case ENGINE_SOUND_STATE_STARTING: {
    ...
    if ((CachedEngineSound == NULL) ||
        ((CachedEngineSound != NULL) && (CachedEngineSound->Get_State() == AudibleSoundClass::STATE_STOPPED))) {
...
bool VehicleGameObj::Is_Entry_Permitted(SoldierGameObj *p_soldier) {
  ...
  for (i = 0; i < SeatOccupants.Length(); i++) {
    ...
    if (SeatOccupants[i] != NULL) {
      ...
      if (seated_pt != player_type || (seated_pt != PLAYERTYPE_NOD && seated_pt != PLAYERTYPE_GDI)) {
...
void VehicleGameObj::Set_Precision(void) {
  cEncoderList::Set_Precision(BITPACK_VEHICLE_VELOCITY, -90.0f, 90.0f, 0.01f);         // 200 mph = approx. 90 m/s
  cEncoderList::Set_Precision(BITPACK_VEHICLE_ANGULAR_VELOCITY, -20.0f, 20.0f, 0.01f); // to approx 6 PI
  cEncoderList::Set_Precision(BITPACK_VEHICLE_QUATERNION, -1.0f, 1.0f, 0.0005f);       // -1 to 1
  cEncoderList::Set_Precision(BITPACK_VEHICLE_LOCK_TIMER, 0.0f, 16.0f, 0.25f);
}
                ...
                case VEHICLE_TYPE_TANK:
                {
                        ...
                        switch (CombatManager::Get_Client_Interpolation_Model()) {
                                ...
                                case CLIENT_INTERPOLATION_PATHFIND: {
                                        ...
                                        if (p_driver != NULL && p_driver->Get_Control_Owner() ==
CombatManager::Get_My_Id()) {
