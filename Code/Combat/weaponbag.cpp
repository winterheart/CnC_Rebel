...
WeaponBagClass::WeaponBagClass(ArmedGameObj *owner)
    : Owner(owner), WeaponIndex(0), IsChanged(true), HUDIsChanged(true) {
  Mark_Owner_Dirty();
...
void WeaponBagClass::Import_Weapon_List(BitStreamClass &packet) {
  ...
  int weapon_id;
  // TODO: WH: 'weapon_index' was 'weapon'. Recheck this part.
...
bool WeaponBagClass::Move_Contents(WeaponBagClass *source) {
  ...
  for (int i = 1; i < source->WeaponList.Count(); i++) {
    ...
    if (my_weapon) {
      ...
      //			Debug_Say(( "Add %s %d\n", weapon->Get_Definition()->Get_Name(),
      //weapon->Get_Total_Rounds() ));
    } else {
      // else, give it to me
