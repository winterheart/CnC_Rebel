...
SpawnerDefClass::SpawnerDefClass(void)
    : PlayerType(PLAYERTYPE_NEUTRAL), SpawnMax(-1), // unlimited
      SpawnDelay(10), SpawnDelayVariation(0), IsPrimary(false), IsSoldierStartup(false), PostVisualSpawnDelay(0),
      SpecialEffectsObjID(0), GotoSpawnerPos(false), GotoSpawnerPosPriority(30), TeleportFirstSpawn(true),
      StartsDisabled(false), KillHibernatingSpawn(true), ApplySpawnMaterialEffect(true),
...
uint32 SpawnerDefClass::Get_Class_ID(void) const { return CLASSID_SPAWNER_DEF; }
...
const PersistFactoryClass &SpawnerDefClass::Get_Factory(void) const { return _SpawnerDefPersistFactory; }
...
const PersistFactoryClass &SpawnerClass::Get_Factory(void) const { return _SpawnerPersistFactory; }

SpawnerClass::SpawnerClass(void)
    : ID(0), TM(1), SpawnTM(1), Definition(NULL), SpawnCount(0), SpawnDelayTimer(0), Enabled(true) {
  SpawnManager::Add_Spawner(this);
}

SpawnerClass::~SpawnerClass(void) { SpawnManager::Remove_Spawner(this); }
...
void SpawnerClass::Check_Auto_Spawn(float dtime) {
  if (LastSpawn.Get_Ptr() == NULL) { // If our last spawn is gone
    ...
    if (Get_Definition().SpawnMax < 0 || SpawnCount < Get_Definition().SpawnMax) {
      spawn_ok = true;
    }
...
PhysicalGameObj *SpawnerClass::Spawn_Object(int obj_def_id) { return Spawn(obj_def_id); }
...
void SpawnManager::Remove_Spawner(SpawnerClass *spawner) { SpawnerList.Delete_Object(spawner); }
...
Matrix3D SpawnManager::Get_Primary_Spawn_Location(void) {
  int index = -1;
  for (int i = 0; i < SpawnerList.Count(); i++) {
    if (SpawnerList[i]->Get_Definition().IsPrimary && SpawnerList[i]->Get_Definition().IsSoldierStartup) {
  ...
  if (suitable_spawners == 0) {
    ...
    if (phys_obj != NULL) {
      for (i = 0; i < SpawnerList.Count(); i++) {
        ...
        if (!SpawnerList[index]->Get_Definition().IsPrimary && SpawnerList[index]->Get_Definition().IsSoldierStartup &&
      ...
      WWDEBUG_SAY(
          ("Failed to find clear multiplayer spawn point for object: %s\r\n", phys_obj->Peek_Model()->Get_Name()));
    }
  }
 ...
 *=============================================================================================*/
SpawnManager::~SpawnManager(void) { Remove_All_Spawners(); }
