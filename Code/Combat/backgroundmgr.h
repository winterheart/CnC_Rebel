...
protected:
  enum { ROW_COUNT = 3, VERTICES_PER_TRIANGLE = 3 };
...
public:
  ...
  void Configure(const Vector3 &orientation, float length, float radius, const Vector3 &color0, const Vector3 &color1,
                 float alpha);
...
protected:
  enum { ROW_COUNT = 5, VERTICES_PER_TRIANGLE = 3 };
...
public:
  enum MoonTypeEnum { MOON_TYPE_FULL, MOON_TYPE_PART, MOON_TYPE_COUNT };
...
public:
  enum LightSourceTypeEnum { LIGHT_SOURCE_TYPE_SUN, LIGHT_SOURCE_TYPE_MOON };
  ...
  static void Init(SimpleSceneClass *renderscene, SoundEnvironmentClass *soundenvironment,
  ...
  static SkyClass::MoonTypeEnum Get_Moon_Type() { return (_MoonType); }
  ...
  static bool Set_Lightning(float intensity, float startdistance, float enddistance, float heading, float distribution,
                            float ramptime = 0.0f);
  static bool Override_Lightning(float intensity, float startdistance, float enddistance, float heading,
                                 float distribution, float ramptime = 0.0f);
  static void Get_Lightning(float &intensity, float &startdistance, float &enddistance, float &heading,
                            float &distribution);
  static void Restore_Lightning(float ramptime);

  static bool Set_War_Blitz(float intensity, float ramptime = 0.0f);
  static bool Set_War_Blitz(float intensity, float startdistance, float enddistance, float heading, float distribution,
                            float ramptime = 0.0f, bool override = false);
  static void Get_War_Blitz(float &intensity, float &startdistance, float &enddistance, float &heading,
                            float &distribution);
...
#define VARID_PARAMETER(varname)                                                                                       \
  ...
  // Constants.
  enum { CHUNKID_MICRO_CHUNKS = 0x11080732, CHUNKID_DYNAMIC_MICRO_CHUNKS = 0x11020216 };
