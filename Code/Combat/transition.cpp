...
const char *TransitionTypeNames[] = {
    "LADDER_EXIT_TOP",        "LADDER_EXIT_BOTTOM",     "LADDER_ENTER_TOP",      "LADDER_ENTER_BOTTOM",
    "LEGACY_VEHICLE_ENTER_0", "LEGACY_VEHICLE_ENTER_1", "LEGACY_VEHICLE_EXIT_0", "LEGACY_VEHICLE_EXIT_1",
    "VEHICLE_ENTER",          "VEHICLE_EXIT",
};
...
**
*/
TransitionDataClass::TransitionDataClass() : Type(LADDER_EXIT_TOP) {}
...
TransitionInstanceClass::TransitionInstanceClass(const TransitionDataClass &data)
    : Data(data), EndingTM(1), LadderIndex(-1), Zone(Vector3(0, 0, 0), Vector3(0, 0, 0)) {}
...
bool TransitionInstanceClass::Check(SoldierGameObj *obj, bool action_trigger) {
  ...
  if (!action_trigger && (Get_Type() != TransitionDataClass::LADDER_EXIT_TOP) &&
  ...
  case TransitionDataClass::VEHICLE_ENTER:
    ...
    if (!obj->Is_Permitted_To_Enter_Vehicle() || !p_vehicle->Is_Entry_Permitted(obj)) {
      condition = false;
    }
  ...
  case TransitionDataClass::VEHICLE_EXIT:
    ...
    if (move_phys != NULL) {
      ...
      if ((can_move_to == false) || (move_phys->Can_Teleport_And_Stand(EndingTM, &dummy_tm) == false)) {
        condition = false;
      }
    }
...
void TransitionInstanceClass::End(SoldierGameObj *obj, TransitionCompletionDataStruct *completion_data) {
...
void TransitionManager::Add(TransitionInstanceClass *transition) { Transitions.Add_Tail(transition); }

void TransitionManager::Destroy(TransitionInstanceClass *transition) { TransitionsDestroy.Add_Tail(transition); }
