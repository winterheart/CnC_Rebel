...
**
*/
void WeaponViewClass::Enable(bool enable) { WeaponViewEnabled = enable; }
...
void WeaponViewClass::Think() {
  ...
  if (weapon != NULL) {
    if ((weapon->Get_Style() == WEAPON_HOLD_STYLE_C4) || (weapon->Get_Style() == WEAPON_HOLD_STYLE_BEACON)) {
  ...
  if (HandsPhysObj != NULL && WeaponModel != NULL) {
    ...
    if (is_current_complete && WeaponState == WEAPON_STATE_FIRE && new_weapon_state == WEAPON_STATE_IDLE) {
      // if the anims exist...
    ...
    if (WeaponState != WEAPON_STATE_IDLE) {
      if (is_current_complete) {
        ...
        if (WeaponState == WEAPON_STATE_FIRE && ForceFireLoop) {
          ...
          RenderObjClass *hands_model = HandsPhysObj->Peek_Model();
          if (hands_model != NULL) {
            //						hands_model->Set_Animation( HandsAnims[ WeaponState ], 0,
          ...
          //					WeaponModel->Set_Animation( WeaponAnims[ WeaponState ], 0,
          //RenderObjClass::ANIM_MODE_ONCE );
    ...
    if (WeaponModel && COMBAT_CAMERA) {
      ...
      /*			if ( raytest.CollidedPhysObj != NULL && raytest.CollidedPhysObj->Get_Observer() != NULL
         ) { DamageableGameObj * obj = ((CombatPhysObserverClass
         *)raytest.CollidedPhysObj->Get_Observer())->As_DamageableGameObj(); if ( obj ) { Debug_Say(( "Hit %s\n",
...
** For each weapon state, except ENTER and EXIT, we have a weapon anim tied to the weapon model (use IDLE anim for
*ENTER/EXIT )
...
static void Aquire_Weapon_Assets(const WeaponClass *weapon) {
  ...
  if (WeaponModel != NULL) {
    ...
    for (int i = 0; i < NUM_WEAPON_STATES; i++) {
      ...
      anim_name.Format("%s.F_GA_%s_%s",
        weapon_htree_name.Peek_Buffer(), weapon_name.Peek_Buffer(), WeaponActionNames[state]);
...
static void Set_Bob_Recoil(float amount) { BobRecoil = amount; }
