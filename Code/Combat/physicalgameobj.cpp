...
PhysicalGameObjDef::PhysicalGameObjDef(void)
    : Type(0), BullseyeOffsetZ(0.0f), RadarBlipType(0), PhysDefID(0), KilledExplosion(0),
      OratorType(ORATOR_TYPE_START - 1), DefaultHibernationEnable(true), AllowInnateConversations(false),
  ...
  for (int index = 0; index < count; index++) {
    orator_type_param->Add_Value(OratorTypeClass::Get_Description(index), OratorTypeClass::Get_ID(index));
  }
...
PhysicalGameObj::PhysicalGameObj(void)
    : PhysObj(NULL), AnimControl(NULL),
      // TintColor(1, 1, 1),
      HibernationTimer(0), // Start alseep
      HibernationEnable(true), HostGameObjBone(0), RadarBlipShapeType(0), RadarBlipColorType(0), RadarBlipIntensity(0),
      ActiveConversation(NULL), PendingHostObjID(0), HUDPokableIndicatorEnabled(false),
...
AnimControlClass *PhysicalGameObj::Get_Anim_Control(void) { return AnimControl; }
...
bool PhysicalGameObj::Is_Soft(void) { return DefenseObject.Is_Soft(); }
...
void PhysicalGameObj::Apply_Damage_Extended(const OffenseObjectClass &damager, float scale, const Vector3 &direction,
...
void PhysicalGameObj::Completely_Damaged(const OffenseObjectClass &damager) {
  if (Get_Definition().KilledExplosion != 0) {
    ...
    ExplosionManager::Create_Explosion_At(Get_Definition().KilledExplosion, tm /*pos*/,
...
void PhysicalGameObj::Set_Collision_Group(int group) { Peek_Physical_Object()->Set_Collision_Group(group); }
...
void PhysicalGameObj::Reset_Server_Skips(BYTE value) { ServerUpdateSkips = value; }
...
void PhysicalGameObj::Reset_Radar_Blip_Color_Type(void) {
  ...
  case PLAYERTYPE_NOD:
    RadarBlipColorType = RadarManager::BLIP_COLOR_TYPE_NOD;
    break;
  case PLAYERTYPE_GDI:
    RadarBlipColorType = RadarManager::BLIP_COLOR_TYPE_GDI;
    break;
  case PLAYERTYPE_MUTANT:
    RadarBlipColorType = RadarManager::BLIP_COLOR_TYPE_MUTANT;
    break;
  case PLAYERTYPE_RENEGADE:
    RadarBlipColorType = RadarManager::BLIP_COLOR_TYPE_RENEGADE;
    break;
  default:
    RadarBlipColorType = RadarManager::BLIP_COLOR_TYPE_NEUTRAL;
    break;
  }
}
...
void PhysicalGameObj::Import_Frequent(BitStreamClass &packet) {
  ...
  if ((on_host_bone) && (Peek_Physical_Object()) && (Peek_Physical_Object()->As_MoveablePhysClass())) {
    MoveablePhysClass *movephys = Peek_Physical_Object()->As_MoveablePhysClass();
    movephys->Set_Velocity(Vector3(0, 0, 0));
  }
}
...
void PhysicalGameObj::Object_Shattered_Something(PhysClass *observed_obj, PhysClass *shattered_obj, int surface_type) {
  ...
  SurfaceEffectsManager::Apply_Effect(surface_type, SurfaceEffectsManager::HITTER_TYPE_BULLET, tm, NULL, NULL,
                                      false, // no decals
                                      false  // no emitter
  );
}
