...
const WCHAR *Objective::Type_To_Name(void) {
  switch (Type) {
  case ObjectiveManager::TYPE_PRIMARY:
    return TRANSLATE(IDS_MENU_TEXT145);
  case ObjectiveManager::TYPE_SECONDARY:
    return TRANSLATE(IDS_MENU_TEXT113);
  case ObjectiveManager::TYPE_TERTIARY:
    return TRANSLATE(IDS_MENU_TERTIARY);
  default:
    return TRANSLATE(IDS_LOCALE_UNKNOWN);
  }
}

const WCHAR *Objective::Status_To_Name(void) {
  if (Status == ObjectiveManager::STATUS_ACCOMPLISHED)
    return TRANSLATE(IDS_MENU_OBJ_ACCOMPLISHED);
  if (Status == ObjectiveManager::STATUS_FAILED)
    return TRANSLATE(IDS_MENU_OBJ_FAILED);
  if (Status == ObjectiveManager::STATUS_HIDDEN)
    return TRANSLATE(IDS_MENU_OBJ_HIDDEN);
...
const Vector3 &Objective::Status_To_Color(void) {
  ...
  switch (Status) {
  case ObjectiveManager::STATUS_IS_PENDING:
    return GREEN;
  case ObjectiveManager::STATUS_ACCOMPLISHED:
    return YELLOW;
  case ObjectiveManager::STATUS_FAILED:
    return RED;
  case ObjectiveManager::STATUS_HIDDEN:
    return GREY;
  default:
    return GREEN;
  }
...
Objective::Objective(void)
    : ID(0), Type(0), Status(0), DrawBlip(false), Position(0, 0, 0), BlipIntensity(0), HUDMessageStringID(0),
      LongDescriptionID(0), ShortDescriptionID(0), HUDPriority(0), Age(0) {}

Objective::~Objective(void) { Set_Object(NULL); }
...
**
*/
void ObjectiveManager::Add_Objective(int id, int type, int status, int short_description_id, int long_description_id,
...
void ObjectiveManager::Set_Objective_Status(int id, int status) {
  ...
  if (objective != NULL) {
    ...
    bool is_unhiding =
        (objective->Status == ObjectiveManager::STATUS_HIDDEN) && (status != ObjectiveManager::STATUS_HIDDEN);
...
int ObjectiveManager::Get_Num_HUD_Objectives(void) {
  ...
  int count = 0;
  for (int i = 0; i < ObjectiveList.Count(); i++) {
    if ((ObjectiveList[i]->Status == STATUS_IS_PENDING) && (ObjectiveList[i]->HUDPriority > 0)) {
      count++;
    }
  }
  return count;
...
Vector3 ObjectiveManager::Get_HUD_Objectives_Location(int index) { return ObjectiveList[index]->Get_Position(); }

float ObjectiveManager::Get_HUD_Objectives_Age(int index) { return ObjectiveList[index]->Age; }

int ObjectiveManager::Get_Num_Objectives(int type) {
  int count = 0;
  for (int i = 0; i < ObjectiveList.Count(); i++) {
    if ((ObjectiveList[i]->Type == type) && (ObjectiveList[i]->Status != STATUS_HIDDEN)) {
      count++;
    }
  }
...
int ObjectiveManager::Get_Num_Completed_Objectives(int type) {
  int count = 0;
  for (int i = 0; i < ObjectiveList.Count(); i++) {
    if ((ObjectiveList[i]->Type == type) && (ObjectiveList[i]->Status == STATUS_ACCOMPLISHED)) {
      count++;
    }
  }
  return count;
