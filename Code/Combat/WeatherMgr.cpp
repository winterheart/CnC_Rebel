...
WindClass::WindClass(float heading, float speed, float variability, SoundEnvironmentClass *soundenvironment)
    : Velocity(0.0f, 0.0f), SoundEnvironment(soundenvironment) {
 ...
 *=============================================================================================*/
WeatherSystemClass::WeatherSystemClass(PhysicsSceneClass *scene, float emittersize, float emitterheight,
                                       float particledensity, float particlesperunitlength, float particlewidth,
                                       float particleheight, float particlespeed, const Vector2 &pageoffset,
                                       const Vector2 &pagesize, unsigned pagecount, bool staticpageexists,
                                       float minstatictime, float maxstatictime, RenderModeEnum rendermode,
                                       bool decayaftercollision, bool prime)
    : Scene(scene), EmitterSize(emittersize), EmitterHeight(emitterheight), EmitterPosition(Vector3(0.0f, 0.0f, 0.0f)),
      ParticlesPerUnitLength(particlesperunitlength), ParticleSpeed(particlespeed),
      HalfParticleWidth(particlewidth * 0.5f), HalfParticleHeight(particleheight * 0.5f), RayHead(NULL), RayCount(0),
      RaySpawnPtr(NULL), RayUpdatePtr(NULL), ParticleHead(NULL), ParticleCount(0), MinRayEndZ(FLT_MAX),
      SpawnCountFraction(0.0f), PageCount(pagecount), StaticPageExists(staticpageexists), MinStaticTime(minstatictime),
      MaxStaticTime(maxstatictime), RenderMode(rendermode), DecayAfterCollision(decayaftercollision),
...
// Initialize an index buffer.
  ...
  for (unsigned page = 0; page < pagecount; page++) {
    TextureArray[page * VERTICES_PER_TRIANGLE + 0].Set(pageoffset.U + (((page + 0.5f) * oopagecount) * pagesize.U),
                                                       pageoffset.V + 0.0f);
    TextureArray[page * VERTICES_PER_TRIANGLE + 1].Set(pageoffset.U + (((page + 1.0f) * oopagecount) * pagesize.U),
                                                       pageoffset.V + pagesize.V);
    TextureArray[page * VERTICES_PER_TRIANGLE + 2].Set(pageoffset.U + (((page + 0.0f) * oopagecount) * pagesize.U),
                                                       pageoffset.V + pagesize.V);
  }
}
...
bool WeatherSystemClass::Update(WindClass *wind, const Vector3 &cameraposition) {
  ...
  while (rayptr != NULL) {
    ...
    if (!rayptr->Initialized) {
      ...
      rayptr->RayCast =
          (rayptr->StartPosition.X < emitterbounds.Min.X) || (rayptr->StartPosition.X > emitterbounds.Max.X) ||
          (rayptr->StartPosition.Y < emitterbounds.Min.Y) || (rayptr->StartPosition.Y > emitterbounds.Max.Y);
  ...
  while (particleptr != NULL) {
    ...
    outside = (emitterposition.X < emitterbounds.Min.X) || (emitterposition.X > emitterbounds.Max.X) ||
              (emitterposition.Y < emitterbounds.Min.Y) || (emitterposition.Y > emitterbounds.Max.Y);
...
bool WeatherSystemClass::Spawn(RayStruct *suppliedrayptr) {
  ...
  if (_GlobalParticleCount < maxparticlecount) {
    ...
    if (Can_Spawn(rayptr)) {
      ...
      // If the ray does not have a valid surface normal then the ray's end position intersects the level's bounding
      // box. In this case it is not necessary to sustain the particle past the collision time.
      if (rayptr->ValidSurfaceNormal) {
        particleptr->LifeTime =
            collisiontime + WWMath::Lerp(MinStaticTime, MaxStaticTime, _RandomNumber(0, randomness) * oorandomness);
...
#if WEATHER_PARTICLE_SORT
        SortingRendererClass::Insert_Triangles(0, submittedparticlecount, 0,
...
RainSystemClass::RainSystemClass(PhysicsSceneClass *scene, float particledensity, WindClass *wind,
    ...
    : WeatherSystemClass(scene, 20.0f, 20.0f, particledensity, 0.2f, 0.15f, 0.45f, 15.0f, Vector2(0.0f, 0.0f),
                         Vector2(1.0f, 0.5f), PAGE_COUNT, true, 0.1f, 0.2f,
...
SnowSystemClass::SnowSystemClass(PhysicsSceneClass *scene, float particledensity, WindClass *wind, bool prime)
    : WeatherSystemClass(scene, 40.0f, 20.0f, particledensity, 0.1f, 0.32f, 0.32f, 3.5f, Vector2(0.0f, 0.5f),
                         Vector2(1.0f, 0.25f), PAGE_COUNT, false, 1.0f, 2.0f,
...
AshSystemClass::AshSystemClass(PhysicsSceneClass *scene, float particledensity, WindClass *wind, bool prime)
    : WeatherSystemClass(scene, 40.0f, 20.0f, particledensity, 0.1f, 0.32f, 0.32f, 3.0f, Vector2(0.0f, 0.75f),
                         Vector2(1.0f, 0.25f), PAGE_COUNT, false, 1.0f, 2.0f,
                         WeatherSystemClass::RENDER_MODE_CAMERA_ALIGNED, true, prime) {}
...
bool WeatherMgrClass::Set_Wind(float heading, float speed, float variability, float ramptime, bool override) {
  if ((heading >= 0.0f) && (heading <= 360.0f) && (speed >= 0.0f) && (variability >= 0.0f) && (variability <= 1.0f) &&
      (ramptime >= 0.0f)) {
...
void WeatherMgrClass::Update(PhysicsSceneClass *scene, CameraClass *camera) {
  ...
  if (_Wind != NULL) {
    if (windmodified) {
      _Wind->Set(_Parameters[PARAMETER_WIND_HEADING].Value(), _Parameters[PARAMETER_WIND_SPEED].Value(),
    ...
    if (_Parameters[PARAMETER_WIND_SPEED].Value() > 0.0f) {
      _Wind = new WindClass(_Parameters[PARAMETER_WIND_HEADING].Value(), _Parameters[PARAMETER_WIND_SPEED].Value(),
    ...
    if (_Precipitation[p] != NULL) {
      ...
      if (parameterptr->Value() > 0.0f) {
        ...
        if (!CombatManager::I_Am_Only_Server()) {
          ...
          case PRECIPITATION_RAIN:
            _Precipitation[p] =
                NEW_REF(RainSystemClass, (scene, parameterptr->Value(), _Wind, _SoundEnvironment, _Prime));
            break;
