...
DECLARE_DEFINITION_FACTORY(GlobalSettingsDef, CLASSID_GLOBAL_SETTINGS_DEF_GENERAL, "General")
_GlobalSettingsDefDefFactory;
...
GlobalSettingsDef::GlobalSettingsDef(void)
    : DeathSoundID(0), EVAObjectivesSoundID(0), HUDHelpTextSoundID(0), MaxConversationDist(10.0F),
      MaxCombatConversationDist(10.0F), SoldierWalkSpeed(0.25), SoldierCrouchSpeed(0.25), EncyclopediaEventStringID(0),
      FallingDamageMinDistance(5), FallingDamageMaxDistance(20), FallingDamageWarhead(15), // Default to Earth
      StealthDistanceHuman(15.0f), StealthDistanceVehicle(25.0f), MPStealthDistanceHuman(15.0f),
...
uint32 GlobalSettingsDef::Get_Class_ID(void) const { return CLASSID_GLOBAL_SETTINGS_DEF_GENERAL; }
    ...
    case CHUNKID_DEF_VARIABLES:
      while (cload.Open_Micro_Chunk()) {
        switch (cload.Cur_Micro_Chunk_ID()) {
          ...
          READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_PURCHASE_GDI_MUT_CHARS_TEXTURE,
                                    PurchaseGDIMUTCharactersTexture);
          READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_PURCHASE_GDI_MUT_VEHICLES_TEXTURE,
                                    PurchaseGDIMUTVehiclesTexture);
          READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_PURCHASE_GDI_MUT_EQUIP_TEXTURE,
                                    PurchaseGDIMUTEquipmentTexture);

          READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_PURCHASE_NOD_MUT_CHARS_TEXTURE,
                                    PurchaseNODMUTCharactersTexture);
          READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_PURCHASE_NOD_MUT_VEHICLES_TEXTURE,
                                    PurchaseNODMUTVehiclesTexture);
          READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_PURCHASE_NOD_MUT_EQUIP_TEXTURE,
...
SimplePersistFactoryClass<HumanLoiterGlobalSettingsDef, CHUNKID_GLOBAL_SETTINGS_DEF_HUMAN_LOITER>
    _HumanLoiterGlobalSettingsDefPersistFactory;

DECLARE_DEFINITION_FACTORY(HumanLoiterGlobalSettingsDef, CLASSID_GLOBAL_SETTINGS_DEF_HUMAN_LOITER, "HumanLoiter")
_HumanLoiterGlobalSettingsDefDefFactory;
...
HumanLoiterGlobalSettingsDef::HumanLoiterGlobalSettingsDef(void) : ActivationDelay(20), LoiterFrequency(10) {
...
uint32 HumanLoiterGlobalSettingsDef::Get_Class_ID(void) const { return CLASSID_GLOBAL_SETTINGS_DEF_HUMAN_LOITER; }
...
HumanLoiterGlobalSettingsDef *HumanLoiterGlobalSettingsDef::Get_Default_Loiters(void) {
  if (DefaultLoiters == NULL) {
    DefaultLoiters = (HumanLoiterGlobalSettingsDef *)DefinitionMgrClass::Find_Typed_Definition(
        "Loiter", CLASSID_GLOBAL_SETTINGS_DEF_HUMAN_LOITER);
  }
  if (DefaultLoiters == NULL) {
...
HumanLoiterGlobalSettingsDef *HumanLoiterGlobalSettingsDef::Get_Weapon_Loiters(void) {
  if (WeaponLoiters == NULL) {
    WeaponLoiters = (HumanLoiterGlobalSettingsDef *)DefinitionMgrClass::Find_Typed_Definition(
        "Weapon Loiters", CLASSID_GLOBAL_SETTINGS_DEF_HUMAN_LOITER);
  }
  if (WeaponLoiters == NULL) {
...
HumanLoiterGlobalSettingsDef *HumanLoiterGlobalSettingsDef::Get_Weaponless_Loiters(void) {
  if (WeaponlessLoiters == NULL) {
    WeaponlessLoiters = (HumanLoiterGlobalSettingsDef *)DefinitionMgrClass::Find_Typed_Definition(
        "Weaponless Loiters", CLASSID_GLOBAL_SETTINGS_DEF_HUMAN_LOITER);
  }
  if (WeaponlessLoiters == NULL) {
...
DECLARE_DEFINITION_FACTORY(HUDGlobalSettingsDef, CLASSID_GLOBAL_SETTINGS_DEF_HUD, "HUD")
_HUDGlobalSettingsDefDefFactory;
...
HUDGlobalSettingsDef::HUDGlobalSettingsDef(void)
    : NodColor(1, 0, 0), GDIColor(1, 1, 0), NeutralColor(1, 1, 1), MutantColor(0, 1, 0), RenegadeColor(0, 0, 1),
      PrimaryObjectiveColor(0, 1, 0), SecondaryObjectiveColor(0, 0, 1), TertiaryObjectiveColor(1, 0, 1),
      HealthHighColor(0, 1, 0), HealthMedColor(1, 1, 0), HealthLowColor(1, 0, 0), EnemyColor(1, 0, 0),
      FriendlyColor(0, 1, 0), NoRelationColor(1, 1, 1), InfoTextureSize(INFO_TEXTURE_SIZE),
      StarBracketSize(STAR_BRACKET_SIZE), StarBracketOffset(STAR_BRACKET_OFFSET), StarBracketUV(STAR_BRACKET_UV),
      StarBracketTopSize(STAR_BRACKET_TOP_SIZE), StarBracketTopOffset(STAR_BRACKET_TOP_OFFSET),
      StarBracketTopUV(STAR_BRACKET_TOP_UV), StarBracketTopArmedSize(STAR_BRACKET_TOP_ARMED_SIZE),
      StarBracketTopArmedOffset(STAR_BRACKET_TOP_ARMED_OFFSET), StarBracketTopArmedUV(STAR_BRACKET_TOP_ARMED_UV),
      StarBarSize(STAR_BAR_SIZE), StarBarOffset(STAR_BAR_OFFSET), StarBarUV(STAR_BAR_UV),
      StarBarEndSize(STAR_BAR_END_SIZE), StarBarEndOffset(STAR_BAR_END_OFFSET), StarBarEndUV(STAR_BAR_END_UV),
      StarHealthSize(STAR_HEALTH_SIZE), StarHealthOffset(STAR_HEALTH_OFFSET), StarHealthUV(STAR_HEALTH_UV),
      StarShieldSize(STAR_SHIELD_SIZE), StarShieldOffset(STAR_SHIELD_OFFSET), StarShieldUV(STAR_SHIELD_UV),
      StarWeaponIconSize(STAR_WEAPON_ICON_SIZE), StarWeaponIconOffset(STAR_WEAPON_ICON_OFFSET),
      TargetBracketSize(TARGET_BRACKET_SIZE), TargetBracketOffset(TARGET_BRACKET_OFFSET),
      TargetBracketUV(TARGET_BRACKET_UV), TargetIconSize(TARGET_ICON_SIZE), TargetIconOffset(TARGET_ICON_OFFSET),
      TargetNameBarSize(TARGET_NAME_BAR_SIZE), TargetNameBarOffset(TARGET_NAME_BAR_OFFSET),
      TargetNameBarUV(TARGET_NAME_BAR_UV), TargetNameOffset(TARGET_NAME_OFFSET), TargetBarSize(TARGET_BAR_SIZE),
      TargetBarOffset(TARGET_BAR_OFFSET), TargetBarUV(TARGET_BAR_UV), TargetBarEndSize(TARGET_BAR_END_SIZE),
      TargetBarEndOffset(TARGET_BAR_END_OFFSET), TargetBarEndUV(TARGET_BAR_END_UV),
      TargetHealthSize(TARGET_HEALTH_SIZE), TargetHealthOffset(TARGET_HEALTH_OFFSET), TargetHealthUV(TARGET_HEALTH_UV),
      TargetShieldSize(TARGET_SHIELD_SIZE), TargetShieldOffset(TARGET_SHIELD_OFFSET), TargetShieldUV(TARGET_SHIELD_UV),
      RadarTextureSize(RADAR_TEXTURE_SIZE), RadarOffset(RADAR_OFFSET), RadarRadius(RADAR_RADIUS),
      RadarFrameSize(RADAR_FRAME_SIZE), RadarFrameUV(RADAR_FRAME_UV), RadarCompassOffset(RADAR_COMPASS_OFFSET),
      RadarCompassSize(RADAR_COMPASS_SIZE), RadarCompassBaseUV(RADAR_COMPASS_BASE_UV),
      RadarCompassUVOffset(RADAR_COMPASS_UV_OFFSET), RadarHumanBlipUV(RADAR_HUMAN_BLIP_UV),
      RadarVehicleBlipUV(RADAR_VEHICLE_BLIP_UV), RadarStationaryBlipUV(RADAR_STATIONARY_BLIP_UV),
      RadarObjectiveBlipUV(RADAR_OBJECTIVE_BLIP_UV), RadarBlipBracketUV(RADAR_BLIP_BRACKET_UV),
      RadarSweepUV(RADAR_SWEEP_UV), RadarOnSoundID(0), RadarOffSoundID(0), SniperTextureSize(SNIPER_TEXTURE_SIZE),
      SniperView(SNIPER_VIEW), SniperViewUV(SNIPER_VIEW_UV), SniperScanLineUV(SNIPER_SCAN_LINE_UV),
      SniperBlackCoverUV(SNIPER_BLACK_COVER_UV), SniperTiltBar(SNIPER_TILT_BAR),
      SniperTiltBarRate(SNIPER_TILT_BAR_RATE), SniperTiltBarUV(SNIPER_TILT_BAR_UV), SniperTurnBar(SNIPER_TURN_BAR),
      SniperTurnBarRate(SNIPER_TURN_BAR_RATE), SniperTurnBarUV(SNIPER_TURN_BAR_UV),
      SniperDistanceGraph(SNIPER_DISTANCE_GRAPH), SniperDistanceGraphUV(SNIPER_DISTANCE_GRAPH_UV),
      SniperDistanceGraphMax(SNIPER_DISTANCE_GRAPH_MAX), SniperZoomGraph(SNIPER_ZOOM_GRAPH),
...
uint32 HUDGlobalSettingsDef::Get_Class_ID(void) const { return CLASSID_GLOBAL_SETTINGS_DEF_HUD; }

const PersistFactoryClass &HUDGlobalSettingsDef::Get_Factory(void) const { return _HUDGlobalSettingsDefPersistFactory; }
...
** HumanAnimOverrides
*/
SimplePersistFactoryClass<HumanAnimOverrideDef, CHUNKID_GLOBAL_SETTINGS_DEF_HUMAN_ANIM_OVERRIDE>
    _HumanAnimOverrideDefPersistFactory;

DECLARE_DEFINITION_FACTORY(HumanAnimOverrideDef, CLASSID_GLOBAL_SETTINGS_DEF_HUMAN_ANIM_OVERRIDE, "HUMAN_ANIM_OVERRIDE")
_HumanAnimOverrideDefDefFactory;
...
uint32 HumanAnimOverrideDef::Get_Class_ID(void) const { return CLASSID_GLOBAL_SETTINGS_DEF_HUMAN_ANIM_OVERRIDE; }

const PersistFactoryClass &HumanAnimOverrideDef::Get_Factory(void) const { return _HumanAnimOverrideDefPersistFactory; }
