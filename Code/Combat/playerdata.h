...
public:
  ...
  void Stats_Add_Game_Time(float time) {
    StatTimeInGame += time;
    StatTimeInSession += time;
  }
  ...
  float Get_Session_Time(void) const {
    return StatTimeInSession;
  } // Note: This is per-session, it is not saved or loaded!
