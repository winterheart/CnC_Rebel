...
// bool									CombatManager::IAmServer = true; bool
// CombatManager::IAmClient			= true;
...
void CombatManager::Scene_Init(void) {
  ...
  COMBAT_SCENE->Disable_Collision_Detection(PhysicsSceneClass::COLLISION_GROUP_WORLD,
                                            PhysicsSceneClass::COLLISION_GROUP_WORLD);
...
bool CombatManager::Is_Loading_Level(void) { return (thread.Is_Running()); }
...
**
*/
PhysicsSceneClass *CombatManager::Get_Scene(void) { return PhysicsSceneClass::Get_Instance(); }
...
void CombatManager::Do_Skeleton_Slider_Demo(void) {
  if ((EnableSkeletonSliderDemo == false) || (COMBAT_CAMERA->Is_2D_Targeting() == false)) {
    return;
  }
...
void CombatManager::Update_Combat_Mode(void) {
  ...
  case COMBAT_MODE_IN_VEHICLE: {
    ...
    //			bool target_2d = ( !vehicle->Use_2D_Aiming() ^ Input::Get_State(INPUT_FUNCTION_CURSOR_TARGETING)
    //^ VehicleGameObj::Is_Target_Steering() );
