...
void ArmorWarheadManager::Init(void) {
  ...
  if (armorINI != NULL) {
    ...
    for (entry = 0; entry < count; entry++) {
      char entry_name[80];
      armorINI->Get_String(SECTION_WARHEAD_TYPES, armorINI->Get_Entry(SECTION_WARHEAD_TYPES, entry), "", entry_name,
                           sizeof(entry_name));
    ...
    for (entry = 0; entry < count; entry++) {
      char entry_name[80];
      armorINI->Get_String(SECTION_SOFT_ARMOR, armorINI->Get_Entry(SECTION_SOFT_ARMOR, entry), "", entry_name,
                           sizeof(entry_name));
    ...
    for (i = SPECIAL_DAMAGE_TYPE_FIRE; i < NUM_SPECIAL_DAMAGE_TYPES; i++) {
      const char *SpecialDamageSectionNames[NUM_SPECIAL_DAMAGE_TYPES] = {
          ...
          "Special_Damage_Electric", "Special_Damage_CNC_Fire", "Special_Damage_CNC_Chem",
      };
    ...
    for (int damage = SPECIAL_DAMAGE_TYPE_FIRE; damage < NUM_SPECIAL_DAMAGE_TYPES; damage++) {
      const char *ImperviousSectionNames[NUM_SPECIAL_DAMAGE_TYPES] = {
          "Impervious_None",     "Impervious_Fire",     "Impervious_Chem",
          "Impervious_Electric", "Impervious_CNC_Fire", "Impervious_CNC_Chem",
      };
...
int ArmorWarheadManager::Get_Num_Armor_Types(void) { return ArmorNames.Count(); }

int ArmorWarheadManager::Get_Num_Warhead_Types(void) { return WarheadNames.Count(); }
...
bool ArmorWarheadManager::Is_Armor_Soft(ArmorType armor) { return SoftArmorTable[armor]; }

int ArmorWarheadManager::Get_Armor_Save_ID(ArmorType type) { return ArmorSaveIDs[type]; }
...
int ArmorWarheadManager::Get_Warhead_Save_ID(WarheadType type) { return WarheadSaveIDs[type]; }
...
**
*/
OffenseObjectClass::OffenseObjectClass(const OffenseObjectClass &base) : Damage(base.Damage), Warhead(base.Warhead) {
...
#define PUNISH_DELAY 60
  ...
  if (COMBAT_STAR != NULL && // dedicated server test
    ...
    DIAG_LOG(("DRCV", "%s; %d; %1.2f; %1.2f; %1.2f; %1.2f; %1.2f; %1.2f", weapon_name, hitter_id, points, armor, Health,
              pos.X, pos.Y, pos.Z));
  }
  ...
  if (damage > 0 && (float)Health <= 0) {
    ...
    DIAG_LOG(("OBDE", "%d; %1.2f; %1.2f; %1.2f; %d; %1.2f; %1.2f; %1.2f; %s", victim_id, victim_pos.X, victim_pos.Y,
              victim_pos.Z, killer_id, killer_pos.X, killer_pos.Y, killer_pos.Z, weapon_name));
    if (Get_Owner() == COMBAT_STAR) {
      DIAG_LOG(("STDE", "%1.2f; %1.2f; %1.2f; %d; %1.2f; %1.2f; %1.2f; %s", victim_pos.X, victim_pos.Y, victim_pos.Z,
                killer_id, killer_pos.X, killer_pos.Y, killer_pos.Z, weapon_name));
    }
  }

  if (damage > 0 && (float)Health <= 0 && smart != NULL && smart->As_SoldierGameObj() != NULL && Get_Owner() != NULL &&
...
bool DefenseObjectClass::Would_Damage(const OffenseObjectClass &offense, float scale) {
  ...
  if (smart && Get_Owner() && smart->Is_Teammate(Get_Owner()) && (smart != Get_Owner())) {
    // This is friendly fire!!
    if (!CombatManager::Is_Friendly_Fire_Permitted()) {
      return false;
    }
...
DefenseObjectDefClass::DefenseObjectDefClass(void)
    : Health(100.0f), HealthMax(100.0f), Skin(0), ShieldStrength(0), ShieldStrengthMax(0), ShieldType(0),
      DamagePoints(0), DeathPoints(0) {}
...
float DefenseObjectClass::Get_Health(void) const { return Health; }
...
float DefenseObjectClass::Get_Health_Max(void) const { return HealthMax; }
...
float DefenseObjectClass::Get_Shield_Strength(void) const { return ShieldStrength; }
...
float DefenseObjectClass::Get_Shield_Strength_Max(void) const { return ShieldStrengthMax; }
