...
CCameraProfileClass::CCameraProfileClass(void)
    : FOV(DEG_TO_RADF(65.0f)), Height(1.95f), ViewTilt(DEG_TO_RADF(20.0f)), TiltTweak(0.6f),
      TranslationTilt(DEG_TO_RADF(19.9f)), Distance(3.1f), Lag(0, 0, 0) {}
...
#define Get_Camera_Profile_Float(v, e) profile->v = camerasINI->Get_Float(section_name, e, profile->v)
...
void CCameraProfileClass::Init(void) {
  ...
  if (camerasINI != NULL) {
    ...
    for (entry = 0; entry < count; entry++) {
      ...
      camerasINI->Get_String(section_name, SECTION_PROFILE_LIST, camerasINI->Get_Entry(SECTION_PROFILE_LIST, entry));
...
CCameraClass::CCameraClass()
    : CameraClass(), HostModel(NULL), AnchorPosition(0, 0, 0), IsValid(false), Tilt(0), Heading(0),
      DistanceFraction(1.0f), Enable2DTargeting(false), EnableWeaponHelp(false), CameraTarget2DOffset(0.5f, 0.5f),
      LerpTimeTotal(0.0f), LerpTimeRemaining(0.0f), LastAnchorPosition(0, 0, 0), LastHeading(0), CurrentProfile(NULL),
      LastProfile(NULL), DefaultProfile(NULL), NearClipPlane(CCAMERA_NEARZ), FarClipPlane(CCAMERA_FARZ),
      IsStarSniping(false), WasStarSniping(false), CinematicSnipingEnabled(false), CinematicSnipingDesiredZoom(0),
      SniperZoom(0), SniperDistance(0), SniperListener(NULL), SnapShotMode(SNAPSHOT_OFF), WeaponHelpTimer(0),
      WeaponHelpTargetID(0), LagPersistTimer(0), DisableLag(false) {
...
**
*/
void CCameraClass::Init(void) { CCameraProfileClass::Init(); }

void CCameraClass::Shutdown(void) { CCameraProfileClass::Shutdown(); }
    ...
    case CHUNKID_VARIABLES:
      while (cload.Open_Micro_Chunk()) {
        switch (cload.Cur_Micro_Chunk_ID()) {
          ...
          // (gth) don't save and load NearClipPlane and FarClipPlane
          //						READ_MICRO_CHUNK( cload, MICROCHUNKID_NEAR_CLIP_PLANE,
          //NearClipPlane ); 						READ_MICRO_CHUNK( cload, MICROCHUNKID_FAR_CLIP_PLANE,
          //FarClipPlane );
          ...
          //						READ_MICRO_CHUNK( cload, MICROCHUNKID_SNIPER_LISTENER,
          //SniperListener );
...
void CCameraClass::Update() {
  ...
  if (LerpTimeTotal) {
    ...
    if (profile.Lag.Length() > 0) {
      ...
      if (lag_ok) {
        ...
        Vector3 lerp =
            profile.Lag * WWMath::Clamp(LagPersistTimer, 0, 1); //( 0, 0.5f, 0.2f );	//		( y, z, x )
  ...
  if (profile.Distance != 0) {
    ...
    PhysOBBoxCollisionTestClass boxtest(box, end_pos - intermediate_pos, &res, DEFAULT_COLLISION_GROUP,
    ...
    PhysOBBoxCollisionTestClass boxtest(box, null_vec, &res, DEFAULT_COLLISION_GROUP, COLLISION_TYPE_CAMERA);
    PhysicsSceneClass::Get_Instance()->Cast_OBBox(boxtest);
  ...
  //	Debug_Say(( "Camera at %1.2f, %1.2f, %1.2f\n", tm.Get_Translation().X, tm.Get_Translation().Y,
  //tm.Get_Translation().Z ));
  ...
  if (star && is_star_determining_target) { // and tell the star what we are looking at...
    ...
    if (raytest.CollidedPhysObj != NULL && raytest.CollidedPhysObj->Get_Observer() != NULL) {
      DamageableGameObj *obj =
          ((CombatPhysObserverClass *)raytest.CollidedPhysObj->Get_Observer())->As_DamageableGameObj();
      ...
      if (obj && obj->As_SmartGameObj() && obj->As_SmartGameObj()->Is_Stealthed() && COMBAT_STAR &&
          obj->Is_Enemy(COMBAT_STAR)) {
        obj = NULL;
      }
...
void CCameraClass::Set_Profile_Height(float height) { CurrentProfile->Set_Height(height); }

void CCameraClass::Set_Profile_Distance(float distance) { CurrentProfile->Set_Distance(distance); }

float CCameraClass::Get_Profile_Zoom(void) { return CurrentProfile->Get_Zoom(); }
...
#define FP_TWEAK_RATE 0.1f;
  ...
  if (CombatManager::Is_First_Person()) {
    ...
    if (tweak.Length()) {
      ...
      //			Debug_Say(( "First Person Tweak ( %1.3f, %1.3f, %1.3f )\n", FirstPersonOffsetTweak.X,
      //FirstPersonOffsetTweak.Y, FirstPersonOffsetTweak.Z ));
    }
    ...
    if ((tilt_amount != 0) || (view_tilt_amount != 0) || (distance_amount != 0) || (fov_amount != 0) ||
        (height_amount != 0)) {

      Debug_Say((" Dist: %3.1f Height: %3.1f FOV %3.1f TransTilt: %3.1f ViewTilt: %3.1f\n", CurrentProfile->Distance,
                 CurrentProfile->Height, RAD_TO_DEG(CurrentProfile->FOV), RAD_TO_DEG(CurrentProfile->TranslationTilt),
                 RAD_TO_DEG(CurrentProfile->ViewTilt)));
    }

    if (Input::Get_State(INPUT_FUNCTION_DEBUG_FAR_CLIP_IN) || Input::Get_State(INPUT_FUNCTION_DEBUG_FAR_CLIP_OUT)) {
  ...
  if (!Is_Using_Host_Model() && Is_Star_Sniping()) {
    ...
    float amount = Input::Get_Amount(INPUT_FUNCTION_ZOOM_IN) - Input::Get_Amount(INPUT_FUNCTION_ZOOM_OUT);
  ...
  //	Adjust Tilt
  float tilt_amount = Input::Get_Amount(INPUT_FUNCTION_WEAPON_UP) - Input::Get_Amount(INPUT_FUNCTION_WEAPON_DOWN);

  //	Adjust Facing
  float turn_amount = Input::Get_Amount(INPUT_FUNCTION_WEAPON_LEFT) - Input::Get_Amount(INPUT_FUNCTION_WEAPON_RIGHT);
...
#define _2D_TARGETING_LIMIT 0.85f
  ...
  if (COMBAT_STAR != NULL && COMBAT_STAR->Get_Vehicle() == NULL) {
    ...
    soldier_turn = Input::Get_Amount(INPUT_FUNCTION_TURN_LEFT) - Input::Get_Amount(INPUT_FUNCTION_TURN_RIGHT);
  }
...
Vector3 CCameraClass::Get_First_Person_Offset_Tweak(void) { return FirstPersonOffsetTweak; }

void CCameraClass::Reset_First_Person_Offset_Tweak(void) { FirstPersonOffsetTweak = Vector3(0, 0, 0); }
...
void CCameraClass::Handle_Snap_Shot_Mode(void) {
  ...
  if (turn_amount == 0) {
    turn_amount = Input::Get_Amount(INPUT_FUNCTION_TURN_LEFT) - Input::Get_Amount(INPUT_FUNCTION_TURN_RIGHT);
  }
  ...
  float left = Input::Get_Amount(INPUT_FUNCTION_MOVE_LEFT) - Input::Get_Amount(INPUT_FUNCTION_MOVE_RIGHT);
  float up = Input::Get_Amount(INPUT_FUNCTION_MOVE_UP) - Input::Get_Amount(INPUT_FUNCTION_MOVE_DOWN);
