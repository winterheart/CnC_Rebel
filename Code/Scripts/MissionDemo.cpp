...
DECLARE_SCRIPT(MDD_Objective_Controller, "") {
  ...
  REGISTER_VARIABLES() { SAVE_VARIABLE(convoy_trucks, 1); }
...
DECLARE_SCRIPT(MDD_GDI_Soldier, "Area_ID:int, Soldier_Type=0:int") {
  ...
  REGISTER_VARIABLES() { SAVE_VARIABLE(stop_following, 1); }
...
DECLARE_SCRIPT(MDD_Stationary_Vehicle,
               "Area_ID:int"){void Created(GameObject * obj){// DEMO
...
DECLARE_SCRIPT(MDD_Flying_Vehicle,
               "Unit_ID:int"){void Created(GameObject * obj){Commands->Enable_Hibernation(obj, false);
...
DECLARE_SCRIPT(MDD_Commando, ""){void Created(GameObject * obj){Commands->Control_Enable(obj, false);
Commands->Set_Shield_Type(obj, "Blamo");
}
}
;
