...
DECLARE_SCRIPT(DAK_TestScriptOne, ""){void Damaged(GameObject * obj, GameObject *damager, float amount){
    if (Commands->Is_A_Star(damager)){ActionParamsStruct params;
...
DECLARE_SCRIPT(DAK_PlayerSpotted, ""){void Damaged(GameObject * obj, GameObject *damager,
                                                   float amount){GameObject *NodSAM = Commands->Find_Object(100012);
if (NodSAM) {
  ...
  void Damaged(GameObject * obj, GameObject * damager, float amount) {
    ...
    if (Commands->Get_Health(obj) <= 0.25 * Commands->Get_Max_Health(obj)) {
      ...
      if (firsttime == true) {
        ...
        } else if (strcmp(Get_Parameter("DeathType"), "Electic")) {
          params.Set_Animation("S_A_HUMAN.H_A_6X05", 0); // electric death
        } else
          params.Set_Animation("S_A_HUMAN.H_A_FLMA", 0); // gas death
...
DECLARE_SCRIPT(DAK_Vehicle_Regen_DAK, ""){void Created(GameObject * obj){Commands->Send_Custom_Event(obj, obj, 0, 0, 0);
}
...
DECLARE_SCRIPT(DAK_Electric_Death_DAK, ""){void Created(GameObject * obj){int time = Commands->Get_Random(1, 3);
...
DECLARE_SCRIPT(M00_BUILDING_EXPLODE_NO_DAMAGE_DAK, ""){void Killed(GameObject * obj, GameObject *killer){
