...
DECLARE_SCRIPT(M00_Soldier_Powerup_Grant, "") {
  ...
  REGISTER_VARIABLES() { SAVE_VARIABLE(Disabled, 1); }
  ...
  void Killed(GameObject * obj, GameObject * killer) {
    ...
    // 67% for Medium
    // 33% for Hard
    if (!Disabled && killer && Commands->Is_A_Star(killer) &&
        (Commands->Get_Random_Int(0, 3) >= Commands->Get_Difficulty_Level())) {
...
DECLARE_SCRIPT(M00_Soldier_Powerup_Disable,
               ""){void Created(GameObject * obj){Commands->Start_Timer(obj, this, 1.0f, 1);
}
...
DECLARE_SCRIPT(M00_Powerup_Destroy, ""){
    ...
    void Created(GameObject * obj){float lifespan = (30.0f - (Commands->Get_Difficulty_Level() * 10));
...
DECLARE_SCRIPT(M00_Tiberium_Refinery, "MoneyAmount:int,TimerLength:int"){
    void Created(GameObject * obj){Commands->Start_Timer(obj, this, Get_Int_Parameter("TimerLength"), 1);
}
...
DECLARE_SCRIPT(M00_CNC_Crate, ""){void Custom(GameObject * obj, int type, int param, GameObject *sender){
    if ((type == CUSTOM_EVENT_POWERUP_GRANTED) && (sender)){Commands->Give_Money(sender, 100.0f, false);
}
}
}
;

DECLARE_SCRIPT(M00_Death_Powerup, ""){void Custom(GameObject * obj, int type, int param, GameObject *sender){
    if ((type == CUSTOM_EVENT_POWERUP_GRANTED) && (sender)){Commands->Apply_Damage(sender, 10000.0f, "Death", NULL);
}
}
}
;