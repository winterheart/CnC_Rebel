...
DECLARE_SCRIPT(M00_Debug_Text_File_RMV, "Description=Object:string, Filename=DebugLog.txt:string") {
  ...
  const char *Reason_Lookup(ActionCompleteReason reason) {
    const char *reasons[] = {"ACTION_COMPLETE_NORMAL",
  ...
  void Damaged(GameObject * obj, GameObject * damager, float amount) {
    ...
    fprintf(file, "%s [ID %d] damaged by object %d.   %3.1f sec.\n", desc, Commands->Get_ID(obj),
            Commands->Get_ID(damager), difftime(current_time, start_time));
  }
  ...
  void Enemy_Seen(GameObject * obj, GameObject * enemy) {
    ...
    fprintf(file, "%s [ID %d] saw enemy: object %d.   %3.1f sec.\n", desc, Commands->Get_ID(obj),
            Commands->Get_ID(enemy), difftime(current_time, start_time));
  }
  ...
  void Action_Complete(GameObject * obj, int action_id, ActionCompleteReason reason) {
    ...
    fprintf(file, "Action %d complete on %s [ID %d] -- Reason: %s.   %3.1f sec.\n", action_id, desc,
            Commands->Get_ID(obj), Reason_Lookup(reason), difftime(current_time, start_time));
  }
  ...
  void Destroyed(GameObject * obj) { fclose(file); }
};
...
DECLARE_SCRIPT(M00_Enable_Physical_Collision_JDG, ""){void Created(GameObject * obj){Commands->Enable_Collisions(obj);
}
}
;

DECLARE_SCRIPT(M00_C130_Explosion, ""){void Killed(GameObject * obj, GameObject *killer){
...
DECLARE_SCRIPT(M00_Monitor_Attached_Primary, "") {
  ...
  REGISTER_VARIABLES() { SAVE_VARIABLE(object_detached, 1); }
...
DECLARE_SCRIPT(
    ...
    "Barrel01_Location:vector3, Barrel01_Type (1-8):int, Barrel01_Facing = 0:float, Barrel02_Location:vector3, "
    "Barrel02_Type (1-8):int, Barrel02_Facing = 0:float, Barrel03_Location:vector3, Barrel03_Type (1-8):int, "
    "Barrel03_Facing = 0:float, Barrel04_Location:vector3, Barrel04_Type (1-8):int, Barrel04_Facing = 0:float, "
  ...
  void Created(GameObject * obj) {
    ...
    const char *simple_barrels[8] = {
        "Simple_MetalDrum_01", "Simple_MetalDrum_02", "Simple_MetalDrum_03", "Simple_MetalDrum_04",
        "Simple_MetalDrum_05", "Simple_MetalDrum_06", "Simple_MetalDrum_07", "Simple_MetalDrum_08",
    };

    const char *barrels_animations[8] = {
        "DSP_METDRUM01.DSP_METDRUM01", "DSP_METDRUM02.DSP_METDRUM02", "DSP_METDRUM03.DSP_METDRUM03",
        "DSP_METDRUM04.DSP_METDRUM04", "DSP_METDRUM05.DSP_METDRUM05", "DSP_METDRUM06.DSP_METDRUM06",
        "DSP_METDRUM07.DSP_METDRUM07", "DSP_METDRUM08.DSP_METDRUM08",
    };
  ...
  void Custom(GameObject * obj, int type, int param, GameObject *sender) {
    if (param == M01_MODIFY_YOUR_ACTION_JDG && deadYet == false) // you've been ordered to blow up--DO IT!
    {
      ...
      const char *barrels_animations[8] = {
          ...
          "DSP_METDRUM02.DSP_METDRUM02", "DSP_METDRUM03.DSP_METDRUM03",
          "DSP_METDRUM04.DSP_METDRUM04", "DSP_METDRUM05.DSP_METDRUM05",
...
DECLARE_SCRIPT(M00_Purchase_Terminal_GDI,
               ""){void Poked(GameObject * obj, GameObject *poker){Commands->Display_GDI_Player_Terminal();
}
}
;

DECLARE_SCRIPT(M00_Purchase_Terminal_NOD,
               ""){void Poked(GameObject * obj, GameObject *poker){Commands->Display_NOD_Player_Terminal();
}
}
;

DECLARE_SCRIPT(M00_Purchase_Terminal_Mutant,
               ""){void Poked(GameObject * obj, GameObject *poker){Commands->Display_Mutant_Player_Terminal();
}
}
;
...
DECLARE_SCRIPT(M00_Nod_Obelisk_CNC, "Controller_ID=0:int") {
  ...
  REGISTER_VARIABLES() { SAVE_VARIABLE(obelisk_id, 1); }
  ...
  void Created(GameObject * obj) {
    ...
    if (obelisk) {
      ...
      Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("Controller_ID")), M00_OBELISK_WEAPON_ID,
                                  Commands->Get_ID(obelisk), 0);
    }
  }
  ...
  void Killed(GameObject * obj, GameObject * killer) {
    ...
    if (obelisk != NULL) {
      Commands->Send_Custom_Event(
          obj, obelisk, 3, 0,
          0); // this custom tells the obelisk weapon that the obelisk has been destroyed -- 02/12/2002 JDG
    }
  }
...
DECLARE_SCRIPT(M00_Nod_Obelisk, "Controller_ID=0:int") {
  ...
  void Created(GameObject * obj) {
    ...
    if (obelisk) {
      ...
      Commands->Send_Custom_Event(obj, Commands->Find_Object(controller_id), M00_OBELISK_WEAPON_ID,
                                  Commands->Get_ID(obelisk), 4.0f);
    }
  }
...
DECLARE_SCRIPT(M00_Select_Empty_Hands, "On_Created=1:int"){
    ...
    void Created(GameObject * obj){bool on_created = (Get_Int_Parameter("On_Created") == 1) ? true : false;
if (on_created) {
...
DECLARE_SCRIPT(M00_Change_L3Mutant_RadarMarker_JDG,
               ""){void Created(GameObject * obj){Commands->Set_Obj_Radar_Blip_Color(obj, RADAR_BLIP_COLOR_MUTANT);
}
}
;
...
DECLARE_SCRIPT(M00_Generic_Conv_DME, "ConvName:string"){
    void Created(GameObject * obj){int id = Commands->Create_Conversation(Get_Parameter("ConvName"), 99, 2000, false);
...
DECLARE_SCRIPT(M00_Damage_Modifier_DME, "Damage_multiplier:float, Star_Modifier=1:int, NotStar_Modifier=1:int, "
                                        "Killable_By_Star=1:int, Killable_by_NotStar=1:int") {
  ...
  void Damaged(GameObject * obj, GameObject * damager, float amount) {
    if (mod_on) {
      ...
      if ((!star_modifier && damager == STAR && star_killable) ||
          (!notStar_modifier && damager != STAR && notStar_killable)) {
        last_health = Commands->Get_Health(obj);
      }
