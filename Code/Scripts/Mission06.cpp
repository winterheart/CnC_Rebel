...
DECLARE_SCRIPT(M06_Objective_Controller, "") // 100018
{
  ...
  void Add_An_Objective(int id) {
    ...
    case 601: {
      Commands->Add_Objective(601, OBJECTIVE_TYPE_PRIMARY, OBJECTIVE_STATUS_PENDING, IDS_Enc_ObjTitle_Primary_M06_02,
                              NULL, IDS_Enc_Obj_Primary_M06_02);
      object = Commands->Find_Object(106952);
      if (object) {
        Commands->Set_Objective_Radar_Blip_Object(601, object);
        Commands->Set_Objective_HUD_Info_Position(601, 90.0f, "POG_M06_1_03.tga", IDS_POG_HACK,
                                                  Commands->Get_Position(object));
      }
    } break;
    // Rescue Scientists
    case 603: {
      Commands->Add_Objective(603, OBJECTIVE_TYPE_PRIMARY, OBJECTIVE_STATUS_PENDING, IDS_Enc_ObjTitle_Primary_M06_01,
                              NULL, IDS_Enc_Obj_Primary_M06_01);
      object = Commands->Find_Object(101010);
      if (object) {
        Commands->Set_Objective_Radar_Blip_Object(603, object);
        Commands->Set_Objective_HUD_Info_Position(603, 90.0f, "POG_M06_1_04.tga", IDS_POG_RESCUE,
                                                  Commands->Get_Position(object));
      }
    } break;
    // Eliminate Mendoza
    case 604: {
      Commands->Add_Objective(604, OBJECTIVE_TYPE_PRIMARY, OBJECTIVE_STATUS_PENDING, IDS_Enc_ObjTitle_Primary_M06_04,
                              NULL, IDS_Enc_Obj_Primary_M06_04);
      ...
      if (object) {
        ...
        Commands->Set_Objective_HUD_Info_Position(604, 90.0f, "POG_M06_1_01.tga", IDS_POG_ELIMINATE,
                                                  Commands->Get_Position(object));
      }
    } break;
    // Rescue Resistance
    case 605: {
      Commands->Add_Objective(605, OBJECTIVE_TYPE_TERTIARY, OBJECTIVE_STATUS_HIDDEN, IDS_Enc_ObjTitle_Hidden_M06_02,
                              NULL, IDS_Enc_Obj_Hidden_M06_02);
      object = Commands->Find_Object(101016);

    } break;
    // Release GDI prisoner
    case 607: {
      Commands->Add_Objective(607, OBJECTIVE_TYPE_TERTIARY, OBJECTIVE_STATUS_HIDDEN, IDS_Enc_ObjTitle_Hidden_M06_03,
                              NULL, IDS_Enc_Obj_Hidden_M06_03);
    } break;
    // Plunder Raveshaw's hidden cache
    case 608: {
      Commands->Add_Objective(608, OBJECTIVE_TYPE_TERTIARY, OBJECTIVE_STATUS_HIDDEN, IDS_Enc_ObjTitle_Hidden_M06_04,
                              NULL, IDS_Enc_Obj_Hidden_M06_04);
    } break;
    // Deactivate alarm system
    case 609: {
      Commands->Add_Objective(609, OBJECTIVE_TYPE_SECONDARY, OBJECTIVE_STATUS_PENDING, IDS_Enc_ObjTitle_Hidden_M06_01,
                              NULL, IDS_Enc_Obj_Hidden_M06_01);
      ...
      if (star_greenhouse == (WWMath::Min(star_greenhouse, star_shower))) {
        ...
        if (object) {
          ...
          Commands->Set_Objective_HUD_Info_Position(609, 90.0f, "POG_M08_2_02.tga", IDS_POG_DISABLE,
                                                    Commands->Get_Position(object));
        }
      } else {
    ...
    case 611: {
      ...
      Commands->Add_Objective(611, OBJECTIVE_TYPE_PRIMARY, OBJECTIVE_STATUS_PENDING, IDS_Enc_ObjTitle_Primary_M06_03,
                              NULL, IDS_Enc_Obj_Primary_M06_03);
      object = Commands->Find_Object(101527);
      if (object) {
        Commands->Set_Objective_Radar_Blip_Object(611, object);
        Commands->Set_Objective_HUD_Info_Position(611, 90.0f, "POG_M06_1_05.tga", IDS_POG_ESCORT,
                                                  Commands->Get_Position(object));
      }
    } break;
    }
...
DECLARE_SCRIPT(M06_WarRoom_Computer, "") {
  ...
  void Poked(GameObject * obj, GameObject * poker) {
    if (Commands->Is_A_Star(poker) && (!already_poked)) {
      // Data decryption complete. Internal computer systems indicate Dr. Ignatio Mobius is located beneath the Chateau.
      // Access is gained through the dining hall.\n
  ...
  void Action_Complete(GameObject * obj, int action_id, ActionCompleteReason reason) {
    ...
    if (action_id == 300603 &&
        (reason == ACTION_COMPLETE_CONVERSATION_ENDED || reason == ACTION_COMPLETE_CONVERSATION_INTERRUPTED)) {
      // New Primary Mission Objective: Evacuate Doctors' Ignatio and Sydney Mobius.  Basement, NE section.  Requires
      // level 2 security keycard.
...
const int Sydney_Move_Table[] = {101567, 101569, 101571, 101573, 101575, 101577, 101579,
                                 101581, 101582, 101584, 101586, 101587, 101588};
...
DECLARE_SCRIPT(M06_Sydney_Mobius, "") {
  ...
  void Custom(GameObject * obj, int type, int param, GameObject *sender) {
    ...
    if (type == M06_RELOCATE) {
      ...
      if (param == 1) {
        ...
        //		GameObject *mendoza = Commands->Create_Object ("Nod_FlameThrower_3Boss", Vector3
        //(-40.572F, 28.942F, 1.605F)); 		if (mendoza != NULL) { 			Commands->Set_Facing (mendoza, -50.0F);
  ...
  void Action_Complete(GameObject * obj, int action_id, ActionCompleteReason reason) {
    ...
    if ((action_id == SYDNEY_MOVE_LOC) && (action_id != Commands->Get_Action_ID(obj)) &&
...
Commands->Create_Explosion("Chateau_Explosions_Twiddler",
                           Commands->Get_Position(Commands->Find_Object(explode_loc[param])), NULL);
}
}
}
;
...
DECLARE_SCRIPT(M06_Mendoza, ""){
    ...
    void Killed(GameObject * obj,
                GameObject *killer){Commands->Send_Custom_Event(obj, Commands->Find_Object(100018), 604, 1, 0.0f);
}
}
;
...
DECLARE_SCRIPT(M06_Gate_Guards, "Soldier_ID=0:int") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(disable_tower_spawn, 1); }
...
DECLARE_SCRIPT(M06_Escort_Tank, "") {
  ...
  void Enemy_Seen(GameObject * obj, GameObject * enemy) {
    if (!attacking) {
      ...
      attacking = true;
      Commands->Attach_Script(obj, "M08_Mobile_Vehicle",
...
DECLARE_SCRIPT(M06_Alarm_Controller, "") {
  ...
  void Custom(GameObject * obj, int type, int param, GameObject *sender) {
    ...
    Commands->Debug_Message(
        "Custom received Alarm_Controller.  type %d param %d sender_id %d sender_pos X %d, Y %d, Z %d.\n", type, param,
        Commands->Get_ID(sender), sender_pos.X, sender_pos.Y, sender_pos.Z);
...
DECLARE_SCRIPT(M06_Alarm_Switch, "") {
  ...
  void Created(GameObject * obj) {
    ...
    Commands->Debug_Message("Position of alarm switch %d: X: %d, Y: %d, Z:%d", Commands->Get_ID(obj), alarm_pos.X,
                            alarm_pos.Y, alarm_pos.Z);
...
DECLARE_SCRIPT(M06_Courtyard_Controller, "") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(dead_courtyard_eagle, 1); }
  ...
  void Custom(GameObject * obj, int type, int param, GameObject *sender) {
    ...
    if (type == M06_DEAD_COURTYARD_EAGLE) {
      ...
      if (dead_courtyard_eagle == 3) {
        ...
        //		GameObject * search_apache = Commands->Create_Object ( "Nod_Apache", Vector3(-99.996f,
        //-135.659f, 37.552f)); 		Commands->Attach_Script(search_apache, "M06_Courtyard_Apache", "");
      }
    }
  }
};
...
DECLARE_SCRIPT(M06_Hedgemaze_Controller, "") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(dead_hedgemaze_eagle, 1); }
...
DECLARE_SCRIPT(M06_Interior_Controller, "") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(dead_interior_patrol, 1); }
...
DECLARE_SCRIPT(M06_Apply_Damage, "") {
  ...
  void Created(GameObject * obj) { Commands->Start_Timer(obj, this, 3.0f, APPLY_DAMAGE); }
...
DECLARE_SCRIPT(M06_Alarm_Engineer, "") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(fixing_alarm, 1); }
...
DECLARE_SCRIPT(M06_Resistance_Raider_DLS, "") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(talking, 1); }
...
DECLARE_SCRIPT(M06_Greenhouse_Shaft_Unit, "") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(soldier_id, 1); }
...
DECLARE_SCRIPT(M06_WarRoom_Officer_DLS, "") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(has_keycard, 1); }
...
DECLARE_SCRIPT(M06_KaneHead, "") {
  ...
  void Action_Complete(GameObject * obj, int action_id, ActionCompleteReason reason) {
    ...
    if ((action_id == 300124 && reason == ACTION_COMPLETE_CONVERSATION_ENDED) ||
        (action_id == 300124 && reason == ACTION_COMPLETE_CONVERSATION_INTERRUPTED)) {
      Commands->Destroy_Object(obj);
    }
  }
};
...
void Custom(GameObject *obj, int type, int param, GameObject *sender) {
  ...
  if (type == M06_START_CONVERSATION) {
    if (Commands->Find_Object(101384) && Commands->Find_Object(101385)) {
      // Both of you have proven yourself loyal, and as such I feel you are prime candidates for a very sensitive
      // mission.\n
...
DECLARE_SCRIPT(M06_Courtyard_Apache, "") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(out_of_range, 1); }
  ...
  void Action_Complete(GameObject * obj, int action_id,
                       ActionCompleteReason reason) // this is called when the servant reaches the destination location.
  {
    ActionParamsStruct params;
  ...
  void Custom(GameObject * obj, int type, int param, GameObject *sender) {
    ...
    if (type == M06_FLYOVER_COMPLETE && flyovers) {
      ...
      const char *flyovers[11] = {
          "M06_XG_VehicleDrop0.txt", "M06_XG_VehicleDrop1.txt", "M06_XG_VehicleDrop2.txt",

          "X6A_Apache_03.txt",       "X6A_Apache_04.txt",       "X6A_Apache_05.txt",       "X6A_Apache_06.txt",
          "X6A_Apache_07.txt",       "X6A_Apache_08.txt",       "X6A_Apache_09.txt",
...
DECLARE_SCRIPT(M06_Drop_Thunder_Squad, "") {
  ...
  void Entered(GameObject * obj, GameObject * enterer) {
    if ((Commands->Is_A_Star(enterer)) && (!already_entered)) {
      ...
      // This is Nod Helo Delta 14.  We are hot dropping a squad of Black Hand at your doorstep, be advised they are on
      // the ground only long enough to re-equip, skip the standard check in.  Raveshaw's orders.\n
...
DECLARE_SCRIPT(M06_Servant_Behavior,
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(enemy_seen, 1); }
...
DECLARE_SCRIPT(M06_Collapse_Zone, "Zone_ID=0:int") {
  ...
  void Created(GameObject * obj) { already_entered = true; }
...
{void Custom(GameObject * obj, int type, int param, GameObject *sender){
    if (type == CUSTOM_EVENT_POWERUP_GRANTED){// Reveal Mutant Templar
