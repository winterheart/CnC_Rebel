...
DECLARE_SCRIPT(DAY_TestScriptTwo, ""){void Created(GameObject * obj){Commands->Innate_Disable(obj);
}
}
;

DECLARE_SCRIPT(DAY_TestScriptThree, ""){void Created(GameObject * obj){
...
DECLARE_SCRIPT(M00_GrantPowerup_Created, "WeaponDef:string"){
    ...
    void Created(GameObject * obj){const char *Weapon = Get_Parameter("WeaponDef");
Commands->Give_PowerUp(obj, Weapon, false);
}
}
;

DECLARE_SCRIPT(M00_VisceroidInnate_DAY, ""){void Created(GameObject * obj){Commands->Set_Loiters_Allowed(obj, false);
...
DECLARE_SCRIPT(M00_DestroyedStateObject_DAY, "OriginalModelFacing:float,DestroyedModelPreset:string"){
    ...
    void Destroyed(GameObject * obj){Vector3 object_pos = Commands->Get_Position(obj);
...
DECLARE_SCRIPT(M00_PlayAnimation_DestroyObject_DAY, "AnimationName:string"){
    ...
    void Created(GameObject * obj){const char *animname = Get_Parameter("AnimationName");
Commands->Set_Animation(obj, animname, 0, NULL, 0.0f, -1.0f, false);
}

void Animation_Complete(GameObject *obj, const char *animation_name) { Commands->Destroy_Object(obj); }
}
;
...
DECLARE_SCRIPT(M00_Set_Background_Music_DAY, "MusicFile:string"){
    ...
    void Created(GameObject * obj){Commands->Set_Background_Music(Get_Parameter("MusicFile"));
}
}
;
