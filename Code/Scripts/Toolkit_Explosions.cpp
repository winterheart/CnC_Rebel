...
DECLARE_SCRIPT(M00_Explosion_Create_RMV,
               "Start_Now=0:int, Create_At_Obj=0:int, Receive_Type:int, Receive_Param_On=1:int, Explosion_Name:string, "
...
DECLARE_SCRIPT(M00_Explosion_Create_At_Bone_RMV,
  ...
  void Created(GameObject * obj) {
    ...
    if (Get_Int_Parameter("Start_Now")) {
      ...
      if (Commands->Find_Object(target_id) != NULL)
        Commands->Create_Explosion_At_Bone(Get_Parameter("Explosion_Name"), Commands->Find_Object(target_id),
                                           Get_Parameter("Bone_Name"), obj);
    }
  }
...
DECLARE_SCRIPT(
    ...
    "Explosion_Name:string, Delay_Min=0.0f:float, Delay_Max=4.0f:float, Loc_ID0=0:int, Loc_ID1=0:int, Loc_ID2=0:int, "
    "Loc_ID3=0:int, Loc_ID4=0:int, Loc_ID5=0:int, Loc_ID6=0:int, Loc_ID7=0:int, Loc_ID8=0:int, Loc_ID9=0:int") {
  bool active;
  int random;
  int last;
  int loc_id[10];
  ...
  void Timer_Expired(GameObject * obj, int timer_id) {
    if (timer_id == EXPLODE && active) {
      ...
      Commands->Create_Explosion(Get_Parameter("Explosion_Name"),
                                 Commands->Get_Position(Commands->Find_Object(loc_id[random])), obj);
      last = random;
...
DECLARE_SCRIPT(M00_NukeStrike_Anim, ""){
    ...
    void Created(GameObject * obj){Commands->Send_Custom_Event(obj, obj, 0, 1, 206 / 30);
