...
void M00_Controller_Sound_RAD(GameObject *obj, const Vector3 &sound_loc, const char *sound_effect,
...
DECLARE_SCRIPT(
    ...
    "Start_Now=0:int, Receive_Type:int, Receive_Param_On=1:int, Receive_Param_Off=0:int, Sound_Effect:string, "
    "Play_Count=1:int, Sound_Delay_Min=0.1:float, Sound_Delay_Max=0.1:float, Debug_Mode=0:int") {
  int play_count;
  int current_count;
...
DECLARE_SCRIPT(
    ...
    "Sound_Effect:string, Play_Count=1:int, Sound_Delay_Min=0.1:float, Sound_Delay_Max=0.1:float, Debug_Mode=0:int") {
  int play_count;
  int current_count;
  float delay_time_min;
  float delay_time_max;
...
DECLARE_SCRIPT(
    ...
    "Start_Now=0:int, Receive_Type:int, Receive_Param_On=1:int, Receive_Param_Off=0:int, Sound_Effect:string, "
    "Play_Count=1:int, Sound_Delay_Min=0.1:float, Sound_Delay_Max=0.1:float, Origin:vector3, Debug_Mode=0:int") {
  int play_count;
  int current_count;
  float delay_time_min;
  float delay_time_max;
...
DECLARE_SCRIPT(M00_Sound_Play_3D_At_Bone_RMV,
               "Start_Now=0:int, Receive_Type:int, Receive_Param_On=1:int, Receive_Param_Off=0:int, Object_ID:int, "
...
DECLARE_SCRIPT(RMV_Audio_Timer_Delay, "Target_ID:int, Custom_Type=0:int, Custom_Param=0:int, Delay_Min=0.0:float, "
                                      "Delay_Max=0.0:float, Timer_ID=0:int, Repeat=1:int, Randomize_Each_Time=1:int") {
  int type, param, id;
  float min, max, delay;
  bool repeat, random;
...
DECLARE_SCRIPT(RMV_Audio_Sound_Player_Preset,
...
DECLARE_SCRIPT(RMV_Sound_Play_Near_Player, "Receive_Type:int, Receive_Param:int, Max_Offset:vector3, "
                                           "Sound_Preset:string, Frequency_Min:float, Frequency_Max=0.0:float") {
  ...
  void Timer_Expired(GameObject * obj, int timer_id) {
    if ((enabled) && (timer_id == SOUND_PLAYER_TIMER)) {
      ...
      if (freq_max == 0.0)
        ...
        freq = Commands->Get_Random(freq_min, freq_max);
...
DECLARE_SCRIPT(M00_BuildingStateSoundSpeaker,
               "Sound_Normal:string,Sound_Destroyed:string,BuildingController_ID:int,Is_3D=1:int,Offset:vector3,Offset_"
               "Randomness:vector3,Frequency_Min=-1:float,Frequency_Max:float,Is_3D_Destroyed=1:int,Offset_Destroyed:"
               "vector3,Offset_Randomness_Destroyed:vector3,Frequency_Min_Destroyed=-1:float,Frequency_Max_Destroyed:"
               "float, Explosion_Name:string") {
    ...
    if (type == CUSTOM_EVENT_SOUND_ENDED) {
      if (building_destroyed == false) {
        ...
        if (Get_Int_Parameter("Frequency_Min_Destroyed") != -1) {
          float time = Commands->Get_Random(Get_Float_Parameter("Frequency_Min_Destroyed"),
                                            Get_Float_Parameter("Frequency_Max_Destroyed"));
          Commands->Start_Timer(obj, this, time, 0);
        }
      }
    }
...
DECLARE_SCRIPT(M00_BuildingStateSoundController, "BuildingSpeaker_ID:int") {
  ...
  REGISTER_VARIABLES() { SAVE_VARIABLE(object_from, 1); }

  void Created(GameObject * obj) { object_from = Get_Int_Parameter("BuildingSpeaker_ID"); }
