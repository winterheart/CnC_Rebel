...
DECLARE_SCRIPT(M00_Siege_Zone_RAD,
...
DECLARE_SCRIPT(M00_Siege_Actor_RAD, "Start_Now=1:int, Receive_Type:int, Receive_Param_On=1:int, "
                                    "Receive_Param_Off=0:int, My_Group_ID=0:int, Debug_Mode=0:int") {
  int registered_zone;
  int my_group_id;
  bool script_active;
  bool debug_mode;
    ...
    if (script_active) {
      if ((sound.Type == M00_SOUND_SIEGE_ZONE) && (!registered_zone)) {
        SCRIPT_DEBUG_MESSAGE(
            ("M00_Siege_Actor_RAD has heard a zone shouting for an actor, sound ID = %d.\n", sound.Type));
