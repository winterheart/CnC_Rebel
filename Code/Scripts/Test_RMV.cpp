...
DECLARE_SCRIPT(RMV_Test_Script, ""){
    void Sound_Heard(GameObject * obj, const CombatSound &sound){Commands->Debug_Message("Sound heard.\n");
}
}
;
...
DECLARE_SCRIPT(M00_C130_Dropoff_RMV, "ObjToCreate=:string") {
  ...
  void Animation_Complete(GameObject * obj, const char *anim) { Commands->Destroy_Object(obj); }
};

DECLARE_SCRIPT(M00_Commando_Death_Taunt, ""){void Killed(GameObject * obj, GameObject *killer){
    if (Commands->Is_A_Star(killer)){float random = Commands->Get_Random(1, 20);
switch (int(random)) {
case 1:
  Commands->Create_Sound("tuffguy1", Vector3(0, 0, 0), killer);
  break;
case 5:
  Commands->Create_Sound("laugh1", Vector3(0, 0, 0), killer);
  break;
case 10:
  Commands->Create_Sound("keepem1", Vector3(0, 0, 0), killer);
  break;
case 15:
  Commands->Create_Sound("lefty1", Vector3(0, 0, 0), killer);
  break;
}
}
}
...
DECLARE_SCRIPT(M00_Damaged_Warning, "") {
  ...
  REGISTER_VARIABLES() { SAVE_VARIABLE(just_sent, 1); }

  void Created(GameObject * obj) { just_sent = false; }
  ...
  void Timer_Expired(GameObject * obj, int timer_id) { just_sent = false; }
};

DECLARE_SCRIPT(M00_Put_Script_On_Commando, "") {
  enum { SCRIPT_ON_COMMANDO_TIMER };

  void Created(GameObject * obj) { Commands->Start_Timer(obj, this, 0.2f, SCRIPT_ON_COMMANDO_TIMER); }
...
DECLARE_SCRIPT(RMV_Hostage_Rescue_Point, ""){void Entered(GameObject * obj, GameObject *enterer){
    if (!Commands->Is_A_Star(enterer)){Commands->Send_Custom_Event(obj, enterer, 999, 999, 0.0f);
}
}
}
;

DECLARE_SCRIPT(RMV_Trigger_Killed, "ID:int, Type:int, Param:int"){void Killed(GameObject * obj, GameObject *killer){
...
DECLARE_SCRIPT(RMV_Home_Point, "Radius:float"){void Created(GameObject * obj){
...
DECLARE_SCRIPT(RMV_Test_Facing, ""){
    void Damaged(GameObject * obj, GameObject *damager, float amount){Vector3 my_pos = Commands->Get_Position(obj);
...
DECLARE_SCRIPT(RMV_Camera_Behavior, "Angle:float, Alarm_ID=0:int, Is_Gun=0:int, Delay=0.0:float") {
  ...
  void Killed(GameObject * obj, GameObject * killer) { Commands->Stop_Sound(sound_id, true); }
};

DECLARE_SCRIPT(RMV_Cinematic_Position,
               "Bone:string"){void Created(GameObject * obj){Commands->Start_Timer(obj, this, 1.0f, 0);
}
...
DECLARE_SCRIPT(M00_Destroyed_Turret,
               ""){void Killed(GameObject * obj, GameObject *killer){Vector3 my_pos = Commands->Get_Position(obj);
...
DECLARE_SCRIPT(RMV_Transport_Evac, "Number:int, Nod=0:int") {
  ...
  void Custom(GameObject * obj, int type, int param, GameObject *sender) { loading_complete = true; }
};

DECLARE_SCRIPT(Poke_And_Play_Cinematic, "Text_File:string, Location=0 0 0:vector3"){
    void Poked(GameObject * obj, GameObject *poker){Vector3 pos = Get_Vector3_Parameter("Location");
...
DECLARE_SCRIPT(M00_Ion_Cannon_Sound, "Number=0:int"){void Created(GameObject * obj){int num = Get_Int_Parameter(0);
Vector3 pos = Commands->Get_Position(obj);
...
DECLARE_SCRIPT(RMV_Trigger_Poked_2, "Target:int, Type:int, Param:int"){void Poked(GameObject * obj, GameObject *poker){
...
DECLARE_SCRIPT(RMV_Test_Stealth, ""){void Created(GameObject * obj){Commands->Enable_Stealth(obj, true);
}
}
;
