...
DECLARE_SCRIPT(DLS_Spawner, ""){// Register variables to be Auto-Saved
                                // All variables must have a unique ID, less than 256, that never changes
                                REGISTER_VARIABLES(){

                                }

                                void Created(GameObject * obj){Commands->Enable_Spawner(Commands->Get_ID(obj), false);
}
}
;

DECLARE_SCRIPT(DLS_Two_Way, ""){// Register variables to be Auto-Saved
                                // All variables must have a unique ID, less than 256, that never changes
                                REGISTER_VARIABLES(){
...
DECLARE_SCRIPT(DLS_Invulnerable_Test, "") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(debug_mode, 2); }
...
DECLARE_SCRIPT(DLS_Tank_Path_Test, "Debug_Mode=0:int") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(debug_mode, 1); }

  void Created(GameObject * obj) { Commands->Start_Timer(obj, this, 2.0f, STAR_VISIBLE); }
...
DECLARE_SCRIPT(DLS_Flame_Tank_Test, ""){// Register variables to be Auto-Saved
                                        // All variables must have a unique ID, less than 256, that never changes
                                        REGISTER_VARIABLES(){
...
DECLARE_SCRIPT(DLS_Innate_Force_State, "") {
  ...
  void Created(GameObject * obj) { Commands->Start_Timer(obj, this, 8.0f, FOOTSTEPS_HEARD); }
...
DECLARE_SCRIPT(M00_Reinforcement_C130, "Controller_ID:int") {
  ...
  void Created(GameObject * obj) { sound_id = Commands->Create_3D_Sound_At_Bone("C130_Idle_01", obj, "BODYMAIN"); }
...
DECLARE_SCRIPT(DLS_Drop_Unit, "Box_ID:int"){
    ...
    void Damaged(GameObject * obj, GameObject *damager, float amount){int box_id = Get_Int_Parameter("Box_ID");
switch (box_id) {
case 1: {
  GameObject *controller = Commands->Create_Object("Invisible_Object", Vector3(0.0f, 0.0f, 0.0f));
  Commands->Set_Facing(controller, 0.000f);
...
DECLARE_SCRIPT(DLS_CNC_Sound, "Box_ID:int"){
    ...
    void Damaged(GameObject * obj, GameObject *damager,
                 float amount){Commands->Create_Sound("CnC_Healer_Sound", Commands->Get_Position(obj), obj);
}
}
;
...
DECLARE_SCRIPT(DLS_Spawn_Engineer, ""){
    ...
    void Damaged(GameObject * obj, GameObject *damager,
                 float amount){Commands->Set_Health(obj, Commands->Get_Max_Health(obj));
...
DECLARE_SCRIPT(DLS_Playertype, ""){
    ...
    void Poked(GameObject * obj, GameObject *poker){Commands->Set_Player_Type(obj, SCRIPT_PLAYERTYPE_GDI);
...
DECLARE_SCRIPT(DLS_SSM_Test, "") {
  ...
  void Damaged(GameObject * obj, GameObject * damager) {
    ...
    Commands->Set_Animation(Commands->Find_Object(ssm_missile_id), "v_nod_ssm_Missl.v_nod_ssm_Missl", 0, NULL, 0.0f,
                            -1.0f, false);
  }
};
...
DECLARE_SCRIPT(DLS_Test_Flyovers, "") {
  ...
  void Created(GameObject * obj) { Commands->Start_Timer(obj, this, 17.0f, WHERE_AM_I); }
...
DECLARE_SCRIPT(DLS_Math, ""){void Created(GameObject * obj){Vector3 pos = Commands->Get_Position(obj);
float facing = Commands->Get_Facing(obj);
  ...
  void Custom(GameObject * obj, int type, int param, GameObject *sender) {
    ...
    if (type == MX0_STAR_AREA) {
      ...
      if (star_area == 1 && !area4_activated) {
        ...
        //		GameObject * chinook_obj2 = Commands->Create_Object ( "Invisible_Object",
        //Commands->Get_Position(Commands->Find_Object(1500051))); 		Commands->Set_Facing(chinook_obj2, 0.0f);
        ...
        Commands->Send_Custom_Event(obj, Commands->Find_Object(humvee_id), MX0_SPECIFIC_ACTION, MX0_DISCOVERS_NOD_BASE,
                                    0.0f);
      }
    }
    // Actor IDs
    ...
    if (type == MX0_GDI_REINFORCEMENT_KILLED) {
      ...
      if (mx0_gdi_reinforcement_killed == 2) {
        ...
        GameObject *chinook_obj2 =
            Commands->Create_Object("Invisible_Object", Commands->Get_Position(Commands->Find_Object(1500102)));
  ...
  void Timer_Expired(GameObject * obj, int timer_id) {
    ...
    if (timer_id == AREA4_ACTIVATED) {
      if (star_area > 0) // Area 2 or Area 3
      {
        ...
        Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper1_id), MX0_SPECIFIC_ACTION,
                                    MX0_ROCKETTROOPER_SEES_OBELISK, 4.0f);

      } else {
        if (!say_need_here) {
          say_need_here = true;
          // We need you up here, Sir.
          Commands->Send_Custom_Event(obj, Commands->Find_Object(humvee_id), MX0_SPECIFIC_ACTION, MX0_NEED_YOU_HERE,
                                      0.0f);
        }
    ...
    if (timer_id == HUMMVEE_DESTRUCTION) {
      if (star_area > 1) // Area 2 or Area 3
      {
        ...
        Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper1_id), MX0_SPECIFIC_ACTION,
                                    MX0_ROCKETTROOPER_HOT_OBELISK, 4.0f);

        // Relocate Troops
        Relocate_Soldiers(obj);
      } else {
        // We need your help - over here, Sir!
        Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper2_id), MX0_SPECIFIC_ACTION,
                                    MX0_ROCKETTROOPER_CANT_TURN_BACK, 0.0f);
        // "We need you Commando"
    ...
    if (timer_id == MEDIUM_TANK_DESTRUCTION) {
      ...
      case 3: {
        ...
        //		Commands->Send_Custom_Event( obj, Commands->Find_Object(1500020), MX0_OBELISK_MEDIUM_TANK, 2,
        //0.0f);
    ...
    if (timer_id == CONVERSATION_HAVOC_TAKE_OUT_SAMS) {
      ...
      Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper2_id), MX0_SPECIFIC_ACTION,
                                  MX0_ROCKETTROOPER_OBELISK_DOWN, 3.0f);
    }
      ...
      case 3: {
        ...
        if (Commands->Find_Object(1500015) || Commands->Find_Object(1500016)) {
          ...
          if (!say_take_out_sams) {
            ...
            Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper2_id), MX0_SPECIFIC_ACTION,
                                        MX0_ROCKETTROOPER_TAKE_SAMS, 9.0f);
        ...
        // If SAMs no longer exist, trigger A10 strike
          ...
          Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper2_id), MX0_SPECIFIC_ACTION,
                                      MX0_ROCKETTROOPER_SAMS_HISTORY, 0.0f);
          // Trooper1 - There, on the wall!
          Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper1_id), MX0_SPECIFIC_ACTION,
                                      MX0_TROOPER_UP_ON_WALL, 2.0f);
          // Nod RocketSoldier on base wall fire at Havoc's tank
          Commands->Send_Custom_Event(obj, Commands->Find_Object(nod_base_rocketsoldier_a_id), MX0_SPECIFIC_ACTION,
                                      player_tank_id, 0.0f);
          Commands->Send_Custom_Event(obj, Commands->Find_Object(nod_base_rocketsoldier_b_id), MX0_SPECIFIC_ACTION,
                                      player_tank_id, 0.0f);
    ...
    if (timer_id == ION_CANNON_STRIKE) {
      ...
      Commands->Start_Conversation(conv_id, 1);
      // Ion Cannon strike
      GameObject *ion_cannon_strike =
          Commands->Create_Object("Nod_RocketSoldier_1Off", Commands->Get_Position(Commands->Find_Object(1500087)));
...
DECLARE_SCRIPT(MX0_Vehicle_DLS,
...
DECLARE_SCRIPT(MX0_GDI_Killed_DLS, "Unit_ID=0:int"){
    ...
    void Damaged(GameObject * obj, GameObject *damager, float amount){
        ...
        if ((damager == Commands->Find_Object(1500020)) &&
            (Commands->Find_Object(1500020))){Commands->Apply_Damage(obj, 50000.0f, "STEEL", NULL);
}
}
...
void Killed(GameObject *obj, GameObject *killer) {
  ...
  case 2: {
    GameObject *destroyed_medium_tank =
        Commands->Create_Object("MX0_GDI_Medium_Tank_Destroyed", Commands->Get_Position(obj));
...
DECLARE_SCRIPT(MX0_GDI_Soldier_DLS,
               "Attack_Loc0=0:int, Attack_Loc1=0:int, Attack_Loc2=0:int, Attack_Loc3=0:int, Speed=1.0:float") {
  int attack_loc[4];
  int loc;
  float speed;
...
DECLARE_SCRIPT(MX0_Gun_Emplacement_DLS, "Left_Point=0:int, Right_Point=0:int") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(left, 1); }

  void Created(GameObject * obj) { left = true; }
...
DECLARE_SCRIPT(MX0_SAM_DLS, ""){// Register variables to be Auto-Saved
                                // All variables must have a unique ID, less than 256, that never changes
                                REGISTER_VARIABLES(){
                                    //		SAVE_VARIABLE( left, 1 );
                                }
...
DECLARE_SCRIPT(MX0_Plant_Ion_Beacon_DLS, ""){// Register variables to be Auto-Saved
                                             // All variables must have a unique ID, less than 256, that never changes
                                             REGISTER_VARIABLES(){
                                                 //		SAVE_VARIABLE( left, 1 );
                                             }
...
DECLARE_SCRIPT(DLS_Star_No_Fall, "") {
  ...
  void Created(GameObject * obj) { Commands->Start_Timer(obj, this, 1.0f, ATTACH_SCRIPT); }
...
DECLARE_SCRIPT(MX0_Explosive_Barrels_DLS, "Logical_Sound=0:int, Radius:float"){
    ...
    void Sound_Heard(GameObject * obj, const CombatSound &sound){
        if (sound.Type == Get_Int_Parameter("Logical_Sound")){Commands->Apply_Damage(obj, 50000.0f, "STEEL", NULL);
}
}
