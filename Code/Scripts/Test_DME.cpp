...
DECLARE_SCRIPT(DME_Test_Powerup, "") {
  ...
  REGISTER_VARIABLES() { SAVE_VARIABLE(reward_type, 1); }
...
DECLARE_SCRIPT(DME_Destroy_Item, "timer_length: float"){
    void Created(GameObject * obj){float timer_length = Get_Float_Parameter("timer_length");
...
DECLARE_SCRIPT(DME_Waypath_test, ""){void Created(GameObject * obj){ActionParamsStruct params;
...
DECLARE_SCRIPT(DME_Test_Ejected_Soldier,
               ""){void Killed(GameObject * obj, GameObject *killer){Vector3 spawn_loc = Commands->Get_Position(obj);
spawn_loc.Z -= 30.0f;
...
DECLARE_SCRIPT(DME_Test_Paradrop, ""){void Created(GameObject * obj){
...
DECLARE_SCRIPT(DME_Test_Worker_Wander, "Work_Area=3:int") {
  ...
  void Custom(GameObject * obj, int type, int param,
              GameObject *sender) // recieves custom from dave's arrow as to if the location is occupied or not.
  ...
  void Action_Complete(
      GameObject * obj, int action_id,
      ActionCompleteReason complete_reason) // this is called when the servant reaches the destination location.
  ...
  void Timer_Expired(GameObject * obj, int timer_id) {
    if (timer_id == 500) // this timer resets the servant from his animation loop and calls Worker_Loop to send him to a
                         // new location.
    {
      anim_playing = false;
    ...
    else if (timer_id == 510) // this timer resets the servant after seeing the star.  He completes his animation and
                              // returns to work.
    {
      star_seen = false;
  ...
  void Enemy_Seen(GameObject * obj,
                  GameObject * enemy) // this function sets the servant in a "don't hurt me!" stance (based on a timer)
  {
    if (star_seen == false) {
...
DECLARE_SCRIPT(M05_Tech_Wander_DME, "Work_Area=1:int") {
  ...
  {                               // 100 not occupied, 200 is occupied.
    if (type == DME_OCCUPIED && param == 100) {
      Commands->Send_Custom_Event(obj, sender, 70, 70, 0.0f); // if not occupied, sends custom to set status to
                                                              // occupied.
    }
...
DECLARE_SCRIPT(test_Ssm_Trigger, ""){void Damaged(GameObject * obj, GameObject *damager, float amount){
    if (damager == STAR){Commands->Send_Custom_Event(obj, obj, M00_LAUNCH_SSM, 0, 0.0f);
}
}
}
;
