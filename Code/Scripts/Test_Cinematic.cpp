...
DECLARE_SCRIPT(M00_Cinematic_Attack_Command_DLS, "AttackDuration=1.0:float"){
    ...
    void Created(GameObject * obj){float facing = Commands->Get_Facing(Owner());
...
void Custom(GameObject *obj, int type, int param, GameObject *sender) { Commands->Action_Reset(obj, 100); }
}
;
...
DECLARE_SCRIPT(Test_Cinematic_Primary_Killed, "CallbackID=:int") {
  ...
  REGISTER_VARIABLES() { SAVE_VARIABLE(custom_sent, 1); }

  virtual void Created(GameObject * game_obj) { custom_sent = false; }
