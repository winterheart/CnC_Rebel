...
DECLARE_SCRIPT(Unit_Combat, "Scoreboard_ID=0:int,Controller_ID=0:int,Script_Override=0:int,Soldier_Type=0:int") {
  ...
  ************************************************************************************************/
  ...
  void Enemy_Seen(GameObject * obj, GameObject * enemy) {
    if (script_override > 0) {
      ...
      if (Commands->Is_Object_Visible(obj, enemy) == true) {
        ...
        //				Commands->Create_Instant_Logical_Sound(
        //(CombatSoundType)(SOUND_ENEMY_SEEN), 20.0, obj, Commands->Get_Position(obj) );
        Commands->Enable_Enemy_Seen(obj, false);
  ...
  void Killed(GameObject * obj, GameObject * killer) {
    ...
    //		Commands->Create_Instant_Logical_Sound( (CombatSoundType)(SOUND_DEATH), 30.0, obj,
  ...
  void Sound_Heard(GameObject * obj, const CombatSound &sound) {
    if (script_override > 0) {
      //			Commands->Send_Custom_Event( obj, Commands->Find_Object(controller_id), OV_SOUNDHEARD,
      //0);
      return;
    }
  ...
  */
  ...
  void Function_Circle_Attack(GameObject * obj, GameObject * Enemy, float Accuracy, float Range, float Radius) {
    ...
    Commands->Enable_Enemy_Seen(obj, false);
    //		Commands->Enable_Sound_Heard( Me, false);
    //		Commands->Create_Instant_Logical_Sound( (CombatSoundType)(SOUND_ENEMY_SEEN), 20.0, Me,
    //Commands->Get_Position( Me ) );
  ...
  void Function_Play_Idle_Anim(GameObject * obj) {
    ...
    if (animnum <= 10) {
      ...
      anim_script = "human.j04c01";
    } else if (animnum <= 30) {
      anim_script = "human.j13c01";
    } else if (animnum <= 40) {
      anim_script = "human.j14c01";
    } else if (animnum <= 50) {
      anim_script = "human.j15c01";
    } else if (animnum <= 60) {
      anim_script = "human.j17c01";
    } else if (animnum <= 70) {
      anim_script = "human.j24c01";
    } else if (animnum <= 80) {
      anim_script = "human.j22c01";
    } else if (animnum <= 90) {
