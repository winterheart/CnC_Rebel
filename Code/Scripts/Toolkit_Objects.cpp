...
DECLARE_SCRIPT(M00_Object_Create_RMV,
...
DECLARE_SCRIPT(M00_Object_Create_Attach_Script_RMV,
               "Start_Now=0:int, Receive_Type:int, Receive_Param_On=1:int, Object_To_Create:string, Location:vector3, "
...
DECLARE_SCRIPT(M00_Object_Destroy_Self_RMV, "Start_Now=1:int, Receive_Type=3:int, Receive_Param_On=1:int, "
                                            "Receive_Param_Off=0:int, Receive_Param_Activate:int, Debug_Mode=0:int") {
...
DECLARE_SCRIPT(M00_PowerUp_Create_When_Killed_JDG, "Preset_Name:string,Drop_Percentage=100:float,Create_At_Death_Pos=1:"
                                                   "int,Position:vector3,Z_Offset=0.75:float,Spawn_Effect=0:int"){
...
DECLARE_SCRIPT(M00_No_Falling_Damage_DME, "") {
  ...
  void Created(GameObject * obj) { ignore_next_dmg = false; }
...
DECLARE_SCRIPT(M00_Permanent_No_Falling_Damage_IML, "") {
  ...
  // Register variables to be Auto-Saved
  // All variables must have a unique ID, less than 256, that never changes
  REGISTER_VARIABLES() { SAVE_VARIABLE(initial_health, 1); }

  void Created(GameObject * obj) { initial_health = Commands->Get_Health(obj); }
...
DECLARE_SCRIPT(M00_Disable_Transition,
               ""){void Created(GameObject * obj){Commands->Enable_Vehicle_Transitions(obj, false);
}
}
;
...
DECLARE_SCRIPT(M00_Fire_Gas_Elec_Death_DAK, "DeathType=1:int,HealthThreshhold=0.25:float") {
  ...
  void Damaged(GameObject * obj, GameObject * damager, float amount) {
    ...
    if (Commands->Get_Health(obj) <= bleah * Commands->Get_Max_Health(obj)) {
      ...
      if (firsttime == true) {
        ...
        } else if (Get_Int_Parameter("DeathType") == 2) {
  ...
  void Killed(GameObject * obj, GameObject * killer) {
    ...
    if (strcmp(Get_Parameter("DeathType"), "Fire")) {
      params.Set_Animation("S_A_HUMAN.H_A_FLMA", 0); // Fire Death
    } else if (strcmp(Get_Parameter("DeathType"), "Gas")) {
      params.Set_Animation("S_A_HUMAN.H_A_6X01", 0); // Gas Death
    } else
      params.Set_Animation("S_A_HUMAN.H_A_6X05", 0); // Electrocution

    Commands->Action_Play_Animation(obj, params);
  }

  void Created(GameObject * obj) { firsttime = true; }
...
DECLARE_SCRIPT(M00_Vehicle_Regen_DAK, ""){void Created(GameObject * obj){Commands->Send_Custom_Event(obj, obj, 0, 0, 0);
}
...
DECLARE_SCRIPT(M00_Send_Object_ID, "Receiver_ID:int, Param=0:int, Delay=1.0f:int") {
  ...
  void Created(GameObject * obj) {
    ...
    if (receiver) {
      ...
      Commands->Debug_Message("M00_Send_Object_ID on object_id %d Warning! Receiver_ID %d does not exist! /n",
                              Commands->Get_ID(obj), receiver_id);
    }
  }
};