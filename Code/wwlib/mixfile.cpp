...
MixFileFactoryClass::MixFileFactoryClass(const char *mix_filename, FileFactoryClass *factory)
    : FileCount(0), NamesOffset(0), IsValid(false), BaseOffset(0), Factory(NULL), IsModified(false) {
...
**
*/
bool MixFileFactoryClass::Get_Temp_Filename(const char *path, StringClass &full_path) {
  bool retval = false;
...
void MixFileCreator::Add_File(const char *source_filename, const char *saved_filename) {
  ...
  if (MixFile != NULL) {
    ...
    if (file && file->Is_Available()) {
      ...
      WWDEBUG_SAY(("Saving File %s CRC %08X Offset %d (0x%08X) Size %d (0x%08X)\n", saved_filename, info.CRC,
                   info.Offset, info.Offset, info.Size, info.Size));
...
void MixFileCreator::Add_File(const char *filename, FileClass *file) {
  if (MixFile != NULL) {
    ...
    WWDEBUG_SAY(("Saving File %s CRC %08X Offset %d (0x%08X) Size %d (0x%08X)\n", filename, info.CRC, info.Offset,
                 info.Offset, info.Size, info.Size));
