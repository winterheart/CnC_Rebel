...
public:
  ...
  static int Calc_Tag_Offset(int blockoffset) { return (blockoffset + sizeof(BlockHeader)); }
...
protected:
  ...
  int Handle_Can_Write(TagBlockHandle *handle) { return (CreateHandle == handle); }
...
public:
  ...
  // Stop write access - flushes data out but keeps handle available for reading.
  int End_Write_Access() { return (File->End_Write_Access(this)); }

  int Tell() { return (Position); }

  int Get_Data_Size() { return (BlockHeader->DataSize); }
...
private:
  ...
  int Called_By_Destructor() { return (_InDestructor); }
};
