...
public:
  ...
  void Link(GenericNode *node) {
    assert(node != (GenericNode *)nullptr);
    node->Unlink();
  ...
  GenericNode *Next_Valid() const { return ((NextNode && NextNode->NextNode) ? NextNode : (GenericNode *)nullptr); }
  GenericNode *Prev() const { return (PrevNode); }
  GenericNode *Prev_Valid() const { return ((PrevNode && PrevNode->PrevNode) ? PrevNode : (GenericNode *)nullptr); }
  bool Is_Valid() const {
    return (this != (GenericNode *)nullptr && NextNode != (GenericNode *)nullptr && PrevNode != (GenericNode *)nullptr);
  }
...
/*
        ...
        data value associated with it. Commonly used for tracking the owner of a DataNode.
                - EHC
*/
template <class C, class D> class ContextDataNode : public DataNode<D> {
  C Context;

public:
  ContextDataNode() = default;
...
*/
template <class C, class D> class SafeContextDataNode : public ContextDataNode<C, D> {
public:
...
/*
        ...
        Planet mode.
*/
template <class PRIMARY, class SECONDARY> class DoubleNode {
  void Initialize() {
    Primary.Set(this);
    Secondary.Set(this);
  };
