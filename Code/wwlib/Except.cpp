...
using SymGetSymFromAddrType = BOOL(WINAPI *)(HANDLE hProcess, DWORD Address, LPDWORD Displacement,
                                             PIMAGEHLP_SYMBOL Symbol);
using SymInitializeType = BOOL(WINAPI *)(HANDLE hProcess, LPSTR UserSearchPath, BOOL fInvadeProcess);
using SymLoadModuleType = BOOL(WINAPI *)(HANDLE hProcess, HANDLE hFile, LPSTR ImageName, LPSTR ModuleName,
...
using StackWalkType = BOOL(WINAPI *)(DWORD MachineType, HANDLE hProcess, HANDLE hThread, LPSTACKFRAME StackFrame,
...
void Dump_Exception_Info(EXCEPTION_POINTERS *e_info) {
  ...
  */
  static char const *_code_txt[] = {
      ...
      "Error code: EXCEPTION_ARRAY_BOUNDS_EXCEEDED\r\r\nDescription: The thread tried to access an array element that "
      "is out of bounds and the underlying hardware supports bounds checking.",
      ...
      "Error code: EXCEPTION_FLT_DENORMAL_OPERAND\r\r\nDescription: One of the operands in a floating-point operation "
      "is denormal. A denormal value is one that is too small to represent as a standard floating-point value.",
      "Error code: EXCEPTION_FLT_DIVIDE_BY_ZERO\r\r\nDescription: The thread tried to divide a floating-point value by "
      ...
      "attempted.",
      "Error code: EXCEPTION_FLT_OVERFLOW\r\r\nDescription: The exponent of a floating-point operation is greater than "
      ...
      "Error code: EXCEPTION_FLT_UNDERFLOW\r\r\nDescription:	The exponent of a floating-point operation is less "
      "than the magnitude allowed by the corresponding type.",
      "Error code: EXCEPTION_ILLEGAL_INSTRUCTION\r\r\nDescription:	The thread tried to execute an invalid "
      "instruction.",
      "Error code: EXCEPTION_IN_PAGE_ERROR\r\r\nDescription:	The thread tried to access a page that was not "
      "present, and the system was unable to load the page. For example, this exception might occur if a network "
      "connection is lost while running a program over the network.",
      "Error code: EXCEPTION_INT_DIVIDE_BY_ZERO\r\r\nDescription: The thread tried to divide an integer value by an "
      "integer divisor of zero.",
      "Error code: EXCEPTION_INT_OVERFLOW\r\r\nDescription: The result of an integer operation caused a carry out of "
      "the most significant bit of the result.",
      "Error code: EXCEPTION_INVALID_DISPOSITION\r\r\nDescription: An exception handler returned an invalid "
      "disposition to the exception dispatcher. Programmers using a high-level language such as C should never "
      ...
      "Error code: EXCEPTION_PRIV_INSTRUCTION\r\r\nDescription: The thread tried to execute an instruction whose "
      "operation is not allowed in the current machine mode.",
      "Error code: EXCEPTION_SINGLE_STEP\r\r\nDescription: A trace trap or other single-instruction mechanism signaled "
  ...
  ** CPU type
  */
  sprintf(scrap, "\r\nCPU %s, %d Mhz, Vendor: %s\r\n", (char *)CPUDetectClass::Get_Processor_String(),
          Get_RDTSC_CPU_Speed(), (char *)CPUDetectClass::Get_Processor_Manufacturer_Name());
  Add_Txt(scrap);
      ...
      */
      if (IsBadCodePtr((FARPROC)*stackptr)) {
        ...
        if (symbols_available) {
          ...
          if (_SymGetSymFromAddr != nullptr &&
              _SymGetSymFromAddr(GetCurrentProcess(), *stackptr, &displacement, symptr)) {
            char symbuf[256];
...
int Exception_Handler(int exception_code, EXCEPTION_POINTERS *e_info) {
  ...
  if (!WindowedMode) {
    ...
    MessageBox(MainWindow, "This demo has timed out. Thank you for playing Red Alert 2.", "Byeee!",
 ...
 *=============================================================================================*/
int Get_Num_Threads(void) { return (ThreadList.Count()); }
...
void Unregister_Thread_ID(unsigned long thread_id, const char *thread_name) {
  ...
  std::erase_if(ThreadList,
                [thread_name](const ThreadInfoType *entry) { return strcmp(entry->ThreadName, thread_name) == 0; });
  ReleaseMutex(MutexThreadList);
}
...
bool Lookup_Symbol(void *code_ptr, char *symbol, int &displacement) {
  ...
  */
  if (_SymGetSymFromAddr(GetCurrentProcess(), (unsigned long)code_ptr, (unsigned long *)&displacement,
                         symbol_struct_ptr)) {
...
here:
  ...
  */
  for (int i = 0; i < num_addresses + 1; i++) {
    if (_StackWalk(IMAGE_FILE_MACHINE_I386, GetCurrentProcess(), GetCurrentThread(), &stack_frame, nullptr, nullptr,
                   _SymFunctionTableAccess, _SymGetModuleBase, nullptr)) {
...
void Register_Application_Exception_Callback(void (*app_callback)()) { AppCallback = app_callback; }

void Register_Application_Version_Callback(const char *(*app_ver_callback)()) { AppVersionCallback = app_ver_callback; }

void Set_Exit_On_Exception(bool set) { ExitOnException = true; }
