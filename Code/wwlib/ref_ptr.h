          ...
          The reference counted objects must implement the following interface:
                ...
                wrappers, it does not support:
                        1. type hierarchy relationships.  RefCountPtr<Derived> is unrelated to RefCountPtr<Base>.
                                ...
                                There is no support for casting upward (i.e. dynamic cast from RefCountPtr<Base> to
  RefCountPtr<Derived> This may be added in the future.
                        ...
                        4. direct conversion to T*.  Please keep object wrappered in RefCountPtr<T> and use
  dereferencing operators (*myptr.f() and/or myptr->f()) You may also use Peek() to get a raw pointer.  Peek does not
                        ...
                        3. As a return value.  use RefCountPtr<T> or const RefCountPtr<T> & (member variables)
                                        class MyClass
                                        {
                                                public:
                                                        ...
                                                        const RefCountPtr<T> &	Get_Something(void) const {return
  MyObject;} private: RefCountPtr<T>				MyObject;
                                        };
                        4. As a function parameter. user const RefCountPtr<T> & or RefCountPtr<T> & (for modifiable
  parameter) void My_Function(const RefCountPtr<T> & my_parameter)
                                        {
...
public:
  friend RefCountPtr<T> Create_NEW(T *t) { return RefCountPtr<T>(t, RefCountPtr<T>::GET); }

  friend RefCountPtr<T> Create_Get(T *t) { return RefCountPtr<T>(t, RefCountPtr<T>::GET); }

  friend RefCountPtr<T> Create_Peek(T *t) { return RefCountPtr<T>(t, RefCountPtr<T>::PEEK); }
  ...
  RefCountPtr(DummyPtrType *dummy) : Referent(0) { G_ASSERT(dummy == 0); }
#endif

  template <class RHS> RefCountPtr(const RefCountPtr<RHS> &rhs) : Referent(rhs.Peek()) {
    if (Referent) {
      Referent->Add_Ref();
    }
  }
  ...
  template <class RHS> const RefCountPtr<T> &operator=(const RefCountPtr<RHS> &rhs) {
    if (rhs.Peek()) {
      rhs.Peek()->Add_Ref();
    }
...
template <class LHS, class RHS> bool operator==(const RefCountPtr<LHS> &lhs, const RefCountPtr<RHS> &rhs) {
...
//  0 == my_ptr
template <class RHS> bool operator==(DummyPtrType *dummy, const RefCountPtr<RHS> &rhs) {
  FAIL_IF(0 != dummy) { return false; }
