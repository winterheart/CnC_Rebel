...
IMEManager::IMEManager()
    : mHWND(NULL), mHIMC(NULL), mDisabledHIMC(NULL), mDisableCount(0), mCodePage(CP_ACP), mIMEProperties(0),
      mHilite(true), mStartCandListFrom1(true), mOSCanUnicode(false), mUseUnicode(false), mInComposition(false) {
...
bool IMEManager::IsDisabled(void) const { return (mDisableCount > 0); }
...
#ifdef WWDEBUG
    {
      ...
      if (imc) {
        ...
        if (okay) {
          ...
          static struct convmodestruct {
            long flag;
            const char *ondesc;
            const char *offdesc;
          } _convModes[] = {{IME_CMODE_CHARCODE, "CharCode:On", "CharCode:Off"},
...
#ifdef WWDEBUG
    {
      ...
      if (imc) {
        ...
        if (okay) {
          ...
          static struct smodestruct {
            long flag;
            const char *ondesc;
            const char *offdesc;
          } _sModes[] = {{IME_SMODE_AUTOMATIC, "Automatic:On", "Automatic:Off"},
...
void IMEManager::InputLanguageChanged(HKL hkl) {
  ...
  static const wchar_t _TradChImeName[] = {0x6CE8, 0x97F3, 0x8F38, 0x5165, 0x6CD5, 0x0020,
                                           0x0034, 0x002E, 0x0031, 0x0020, 0x7248, 0x0000};
...
void IMEManager::DoComposition(unsigned int dbcs, long compFlags) {
  ...
  static struct flagstruct {
    long flag;
    const char *desc;
  } _gcsFlags[] = {{GCS_COMPATTR, "GCS_COMPATTR"},
