...
public:
  ...
  TPoint2D<T> const operator/(T rvalue) const {
    if (rvalue == T(0))
      return (TPoint2D<T>(0, 0));
    return (TPoint2D<T>(X / rvalue, Y / rvalue));
  }
  TPoint2D<T> const &operator/=(T rvalue) {
    if (rvalue != T(0)) {
      X /= rvalue;
...
template <class T> TPoint2D<T> const operator*(T lvalue, TPoint2D<T> const &rvalue) { return (rvalue * lvalue); }
...
**	the next best thing.
*/
template <class T> class TPoint3D : public TPoint2D<T> {
...
public:
  ...
  TPoint3D<T> const operator-(TPoint3D<T> const &rvalue) const {
    return (TPoint3D<T>(this->X - rvalue.X, this->Y - rvalue.Y, this->Z - rvalue.Z));
  }
  TPoint3D<T> const operator-(TPoint2D<T> const &rvalue) const {
    return (TPoint3D<T>(this->X - rvalue.X, this->Y - rvalue.Y, this->Z));
  }
  TPoint3D<T> const operator+(TPoint3D<T> const &rvalue) const {
    return (TPoint3D<T>(this->X + rvalue.X, this->Y + rvalue.Y, this->Z + rvalue.Z));
  }
  ...
  TPoint3D<T> const operator*(T rvalue) const {
    return (TPoint3D<T>(this->X * rvalue, this->Y * rvalue, this->Z * rvalue));
  }
  ...
  TPoint3D<T> const Cross_Product(TPoint3D<T> const &rvalue) const {
    return TPoint3D<T>(this->Y * rvalue.Z - this->Z * rvalue.Y, this->Z * rvalue.X - this->X * rvalue.Z,
                       this->X * rvalue.Y - this->Y * rvalue.X);
  }
...
template <class T> TPoint3D<T> const operator*(T lvalue, TPoint3D<T> const &rvalue) { return (rvalue * lvalue); }
