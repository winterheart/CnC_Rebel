...
WWFontClass::WWFontClass(void const *fontdata, bool isoutlined, int shadow, ConvertClass *convert, unsigned char *remap)
    : IsOutlinedData(isoutlined), Shadow(shadow), Converter(convert), RemapPalette(remap), FontXSpacing(0),
      FontYSpacing(0) {
    ...
    while (*string != '\0') {
      ...
      */
      Rect crect(xpos, ypos, width + ((xspacing > 0) ? xspacing : 0),
                 *(((unsigned char *)FontData) + FontData->InfoBlockOffset + FONTINFOMAXHEIGHT) +
        ...
        */
        if (FontData->FontCompress != (char)2) { // if the font is the old style
          ...
          for (int h = 0; h < dheight; h++) {
            ...
            */
            if (ypos + firstrow + h >= crect.Y + crect.Height)
              break;
