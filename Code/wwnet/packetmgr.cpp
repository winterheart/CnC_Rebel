  ...
  *build_delta_ptr = 0;
  ...
  if (base_packet_size == add_packet_size) {
    ...
    */
    ...
    /*
    ** Try a byte for byte check in the chunks that don't match. For any bytes that are different, save the changed
    *value
    ...
    */
    ...
    /*
    ** OK, now we have bitfields that mark the differences. A 1 in the bitfield means that the corresponding chunk or
    *byte
...
int PacketManagerClass::Reconstruct_From_Delta(unsigned char *base_packet, unsigned char *reconstructed_packet,
  ...
  if (base_packet_size > 500) {
    WWDEBUG_SAY(
        ("*** WARNING: MALFORMED PACKET - PacketManagerClass::Reconstruct_From_Delta -- Bad base packet size\n"));
    return (0);
  }
  ...
  /*
  ** If there is chunk info then extract that first. Use it to copy the approprate parts of the base packet into the add
  *packet.
  */
  if (header->ChunkPack) {
    for (int i = 0; i < base_packet_size - 7; i += 8) {
    ...
    /*
    ** If the packet isn't an even number of chunks long then pull out an extra bit and throw it away. It should always
    *be 0.
    */
    if ((base_packet_size & 7) != 0) {
      // #ifdef WWDEBUG
...
void PacketManagerClass::Flush(bool forced) {
  {
    ...
    */
            ...
            if (SendBuffers[index].PacketLength == length && SendBuffers[index].PacketSendSocket == socket) {
              ...
              */
              if (SendBuffers[base_index].Port == SendBuffers[index].Port &&
                  (*(unsigned long *)(&SendBuffers[index].IPAddress[0])) ==
                ...
                */
                int bytes = Build_Delta_Packet_Patch(base_packet, SendBuffers[index].PacketBuffer->Buffer, DeltaPacket,
                                                     length, length);
    ...
    /*
    ** Merge ready buffers where possible. This is the step that adds runs of different sizeed packets into the same
    *packet.
    */
    if (AllowCombos) {
      for (i = 0; i < NumSendBuffers; i++) {
          ...
          for (int j = i + 1; j < NumSendBuffers; j++) {
            if (SendBuffers[j].PacketReady && SendBuffers[j].PacketSendSocket == socket) {
              if (SendBuffers[i].PacketSendLength + SendBuffers[j].PacketSendLength < PACKET_MANAGER_MTU) {
                if (SendBuffers[i].Port == SendBuffers[j].Port &&
                    (*(unsigned long *)(&SendBuffers[i].IPAddress[0])) ==
                        (*(unsigned long *)(&SendBuffers[j].IPAddress[0]))) {
       ...
       // ((PacketPackHeaderStruct*)&PacketBuffers[j][0])->NumPackets, i)); WWDEBUG_SAY(("Packet %d has %d packets
       // total\n", i, num_sub_packets));
                }
              }
            }
    ...
    */
    for (i = 0; i < NumSendBuffers; i++) {
      if (SendBuffers[i].PacketReady) {
        ...
        // PacketSendLength[i], Addr_As_String(&addr), debug_num_packets, debug_packet_size));
#endif // WWDEBUG
        ...
        *((unsigned long *)crc_and_buffer) = crc;
        ...
        Register_Packet_Out(&SendBuffers[i].IPAddress[0], SendBuffers[i].Port,
                            SendBuffers[i].PacketSendLength + UDP_HEADER_SIZE + sizeof(crc), 0);
        int result = sendto(socket, crc_and_buffer, SendBuffers[i].PacketSendLength + sizeof(crc), 0, (LPSOCKADDR)&addr,
                            sizeof(SOCKADDR_IN));

#else // WRAPPER_CRC

        Register_Packet_Out(&SendBuffers[i].IPAddress[0], SendBuffers[i].Port,
                            SendBuffers[i].PacketSendLength + UDP_HEADER_SIZE, 0);
        int result = sendto(socket, (const char *)SendBuffers[i].PacketBuffer, SendBuffers[i].PacketSendLength, 0,
        ...
        if (result == SOCKET_ERROR) {
          if (WSAGetLastError() != WSAEWOULDBLOCK) {
            ...
            WWDEBUG_SAY(("PacketManagerClass - sendto returned error code %d - %s\n", error_code,
                         cNetUtil::Winsock_Error_Text(error_code)));
 ...
 *=============================================================================================*/
bool PacketManagerClass::Break_Packet(unsigned char *packet, int original_packet_size, unsigned char *ip_address,
                                      unsigned short port) {
  /*
  ...
  */
  ...
  if (num_packets < 1 || packet_size > PACKET_MANAGER_MTU) {
    WWDEBUG_SAY(
        ("PacketManager - Packet decode error. num_packets = %d\n, packet_size = %d\n", num_packets, packet_size));
    return (false);
  }
  ...
  for (int i = 0; i < num_packets - 1; i++) {
    ...
    */
    if (delta_header->ChunkPack || delta_header->BytePack) {
      ...
      int bytes =
          // #endif //WWDEBUG
          Reconstruct_From_Delta(&ReceiveBuffers[delta_base_index].ReceiveHoldingBuffer[0],
                                 &ReceiveBuffers[NumReceivePackets].ReceiveHoldingBuffer[0],
                                 (unsigned char *)delta_header, packet_size, delta_size);
 ...
 *=============================================================================================*/
int PacketManagerClass::Get_Packet(SOCKET socket, unsigned char *packet_buffer, int packet_buffer_size,
                                   unsigned char *ip_address, unsigned short &port) {
  {
    ...
    if (CurrentPacket < NumReceivePackets && socket == ReceiveSocket) {
      /*
      ** Copy the current packet into the return buffer. We have to zero out the rest of the buffer or the CRC won't
      *come out
 ...
 *=============================================================================================*/
void PacketManagerClass::Register_Packet_In(unsigned char *ip_address, unsigned short port,
 ...
 *   10/9/2001 8:56AM ST : Created                                                             *
 *=============================================================================================*/
void PacketManagerClass::Register_Packet_Out(unsigned char *ip_address, unsigned short port,
                                             unsigned long compressed_size, unsigned long uncompressed_size) {
  static unsigned long _last_ip = 0;
  static unsigned short _last_port = 0;
  static int _last_stats = -1;
