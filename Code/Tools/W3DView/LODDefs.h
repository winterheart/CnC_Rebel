...
//	Data types
//
typedef enum { TYPE_COMMANDO = 0, TYPE_G, TYPE_COUNT } LOD_NAMING_TYPE;
...
__inline bool Is_LOD_Name_Valid(LPCTSTR name, LOD_NAMING_TYPE &type) {
  ...
  if (((last_2_chars[0] == 'L') || (last_2_chars[0] == 'l')) &&
    ...
    type = TYPE_COMMANDO;
    retval = true;
  } else if ((last_2_chars[1] >= 'a' && last_2_chars[1] <= 'z') || (last_2_chars[1] >= 'A' && last_2_chars[1] <= 'Z')) {
    type = TYPE_G;
    retval = true;
  }
...
__inline bool Is_Model_Part_of_LOD(LPCTSTR name, LPCTSTR base, LOD_NAMING_TYPE &type) {
  ...
  if (::strstr(name, base) == name) {
    ...
    if (type == TYPE_COMMANDO) {
      if (((extension[0] == 'L') || (extension[0] == 'l')) && ((extension[1] >= '0') && (extension[1] <= '9'))) {
        retval = (extension[2] == 0);
      }
    } else if (type == TYPE_G) {
      if ((extension[0] >= 'a' && extension[0] <= 'z') || (extension[0] >= 'A' && extension[0] <= 'Z')) {
