...
ColorBarClass::ColorBarClass(void)
    : m_hBitmap(NULL), m_iBMPWidth(0), m_iBMPHeight(0), m_pBits(NULL), m_hMemDC(NULL), m_iColorPoints(0),
      m_iMarkerWidth(0), m_iMarkerHeight(0), m_KeyFrameDIB(NULL), m_pKeyFrameBits(NULL), m_iCurrentKey(0), m_MinPos(0),
      m_MaxPos(1), m_iScanlineSize(0), m_bMoving(false), m_bMoved(false), m_SelectionPos(0), m_bRedraw(true), CWnd() {
...
// Create
//
BOOL ColorBarClass::Create(LPCTSTR /*lpszClassName*/, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT &rect,
                           CWnd *pparent_wnd, UINT nID, CCreateContext * /*pContext*/
) {
  // Create the window (it will force the message map and everthing)
  HWND hparent_wnd = (pparent_wnd != NULL) ? pparent_wnd->m_hWnd : NULL;
  HWND hwnd = ::CreateWindow("WWCOLORBAR", lpszWindowName, dwStyle, rect.left, rect.top, rect.right - rect.left,
                             rect.bottom - rect.top, hparent_wnd, (HMENU)nID, ::AfxGetInstanceHandle(), this);
...
// Create_Bitmap
//
void ColorBarClass::Create_Bitmap(void) {
  // Start fresh
  Free_Bitmap();
...
// Free_Bitmap
//
void ColorBarClass::Free_Bitmap(void) {
  if (m_hBitmap != NULL) {
...
// Paint_Bar_Vert
//
void ColorBarClass::Paint_Bar_Vert(int x_pos, int y_pos, int width, int height, UCHAR *pbits) {
...
// Paint_Bar_Horz
//
void ColorBarClass::Paint_Bar_Horz(int x_pos, int y_pos, int width, int height, UCHAR *pbits) {
  LONG style = ::GetWindowLong(m_hWnd, GWL_STYLE);
...
// Paint_DIB
//
void ColorBarClass::Paint_DIB(void) {
  ...
  //
  if (style & CBRS_HORZ) {
    Paint_Bar_Horz(m_ColorArea.left, m_ColorArea.top, m_ColorArea.Width(), m_ColorArea.Height(), m_pBits);
  } else {
    Paint_Bar_Vert(m_ColorArea.left, m_ColorArea.top, m_ColorArea.Width(), m_ColorArea.Height(), m_pBits);
  }
...
// OnPaint
//
void ColorBarClass::OnPaint(void) {
  CPaintDC dc(this);
  Paint_Screen(dc);
  return;
}
...
// Paint_Screen
//
void ColorBarClass::Paint_Screen(HDC hwnd_dc) {
  if (m_hMemDC != NULL) {
...
// Insert_Point
//
bool ColorBarClass::Insert_Point(CPoint point, DWORD flags) {
  int new_index = 0;
  int position = 0;
  float percent = 0;
  ...
  for (int index = 0; (index < m_iColorPoints) && !found; index++) {
    if ((position > m_ColorPoints[index].StartPos) && (position < m_ColorPoints[index].EndPos)) {
      new_index = index + 1;
      found = true;
      ...
      // is in graph mode
      //
      float key_percent = (((float)position) - ((float)m_ColorPoints[index].StartPos)) /
                          float(m_ColorPoints[index].EndPos - m_ColorPoints[index].StartPos);
      float graph_start = m_ColorPoints[index].StartGraphPercent;
      float graph_delta =
          (m_ColorPoints[index].EndPos - m_ColorPoints[index].StartPos) * m_ColorPoints[index].GraphPercentInc;
...
// Insert_Point
//
bool ColorBarClass::Insert_Point(int index, float position, float red, float green, float blue, DWORD flags) {
  // Assume failure
  bool retval = false;

  // Params valid?
  if (((m_iColorPoints + 1) < MAX_COLOR_POINTS) && (index >= 0) && (index <= m_iColorPoints)) {

    // Bump all the points up one position in the array
    ::memmove(&m_ColorPoints[index + 1], &m_ColorPoints[index], sizeof(COLOR_POINT) * (m_iColorPoints - index));
...
bool ColorBarClass::Delete_Point(int index) {
  ...
  if ((index > 0) && (index < m_iColorPoints)) {
    ...
    // Bump all the points up one position in the array
    ::memmove(&m_ColorPoints[index], &m_ColorPoints[index + 1], sizeof(COLOR_POINT) * (m_iColorPoints - (index + 1)));
...
// Modify_Point
//
bool ColorBarClass::Modify_Point(int index, float position, float red, float green, float blue, DWORD flags) {
  // Assume failure
  bool retval = false;
...
// Update_Point_Info
//
void ColorBarClass::Update_Point_Info(void) {
  ...
  if (style & CBRS_HORZ) {
    ...
    for (index = 0; index < m_iColorPoints; index++) {
      ...
      if (index > 0) {
        ...
        m_ColorPoints[index - 1].RedInc =
            (m_ColorPoints[index].StartRed - m_ColorPoints[index - 1].StartRed) / ((float)width);
        m_ColorPoints[index - 1].GreenInc =
            (m_ColorPoints[index].StartGreen - m_ColorPoints[index - 1].StartGreen) / ((float)width);
        m_ColorPoints[index - 1].BlueInc =
            (m_ColorPoints[index].StartBlue - m_ColorPoints[index - 1].StartBlue) / ((float)width);

        // Assign the graph increment to the previous point
        m_ColorPoints[index - 1].GraphPercentInc =
            (m_ColorPoints[index].StartGraphPercent - m_ColorPoints[index - 1].StartGraphPercent) / ((float)width);
      }
    }
    ...
    for (index = 0; index < m_iColorPoints; index++) {
      ...
      if (index > 0) {
        ...
        m_ColorPoints[index - 1].RedInc =
            (m_ColorPoints[index].StartRed - m_ColorPoints[index - 1].StartRed) / ((float)height);
        m_ColorPoints[index - 1].GreenInc =
            (m_ColorPoints[index].StartGreen - m_ColorPoints[index - 1].StartGreen) / ((float)height);
        m_ColorPoints[index - 1].BlueInc =
            (m_ColorPoints[index].StartBlue - m_ColorPoints[index - 1].StartBlue) / ((float)height);
      }
    }
...
// Free_Marker_Bitmap
//
void ColorBarClass::Free_Marker_Bitmap(void) {
...
// Load_Key_Frame_BMP
//
void ColorBarClass::Load_Key_Frame_BMP(void) {
  Free_Marker_Bitmap();
  ...
  // Create a bitmap that we can access the bits directly of
  m_KeyFrameDIB = ::CreateDIBSection(hscreen_dc, (const BITMAPINFO *)&bitmap_info, DIB_RGB_COLORS,
                                     (void **)&m_pKeyFrameBits, NULL, 0L);
...
// Paint_Key_Frame
//
void ColorBarClass::Paint_Key_Frame(int x_pos, int y_pos) {
  // Window's bitmaps are DWORD aligned, so make sure
  // we take that into account.
...
// Marker_From_Point
//
int ColorBarClass::Marker_From_Point(CPoint point) {
  int marker_index = -1;
...
// OnLButtonDown
//
void ColorBarClass::OnLButtonDown(UINT nFlags, CPoint point) {
  //
...
// OnLButtonUp
//
void ColorBarClass::OnLButtonUp(UINT nFlags, CPoint point) {
  if (m_bMoving) {
    ::ReleaseCapture();
    m_bMoving = false;
...
//
void ColorBarClass::Get_Selection_Rectangle(CRect &rect) {
...
// Move_Selection
//
void ColorBarClass::Move_Selection(CPoint point, bool send_notify) {
  //
...
void ColorBarClass::Move_Selection(float new_pos, bool send_notify) {
  if (new_pos != m_SelectionPos) {
    ...
    if (send_notify) {
      ...
      Get_Color(m_SelectionPos, &notify_hdr.red, &notify_hdr.green, &notify_hdr.blue);

      //
      //	Send the notification to the parent
      //
      ::SendMessage(::GetParent(m_hWnd), WM_NOTIFY, id, (LPARAM)&notify_hdr);
    }
...
void ColorBarClass::Get_Color(float position, float *red, float *green, float *blue) {
  ...
  int key_index = 0;
  for (int index = 0; (index < m_iColorPoints) && !found; index++) {
    if ((pixel_pos >= m_ColorPoints[index].StartPos) && (pixel_pos <= m_ColorPoints[index].EndPos)) {
      key_index = index;
      found = true;
    }
  }
...
// Clear_Points
//
void ColorBarClass::Clear_Points(void) {
...
// Set_User_Data
//
bool ColorBarClass::Set_User_Data(int index, DWORD data) {
  bool retval = false;
  if ((index >= 0) && (index < m_iColorPoints)) {
...
// Repaint
//
void ColorBarClass::Repaint(void) {
