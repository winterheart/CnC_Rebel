...
CSceneLightDialog::CSceneLightDialog(CWnd *pParent /*=NULL*/)
    : m_CurrentChannel(DIFFUSE), m_InitialStartAtten(0), m_InitialEndAtten(0), m_InitialDistance(0),
      m_InitialIntensity(0), m_InitialAttenOn(TRUE), CDialog(CSceneLightDialog::IDD, pParent) {
...
//  OnInitDialog
//
BOOL CSceneLightDialog::OnInitDialog(void) {
...
//
LRESULT
CSceneLightDialog::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) {
  switch (message) {
  case WM_NOTIFY: {
...
//  OnGrayscaleCheck
//
void CSceneLightDialog::OnGrayscaleCheck(void) {
  if (SendDlgItemMessage(IDC_GRAYSCALE_CHECK, BM_GETCHECK)) {
...
//  OnChannelBothRadio
//
void CSceneLightDialog::OnChannelBothRadio(void) {
...
//
void CSceneLightDialog::OnChannelDiffuseRadio(void) {
...
//
void CSceneLightDialog::OnChannelSpecularRadio(void) {
...
//
void CSceneLightDialog::Set_Color_Control_State(const Vector3 &color) {
...
//  Update_Attenuation
//
void CSceneLightDialog::Update_Attenuation(void) {
  // Get a pointer to the document so we can change the scene light's
  // settings
  CW3DViewDoc *pCDoc = ::GetCurrentDocument();
  if (pCDoc && pCDoc->GetScene()) {
...
//  Update_Attenuation_Controls
//
void CSceneLightDialog::Update_Attenuation_Controls(void) {
...
//  OnAttenuationCheck
//
void CSceneLightDialog::OnAttenuationCheck(void) {
