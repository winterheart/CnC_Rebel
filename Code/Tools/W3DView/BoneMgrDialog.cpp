...
//  BoneMgrDialogClass
//
BoneMgrDialogClass::BoneMgrDialogClass(RenderObjClass *prender_obj, CWnd *pparent)
    : m_pBaseModel(prender_obj), m_pBackupModel(NULL), m_bAttach(true), CDialog(BoneMgrDialogClass::IDD, pparent) {
...
//  DoDataExchange
//
void BoneMgrDialogClass::DoDataExchange(CDataExchange *pDX) {
...
//  OnInitDialog
//
BOOL BoneMgrDialogClass::OnInitDialog(void) {
  ...
  pimagelist->Add(
      (HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_FOLDER), IMAGE_ICON, 16, 16, LR_SHARED));

  pimagelist->Add(
      (HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_OBJECT), IMAGE_ICON, 16, 16, LR_SHARED));
...
//  Fill_Bone_Item
//
void BoneMgrDialogClass::Fill_Bone_Item(HTREEITEM hbone_item, int bone_index) {
  ...
  for (int index = 0; index < porig_model->Get_Num_Sub_Objects_On_Bone(bone_index); index++) {
  ...
  for (int index = 0; index < m_pBaseModel->Get_Num_Sub_Objects_On_Bone(bone_index); index++) {
  ...
  if (node_list.Count() > 0) {
    ...
    for (int node_index = 0; node_index < node_list.Count(); node_index++) {
      ...
      if (psubobject != NULL && (Is_Object_In_List(psubobject->Get_Name(), orig_node_list) == false)) {
...
//	Is_Object_In_List
//
bool BoneMgrDialogClass::Is_Object_In_List(const char *passet_name, DynamicVectorClass<RenderObjClass *> &node_list) {
  // Assume failure
  bool retval = false;
  ...
  for (int node_index = 0; (node_index < node_list.Count()) && (retval == false); node_index++) {
    ...
    if (prender_obj != NULL && ::lstrcmpi(prender_obj->Get_Name(), passet_name) == 0) {
      retval = true;
    }
  }
...
//
void BoneMgrDialogClass::OnSelchangedBoneTree(NMHDR *pNMHDR, LRESULT *pResult) {
...
//	OnSelchangeObjectCombo
//
void BoneMgrDialogClass::OnSelchangeObjectCombo(void) {
...
//
bool BoneMgrDialogClass::Is_Render_Obj_Already_Attached(const CString &name) {
  ...
  if (htree_item != NULL) {
    ...
    for (HTREEITEM hchild_item = m_BoneTree.GetChildItem(htree_item); (hchild_item != NULL) && (retval == false);
...
//	Update_Controls
//
void BoneMgrDialogClass::Update_Controls(HTREEITEM selected_item) {
...
//	OnDestroy
//
void BoneMgrDialogClass::OnDestroy(void) {
...
//	OnOK
//
void BoneMgrDialogClass::OnOK(void) {
...
//	OnCancel
//
void BoneMgrDialogClass::OnCancel(void) {
...
//	OnAttachButton
//
void BoneMgrDialogClass::OnAttachButton(void) {
  // Get the name of the currently selected render object
  CString name;
  int index = m_ObjectCombo.GetCurSel();
  m_ObjectCombo.GetLBText(index, name);
  ...
  if (m_bAttach) {
    ...
    for (int index = 0; (index < count) && !found; index++) {
      ...
      if ((psub_obj != NULL) && (::lstrcmpi(psub_obj->Get_Name(), name) == 0)) {
...
//
void BoneMgrDialogClass::Remove_Object_From_Bone(HTREEITEM bone_item, const CString &name) {
  // Loop through all the children of this bone
  for (HTREEITEM hchild_item = m_BoneTree.GetChildItem(bone_item); (hchild_item != NULL);
       hchild_item = m_BoneTree.GetNextSiblingItem(hchild_item)) {
