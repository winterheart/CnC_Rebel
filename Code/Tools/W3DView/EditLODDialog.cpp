...
//  CEditLODDialog
//
CEditLODDialog::CEditLODDialog(CWnd *pParent /*=NULL*/) : m_spinIncrement(0.5F), CDialog(CEditLODDialog::IDD, pParent) {
  ...
  if (switchUp < switchDown) {
    ...
    if (pCDoc) {
      ...
      if (pLOD) {
        ...
        for (int iObject = m_hierarchyListCtrl.GetNextItem(-1, LVNI_ALL | LVNI_SELECTED) + 1; (iObject < iSubObjects);
             iObject++) {
