    ...
    if (pCMainWnd) {
      ...
      if (pCDataTreeView) {
        ...
        ::SetWindowPos(hDlg, NULL,
                       ...
                       rect.top + ((rect.bottom - rect.top) >> 1) - ((dialogRect.bottom - dialogRect.top) >> 1), 0, 0,
...
void Paint_Gradient(HWND hWnd, BYTE baseRed, BYTE baseGreen, BYTE baseBlue) {
  ...
  for (int iShade = 0; iShade < 256; iShade++) {
    // Paint this sliver
    cDC.FillSolidRect((int)posX, 0, (widthPerShade >= 1.00F) ? ((int)widthPerShade) + 1 : 1, iHeight,
...
//  Initialize_Spinner
//
void Initialize_Spinner(CSpinButtonCtrl &ctrl, float pos, float min, float max) {
  //
...
//
void Update_Spinner_Buddy(CSpinButtonCtrl &ctrl, int delta) {
  //
...
//
HBITMAP
Create_DIB_Section(UCHAR **pbits, int width, int height) {
  ...
  HBITMAP hbitmap =
      ::CreateDIBSection(hscreen_dc, (const BITMAPINFO *)&bitmap_info, DIB_RGB_COLORS, (void **)pbits, NULL, 0L);
...
//
////////////////////////////////////////////////////////////////////////////
void Resolve_Path(CString &filename) {
  if (filename.Find('\\') == -1) {
    char path[MAX_PATH];
...
//
////////////////////////////////////////////////////////////////////////////
void Find_Missing_Textures(DynamicVectorClass<CString> &list, LPCTSTR name, int frame_count) {
  //
...
bool Copy_File(LPCTSTR existing_filename, LPCTSTR new_filename, bool force_copy) {
  SANITY_CHECK((existing_filename != NULL && new_filename != NULL)) { return false; }
  ...
  if (allow_copy && (attributes != 0xFFFFFFFF) && ((attributes & FILE_ATTRIBUTE_READONLY) == FILE_ATTRIBUTE_READONLY)) {
    if (force_copy) {
