...
public:
  ...
  //
  AssetInfoClass(void) : m_AssetType(TypeUnknown), m_dwUserData(0L), m_pRenderObj(NULL) { Initialize(); }

  AssetInfoClass(LPCTSTR passet_name, ASSET_TYPE type, RenderObjClass *prender_obj = NULL, DWORD user_data = 0L)
      : m_Name(passet_name), m_AssetType(type), m_dwUserData(user_data), m_pRenderObj(NULL) {
