...
public:
  ...
  //
  void Set_Render_Device_Info(int device_index, int bits_per_pixel) {
    m_iDeviceIndex = device_index;
    m_iBitsPerPixel = bits_per_pixel;
  }
  bool Initialize_Render_Device(void);
  bool Is_Windowed(void) { return bool(m_iWindowed == 1); }
  void Set_Windowed(bool windowed) {
    m_iWindowed = (int)windowed;
...
inline CLevelEditDoc *CLevelEditView::GetDocument() { return (CLevelEditDoc *)m_pDocument; }
#endif
