...
Sys_String_To_ANSI(BSTR bstr) {
  ...
  ::WideCharToMultiByte(CP_ACP, 0, bstr, len, ansi_string, len + 1, NULL, NULL);
...
//	Alloc_Sys_String
//
BSTR Alloc_Sys_String(LPCTSTR string) {
...
//
bool IDispatch_Get_Property(LPDISPATCH pdispatch, DISPID dispid, VARIANT *presult) {
  // Assume failure
  bool retval = false;
  ...
  if (pdispatch != NULL) {
    ...
    retval = SUCCEEDED(pdispatch->Invoke(dispid, IID_NULL, NULL, DISPATCH_PROPERTYGET, &no_args, presult, NULL, NULL));
  }
...
//	IDispatch_Get_Property
//
bool IDispatch_Get_Property(LPDISPATCH pdispatch, DISPID dispid, VARIANT &var_arg, VARIANT *presult) {
  // Assume failure
  bool retval = false;
  ...
  if (pdispatch != NULL) {
    ...
    retval =
        SUCCEEDED(pdispatch->Invoke(dispid, IID_NULL, NULL, DISPATCH_PROPERTYGET, &args, presult, &excep_info, NULL));
  }
...
//	Get_VSS_Interface
//
void VSSClass::Get_VSS_Interface(void) {
  if (m_pIVSSDatabase == NULL) {

    // Attempt to create an instance of the VSS Database interface
    LPUNKNOWN punknown = NULL;
    HRESULT hresult =
        ::CoCreateInstance(CLSID_VSSDatabase, NULL, CLSCTX_INPROC_SERVER, IID_IVSSDatabase, (LPVOID *)&punknown);
...
//	Open_Database
//
bool VSSClass::Open_Database(LPCTSTR ini_filename, LPCTSTR username, LPCTSTR password) {
...
//
IVSSItem *VSSClass::Get_VSS_Item_From_Local_Path(LPCTSTR local_filename) {
  // Assume failure
  IVSSItem *pitem = NULL;
  ...
  // State OK?
  if ((m_pIVSSDatabase != NULL) && (local_filename != NULL)) {
...
//	Get_VSS_Item
//
IVSSItem *VSSClass::Get_VSS_Item(LPCTSTR vss_path) {
  // Assume failure
  IVSSItem *pitem = NULL;
  ...
  // State OK?
  if ((m_pIVSSDatabase != NULL) && (vss_path != NULL)) {
...
//	Does_File_Exist
//
bool VSSClass::Does_File_Exist(LPCTSTR local_filename) {
  // Assume failure
  bool retval = false;
...
//	Get_Type
//
VSSItemType VSSClass::Get_Type(LPCTSTR local_filename) {
...
//	Get
//
bool VSSClass::Get(LPCTSTR local_filename) {
  // Assume failure
  bool retval = false;
...
//	Get
//
bool VSSClass::Get(LPCTSTR local_filename, IVSSItem *pitem) {
  // Assume failure
  bool retval = false;
...
//	Get_Subproject
//
bool VSSClass::Get_Subproject(LPCTSTR local_filename) {
  // Assume failure
  bool retval = false;
...
//	Check_Out
//
bool VSSClass::Check_Out(LPCTSTR local_filename, bool get_locally) {
  // Assume failure
  bool retval = false;
  ...
  if (pitem != NULL) {
    ...
    retval = SUCCEEDED(
        pitem->Checkout(NULL, bstr_filename, (get_locally ? VSSFLAG_TIMEMOD : VSSFLAG_GETNO) | VSSFLAG_CHKEXCLUSIVENO));
...
//	Check_In
//
bool VSSClass::Check_In(LPCTSTR local_filename, LPCTSTR comment) {
  // Assume failure
  bool retval = false;
  ...
  if (pitem != NULL) {
    ...
    retval = SUCCEEDED(
        pitem->Checkin(bstr_comment, bstr_filename, VSSFLAG_CMPCHKSUM | VSSFLAG_RECURSNO | VSSFLAG_UPDUPDATE));
...
//	Add_File
//
bool VSSClass::Add_File(LPCTSTR local_filename, LPCTSTR comment) {
  // Assume failure
  bool retval = false;
...
//	Build_Tree
//
bool VSSClass::Build_Tree(LPCTSTR local_filename) {
  // Assume failure
  bool retval = true;
...
//	Undo_Check_Out
//
bool VSSClass::Undo_Check_Out(LPCTSTR local_filename) {
  // Assume failure
  bool retval = false;
...
//	Destroy
//
bool VSSClass::Destroy(LPCTSTR local_filename) {
  // Assume failure
  bool retval = false;
...
//	Get_File_Version
//
long VSSClass::Get_File_Version(LPCTSTR local_filename) {
  long version = 0L;
...
//	Is_File_Different
//
bool VSSClass::Is_File_Different(LPCTSTR local_filename) {
...
//	Get_File_Date
//
bool VSSClass::Get_File_Date(
    // LPCTSTR local_filename,
    IVSSItem *pitem, SYSTEMTIME &system_time) {
  // Params OK?
  ASSERT(pitem != NULL);
  if (pitem != NULL) {
...
//	Get_File_Status
//
VSSFileStatus VSSClass::Get_File_Status(LPCTSTR local_filename, LPTSTR checked_out_username, DWORD buffer_size,
  ...
  if (pitem != NULL) {
    ...
    if ((status != VSSFILE_NOTCHECKEDOUT) && (checked_out_username != NULL)) {
...
VSSClass::Get_Error_Description(HRESULT hresult) {
  ...
  for (int error = 0; (error < error_count) && (description == NULL); error++) {
...
//	Retry_Check_Out
//
bool VSSClass::Retry_Check_Out(LPCTSTR local_filename, int iattempts, int idelay) {
  // Assume failure
  bool retval = false;
...
//	Retry_Check_In
//
bool VSSClass::Retry_Check_In(LPCTSTR local_filename, int iattempts, int idelay) {
  // Assume failure
  bool retval = false;
...
//	Get_Recursive
//
void VSSClass::Get_Recursive(LPCTSTR local_path, IVSSItem *pparent) {
...
//
///////////////////////////////////////////////////////////////////
bool VSSClass::Check_Out_Ex(LPCTSTR local_filename, HWND parent_wnd) {
  bool retval = false;
...
//
///////////////////////////////////////////////////////////////////
bool VSSClass::Check_In_Ex(LPCTSTR local_filename, HWND parent_wnd) {
  bool retval = false;
...
//
///////////////////////////////////////////////////////////////////
AssetDatabaseClass::FILE_STATUS VSSClass::Get_File_Status(LPCTSTR local_filename, StringClass *checked_out_user_name) {
...
//
///////////////////////////////////////////////////////////////////
FileClass *VSSClass::Get_File(LPCTSTR local_filename) {
  //
