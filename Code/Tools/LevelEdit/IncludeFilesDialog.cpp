...
IncludeFilesDialogClass::IncludeFilesDialogClass(CWnd *pParent /*=NULL*/)
    : m_hGlobalFolder(NULL), m_hLevelFolder(NULL), m_CurrentItem(NULL), CDialog(IncludeFilesDialogClass::IDD, pParent) {
...
// DoDataExchange
//
void IncludeFilesDialogClass::DoDataExchange(CDataExchange *pDX) {
...
// OnInitDialog
//
BOOL IncludeFilesDialogClass::OnInitDialog(void) {
  ...
  ScreenToClient(&rect);
  m_FilePicker.Create_Picker(WS_CHILD | WS_TABSTOP | WS_VISIBLE, rect, this, IDC_SPEC_EDIT);

  ::SetWindowPos(m_FilePicker, ::GetDlgItem(m_hWnd, IDC_SPEC_STATIC), 0, 0, 0, 0,
...
// OnOK
//
void IncludeFilesDialogClass::OnOK(void) {
  DynamicVectorClass<CString> &global_list = ::Get_File_Mgr()->Get_Global_Include_File_List();
  DynamicVectorClass<CString> &level_list = ::Get_File_Mgr()->Get_Include_File_List();
  ...
  for (HTREEITEM hchild = m_IncludesTreeCtrl.GetChildItem(m_hLevelFolder); hchild != NULL;
       hchild = m_IncludesTreeCtrl.GetNextSiblingItem(hchild)) {
...
//
void IncludeFilesDialogClass::OnSelchangedIncludeTree(NMHDR *pNMHDR, LRESULT *pResult) {
...
// Update_Add_Remove_Button
//
void IncludeFilesDialogClass::Update_Add_Remove_Button(void) {
  CString filespec;
...
IncludeFilesDialogClass::Find_Spec(LPCTSTR filespec, HTREEITEM hroot) {
  ...
  // Loop through all the children of the global folder
  for (HTREEITEM hchild = m_IncludesTreeCtrl.GetChildItem(hroot); (hchild != NULL) && (hitem == NULL);
       hchild = m_IncludesTreeCtrl.GetNextSiblingItem(hchild)) {
...
// OnChangeSpecEdit
//
void IncludeFilesDialogClass::OnChangeSpecEdit(void) {
  Update_Add_Remove_Button();
  return;
}
