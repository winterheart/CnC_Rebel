...
//
///////////////////////////////////////////////////////////////////////
void PresetExportClass::Import(const char *filename) {
  //
  ...
  if (file.Open()) {
    ...
    if (class_id != 0) {
      ...
      if (Read_Column_Headers(file, column_headers) && Validate_Columns(class_id, column_headers)) {
        //
    ...
    if (is_valid_file == false) {
      CString message;
      message.Format(
          "%s does not appear to be a valid preset export text file or there was an error parsing the data.\nDue to "
          "the severe side-effects of improperly importing preset settings, all changes were ignored.",
          filename);
...
//
///////////////////////////////////////////////////////////////////////
bool PresetExportClass::Read_Column_Headers(TextFileClass &file, DynamicVectorClass<StringClass> &column_headers) {
  bool retval = false;
...
//
///////////////////////////////////////////////////////////////////////
bool PresetExportClass::Validate_Columns(int class_id, DynamicVectorClass<StringClass> &column_headers) {
  bool retval = false;
...
//
///////////////////////////////////////////////////////////////////////
bool PresetExportClass::Import_Presets(TextFileClass &file, int class_id,
                                       DynamicVectorClass<StringClass> &column_headers) {
  bool retval = true;
...
//
///////////////////////////////////////////////////////////////////////
bool PresetExportClass::Import_Setting(PresetClass *preset, ParameterClass *parameter, const char *value) {
  bool retval = true;
...
ParameterClass *PresetExportClass::Find_Parameter(PresetClass *preset, const char *parameter_name) {
  ...
  for (int index = 0; !found && index < count; index++) {
    ...
    //	Is this the parameter we are looking for?
    //
    if (::lstrcmpi(parameter_name, curr_parameter->Get_Name()) == 0 && Can_Export_Parameter(curr_parameter)) {
...
//
///////////////////////////////////////////////////////////////////////
int PresetExportClass::Find_Header(TextFileClass &file) {
  int class_id = 0;
  StringClass line;
...
//
///////////////////////////////////////////////////////////////////////
void PresetExportClass::Export(uint32 class_id, const char *filename) {
  //
  //	Try to open the log file
  //
  TextFileClass file(filename);
...
//
///////////////////////////////////////////////////////////////////////
bool PresetExportClass::Can_Export_Parameter(ParameterClass *parameter) {
  bool retval = false;

  //
  //	We can only export simple parameter types
  //
  if (parameter->Get_Type() == ParameterClass::TYPE_INT || parameter->Get_Type() == ParameterClass::TYPE_FLOAT ||
      parameter->Get_Type() == ParameterClass::TYPE_STRING || parameter->Get_Type() == ParameterClass::TYPE_VECTOR3 ||
      parameter->Get_Type() == ParameterClass::TYPE_BOOL || parameter->Get_Type() == ParameterClass::TYPE_FILENAME ||
...
//
///////////////////////////////////////////////////////////////////////
void PresetExportClass::Write_Column_Headers(uint32 class_id, TextFileClass &file) {
  //
...
//
///////////////////////////////////////////////////////////////////////
void PresetExportClass::Export_Preset(TextFileClass &file, PresetClass *preset) {
  //
