...
uint32 LightSolveSaveSystemClass::Chunk_ID(void) const { return CHUNKID_EDITOR_LIGHT_SOLVE_SAVELOAD; }
...
bool LightSolveSaveSystemClass::Save(ChunkSaveClass &csave) {
  ...
  while (!it.Is_Done()) {
    ...
    if ((pobj != NULL) && (pobj->Peek_Model() != NULL) && (pobj->Peek_Model()->Has_User_Lighting())) {
...
bool LightSolveSaveSystemClass::Load_Lighting_For_Object(ChunkLoadClass &cload) {
  ...
  */
  ...
  if (obj != NULL) {
    ...
    if ((model != NULL) && (obj->Get_ID() == id) && (model->Class_ID() == (int)classid) &&
