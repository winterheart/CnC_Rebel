...
public:
  ...
  GrabHandlePhysClass(void)
      : m_iVertex(0), m_pParent(NULL), m_HitTestInfo(this), NodeFunctionClass(), Box3DPhysClass() {
    Initialize();
  }

  GrabHandlePhysClass(int ivertex, GrabHandlesClass *pparent)
      : m_iVertex(ivertex), m_pParent(pparent), m_HitTestInfo(this), NodeFunctionClass(), Box3DPhysClass() {
    Initialize();
  }
