...
void Paint_Gradient(HWND hwnd, BYTE base_red, BYTE base_green, BYTE base_blue) {
  ...
  for (int shade = 0; shade < 256; shade++) {
    // Paint this sliver
    cdc.FillSolidRect((int)posx, 0, (width_per_shade >= 1.00F) ? (int)width_per_shade : 1, height,
...
int Build_List_From_String(LPCTSTR buffer, LPCTSTR delimiter, CString **pstring_list) {
  ...
  if ((buffer != NULL) && (delimiter != NULL) && (pstring_list != NULL)) {
    ...
    for (LPCTSTR entry = buffer; (entry != NULL) && (entry[1] != 0); entry = ::strstr(entry, delimiter)) {
...
//
////////////////////////////////////////////////////////////////////////////
UINT Message_Box(HWND hparentwnd, UINT message_id, UINT title_id, UINT style) {
  UINT ret_code = IDOK;
...
//
////////////////////////////////////////////////////////////////////////////
int Get_LOD_File_Count(LPCTSTR first_lod_filename, CString *pbase_filename) {
...
//
////////////////////////////////////////////////////////////////////////////
bool Browse_For_Folder(CString &folder, HWND hparentwnd, LPCTSTR default_path, LPCTSTR title, UINT flags) {
  bool retval = false;
...
UINT fnWorkerThread(LPVOID pParam) {
  ...
  if (pthread_params != NULL) {
    ...
    (thread_proc)(pthread_params->dwparam1, pthread_params->dwparam2, pthread_params->dwparam3,
...
UINT fnUIThread(LPVOID pParam) {
  ...
  if (pthread_params != NULL) {
    ...
    (thread_proc)(pthread_params->dwparam1, pthread_params->dwparam2, pthread_params->dwparam3, pthread_params->presult,
...
void Create_UI_Thread(MY_UITHREADPROC fnthread_proc, DWORD dwparam1, DWORD dwparam2, DWORD dwparam3, HRESULT *presult,
  ...
  while (::MsgWaitForMultipleObjects(1, &(pthread_params->hevent), FALSE, INFINITE,
...
//
////////////////////////////////////////////////////////////////////////////
float GetWindowFloat(HWND hwnd, bool interpret) {
...
//
////////////////////////////////////////////////////////////////////////////
float GetDlgItemFloat(HWND hdlg, UINT child_id, bool interpret) {
  // Get the string from the window
  TCHAR string_value[20];
...
//
////////////////////////////////////////////////////////////////////////////
void Fill_Node_Instance_Combo(HWND hcombobox, NodeClass *default_node) {
...
//
////////////////////////////////////////////////////////////////////////////
void Fill_Group_Combo(HWND hcombobox, GroupMgrClass *pdefault) {
...
//
////////////////////////////////////////////////////////////////////////////
UINT fnUpdatingVSSThread(DWORD dwparam1, DWORD /*dwparam2*/, DWORD /*dwparam3*/, HRESULT * /*presult*/,
                         HWND *phmain_wnd) {
...
//
////////////////////////////////////////////////////////////////////////////
bool Get_File_Time(LPCTSTR path, LPFILETIME pcreation_time, LPFILETIME paccess_time, LPFILETIME pwrite_time) {
  // Assume failure
  bool retval = false;
...
//
////////////////////////////////////////////////////////////////////////////
int Type_To_Icon(NODE_TYPE type, bool btemp) {
  int index = TEMP_ICON;
...
//
/////////////////////////////////////////////////////////////////////////////
FileAccessRightsClass::FileAccessRightsClass(LPCTSTR filename, ACCESS_TYPE type, bool should_restore)
    : m_Filename(filename), m_FileAttrs(0xFFFFFFFF), m_bNeedsRestoring(false) {
  // Does the file exist?
  m_FileAttrs = ::GetFileAttributes(m_Filename);
  if (m_FileAttrs != 0xFFFFFFFF) {

    if ((type == WANTS_READONLY) && (m_FileAttrs & FILE_ATTRIBUTE_READONLY) != FILE_ATTRIBUTE_READONLY) {
    ...
    } else if ((type == WANTS_WRITEABLE) && (m_FileAttrs & FILE_ATTRIBUTE_READONLY) == FILE_ATTRIBUTE_READONLY) {
...
//
/////////////////////////////////////////////////////////////////////////////
void Restore_Edit_Ctrl(HWND edit_wnd) {
...
//
/////////////////////////////////////////////////////////////////////////////
LRESULT CALLBACK fnEditToIntProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
  WNDPROC old_proc = (WNDPROC)::GetProp(hwnd, "OLD_WND_PROC");
  LRESULT result = 0L;
...
//
/////////////////////////////////////////////////////////////////////////////
bool Is_Silent_Mode(void) { return _IsSilent; }
...
PersistClass *Instance_Definition(DefinitionClass *definition) {
  ...
  if (new_object == NULL) {
    ...
    CString message;
    message.Format("Unable to create an instance of %s.\nCheck to make sure the preset is properly configured with a "
                   "model and/or physics object.",
                   name);
...
void Perform_Job(LPCTSTR filename, bool delete_on_completion) {
  ...
  ::GetPrivateProfileString("Job Description", "Granularity", "1", granularity_string.GetBufferSetLength(20), 20,
                            filename);
  ...
  CString output_file;
  ::GetPrivateProfileString("Job Description", "Output", "", output_file.GetBufferSetLength(MAX_PATH), MAX_PATH,
                            filename);
