...
BOOL WeatherPropPageClass::OnInitDialog() {
  ...
  BackgroundMgrClass::Get_Lightning(lightningintensity, lightningstartdistance, lightningenddistance, lightningheading,
                                    lightningdistribution);
  if (lightningintensity > 0.0f) {
    OnLightningLightning();
  } else {
    BackgroundMgrClass::Get_War_Blitz(lightningintensity, lightningstartdistance, lightningenddistance,
                                      lightningheading, lightningdistribution);
    if (lightningintensity > 0.0f) {
      OnLightningWarBlitz();
    } else {
      OnLightningNone();
...
void WeatherPropPageClass::OnFogColor() {
  ...
  if (::Show_Color_Picker(&red, &green, &blue)) {
    ...
    FogColor.Set((red + 0.5f) * ooucharmaxplusone, (green + 0.5f) * ooucharmaxplusone,
...
void WeatherPropPageClass::OnOK() {
  ...
  if (IsDlgButtonChecked(IDC_LIGHTNING_NONE) == 1) {
    ...
    if (IsDlgButtonChecked(IDC_LIGHTNING_LIGHTNING) == 1) {
      BackgroundMgrClass::Set_Lightning(lightningintensity, lightningstartdistance, lightningenddistance,
                                        lightningheading, lightningdistribution);
      BackgroundMgrClass::Set_War_Blitz(0.0f);
    } else {
      WWASSERT(IsDlgButtonChecked(IDC_LIGHTNING_WAR_BLITZ) == 1);
      BackgroundMgrClass::Set_Lightning_Intensity(0.0f);
      BackgroundMgrClass::Set_War_Blitz(lightningintensity, lightningstartdistance, lightningenddistance,
                                        lightningheading, lightningdistribution);
    }
  }
