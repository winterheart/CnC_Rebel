...
static bool Get_Ping_Response(SOCKET socket, int &seq_id, struct sockaddr *address, unsigned long validate_addr,
                              unsigned long &my_address);
static unsigned short Get_IP_Checksum(unsigned short *buffer, int size);
static bool Send_Raw_UDP(char *payload, int payload_size, SOCKET socket, struct sockaddr *address,
                         unsigned short source_port, unsigned short dest_port);
static unsigned long Upstream_Detect(unsigned long server_ip, unsigned long my_ip, int &failure_code,
                                     unsigned long &downstream, BandtestSettingsStruct *settings);
static int Ping_Host(unsigned long host_ip, unsigned long my_ip, int times, int payload_size, unsigned long *ping_times,
 ...
 *=============================================================================================*/
bool APIENTRY DllMain(HANDLE, DWORD, void *) { return (true); }
 ...
 *   10/3/2001 11:21AM ST : Created                                                            *
 *=============================================================================================*/
unsigned long Upstream_Detect(unsigned long server_ip, unsigned long my_ip, int &failure_code,
                              unsigned long &downstream, BandtestSettingsStruct *settings) {
  ...
  */
  if (my_ip == 0) {
    ...
    */
    ...
    seq_id = -1;
    while (seq_id == -1) {
      Get_Ping_Response(ICMPRawSocket, seq_id, (struct sockaddr *)&address, ntohl(host_address.sin_addr.s_addr),
                        ping_dest_address);
  ...
  */
  ...
  /*
  ** If the ping time is low, it's *probably* a high bandwidth connection so we can get a more accurate test by sending
  *more
  ...
  */
  ...
  /*
  ** Now see what the ping time to the router is. Since there are n UDP packets ahead of this ping before it goes out,
  *the
  ...
  */
  ...
  /*
  ** If the bandwidth in the registry is close to what we just calculated then use the old downstream calculation from
  *the
  ** registry.
  */
 ...
 *=============================================================================================*/
int Ping_Host(unsigned long host_ip, unsigned long my_ip, int times, int payload_size, unsigned long *ping_times,
              unsigned long timeout) {
  ...
  for (int i = 0; i < times; i++) {
    ...
    */
    while (seq_id != _packet_sequencer) {
      Get_Ping_Response(ICMPRawSocket, seq_id, (struct sockaddr *)&address, ntohl(host_addr.sin_addr.s_addr),
                        ping_dest_address);
 ...
 *=============================================================================================*/
bool Send_Raw_UDP(char *payload, int payload_size, SOCKET socket, struct sockaddr *address, unsigned short source_port,
...
bool Send_Ping(char *payload, int payload_size, SOCKET socket, struct sockaddr *address, int sequence_id) {
  // DebugString("Send ping. payload=%08X, size=%d, socket=%d, address=%s, seq_id=%d\n", payload, payload_size, socket,
  // Addr_As_String2((struct sockaddr_in *)address), sequence_id);
  /*
  ** Asserts.
  */
  assert(payload_size < 550);
 ...
 *=============================================================================================*/
bool Get_Ping_Response(SOCKET socket, int &seq_id, struct sockaddr *address, unsigned long validate_address,
                       unsigned long &my_address) {
        ...
        */
        case ICMP_TIME_EXCEEDED:
          // DebugString("Type is ICMP_TIME_EXCEEDED\n");
          {
            ...
            ** header followed by the original ICMP header followed by 64 bits of the original payload. Phew. (the
            *original
...
void Close_Registry(void) { RegCloseKey(RegistryKey); }
...
char *Addr_As_String2(struct sockaddr_in *addr) {
  ...
  sprintf(_string, "%d.%d.%d.%d ; %d", (int)(addr->sin_addr.S_un.S_un_b.s_b1), (int)(addr->sin_addr.S_un.S_un_b.s_b2),
          (int)(addr->sin_addr.S_un.S_un_b.s_b3), (int)(addr->sin_addr.S_un.S_un_b.s_b4), (int)(addr->sin_port));
  return (_string);
}
  ...
  */
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (seq_id == packet_sequencer) {
      unsigned long router_ping_time = timeGetTime() - start_time;
      DebugString("Ping time %d to external router %s is %d ms\n", num_pings, Addr_As_String2(&router_addr),
                  router_ping_time);
      ping_times[num_pings++] = router_ping_time;
    }
    packet_sequencer++;
