...
public:
  ...
  ParticleEmitterClass(float emit_rate, unsigned int burst_size, Vector3Randomizer *pos_rnd, Vector3 base_vel,
                       Vector3Randomizer *vel_rnd, float out_vel, float vel_inherit_factor,
                       ParticlePropertyStruct<Vector3> &color, ParticlePropertyStruct<float> &opacity,
                       ParticlePropertyStruct<float> &size, ParticlePropertyStruct<float> &rotation, float orient_rnd,
                       ParticlePropertyStruct<float> &frames, ParticlePropertyStruct<float> &blur_times, Vector3 accel,
                       float max_age, TextureClass *tex, ShaderClass shader = ShaderClass::_PresetAdditiveSpriteShader,
  ...
  void Set_One_Time_Burst(int size) {
    OneTimeBurstSize = size != 0 ? size : 1;
    OneTimeBurst = true;
  }
  ...
  // float						Get_Velocity_Random (void) const	{ return VelRand ?
  // (VelRand->Get_Maximum_Extent() * 1000.0f) : 0.0f; }
  ...
  void Get_Opacity_Key_Frames(ParticlePropertyStruct<float> &opacities) const {
    Buffer->Get_Opacity_Key_Frames(opacities);
  }
  ...
  void Get_Rotation_Key_Frames(ParticlePropertyStruct<float> &rotations) const {
    Buffer->Get_Rotation_Key_Frames(rotations);
  }
  ...
  void Get_Blur_Time_Key_Frames(ParticlePropertyStruct<float> &blurtimes) const {
    Buffer->Get_Blur_Time_Key_Frames(blurtimes);
  }
