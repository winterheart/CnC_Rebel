...
ParticleEmitterDefClass::ParticleEmitterDefClass(void)
    : m_pName(NULL), m_Version(0L), m_pUserString(NULL), m_iUserType(EMITTER_TYPEID_DEFAULT),
      m_InitialOrientationRandom(0), m_pCreationVolume(NULL), m_pVelocityRandomizer(NULL) {
...
ParticleEmitterDefClass::ParticleEmitterDefClass(const ParticleEmitterDefClass &src)
    : m_pName(NULL), m_Version(0L), m_pUserString(NULL), m_iUserType(EMITTER_TYPEID_DEFAULT),
      m_InitialOrientationRandom(src.m_InitialOrientationRandom), m_pCreationVolume(NULL), m_pVelocityRandomizer(NULL) {
  ::memset(&m_Info, 0, sizeof(m_Info));
  ::memset(&m_InfoV2, 0, sizeof(m_InfoV2));
...
WW3DErrorType ParticleEmitterDefClass::Load_W3D(ChunkLoadClass &chunk_load) {
  ...
  if ((Read_Header(chunk_load) == WW3D_ERROR_OK) && (Read_User_Data(chunk_load) == WW3D_ERROR_OK) &&
    ...
    if (m_Version > 0x00010000) {
      ...
      //
      if ((Read_InfoV2(chunk_load) == WW3D_ERROR_OK) && (Read_Props(chunk_load) == WW3D_ERROR_OK)) {
...
WW3DErrorType ParticleEmitterDefClass::Save_W3D(ChunkSaveClass &chunk_save) {
  ...
  if (chunk_save.Begin_Chunk(W3D_CHUNK_EMITTER) == TRUE) {
    ...
    if ((Save_Header(chunk_save) == WW3D_ERROR_OK) && (Save_User_Data(chunk_save) == WW3D_ERROR_OK) &&
        (Save_Info(chunk_save) == WW3D_ERROR_OK) && (Save_InfoV2(chunk_save) == WW3D_ERROR_OK) &&
        (Save_Props(chunk_save) == WW3D_ERROR_OK) && (Save_Rotation_Keyframes(chunk_save) == WW3D_ERROR_OK) &&
  ...
  if (chunk_save.Begin_Chunk(W3D_CHUNK_EMITTER_PROPS) == TRUE) {
    ...
    if (chunk_save.Write(&info, sizeof(info)) == sizeof(info)) {
      ...
      if ((Save_Color_Keyframes(chunk_save) == WW3D_ERROR_OK) &&
          (Save_Opacity_Keyframes(chunk_save) == WW3D_ERROR_OK) && (Save_Size_Keyframes(chunk_save) == WW3D_ERROR_OK)) {
