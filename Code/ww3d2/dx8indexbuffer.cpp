...
IndexBufferClass::IndexBufferClass(unsigned type_, unsigned short index_count_)
  ...
  WWDEBUG_SAY(("Total IB count: %d, total %d indices, total size %d bytes\n", _IndexBufferCount,
               _IndexBufferTotalIndices, _IndexBufferTotalSize));
#endif
}
...
IndexBufferClass::WriteLockClass::WriteLockClass(IndexBufferClass *index_buffer_) : index_buffer(index_buffer_) {
  ...
  case BUFFER_TYPE_DX8:
    ...
    DX8_ErrorCode(static_cast<DX8IndexBufferClass *>(index_buffer)
                      ...
                      ->Lock(0, index_buffer->Get_Index_Count() * sizeof(WORD),
...
IndexBufferClass::AppendLockClass::AppendLockClass(IndexBufferClass *index_buffer_, unsigned start_index,
  ...
  case BUFFER_TYPE_DX8:
    DX8_Assert();
    DX8_ErrorCode(static_cast<DX8IndexBufferClass *>(index_buffer)
                      ->index_buffer->Lock(start_index * sizeof(unsigned short), index_range * sizeof(unsigned short),
#if (DIRECT3D_VERSION < 0x0900)
                                           (unsigned char **)&indices,
#else
                                           (void **)&indices,
...
DX8IndexBufferClass::DX8IndexBufferClass(unsigned short index_count_, UsageType usage)
  ...
  HRESULT ret = DX8Wrapper::_Get_D3D_Device8()->CreateIndexBuffer(
      ...
      (usage & USAGE_DYNAMIC) ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED, &index_buffer
#if (DIRECT3D_VERSION >= 0x0900)
      ,
      NULL
#endif
  ...
  // Try again...
  ret = DX8Wrapper::_Get_D3D_Device8()->CreateIndexBuffer(sizeof(WORD) * index_count, usage_flags, D3DFMT_INDEX16,
...
DynamicIBAccessClass::WriteLockClass::WriteLockClass(DynamicIBAccessClass *ib_access_) : DynamicIBAccess(ib_access_) {
  ...
  case BUFFER_TYPE_DYNAMIC_DX8:
    ...
    DX8_Assert();
    DX8_ErrorCode(static_cast<DX8IndexBufferClass *>(DynamicIBAccess->IndexBuffer)
                      ->Get_DX8_Index_Buffer()
...
void DynamicIBAccessClass::Allocate_DX8_Dynamic_Buffer() {
  ...
  if (!_DynamicDX8IndexBuffer) {
    ...
    _DynamicDX8IndexBuffer =
        NEW_REF(DX8IndexBufferClass, (_DynamicDX8IndexBufferSize, (DX8IndexBufferClass::UsageType)usage));
    _DynamicDX8IndexBufferOffset = 0;
  }
