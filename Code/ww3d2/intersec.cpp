...
bool IntersectionClass::Intersect_Screen_Point_RenderObject(float screen_x, float screen_y, const LayerClass &Layer,
                                                            RenderObjClass *RObj,
...
bool IntersectionClass::Intersect_Screen_Point_Layer_Range(float screen_x, float screen_y, const LayerClass &TopLayer,
...
bool IntersectionClass::Intersect_Screen_Point_Layer(float screen_x, float screen_y, const LayerClass &Layer) {
  ...
  if ((screen_x < v.Min.X) || (screen_x > v.Max.X) || (screen_y < v.Min.Y) || (screen_y > v.Max.Y))
    return false;
...
void IntersectionClass::Append_Object_Array(int MaxCount, int &CurrentCount, RenderObjClass **ObjectArray,
...
void IntersectionClass::Append_Hierarchy_Objects(int MaxCount, int &CurrentCount, RenderObjClass **ObjectArray,
...
bool IntersectionClass::Intersect_Hierarchy(RenderObjClass *Hierarchy, IntersectionResultClass *FinalResult,
  ...
  Append_Hierarchy_Objects(MAX_HIERARCHY_NODE_COUNT, candidate_count, candidate_objects, Hierarchy,
                           Test_Bounding_Sphere, Convex);
...
RenderObjClass *IntersectionClass::Intersect_Sub_Object(float screenx, float screeny, LayerClass &layer,
                                                        RenderObjClass *robj, IntersectionResultClass *result) {
...
bool IntersectionClass::Intersect_Object_Array(int Object_Count, RenderObjClass **ObjectArray,
...
bool IntersectionClass::Intersect_Object_Array(int Object_Count, RenderObjClass **ObjectArray,
  ...
  // if it's a convex hierarchy (ie a control panel) then find the first hit otherwise use the more expensive exact
  // intersection routine for use with potentially concave hierarchies.
  int nearest_index = -1;
