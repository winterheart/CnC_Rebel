...
SurfaceClass::SurfaceClass(DX_IDirect3DSurface *d3d_surface) : D3DSurface(NULL) {
  Attach(d3d_surface);
  SurfaceDescription desc;
  Get_Description(desc);
  SurfaceFormat = desc.Format;
 ...
 *   2/13/2001  hy : Created.                                                                  *
 *=============================================================================================*/
void SurfaceClass::Copy(unsigned int dstx, unsigned int dsty, unsigned int srcx, unsigned int srcy, unsigned int width,
  ...
  if (sd.Format == osd.Format && sd.Width == osd.Width && sd.Height == osd.Height) {
    ...
    DX8_ErrorCode(
        D3DXLoadSurfaceFromSurface(D3DSurface, NULL, &dest, other->D3DSurface, NULL, &src, D3DX_FILTER_NONE, 0));
  } else {
    RECT dest;
    dest.left = dstx;
    dest.right = dstx + width;
...
void SurfaceClass::Stretch_Copy(unsigned int dstx, unsigned int dsty, unsigned int dstwidth, unsigned int dstheight,
  ...
  DX8_ErrorCode(
      D3DXLoadSurfaceFromSurface(D3DSurface, NULL, &dest, other->D3DSurface, NULL, &src, D3DX_FILTER_TRIANGLE, 0));
}
...
void SurfaceClass::FindBB(Vector2i *min, Vector2i *max) {
  ...
  for (y = min->J; y < max->J; y++) {
    for (x = min->I; x < max->I; x++) {
      ...
      unsigned char *alpha =
          (unsigned char *)((unsigned int)lock_rect.pBits + (y - min->J) * lock_rect.Pitch + (x - min->I) * size);
