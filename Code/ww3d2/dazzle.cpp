...
LensflareTypeClass::LensflareTypeClass(const LensflareInitClass &is) : lic(is), texture(NULL) {}
...
void LensflareTypeClass::Generate_Vertex_Buffers(VertexFormatXYZNDUV2 *vertex, int &vertex_count, float screen_x_scale,
  ...
  float distance_multiplier =
      sqrt(transformed_location[0] * transformed_location[0] + transformed_location[1] * transformed_location[1]) +
      1.0f;
...
DazzleTypeClass::DazzleTypeClass(const DazzleInitClass &is)
    : ic(is), dazzle_shader(default_dazzle_shader), halo_shader(default_halo_shader), primary_texture(NULL),
      secondary_texture(NULL), lensflare_id(DazzleRenderObjClass::Get_Lensflare_ID(is.lensflare_name)),
      radius(is.radius) {
  fadeout_end_sqr = ic.fadeout_end * ic.fadeout_end;
  fadeout_start_sqr = ic.fadeout_start * ic.fadeout_start;
  dazzle_test_color_integer = 0xff000000 | (unsigned(255.0f * ic.dazzle_test_color[2]) << 16) |
...
void DazzleTypeClass::Calculate_Intensities(float &dazzle_intensity, float &dazzle_size, float &halo_intensity,
                                            const Vector3 &camera_dir, const Vector3 &dazzle_dir,
...
void DazzleRenderObjClass::Init_From_INI(const INIClass *ini) {
  ...
  for (entry = 0; entry < count; entry++) {
    ...
    ini->Get_String(LENSFLARE_LIST_STRING, ini->Get_Entry(LENSFLARE_LIST_STRING, entry), "", section_name,
                    sizeof(section_name));
  ...
  for (entry = 0; entry < count; entry++) {
    char section_name[80];
    ini->Get_String(DAZZLE_LIST_STRING, ini->Get_Entry(DAZZLE_LIST_STRING, entry), "", section_name,
                    sizeof(section_name));
...
DazzleRenderObjClass::DazzleRenderObjClass(unsigned t)
    : succ(NULL), type(t), current_dazzle_intensity(0.0f), current_dazzle_size(0.0f), dazzle_color(1.0f, 1.0f, 1.0f),
      halo_color(1.0f, 1.0f, 1.0f), lensflare_intensity(1.0f), on_list(false), visibility(0.0f) {
  if (types && types[t]) {
...
DazzleRenderObjClass::DazzleRenderObjClass(const char *type_name)
    : succ(NULL), type(Get_Type_ID(type_name)), current_dazzle_intensity(0.0f), current_dazzle_size(0.0f),
      dazzle_color(1.0f, 1.0f, 1.0f), halo_color(1.0f, 1.0f, 1.0f), lensflare_intensity(1.0f), on_list(false),
      visibility(0.0f) {
...
DazzleRenderObjClass::DazzleRenderObjClass(const DazzleRenderObjClass &src)
    : succ(NULL), type(src.type), current_dazzle_intensity(src.current_dazzle_intensity),
      current_dazzle_size(src.current_dazzle_size), current_dir(src.current_dir), dazzle_color(src.dazzle_color),
      halo_color(src.halo_color), lensflare_intensity(src.lensflare_intensity), on_list(false),
      visibility(src.visibility), radius(src.radius) {
  creation_time = WW3D::Get_Sync_Time();
}
...
void DazzleRenderObjClass::Set_Current_Dazzle_Layer(DazzleLayerClass *layer) { current_dazzle_layer = layer; }

/////////////////////////////////////////////////////////////////////////////
// Render Object Interface
/////////////////////////////////////////////////////////////////////////////

RenderObjClass *DazzleRenderObjClass::Clone(void) const { return NEW_REF(DazzleRenderObjClass, (*this)); }
...
void DazzleRenderObjClass::Render(RenderInfoClass &rinfo) {
  ...
  if (Is_Not_Hidden_At_All() && _dazzle_rendering_enabled && !DX8Wrapper::Is_Render_To_Texture()) {
    ...
    if (is_on == false) {
      ...
      params->Calculate_Intensities(dazzle_intensity, dazzle_size, current_halo_intensity, camera_dir, current_dir,
                                    current_distance);
...
void DazzleRenderObjClass::Render_Dazzle(CameraClass *camera) {
  ...
  DynamicVBAccessClass vb_access(BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type, vertex_count * 2 + lens_max_verts);
  {
    ...
    if (current_dazzle_intensity > 0.0f) {
      ...
      Vector3 col(dazzle_color[0] * params->ic.dazzle_color[0], dazzle_color[1] * params->ic.dazzle_color[1],
    ...
    if (current_halo_intensity) {
      ...
      Vector3 col(halo_color[0] * params->ic.halo_color[0], halo_color[1] * params->ic.halo_color[1],
    ...
    if (lensflare && current_dazzle_intensity > 0.0f) {
      ...
      lensflare->Generate_Vertex_Buffers(vertex, lensflare_vertex_count, screen_x_scale, screen_y_scale,
...
DazzleTypeClass *
DazzleRenderObjClass::Get_Type_Class(unsigned id) // Return dazzle type class pointer, or NULL if not found
{
  if (id >= type_count)
    return NULL;
  return types[id];
...
LensflareTypeClass *
DazzleRenderObjClass::Get_Lensflare_Class(unsigned id) // Return lensflare type class pointer, or NULL if not found
{
...
uint32 DazzlePersistFactoryClass::Chunk_ID(void) const { return WW3D_PERSIST_CHUNKID_DAZZLE; }
...
*/
const PersistFactoryClass &DazzleRenderObjClass::Get_Factory(void) const { return _DazzleFactory; }
...
float DazzleVisibilityClass::Compute_Dazzle_Visibility(RenderInfoClass &rinfo, DazzleRenderObjClass *dazzle,
...
RenderObjClass *DazzlePrototypeClass::Create(void) { return NEW_REF(DazzleRenderObjClass, (DazzleType)); }
