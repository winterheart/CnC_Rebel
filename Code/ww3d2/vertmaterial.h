...
public:
  ...
  */
  ...
  VertexMaterialClass *Clone(void) {
    VertexMaterialClass *mat = NEW_REF(VertexMaterialClass, ());
    *mat = *this;
    return mat;
  }
...
inline TextureMapperClass *VertexMaterialClass::Peek_Mapper(int stage) { return Mapper[stage]; }
...
inline bool VertexMaterialClass::Do_Mappers_Need_Normals(void) const {
  for (int stage = 0; stage < MeshBuilderClass::MAX_STAGES; stage++) {
    if (Mapper[stage] && (Mapper[stage]->Needs_Normals()))
      return true;
  }
  return false;
}
