...
WW3DAssetManager::WW3DAssetManager(void)
    : PrototypeLoaders(PROTOLOADERS_VECTOR_SIZE), Prototypes(PROTOTYPES_VECTOR_SIZE),

#ifdef WW3D_DX8
      TextureCache(NULL),
#endif // WW3D_DX8

      WW3D_Load_On_Demand(false), Activate_Fog_On_Load(false), MetalManager(0) {
...
static void Log_Textures(bool inited, unsigned &total_count, unsigned &total_mem) {
  ...
  for (ite.First(); !ite.Is_Done(); ite.Next()) {
    ...
    DX_IDirect3DTexture *d3d_texture = tex->Peek_DX8_Texture();
    if (!d3d_texture)
      continue;
 ...
 *   12/21/97   GTH : Created.                                                                 *
 *=============================================================================================*/
RenderObjIterator *WW3DAssetManager::Create_Render_Obj_Iterator(void) { return new RObjIterator(); }
 ...
 *   12/21/97   GTH : Created.                                                                 *
 *=============================================================================================*/
AssetIterator *WW3DAssetManager::Create_HAnim_Iterator(void) { return new HAnimIterator(); }
 ...
 *   3/11/98    GTH : Created.                                                                 *
 *=============================================================================================*/
AssetIterator *WW3DAssetManager::Create_HTree_Iterator(void) { return new HTreeIterator(); }
 ...
 *   1/6/98     GTH : Created.                                                                 *
 *=============================================================================================*/
AssetIterator *WW3DAssetManager::Create_Font3DData_Iterator(void) { return new Font3DDataIterator(); }
 ...
 *=============================================================================================*/
TextureClass *WW3DAssetManager::Get_Texture(const char *filename, TextureClass::MipCountType mip_level_count,
...
void WW3DAssetManager::Log_All_Textures(void) {
  ...
  WWDEBUG_SAY(("Lightmap textures: %d\n\n"
               "size     name\n"
               "--------------------------------------\n",
  ...
  WWDEBUG_SAY(("Procedural textures: %d\n\n"
               "size     name\n"
               "--------------------------------------\n",
  ...
  WWDEBUG_SAY(("Ordinary textures: %d\n\n"
               "size     name\n"
               "--------------------------------------\n",
               TextureClass::_Get_Total_Texture_Count() - TextureClass::_Get_Total_Lightmap_Texture_Count() -
                   TextureClass::_Get_Total_Procedural_Texture_Count()));
...
void WW3DAssetManager::Remove_Prototype(PrototypeClass *proto) {
  ...
  if (proto != NULL) {
    ...
    for (PrototypeClass *test = PrototypeHashTable[hash]; (test != NULL) && (bfound == false); test = test->NextHash) {
...
bool RObjIterator::Is_Done(void) { return !(Index < WW3DAssetManager::Get_Instance()->Prototypes.Count()); }
...
bool HTreeIterator::Is_Done(void) { return !(Index < WW3DAssetManager::Get_Instance()->HTreeManager.Num_Trees()); }
