...
TextureClass::TextureClass(unsigned width, unsigned height, WW3DFormat format, MipCountType mip_level_count,
                           PoolType pool, bool rendertarget)
    : D3DTexture(NULL), texture_id(unused_texture_id++), Initialized(true), TextureMinFilter(FILTER_TYPE_DEFAULT),
      TextureMagFilter(FILTER_TYPE_DEFAULT),
      MipMapFilter((mip_level_count != MIP_LEVELS_1) ? FILTER_TYPE_DEFAULT : FILTER_TYPE_NONE),
      UAddressMode(TEXTURE_ADDRESS_REPEAT), VAddressMode(TEXTURE_ADDRESS_REPEAT), MipLevelCount(mip_level_count),
      Pool(pool), Dirty(false), IsLightmap(false), IsProcedural(true), Name(""), TextureFormat(format),
      IsCompressionAllowed(false), TextureLoadTask(NULL), ThumbnailLoadTask(NULL), Width(width), Height(height),
      InactivationTime(0), // Don't inactivate!
      ExtendedInactivationTime(0), LastInactivationSyncTime(0) {
  switch (format) {
  ...
  if (pool == POOL_DEFAULT) {
    Dirty = true;
    DX8TextureTrackerClass *track =
        new DX8TextureTrackerClass(width, height, format, mip_level_count, rendertarget, this);
...
TextureClass::TextureClass(const char *name, const char *full_path, MipCountType mip_level_count,
                           WW3DFormat texture_format, bool allow_compression)
    : D3DTexture(NULL), texture_id(unused_texture_id++), Initialized(false), TextureMinFilter(FILTER_TYPE_DEFAULT),
      TextureMagFilter(FILTER_TYPE_DEFAULT),
      MipMapFilter((mip_level_count != MIP_LEVELS_1) ? FILTER_TYPE_DEFAULT : FILTER_TYPE_NONE),
      UAddressMode(TEXTURE_ADDRESS_REPEAT), VAddressMode(TEXTURE_ADDRESS_REPEAT), MipLevelCount(mip_level_count),
      Pool(POOL_MANAGED), Dirty(false), IsLightmap(false), IsProcedural(false), TextureFormat(texture_format),
      IsCompressionAllowed(allow_compression), TextureLoadTask(NULL), ThumbnailLoadTask(NULL), Width(0), Height(0),
...
TextureClass::TextureClass(SurfaceClass *surface, MipCountType mip_level_count)
    : D3DTexture(NULL), texture_id(unused_texture_id++), Initialized(true), TextureMinFilter(FILTER_TYPE_DEFAULT),
      ...
      MipMapFilter((mip_level_count != MIP_LEVELS_1) ? FILTER_TYPE_DEFAULT : FILTER_TYPE_NONE),
      UAddressMode(TEXTURE_ADDRESS_REPEAT), VAddressMode(TEXTURE_ADDRESS_REPEAT), MipLevelCount(mip_level_count),
      Pool(POOL_MANAGED), Dirty(false), IsLightmap(false), Name(""), IsProcedural(true),
      TextureFormat(surface->Get_Surface_Format()), IsCompressionAllowed(false), TextureLoadTask(NULL),
      ThumbnailLoadTask(NULL), Width(0), Height(0), InactivationTime(0), // Don't inactivate
      ExtendedInactivationTime(0), LastInactivationSyncTime(0) {
...
TextureClass::TextureClass(DX_IDirect3DTexture *d3d_texture)
    : D3DTexture(d3d_texture), texture_id(unused_texture_id++), Initialized(true),
      ...
      MipLevelCount((MipCountType)d3d_texture->GetLevelCount()), Pool(POOL_MANAGED), Dirty(false), IsLightmap(false),
      Name(""), IsProcedural(true), IsCompressionAllowed(false), TextureLoadTask(NULL), ThumbnailLoadTask(NULL),
      Width(0), Height(0), InactivationTime(0), // Don't inactivate!
      ExtendedInactivationTime(0), LastInactivationSyncTime(0) {
  D3DTexture->AddRef();
...
bool TextureClass::Is_Missing_Texture() {
  bool flag = false;
  DX_IDirect3DTexture *missing_texture = MissingTexture::_Get_Missing_Texture();
...
DX_IDirect3DSurface *TextureClass::Get_D3D_Surface_Level(unsigned int level) {
  if (!D3DTexture) {
...
void TextureClass::Apply_New_Surface(DX_IDirect3DTexture *d3d_texture, bool initialized) {
  if (D3DTexture)
    D3DTexture->Release();
...
void TextureClass::_Init_Filters(TextureClass::TextureFilterMode filter_type) {
  ...
  if (dx8caps.TextureFilterCaps & D3DPTFILTERCAPS_MAGFLINEAR)
    _MagTextureFilters[0][FILTER_TYPE_BEST] = D3DTEXF_LINEAR;
  if (dx8caps.TextureFilterCaps & D3DPTFILTERCAPS_MINFLINEAR)
  ...
  if (filter_type == TextureClass::TEXTURE_FILTER_ANISOTROPIC ||
      filter_type == TextureClass::TEXTURE_FILTER_TRILINEAR) {
    ...
    if (hastexinfo) {
      ...
      case W3DTEXTURE_TYPE_BUMPMAP: {
        if (DX8Wrapper::Is_Initted() && DX8Wrapper::Get_Current_Caps()->Support_Bump_Envmap()) {
          ...
          if (DX8Wrapper::Get_Current_Caps()->Support_Texture_Format(WW3D_FORMAT_U8V8))
            format = WW3D_FORMAT_U8V8;
          else if (DX8Wrapper::Get_Current_Caps()->Support_Texture_Format(WW3D_FORMAT_X8L8V8U8))
            format = WW3D_FORMAT_X8L8V8U8;
          else if (DX8Wrapper::Get_Current_Caps()->Support_Texture_Format(WW3D_FORMAT_L6V5U5))
