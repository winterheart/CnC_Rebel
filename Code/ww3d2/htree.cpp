 ...
 *   3/4/98     GTH : Created.                                                                 *
 *=============================================================================================*/
HTreeClass::HTreeClass(const HTreeClass &src) : NumPivots(0), Pivot(NULL), ScaleFactor(1.0f) {
  ...
  */
  ...
  for (int pidx = first_piv; pidx < NumPivots; pidx++) {
    ...
    Pivot[pidx].BaseTransform =
        Pivot[pidx].BaseTransform *
        Build_Matrix3D(Quaternion(piv.Rotation.Q[0], piv.Rotation.Q[1], piv.Rotation.Q[2], piv.Rotation.Q[3]));
 ...
 *=============================================================================================*/
bool HTreeClass::Simple_Evaluate_Pivot(HAnimClass *motion, int pivot_index, float frame, const Matrix3D &obj_tm,
  ...
  if (motion != NULL && end_tm != NULL && pivot_index >= 0 && pivot_index < NumPivots) {
    ...
    //
    for (PivotClass *pivot = &Pivot[pivot_index]; pivot != NULL && pivot->Parent != NULL; pivot = pivot->Parent) {
      //
 ...
 *   3/4/98     GTH : Created.                                                                 *
 *=============================================================================================*/
void HTreeClass::Blend_Update(const Matrix3D &root, HAnimClass *motion0, float frame0, HAnimClass *motion1,
                              float frame1,
...
HTreeClass *HTreeClass::Create_Morphed(int num_morph_sources, const float morph_weights[],
...
HTreeClass *HTreeClass::Create_Interpolated(const HTreeClass *tree_a0_b0, const HTreeClass *tree_a0_b1,
                                            const HTreeClass *tree_a1_b0, const HTreeClass *tree_a1_b1, float lerp_a,
                                            float lerp_b) {
...
// Create an HTree by Interpolating between others
HTreeClass *HTreeClass::Create_Interpolated(const HTreeClass *tree_base, const HTreeClass *tree_a,
                                            const HTreeClass *tree_b, float a_scale, float b_scale) {
