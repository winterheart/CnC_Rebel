...
template <class T> class DLDestroyListClass : public DLListClass<T> {
public:
...
template <class T> inline void DLListClass<T>::Add_Head(DLNodeClass<T> *n) {
  n->list = this;
  if (head) {
...
template <class T> inline void DLListClass<T>::Add_Tail(DLNodeClass<T> *n) {
  n->list = this;
  if (tail) {
    n->Insert_After(tail);
    tail = n;
...
template <class T> inline void DLListClass<T>::Remove_Head() {
  if (!head)
    return;
...
template <class T> inline void DLListClass<T>::Remove_Tail() {
  if (!tail)
    return;
