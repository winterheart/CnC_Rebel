...
MatrixMapperClass::MatrixMapperClass(int stage)
    : TextureMapperClass(stage), Flags(0), ViewToTexture(true), ViewToPixel(true), GradientUCoord(0.5f) {
...
void MatrixMapperClass::Compute_Texture_Coordinate(const Vector3 &point, Vector3 *set_stq) {
  set_stq->X =
      ViewToPixel[0][0] * point.X + ViewToPixel[0][1] * point.Y + ViewToPixel[0][2] * point.Z + ViewToPixel[0][3];
  set_stq->Y =
      ViewToPixel[1][0] * point.X + ViewToPixel[1][1] * point.Y + ViewToPixel[1][2] * point.Z + ViewToPixel[1][3];
  set_stq->Z =
      ViewToPixel[3][0] * point.X + ViewToPixel[3][1] * point.Y + ViewToPixel[3][2] * point.Z + ViewToPixel[3][3];
}
...
void MatrixMapperClass::Apply(int uv_array_index) {
  ...
  case PERSPECTIVE_PROJECTION:
    ...
    */
    ...
    DX8Wrapper::Set_DX8_Texture_Stage_State(uv_array_index, D3DTSS_TEXCOORDINDEX, D3DTSS_TCI_CAMERASPACEPOSITION);
    DX8Wrapper::Set_DX8_Texture_Stage_State(uv_array_index, D3DTSS_TEXTURETRANSFORMFLAGS,
