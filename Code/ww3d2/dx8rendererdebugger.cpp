...
void DX8RendererDebugger::Enable(bool enable) { Enabled = true; }
...
void DX8RendererDebugger::Get_String(StringClass &s) {
  ...
  for (ite.First(); !ite.Is_Done(); ite.Next()) {
    ...
    tmp.Format("id: %5.5d mesh: %s %d polys, %d verts", ite.Peek_Key(), mesh->Get_Name(), polys, verts);
    s += tmp;
...
void DX8RendererDebugger::Add_Mesh(MeshClass *mesh) {
  ...
  if (MeshHash.Get(mesh->Get_Debug_Id()))
    return;
