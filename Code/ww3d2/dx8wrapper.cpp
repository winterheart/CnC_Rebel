...
DX_IDirect3DBaseTexture *DX8Wrapper::Textures[MAX_TEXTURE_STAGES];
RenderStateStruct DX8Wrapper::render_state;
unsigned DX8Wrapper::render_state_changed;

bool DX8Wrapper::FogEnable = false;
DX_D3DCOLOR DX8Wrapper::FogColor = 0;

DX_IDirect3DX *DX8Wrapper::D3DInterface = NULL;
DX_IDirect3DDevice *DX8Wrapper::D3DDevice = NULL;
DX_IDirect3DSurface *DX8Wrapper::CurrentRenderTarget = NULL;
DX_IDirect3DSurface *DX8Wrapper::DefaultRenderTarget = NULL;
DX_IDirect3DSurface *DX8Wrapper::DefaultDepthBuffer = NULL;
...
DX_D3DADAPTER_IDENTIFIER DX8Wrapper::CurrentAdapterIdentifier;
...
void DX8Wrapper::Shutdown(void) {
  if (D3DDevice) {
    Set_Render_Target((DX_IDirect3DSurface *)NULL);
...
void DX8Wrapper::Set_Default_Global_Render_States(void) {
  DX8_THREAD_ASSERT();
  const DX_D3DCAPS &caps = Get_Current_Caps()->Get_DX8_Caps();
...
bool DX8Wrapper::Create_Device(void) {
  ...
  ::ZeroMemory(&CurrentAdapterIdentifier, sizeof(DX_D3DADAPTER_IDENTIFIER));
#if (DIRECT3D_VERSION < 0x0900)
  ...
  DX8Caps dx8_caps(D3DInterface, caps, D3DFormat_To_WW3DFormat(_PresentParameters.BackBufferFormat),
  ...
  HRESULT hr = D3DInterface->CreateDevice(CurRenderDevice, WW3D_DEVTYPE, _Hwnd, vertex_processing_type,
  ...
  if (FAILED(hr)) {
    ...
    if ((_PresentParameters.BackBufferFormat == D3DFMT_R5G6B5 ||
      ...
      hr = D3DInterface->CreateDevice(CurRenderDevice, WW3D_DEVTYPE, _Hwnd, vertex_processing_type, &_PresentParameters,
                                      &D3DDevice);
      if (FAILED(hr)) {
        return false;
      }
      ...
      */
...
#else
      D3DDISPLAYMODE adapter_d3dmode;
      ::ZeroMemory(&adapter_d3dmode, sizeof(D3DDISPLAYMODE));
      D3DInterface->GetAdapterDisplayMode(adapter_index, &adapter_d3dmode);
      int mode_count = D3DInterface->GetAdapterModeCount(adapter_index, adapter_d3dmode.Format);
#endif
      for (int mode_index = 0; mode_index < mode_count; mode_index++) {
        ...
        if (res == D3D_OK) {
          ...
          case D3DFMT_R8G8B8: // FIXME: WH: Prohibited in DX9?
...
bool DX8Wrapper::Set_Render_Device(int dev, int width, int height, int bits, int windowed, bool resize_window) {
  ...
  */
  ...
  _PresentParameters.SwapEffect =
      IsWindowed ? D3DSWAPEFFECT_DISCARD : D3DSWAPEFFECT_FLIP; // Shouldn't this be D3DSWAPEFFECT_FLIP?
  ...
  */
  ...
  if (IsWindowed) {
    ...
    */
    if (!Find_Z_Mode(_PresentParameters.BackBufferFormat, _PresentParameters.BackBufferFormat,
    ...
    SetWindowPos(_Hwnd, HWND_TOPMOST, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN),
    ...
    */
    Find_Color_And_Z_Mode(ResolutionWidth, ResolutionHeight, BitDepth, &_PresentParameters.BackBufferFormat,
                          &_PresentParameters.AutoDepthStencilFormat);
  }
  ...
  if (resize_window) {
    ...
    if ((rect.right - rect.left) != ResolutionWidth || (rect.bottom - rect.top) != ResolutionHeight) {
      ...
      ::SetWindowPos(_Hwnd, NULL, 0, 0, rect.right - rect.left, rect.bottom - rect.top, SWP_NOZORDER | SWP_NOMOVE);
    }
#else
    ...
    ::SetWindowPos(_Hwnd, HWND_TOP, 0, 0, (rect.right - rect.left), (rect.bottom - rect.top),
                   SWP_SHOWWINDOW | SWP_NOCOPYBITS);
#endif
  }
#endif
...
bool DX8Wrapper::Toggle_Windowed(void) {
  ...
  if (IsInitted) {
    ...
    for (int res = 0; (res < resolutions.Count()) && (curr_res == -1); res++) {
      ...
      if ((resolutions[res].Width == ResolutionWidth) && (resolutions[res].Height == ResolutionHeight) &&
    ...
    if (curr_res == -1) {
      ...
      return Set_Device_Resolution(resolutions[0].Width, resolutions[0].Height, resolutions[0].BitDepth, !IsWindowed,
                                   true);
    } else {
...
void DX8Wrapper::Set_Swap_Interval(int swap) {
  ...
  case 1:
    ...
    break;
  case 2:
    _PresentParameters.FullScreen_PresentationInterval = D3DPRESENT_INTERVAL_TWO;
    break;
  case 3:
    _PresentParameters.FullScreen_PresentationInterval = D3DPRESENT_INTERVAL_THREE;
    break;
  default:
    _PresentParameters.FullScreen_PresentationInterval = D3DPRESENT_INTERVAL_ONE;
    break;
  ...
  case 0:
    ...
    break;
  case 1:
    _PresentParameters.PresentationInterval = D3DPRESENT_INTERVAL_ONE;
    break;
  case 2:
    _PresentParameters.PresentationInterval = D3DPRESENT_INTERVAL_TWO;
    break;
  case 3:
    _PresentParameters.PresentationInterval = D3DPRESENT_INTERVAL_THREE;
    break;
  default:
    _PresentParameters.PresentationInterval = D3DPRESENT_INTERVAL_ONE;
    break;
...
int DX8Wrapper::Get_Render_Device_Count(void) { return _RenderDeviceNameTable.Count(); }
...
bool DX8Wrapper::Registry_Save_Render_Device(const char *sub_key) {
  ...
  return Registry_Save_Render_Device(sub_key, CurRenderDevice, ResolutionWidth, ResolutionHeight, BitDepth, IsWindowed,
                                     TextureBitDepth);
}

bool DX8Wrapper::Registry_Save_Render_Device(const char *sub_key, int device, int width, int height, int depth,
  ...
  registry->Set_String(VALUE_NAME_RENDER_DEVICE_NAME, _RenderDeviceShortNameTable[device]);
...
bool DX8Wrapper::Registry_Load_Render_Device(const char *sub_key, bool resize_window) {
  ...
  if (Registry_Load_Render_Device(sub_key, name, sizeof(name), width, height, depth, windowed, TextureBitDepth) &&
      (*name != 0)) {
...
bool DX8Wrapper::Registry_Load_Render_Device(const char *sub_key, char *device, int device_len, int &width, int &height,
...
bool DX8Wrapper::Find_Color_And_Z_Mode(int resx, int resy, int bitdepth, DX_D3DFORMAT *set_colorbuffer,
                                       DX_D3DFORMAT *set_zmode) {
  static DX_D3DFORMAT _formats16[] = {D3DFMT_R5G6B5, D3DFMT_X1R5G5B5, D3DFMT_A1R5G5B5};

  static DX_D3DFORMAT _formats32[] = {
      D3DFMT_A8R8G8B8,
      D3DFMT_X8R8G8B8,
      D3DFMT_R8G8B8,
  };
  ...
  */
  DX_D3DFORMAT *format_table = NULL;
  int format_count = 0;

  if (BitDepth == 16) {
    format_table = _formats16;
    format_count = sizeof(_formats16) / sizeof(DX_D3DFORMAT);
  } else {
    format_table = _formats32;
    format_count = sizeof(_formats32) / sizeof(DX_D3DFORMAT);
  }
...
// refresh rate
bool DX8Wrapper::Find_Color_Mode(DX_D3DFORMAT colorbuffer, int resx, int resy, UINT *mode) {
  UINT i, j, modemax;
  UINT rx, ry;
...
#else
  D3DInterface->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &dmode);
  modemax = D3DInterface->GetAdapterModeCount(D3DADAPTER_DEFAULT, dmode.Format);
#endif
  i = 0;
...
#else
    D3DInterface->EnumAdapterModes(D3DADAPTER_DEFAULT, dmode.Format, i, &dmode);
#endif
...
#else
    D3DInterface->EnumAdapterModes(D3DADAPTER_DEFAULT, dmode.Format, j, &dmode);
#endif
    if (dmode.Width == rx && dmode.Height == ry && dmode.Format == colorbuffer)
      stillok = true;
    else
...
bool DX8Wrapper::Find_Z_Mode(DX_D3DFORMAT colorbuffer, DX_D3DFORMAT backbuffer, DX_D3DFORMAT *zmode) {
...
bool DX8Wrapper::Test_Z_Mode(DX_D3DFORMAT colorbuffer, DX_D3DFORMAT backbuffer, DX_D3DFORMAT zmode) {
  // See if we have this mode first
  if (FAILED(D3DInterface->CheckDeviceFormat(D3DADAPTER_DEFAULT, WW3D_DEVTYPE, colorbuffer, D3DUSAGE_DEPTHSTENCIL,
  ...
  if (FAILED(D3DInterface->CheckDepthStencilMatch(D3DADAPTER_DEFAULT, WW3D_DEVTYPE, colorbuffer, backbuffer, zmode))) {
    WWDEBUG_SAY(
...
#endif
      D3DLOCKED_RECT rect;
      HRESULT res = bb->LockRect(&rect, NULL, D3DLOCK_READONLY);
      if (res == D3D_OK) {
        bb->UnlockRect();
        bb->Release();
      }
...
void DX8Wrapper::Set_Viewport(CONST DX_D3DVIEWPORT *pViewport) {
  DX8_THREAD_ASSERT();
...
void DX8Wrapper::Draw_Sorting_IB_VB(unsigned primitive_type, unsigned short start_index, unsigned short polygon_count,
...
#if (DIRECT3D_VERSION < 0x0900)
  DX8CALL(SetStreamSource(0, static_cast<DX8VertexBufferClass *>(dyn_vb_access.VertexBuffer)->Get_DX8_Vertex_Buffer(),
                          dyn_vb_access.FVF_Info().Get_FVF_Size()));
  DX8CALL(SetVertexShader(dyn_vb_access.FVF_Info().Get_FVF()));
#else
  DX8CALL(SetStreamSource(0, static_cast<DX8VertexBufferClass *>(dyn_vb_access.VertexBuffer)->Get_DX8_Vertex_Buffer(),
                          0, dyn_vb_access.FVF_Info().Get_FVF_Size()));
  // TODO: WH: this can be no use on DX9...
  ...
  DX8CALL(DrawIndexedPrimitive(D3DPT_TRIANGLELIST,
                               0, // start vertex
                               vertex_count, dyn_ib_access.IndexBufferOffset, polygon_count));
#else
  DX8CALL(DrawIndexedPrimitive(D3DPT_TRIANGLELIST, dyn_vb_access.VertexBufferOffset,
                               0, // start vertex
                               vertex_count, dyn_ib_access.IndexBufferOffset, polygon_count));
#endif
...
void DX8Wrapper::Draw(unsigned primitive_type, unsigned short start_index, unsigned short polygon_count,
                      unsigned short min_vertex_index, unsigned short vertex_count) {
  DX8_THREAD_ASSERT();
  ...
  if (vertex_count < 3) {
    ...
    case BUFFER_TYPE_SORTING:
      vertex_count = render_state.vertex_buffer->Get_Vertex_Count() - render_state.index_base_offset -
  ...
  case BUFFER_TYPE_DYNAMIC_DX8:
    ...
    case BUFFER_TYPE_DYNAMIC_DX8: {
      DX8_RECORD_RENDER(polygon_count, vertex_count, render_state.shader);
#if (DIRECT3D_VERSION < 0x0900)
      DX8CALL(DrawIndexedPrimitive((D3DPRIMITIVETYPE)primitive_type, min_vertex_index, vertex_count,
                                   start_index + render_state.iba_offset, polygon_count));
#else
      DX8CALL(DrawIndexedPrimitive((D3DPRIMITIVETYPE)primitive_type,
                                   render_state.index_base_offset + render_state.vba_offset, min_vertex_index,
                                   vertex_count, start_index + render_state.iba_offset, polygon_count));
...
void DX8Wrapper::Draw_Triangles(unsigned buffer_type, unsigned short start_index, unsigned short polygon_count,
                                unsigned short min_vertex_index, unsigned short vertex_count) {
...
void DX8Wrapper::Draw_Triangles(unsigned short start_index, unsigned short polygon_count,
                                unsigned short min_vertex_index, unsigned short vertex_count) {
  Draw(D3DPT_TRIANGLELIST, start_index, polygon_count, min_vertex_index, vertex_count);
}
...
void DX8Wrapper::Draw_Strip(unsigned short start_index, unsigned short polygon_count, unsigned short min_vertex_index,
...
void DX8Wrapper::Apply_Render_State_Changes() {
  ...
  for (unsigned i = 0; i < MAX_TEXTURE_STAGES; ++i, mask <<= 1) {
    if (render_state_changed & mask) {
      SNAPSHOT_SAY(("DX8 - apply texture %d (%s)\n", i,
                    render_state.Textures[i] ? render_state.Textures[i]->Get_Full_Path().Peek_Buffer() : "NULL"));
...
#if (DIRECT3D_VERSION < 0x0900)
        DX8CALL(
            SetStreamSource(0, static_cast<DX8VertexBufferClass *>(render_state.vertex_buffer)->Get_DX8_Vertex_Buffer(),
                            render_state.vertex_buffer->FVF_Info().Get_FVF_Size()));
#else
        DX8CALL(
            SetStreamSource(0, static_cast<DX8VertexBufferClass *>(render_state.vertex_buffer)->Get_DX8_Vertex_Buffer(),
                            0, render_state.vertex_buffer->FVF_Info().Get_FVF_Size()));
#endif
        DX8_RECORD_VERTEX_BUFFER_CHANGE();
#if (DIRECT3D_VERSION < 0x0900)
  ...
  if (render_state_changed & INDEX_BUFFER_CHANGED) {
    ...
    if (render_state.index_buffer) {
      ...
      case BUFFER_TYPE_DYNAMIC_DX8:
#if (DIRECT3D_VERSION < 0x0900)
        DX8CALL(SetIndices(static_cast<DX8IndexBufferClass *>(render_state.index_buffer)->Get_DX8_Index_Buffer(),
...
DX_IDirect3DTexture *DX8Wrapper::_Create_DX8_Texture(unsigned int width, unsigned int height, WW3DFormat format,
  ...
  if (rendertarget) {
    unsigned ret = D3DXCreateTexture(DX8Wrapper::_Get_D3D_Device8(), width, height, mip_level_count,
    ...
    if (ret == D3DERR_OUTOFVIDEOMEMORY) {
      ...
      ret = D3DXCreateTexture(DX8Wrapper::_Get_D3D_Device8(), width, height, mip_level_count, D3DUSAGE_RENDERTARGET,
  ...
  unsigned ret = D3DXCreateTexture(DX8Wrapper::_Get_D3D_Device8(), width, height, mip_level_count, 0,
                                   WW3DFormat_To_D3DFormat(format), pool, &texture);
  ...
  if (ret == D3DERR_OUTOFVIDEOMEMORY) {
    ...
    ret = D3DXCreateTexture(DX8Wrapper::_Get_D3D_Device8(), width, height, mip_level_count, 0,
                            WW3DFormat_To_D3DFormat(format), pool, &texture);
    if (SUCCEEDED(ret)) {
...
DX_IDirect3DTexture *DX8Wrapper::_Create_DX8_Texture(const char *filename, TextureClass::MipCountType mip_level_count) {
  DX8_THREAD_ASSERT();
  DX8_Assert();
  DX_IDirect3DTexture *texture = NULL;
  ...
  unsigned result = D3DXCreateTextureFromFileExA(_Get_D3D_Device8(), filename, D3DX_DEFAULT, D3DX_DEFAULT,
                                                 mip_level_count, // create_mipmaps ? 0 : 1,
                                                 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_FILTER_BOX, D3DX_FILTER_BOX,
                                                 0, NULL, NULL, &texture);
...
DX_IDirect3DTexture *DX8Wrapper::_Create_DX8_Texture(DX_IDirect3DSurface *surface,
  ...
  DX_IDirect3DSurface *tex_surface = NULL;
...
DX_IDirect3DSurface *DX8Wrapper::_Create_DX8_Surface(unsigned int width, unsigned int height, WW3DFormat format) {
  DX8_THREAD_ASSERT();
  DX8_Assert();
...
#else
  DX8CALL(CreateOffscreenPlainSurface(width, height, WW3DFormat_To_D3DFormat(format), D3DPOOL_DEFAULT, &surface, NULL));
#endif
  return surface;
}

DX_IDirect3DSurface *DX8Wrapper::_Create_DX8_Surface(const char *filename_) {
  DX8_THREAD_ASSERT();
  DX8_Assert();
...
void DX8Wrapper::Set_Light(unsigned index, const LightClass &light) {
  DX_D3DLIGHT dlight;
  Vector3 temp;
  memset(&dlight, 0, sizeof(DX_D3DLIGHT));
...
void DX8Wrapper::Set_Light_Environment(LightEnvironmentClass *light_env) {
  if (light_env) {
    ...
    DX_D3DLIGHT light;
    ::ZeroMemory(&light, sizeof(DX_D3DLIGHT));
...
DX_IDirect3DSurface *DX8Wrapper::_Get_DX8_Front_Buffer() {
  DX8_THREAD_ASSERT();
  D3DDISPLAYMODE mode;
  ...
  DX_IDirect3DSurface *fb = NULL;

#if (DIRECT3D_VERSION < 0x0900)
  DX8CALL(CreateImageSurface(mode.Width, mode.Height, D3DFMT_A8R8G8B8, &fb));
  DX8CALL(GetFrontBuffer(fb));
#else
  DX8CALL(CreateOffscreenPlainSurface(mode.Width, mode.Height, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &fb, NULL));
...
TextureClass *DX8Wrapper::Create_Render_Target(int width, int height, WW3DFormat format) {
  ...
  //
  const DX_D3DCAPS &dx8caps = Get_Current_Caps()->Get_DX8_Caps();
  ...
  //
  TextureClass *tex =
      NEW_REF(TextureClass, (width, height, format, TextureClass::MIP_LEVELS_1, TextureClass::POOL_DEFAULT, true));
...
void DX8Wrapper::Set_Render_Target(TextureClass *texture) {
  ...
  DX_IDirect3DSurface *d3d_surf = texture->Get_D3D_Surface_Level();
...
void DX8Wrapper::Set_Render_Target(DX_IDirect3DSwapChain *swap_chain) {
  DX8_THREAD_ASSERT();
  ...
  //
  DX_LPDIRECT3DSURFACE render_target = NULL;
...
void DX8Wrapper::Set_Render_Target(DX_IDirect3DSurface *render_target, bool use_default_depth_buffer) {
  DX8_THREAD_ASSERT();
  DX8_Assert();
  ...
  if (render_target == NULL || render_target == DefaultRenderTarget) {
    ...
    if (DefaultDepthBuffer == NULL) {
      //		DX_IDirect3DSurface *depth_buffer = NULL;
...
DX_IDirect3DSwapChain *DX8Wrapper::Create_Additional_Swap_Chain(HWND render_window) {
  DX8_Assert();
  ...
  //
  DX_IDirect3DSwapChain *swap_chain = NULL;
...
void DX8Wrapper::Set_Gamma(float gamma, float bright, float contrast, bool calibrate, bool uselimit) {
  ...
  // IML: I'm not really sure what the intent of the 'limit' variable is. It does not produce useful results for my
  // purposes.
  if (uselimit) {
...
const char *DX8Wrapper::Get_DX8_Render_State_Name(DX_D3DRENDERSTATETYPE state) {
  switch (state) {
  case D3DRS_ZENABLE:
...
const char *DX8Wrapper::Get_DX8_Texture_Stage_State_Name(DX_D3DTEXTURESTAGESTATETYPE state) {
  switch (state) {
  case D3DTSS_COLOROP:
...
void DX8Wrapper::Get_DX8_Render_State_Value_Name(StringClass &name, DX_D3DRENDERSTATETYPE state, unsigned value) {
  switch (state) {
  case D3DRS_ZENABLE:
...
void DX8Wrapper::Get_DX8_Texture_Stage_State_Value_Name(StringClass &name, DX_D3DTEXTURESTAGESTATETYPE state,
                                                        unsigned value) {
  switch (state) {
  case D3DTSS_COLOROP:
  case D3DTSS_ALPHAOP:
