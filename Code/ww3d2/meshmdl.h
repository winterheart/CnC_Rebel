...
public:
  ...
  unsigned *Get_Color_Array(int array_index, bool create = true) {
    return CurMatDesc->Get_Color_Array(array_index, create);
  }
  ...
  void Set_Single_Texture(TextureClass *tex, int pass = 0, int stage = 0) {
    CurMatDesc->Set_Single_Texture(tex, pass, stage);
  }
  ...
  void Set_Texture(int pidx, TextureClass *tex, int pass = 0, int stage = 0) {
    CurMatDesc->Set_Texture(pidx, tex, pass, stage);
  }
  ...
  TextureClass *Get_Texture(int pidx, int pass = 0, int stage = 0) const {
    return CurMatDesc->Get_Texture(pidx, pass, stage);
  }
  ...
  TextureClass *Peek_Texture(int pidx, int pass = 0, int stage = 0) const {
    return CurMatDesc->Peek_Texture(pidx, pass, stage);
  }
...
public: // Jani: I need to have an access to these for now...
  ...
  ShaderClass *Get_Shader_Array(int pass, bool create = true) { return CurMatDesc->Get_Shader_Array(pass, create); }
...
protected:
  ...
  void get_deformed_screenspace_vertices(Vector4 *dst_vert, const RenderInfoClass &rinfo, const Matrix3D &mesh_tm,
