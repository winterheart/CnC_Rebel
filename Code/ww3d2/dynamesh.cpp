...
DynamicMeshModel::DynamicMeshModel(unsigned int max_polys, unsigned int max_verts)
    : MeshGeometryClass(), DynamicMeshPNum(0), DynamicMeshVNum(0), MatDesc(NULL), MatInfo(NULL) {
...
DynamicMeshModel::DynamicMeshModel(const DynamicMeshModel &src)
    : MeshGeometryClass(src), DynamicMeshPNum(src.DynamicMeshPNum), DynamicMeshVNum(src.DynamicMeshVNum), MatDesc(NULL),
      MatInfo(NULL) {
...
void DynamicMeshModel::Render(RenderInfoClass &rinfo) {
  ...
  unsigned buffer_type = (Get_Flag(MeshGeometryClass::SORT) && WW3D::Is_Sorting_Enabled()) ? BUFFER_TYPE_DYNAMIC_SORTING
    ...
    */
    ...
    while (!done) {
      ...
      if (done) {
        ...
        material_changed =
            material_array && material_array[tris[cur_tri_idx].I] != material_array[tris[next_tri_idx].I];
      ...
      if (done || texture_changed || material_changed || shader_changed) {
        if (buffer_type == BUFFER_TYPE_DYNAMIC_SORTING) {
          SortingRendererClass::Insert_Triangles(sphere, (start_tri_idx * 3), (1 + cur_tri_idx - start_tri_idx),
                                                 min_vert_idx, 1 + max_vert_idx - min_vert_idx);
        } else {
          DX8Wrapper::Draw_Triangles((start_tri_idx * 3), (1 + cur_tri_idx - start_tri_idx), min_vert_idx,
...
DynamicMeshClass::DynamicMeshClass(int max_poly, int max_vert)
    : Model(NULL), PolyCount(0), VertCount(0), TriVertexCount(0), FanVertex(0), TriMode(TRI_MODE_STRIPS),
...
DynamicMeshClass::DynamicMeshClass(const DynamicMeshClass &src)
    : RenderObjClass(src), Model(NULL), PolyCount(src.PolyCount), VertCount(src.VertCount),
      TriVertexCount(src.TriVertexCount), FanVertex(src.FanVertex), TriMode(src.TriMode), SortLevel(src.SortLevel) {
  int pass = MAX_PASSES;
  while (pass--) {
...
RenderObjClass *DynamicMeshClass::Clone(void) const { return NEW_REF(DynamicMeshClass, (*this)); }
...
int DynamicMeshClass::Set_Vertex_Material(VertexMaterialClass *material, bool dont_search, int pass) {
  ...
  if (Peek_Material_Info()->Vertex_Material_Count() && (VertexMaterialIdx[pass] != -1) &&
      Peek_Material_Info()->Peek_Vertex_Material(VertexMaterialIdx[pass]) == material) {
    return VertexMaterialIdx[pass];
  }
  ...
  if ((!MultiVertexMaterial[pass]) && Peek_Material_Info()->Vertex_Material_Count() &&
      (VertexMaterialIdx[pass] != -1) &&
      Peek_Material_Info()->Peek_Vertex_Material(VertexMaterialIdx[pass]) != material) {
...
int DynamicMeshClass::Set_Texture(TextureClass *texture, bool dont_search, int pass) {
  ...
  if ((!MultiTexture[pass]) && Peek_Material_Info()->Texture_Count() && (TextureIdx[pass] != -1) &&
      Peek_Material_Info()->Peek_Texture(TextureIdx[pass]) != texture) {
