...
cPlayer *cPlayerManager::Find_Player(int id) {
  ...
  for (objnode = PlayerList.Head(); objnode; objnode = objnode->Next()) {
    ...
    if (p_player->Get_Is_Active().Is_True() && p_player->Get_Id() == id) {
...
cPlayer *cPlayerManager::Find_Player(const WideStringClass &name) {
  ...
  for (objnode = PlayerList.Head(); objnode; objnode = objnode->Next()) {
    ...
    if (p_player->Get_Is_Active().Is_True() && !name.Compare_No_Case(p_player->Get_Name())) {
...
cPlayer *cPlayerManager::Find_Inactive_Player(const WideStringClass &name) {
  ...
  for (SLNode<cPlayer> *objnode = PlayerList.Head(); objnode != NULL; objnode = objnode->Next()) {
    cPlayer *p_player = objnode->Data();
    WWASSERT(p_player != NULL);

    if (p_player->Get_Is_Active().Is_False() && !p_player->Get_Name().Compare_No_Case(name)) {
...
cPlayer *cPlayerManager::Find_Team_Player(int team_number) {
  ...
  for (objnode = PlayerList.Head(); objnode; objnode = objnode->Next()) {
    cPlayer *p_player = objnode->Data();
    WWASSERT(p_player != NULL);
    if (p_player->Get_Is_Active().Is_True() && p_player->Get_Player_Type() == team_number) {
      return p_player; // found it
    }
  }
...
cPlayer *cPlayerManager::Find_Random_Team_Player(int team_number) {
  ...
  for (objnode = PlayerList.Head(); objnode; objnode = objnode->Next()) {
    cPlayer *p_player = objnode->Data();
    WWASSERT(p_player != NULL);
    if (p_player->Get_Is_Active().Is_True() && p_player->Get_Player_Type() == team_number && count++ == chosen_player) {
      return p_player; // found it
    }
  }
...
cPlayer *cPlayerManager::Find_Team_Mate(cPlayer *p_player1) {
  ...
  for (objnode = PlayerList.Head(); objnode; objnode = objnode->Next()) {
    ...
    if (p_player2->Get_Is_Active().Is_True() && p_player2->Get_Id() != p_player1->Get_Id() &&
...
//------------------------------------------------------------------------------------
bool cPlayerManager::Is_Player_Present(int id) { return (Find_Player(id) != NULL); }

//------------------------------------------------------------------------------------
bool cPlayerManager::Is_Player_Present(WideStringClass &name) { return (Find_Player(name) != NULL); }
...
void cPlayerManager::Compute_Ladder_Points(int winning_team) {
  ...
  for (i = 0; i < MAX_PLAYERS; i++) {
    ...
    if (Player_Array[i] == NULL || Player_Array[i]->Get_Total_Time() < min_qualifying_time_ms) {
      continue;
    }
...
void cPlayerManager::Increment_Player_Times(void) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (Player_Array[i] != NULL && Player_Array[i]->Is_Active()) {
...
WideStringClass cPlayerManager::Determine_Mvp_Name(void) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (Player_Array[i] != NULL) {
      ...
      if (Player_Array[i]->Is_Active() && Player_Array[i]->Get_Total_Time() > min_qualifying_time_ms) {
...
int cPlayerManager::Tally_Team_Size(int type) {
  ...
  for (objnode = PlayerList.Head(); objnode != NULL; objnode = objnode->Next()) {
    ...
    WWASSERT(p_player != NULL);
    if (p_player->Get_Is_Active().Is_True() && p_player->Get_Player_Type() == type) {
      tally++;
    }
  }
...
bool cPlayerManager::Is_Kill_Treasonous(cPlayer *p_killer, cPlayer *p_victim) {
  return (
      ...
      // The_Game()->Is_Team_Game() &&
      p_killer != NULL && p_victim != NULL && p_killer != p_victim &&
...
void cPlayerManager::List_Print(WideStringClass &text, Vector3 color) {
  ...
  int c = ((int)(color[0] * 255) & 0xFF) << 16 | ((int)(color[1] * 255) & 0xFF) << 8 |
          ((int)(color[2] * 255) & 0xFF) << 0 | 0xFF000000;
...
void cPlayerManager::Render_Player_List(void) {
  ...
  WWASSERT(PTheGameData != NULL);
  if (GameModeManager::Find("Combat") == NULL || !GameModeManager::Find("Combat")->Is_Active() ||
  ...
  bool is_verbose =
      The_Game()->IsIntermission.Get() || (MultiHUDClass::Get_Playerlist_Format() == PLAYERLIST_FORMAT_FULL);
  ...
  if (The_Game()->IsIntermission.Is_True()) {
    ...
    int combined_height = cTeamManager::Compute_Team_List_Height() + cPlayerManager::Compute_Full_Player_List_Height() +
  ...
  for (int j = 0; j < renderer_displayed_player_count; j++) {
    ...
    if ((MultiHUDClass::Get_Playerlist_Format() != PLAYERLIST_FORMAT_TINY) && (renderer_star_index == j)) {
