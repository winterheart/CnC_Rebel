...
DlgMPTeamSelect::DlgMPTeamSelect(void)
    : MenuDialogClass(IDD_MP_TEAM_SELECT), mWOLGame(true), mCanChoose(true), mTimeRemaining(0.0f) {
...
DlgMPTeamSelect::~DlgMPTeamSelect() { WWDEBUG_SAY(("DlgMPTeamSelect destroyed\n")); }
...
void DlgMPTeamSelect::HandleNotification(GameOptionsMessage &message) {
  ...
  if (hostName.Compare_No_Case(sender) == 0) {
    static struct {
      const char *Token;
      GameOptionsDispatchFunc Dispatch;
    } _dispatch[] = {
        {"GINFO:", ProcessWOLGameInfo}, {"TINFO:", ProcessWOLTeamInfo}, {"PINFO:", ProcessWOLPlayerInfo}, {NULL, NULL}};
