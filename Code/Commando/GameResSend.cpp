...
static void AddPlayerStats(GameResPacket &stats, cPlayer *player, WOL::Locale locale, int winnerID, bool teamGame);
...
void SendGameResults(unsigned long gameID, cGameData *theGame, SList<cPlayer> *playerList) {
  ...
  char buildDate[20];
  sprintf(buildDate, "%02d/%02d/%04d %02d:%02d:%02d", time.wMonth, time.wDay, time.wYear, time.wHour, time.wMinute,
          time.wSecond);
  ...
  char startTime[20];
  sprintf(startTime, "%02d/%02d/%04d %02d:%02d:%02d", gameTime->wMonth, gameTime->wDay, gameTime->wYear,
          gameTime->wHour, gameTime->wMinute, gameTime->wSecond);
...
#if (0)
#ifdef _DEBUG
  HANDLE file = CreateFile("GameRes.dat", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
void AddPlayerStats(GameResPacket &stats, cPlayer *player, WOL::Locale locale, int winnerID, bool isTeamed) {
  if (player->Is_Human()) {
    // Players WOL name
    const WideStringClass &playerName = player->Get_Name();
    char name[10];
