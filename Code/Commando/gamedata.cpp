...
cGameData::cGameData(void)
    : DoExeVersionsMatch(true), DoStringVersionsMatch(true), DoMapsLoop(true), IsMapCycleOver(false), MapCycleIndex(0) {
...
//
bool cGameData::Set_Generic_Num(int generic_num, int lower_bound, int upper_bound, int &set_num) {
...
//-----------------------------------------------------------------------------
bool cGameData::Set_Max_Players(int max) { return Set_Generic_Num(max, 0, MAX_PLAYERS, MaxPlayers); }
...
//-----------------------------------------------------------------------------
void cGameData::Set_Radar_Mode(RadarModeEnum mode) { RadarMode = mode; }
...
//-----------------------------------------------------------------------------
bool cGameData::Set_Current_Players(int current) { return Set_Generic_Num(current, 0, MAX_PLAYERS, CurrentPlayers); }

//-----------------------------------------------------------------------------
void cGameData::Set_Owner(const WideStringClass &owner) { Owner = owner; }

//-----------------------------------------------------------------------------
void cGameData::Set_Ip_Address(ULONG ip_address) { IpAddress = ip_address; }
...
//-----------------------------------------------------------------------------
bool cGameData::Is_Limited(void) const { return Is_Time_Limit(); }
...
bool cGameData::Is_Valid_Settings(WideStringClass &outMsg, bool check_as_server) {
  ...
  if (Get_Max_Players() == 0 && ServerSettingsClass::Get_Game_Mode() != ServerSettingsClass::MODE_WOL) {
#else
  ...
  if (check_as_server || ConsoleBox.Is_Exclusive()) {
    ...
    */
    ...
    /*
    ** Check the game title for length. The buffers are pretty much all 256 bytes but 100 seems a more reasonable
    *figure.
    */
...
void cGameData::Soldier_Added(SoldierGameObj *p_soldier) {
  ...
  if (IsFreeWeapons.Is_True() && p_soldier->Get_Player_Type() != PLAYERTYPE_NEUTRAL) {
...
//-----------------------------------------------------------------------------
int cGameData::Get_Duration_Seconds(void) { return (int)(TimeLimitMinutes * 60 - TimeRemainingSeconds); }
...
//-----------------------------------------------------------------------------
void cGameData::Reset_Time_Remaining_Seconds(void) { TimeRemainingSeconds = 60 * TimeLimitMinutes; }

//-----------------------------------------------------------------------------
float cGameData::Get_Time_Remaining_Seconds(void) { return TimeRemainingSeconds; }
...
int cGameData::Choose_Player_Type(cPlayer *player, int team_choice, bool is_grunt) {
  if (IsClanGame.Is_True() && !is_grunt) {
    ...
    if (clanMate) {
      ...
      WWDEBUG_SAY(
          ("CLANS: Player '%S' assigned to team with clanmates (%d)\n", (const WCHAR *)player->Get_Name(), team));
...
cGameData *cGameData::Create_Game_Of_Type(GameTypeEnum game_type) {
  ...
  switch (game_type) {
          ...
          case GAME_TYPE_CNC:					p_game_data = new cGameDataCnc;
  break; default:									DIE;
  break;
  }
...
void cGameData::Get_Time_Limit_Text(WideStringClass &text) {
  if (IsIntermission.Is_True()) {
    ...
    text.Format(L"%s: %d", TRANSLATION(IDS_MP_NEXTGAME_COUNTDOWN), cMathUtil::Round(Get_Intermission_Time_Remaining()));
...
void cGameData::Think(void) {
  ...
  if (cNetwork::I_Am_Server()) {
    ...
    //
    int max = (int)WWMath::Max(cPlayerManager::Count(), cNetwork::PServerConnection->Get_Num_RHosts());
...
//-----------------------------------------------------------------------------
void cGameData::Set_Map_Cycle(int map, const StringClass &map_name) {
  WWASSERT(map >= 0 && map < MAX_MAPS);
...
//-----------------------------------------------------------------------------
void cGameData::Clear_Map_Cycle(void) {
  for (int i = 0; i < MAX_MAPS; i++) {
    MapCycle[i].Format("");
  }
}
...
//-----------------------------------------------------------------------------
void cGameData::Rotate_Map(void) {
...
//-----------------------------------------------------------------------------
void cGameData::Set_Win_Text(WideStringClass &text) { WinText = text; }
...
//------------------------------------------------------------------------------------
void cGameData::Set_Mvp_Name(const WideStringClass name) { MvpName = name; }
...
//------------------------------------------------------------------------------------
void cGameData::Set_Win_Type(WinTypeEnum type) { WinType = type; }

//------------------------------------------------------------------------------------
void cGameData::Set_Game_Duration_S(DWORD seconds) { GameDurationS = seconds; }
...
// cGameData *						The_Game(void)
// {WWASSERT(PTheGameData != NULL); return PTheGameData;}
cGameData *The_Game(void) { return PTheGameData; }

cGameDataSinglePlayer *The_Single_Player_Game(void) {
  WWASSERT(The_Game()->As_Single_Player() != NULL);
  return The_Game()->As_Single_Player();
}
cGameDataSkirmish *The_Skirmish_Game(void) {
  WWASSERT(The_Game()->As_Skirmish() != NULL);
  return The_Game()->As_Skirmish();
}
cGameDataCnc *The_Cnc_Game(void) {
  WWASSERT(The_Game()->As_Cnc() != NULL);
  return The_Game()->As_Cnc();
}
...
// cGameDataDeathMatch *		The_Deathmatch_Game(void)
// {WWASSERT(The_Game()->As_Deathmatch() != NULL);			return The_Game()->As_Deathmatch();}
// cGameDataTeamDeathMatch *	The_Team_Deathmatch_Game(void)	{WWASSERT(The_Game()->As_Team_Deathmatch() != NULL);
// return The_Game()->As_Team_Deathmatch();}
