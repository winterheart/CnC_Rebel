...
if (is_verbose) {
  ...
  */
  ...
  if (cGameSpyAdmin::Is_Gamespy_Game()) {
    substring.Format(L"%-36s", The_Game()->Get_Team_Word().Peek_Buffer());
  } else {
    substring.Format(L"%-11s", The_Game()->Get_Team_Word().Peek_Buffer());
  }
  string += substring;
...
void cTeamManager::Render_Team_List(void) {
  ...
  if (The_Game()->IsIntermission.Is_True()) {
    ...
    int combined_height = Compute_Team_List_Height() + cPlayerManager::Compute_Full_Player_List_Height() +
    ...
    float x = Render2DClass::Get_Screen_Resolution().Center().X - PFont->String_Width(text) / 2.0f;
    float y = YPos;
