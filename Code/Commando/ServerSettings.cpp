  ...
  */
  if (IsActive) {
...
#ifdef ENABLE_GAMESPY
    ini.Get_String(MasterServerSection, "HeartBeatServers", GameSpyQnR.Get_Default_HeartBeat_List(), heartbeat_list,
                   sizeof(heartbeat_list) - 1);
    ...
    */
    ...
    /*
    ** We only need to validate this for the FDS. The regular game can allow the login name to be specified in the
    *registry.
    */
...
#ifdef FREEDEDICATEDSERVER
    /*
    ** We only need to validate the serial number if we are the FDS. For the regular game, the master serial will be
    *stored
    ** in the registry.
    */
    ...
    */
    ...
    if (!gsqport)
      gsqport = cUserOptions::GameSpyQueryPort.Get();
    ...
    */
    ...
    if (!gsgport)
      gsgport = cUserOptions::GameSpyGamePort.Get();
    ...
    */
    for (int i = 0; i < MAX_SLAVES; i++) {
      ...
      */
      if (apply) {
        SlaveMaster.Add_Slave(enabled, slave_nick, slave_serial, (unsigned short)slave_port, slave_settings, slave_bw,
                              slave_pass);
      }
    }
    if (apply) {
...
void ServerSettingsClass::Write_Server_List(const WWOnline::IRCServerList &server_list) {
  ...
  if (server_list.size()) {
    ...
    **
    ** We will do this by reading in the file, looking for a tag that tells us where to write the server list, then
    *writing
