...
void CombatGameModeClass::Combat_Keyboard(void) {
  ...
  if (!IS_MISSION && cNetwork::I_Am_Client() && COMBAT_STAR != NULL) {
    ...
    for (int radioCmdIndex = INPUT_FUNCTION_RADIO_CMD_01; radioCmdIndex <= INPUT_FUNCTION_RADIO_CMD_30;
         ++radioCmdIndex) {
  ...
  if ((The_Game() != NULL && (Input::Get_State(INPUT_FUNCTION_BEGIN_PUBLIC_MESSAGE) ||
...
#define LOADTIME_NETWORK_UPDATE                                                                                        \
  if (!IS_SOLOPLAY)                                                                                                    \
  cNetwork::Update()
...
void CombatGameModeClass::Load_Level(void) {
  ...
  //
  // Stop the network layer from processing packets. This is needed in case a packet import or export accesses the
  // datasafe from the main thread while the loader thread is loading stuff into the datasafe. This won't shut off
  // packet acks so no-one will be disconnected as a result of this.
  //
...
void CombatGameModeClass::Post_Load_Id_Uniqueness_Check(void) {
  ...
  for (objnode = GameObjManager::Get_Game_Obj_List()->Head(); objnode; objnode = objnode->Next()) {
    ...
    if (p_phys_obj != NULL) {
      ...
      if (list_ids) {
        Debug_Say(("  Object %d, model %s\n", p_phys_obj->Get_ID(), p_phys_obj->Peek_Model()->Get_Name()));
      }
      ...
      for (objnode2 = GameObjManager::Get_Game_Obj_List()->Head(); objnode2; objnode2 = objnode2->Next()) {
        ...
        if (p_phys_obj_2 != NULL && p_phys_obj_2->Get_ID() == p_phys_obj->Get_ID()) {
  ...
  for (iter_1.First(); !iter_1.Is_Done(); iter_1.Next()) {
    ...
    if (list_ids) {
      // Debug_Say(("  Object %d\n", object_id_1));
      Debug_Say(("  Object %d, model %s\n", p_obj_1->Get_ID(), p_obj_1->Peek_Model()->Get_Name()));
    }
...
void CombatGameModeClass::Post_Load_Dynamic_Object_Filtering(void) {
  ...
  for (objnode = GameObjManager::Get_Smart_Game_Obj_List()->Head(); objnode; objnode = objnode->Next()) {
    ...
    if (p_smart_obj->As_SoldierGameObj() != NULL) {
      ...
      if (p_smart_obj->Has_Player() && (p_smart_obj->Get_Control_Owner() > 1 || (!cNetwork::I_Am_Client()))) {
        Debug_Say(("* Removing loaded soldier belonging to another player.\n"));
        p_smart_obj->Set_Delete_Pending();
      }

      if (!p_smart_obj->Is_Delete_Pending() && !The_Game()->Is_Valid_Player_Type(p_smart_obj->Get_Player_Type())) {
        Debug_Say(("* Removing loaded soldier due to invalid playertype for this game.\n"));
        p_smart_obj->Set_Delete_Pending();
      }

      if (!p_smart_obj->Is_Delete_Pending() && !p_smart_obj->Has_Player() && !IS_SOLOPLAY) {
...
void CombatGameModeClass::Compute_World_Size(void) {
  ...
  //
  cEncoderList::Set_Precision(BITPACK_WORLD_POSITION_X, min.X - margin, max.X + margin, 0.2);
  cEncoderList::Set_Precision(BITPACK_WORLD_POSITION_Y, min.Y - margin, max.Y + margin, 0.2);
  cEncoderList::Set_Precision(BITPACK_WORLD_POSITION_Z, min.Z - margin, max.Z + margin, 0.2);
  cEncoderList::Set_Precision(BITPACK_BUILDING_RADIUS, 0.0, 50.0, 0.1);
}
...
void CombatGameModeClass::Core_Restart(void) {
  ...
  if (IS_MISSION) {
    ...
    // convert .LSD to .MIX
    if (new_name.Get_Length() > 4 && ::stricmp(&new_name[new_name.Get_Length() - 4], ".LSD") == 0) {
...
//-----------------------------------------------------------------------------
void CombatGameModeClass::Toggle_Multi_Hud(void) { MultiHUDClass::Toggle(); }
...
void CombatGameMiscHandlerClass::Star_Killed(void) { cGod::Star_Killed(); }
