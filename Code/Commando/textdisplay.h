...
public:
  ...
  float Update_Time(float seconds) {
    Time += seconds;
    return Time;
  }
...
public:
  ...
  static void Set_Stat(const char *title, const char *text, unsigned long color = 0xffffffff,
                       const Vector2 &location = Vector2(0, -240));
};
