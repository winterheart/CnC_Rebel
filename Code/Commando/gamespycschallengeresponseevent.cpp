...
//-----------------------------------------------------------------------------
cGameSpyCsChallengeResponseEvent::cGameSpyCsChallengeResponseEvent(void) {
  ClientId = -1;
...
//-----------------------------------------------------------------------------
void cGameSpyCsChallengeResponseEvent::Init(StringClass &challenge_string) {
...
//-----------------------------------------------------------------------------
void cGameSpyCsChallengeResponseEvent::Act(void) {
  ...
  if (p_player != NULL) {
    ...
    if (ChallengeResponseString.Get_Length() > 31) {
      ...
      if (tmpstr.Get_Length() > 32)
        tmpstr.Erase(32, tmpstr.Get_Length() - 32);
...
//-----------------------------------------------------------------------------
void cGameSpyCsChallengeResponseEvent::Export_Creation(BitStreamClass &packet) {
...
//-----------------------------------------------------------------------------
void cGameSpyCsChallengeResponseEvent::Import_Creation(BitStreamClass &packet) {
  ...
  packet.Get(ClientId);
  packet.Get_Terminated_String(ChallengeResponseString.Get_Buffer(MAX_CHALLENGE_RESPONSE_STRING_LENGTH),
                               MAX_CHALLENGE_RESPONSE_STRING_LENGTH);
