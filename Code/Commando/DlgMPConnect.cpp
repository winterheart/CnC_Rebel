...
DlgMPConnect::DlgMPConnect(int teamChoice, unsigned long clanID)
    : PopupDialogClass(IDD_MULTIPLAY_CONNECTING), mTeamChoice(teamChoice), mClanID(clanID), mTheGame(NULL),
      mFailed(false) {
...
DlgMPConnect::~DlgMPConnect() { WWDEBUG_SAY(("DlgMPConnect: Destructing\n")); }
...
void DlgMPConnect::Connected(cGameData *theGame) { mTheGame = theGame; }
...
void DlgMPConnect::Failed_To_Connect(void) { mFailed = true; }
