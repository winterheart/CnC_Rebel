...
// int
// cNetwork::BandwidthScaler					= 28800;
...
void cNetwork::Init_Client(unsigned short my_port) {
  ...
  if (IS_SOLOPLAY || (GameModeManager::Find("LAN")->Is_Active() && !cGameSpyAdmin::Is_Gamespy_Game())) {
  ...
  WWASSERT(PTheGameData != NULL);
  PClientConnection->Init_As_Client(The_Game()->Get_Ip_Address(), The_Game()->Get_Port(), my_port);
...
void cNetwork::Init_Server(void) {
  ...
  */
    ...
    */
    if (The_Game()->IsDedicated.Get() && !SlaveMaster.Am_I_Slave()) {
      if (cUserOptions::Get_Bandwidth_Type() == BANDWIDTH_AUTO) {
        if (ServerSettingsClass::Get_Master_Bandwidth() == 0 ||
            ServerSettingsClass::Get_Master_Bandwidth() == 0xffffffff) {
  ...
  WWASSERT(PTheGameData != NULL);
  PServerConnection->Init_As_Server(The_Game()->Get_Port(), The_Game()->Get_Max_Players(),
                                    The_Game()->IsDedicated.Get(), ntohl(The_Game()->Get_Ip_Address()));
...
void cNetwork::Update(void) {
  WWPROFILENAMED(cnetwork_update, "cNetwork::Update");
  ...
  if (I_Am_Server()) {
    ...
    WWPROFILENAMED(server_read, "Server Read");
...
void cNetwork::Server_Send_Packet(cPacket &packet, int mode, int recipient) {
  ...
  if (recipient == ALL) {
    ...
    for (objnode = cPlayerManager::Get_Player_Object_List()->Head(); objnode; objnode = objnode->Next()) {

      cPlayer *p_player = objnode->Data();
      WWASSERT(p_player != NULL);

      // if (p_player->Is_Human()) {
      if (p_player->Get_Is_Active().Is_True() && p_player->Is_Human() && p_player->Get_Is_In_Game().Is_True()) {

        int client_id = p_player->Get_Id();

        PServerConnection->Send_Packet_To_Individual(packet, client_id, mode);
...
LPCSTR cNetwork::Get_Client_String(int recipient) {
  ...
  if (recipient > 0) {
    ...
    if (recipient == ALL) {
      sprintf(ClientString, "all %d clients (%s)", PServerConnection->Get_Num_RHosts(),
...
void cNetwork::Server_Broken_Connection_Handler(int broken_rhost_id) {
  ...
  widestring.Format(L"%s %d\n", TRANSLATION(IDS_MP_CONNECTION_TO_CLIENT_BROKEN), broken_rhost_id);
...
REFUSAL_CODE cNetwork::Application_Acceptance_Handler(cPacket &packet) {
  ...
  if (!cGameSpyAdmin::Is_Gamespy_Game() && cPlayerManager::Find_Player(player_name)) {
...
void cNetwork::Connection_Handler(int new_rhost_id) {
  ...
  if (!cNetwork::I_Am_Client() || new_rhost_id != cNetwork::Get_My_Id()) {
    for (SLNode<cTeam> *team_node = cTeamManager::Get_Team_Object_List()->Head(); team_node;
