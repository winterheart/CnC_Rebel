  ...
  if (Get_Text_Display()) {
    ...
    if (StatisticsDisplayManager::Is_Current_Display("fps")) {
      ...
      working_string.Format("\nMalloc count: %d\n"
                            ...
                            "FF Count: %d\n",
                            WW3D::Get_Last_Frame_Memory_Allocation_Count(), WW3D::Get_Last_Frame_Memory_Free_Count(),
                            ...
                            actualuse / (1000 * 1000), (actualuse / 1000) % 1000, actualuse % 1000,
                            actualuse / (1024 * 1024), count);
      message += working_string;
      ...
      if (Debug_Statistics::Get_Record_Texture_Mode() != Debug_Statistics::RECORD_TEXTURE_NONE) {
        ...
        working_string.Format("\n"
                              ...
                              "texture changes: %d\n",
                              Debug_Statistics::Get_Record_Texture_Count(), red_size >> 20,
                              (10 * (red_size >> 10) >> 10) % 10, Debug_Statistics::Get_Record_Texture_Change_Count());
        message += working_string;
        ...
        working_string.Format("\n"
                              ...
                              Debug_Statistics::Get_Record_Lightmap_Texture_Count(), lightmap_size >> 20,
        ...
        working_string.Format("\n"
                              ...
                              Debug_Statistics::Get_Record_Procedural_Texture_Count(), procedural_size >> 20,
        ...
        working_string.Format("\n"
                              ...
                              TextureClass::_Get_Total_Texture_Count(), red_size >> 20,
                              (10 * (red_size >> 10) >> 10) % 10, TextureClass::_Get_Total_Lightmap_Texture_Count(),
                              lightmap_size >> 20, (10 * (lightmap_size >> 10) >> 10) % 10,
                              TextureClass::_Get_Total_Procedural_Texture_Count(), procedural_size >> 20,
                              (10 * (procedural_size >> 10) >> 10) % 10);
        message += working_string;

        // Thumbnail info
        red_size = TextureClass::_Get_Total_Locked_Surface_Size();
        working_string.Format("total thumbnails: %5d\n"
                              "total size of thumbnails: %d.%dMb\n",
                              TextureClass::_Get_Total_Locked_Surface_Count(), red_size >> 20,
    ...
    ****************************************************************************************/
    ...
    if (StatisticsDisplayManager::Is_Current_Display("dx8")) {
      ...
      working_string.Format(
          ...
          "Sorted polys: %d\n"
          "Sorted verts: %d\n",
          DX8Wrapper::Get_Last_Frame_DX8_Calls(), Debug_Statistics::Get_DX8_Polygons(),
          Debug_Statistics::Get_DX8_Vertices(), Debug_Statistics::Get_DX8_Skin_Renders(),
          Debug_Statistics::Get_DX8_Skin_Polygons(), Debug_Statistics::Get_DX8_Skin_Vertices(),
          DX8Wrapper::Get_Last_Frame_Matrix_Changes(), Debug_Statistics::Get_Record_Texture_Count(),
          DX8Wrapper::Get_Last_Frame_Material_Changes(), DX8Wrapper::Get_Last_Frame_Vertex_Buffer_Changes(),
          DX8Wrapper::Get_Last_Frame_Index_Buffer_Changes(), DX8Wrapper::Get_Last_Frame_Light_Changes(),
          Debug_Statistics::Get_Sorting_Polygons(), Debug_Statistics::Get_Sorting_Vertices());
      message += working_string;
      ...
      /*			bool empty=true;
         ...
         renderer=n->render_class_types[a].Get_Renderer_Head(); if (!renderer) { message+=" No registered renderers!\n";
                                              }
                                              else {
                                                      message+="\n";
                                              }
                                              ...
                                              while (renderer) {
                                                      if (renderer->stats_last_frame_add_calls ||
         renderer->stats_last_frame_count) { working_string.Format("%24s Add: %3d Render: %3d Polys: %3d\n",
                                                                      renderer->name,
      ...
      */
      ...
      /*		working_string.Format("Total add renders:
         %d\n",DX8MeshRendererClass::Get_Last_Frame_Render_Stats()); message+=working_string; for (int
         ridx=0;ridx<DX8MeshRendererContainerClass::RENDER_CLASS_MAX;++ridx) { working_string.Format("Class:
    ...
    if (StatisticsDisplayManager::Is_Current_Display("audio")) {
      ...
      for (int sample_index = 0; sample_index < count_2d; sample_index++) {
        ...
        if (sound_obj != NULL) {
          ...
          PhysicsSceneClass::Get_Instance()->Add_Debug_AABox(AABoxClass(curr_pos, Vector3(0.25F, 0.25F, 0.25F)),
                                                             Vector3(1, 0, 0));
    ...
    if (StatisticsDisplayManager::Is_Current_Display("culling")) {
      ...
      if (the_scene != NULL) {
        ...
        if (stats.FrameCount > 0) {
          ...
          message += working_string;
          working_string.Format("Accepted: %d  %10.3f pf\n", stats.CullNodesAccepted,
                                (float)stats.CullNodesAccepted / (float)stats.FrameCount);
          message += working_string;
          working_string.Format("Triv Accepted: %d  %10.3f pf\n", stats.CullNodesTriviallyAccepted,
                                (float)stats.CullNodesTriviallyAccepted / (float)stats.FrameCount);
          message += working_string;
          working_string.Format("Rejected: %d  %10.3f pf\n", stats.CullNodesRejected,
                                (float)stats.CullNodesRejected / (float)stats.FrameCount);
          message += working_string;
        }
      }
...
void ConsoleGameModeClass::Update_Suggestion(char *cmd, bool go_to_next) {
  if (ConsoleInputType == INPUT_FUNCTION_BEGIN_CONSOLE) {
    ...
    if ((strlen(cmd) > 0) && (strchr(cmd, ' ') == NULL)) {
      ...
      bool gotone =
          ConsoleFunctionManager::Get_Command_Suggestion(cmd, cur_suggestion, Suggestion, HelpLine, sizeof(Suggestion));
      if (!gotone) {
        Clear_Suggestion();
      }
    }
...
void ConsoleGameModeClass::Update_Profile(void) {
...
#ifdef ATI_DEMO_HACK
  // HACK
  if (Input::Get_State(INPUT_FUNCTION_PROFILE_ENTER_CHILD0))
    WW3D::Set_NPatches_Level(0);
  if (Input::Get_State(INPUT_FUNCTION_PROFILE_ENTER_CHILD1))
    WW3D::Set_NPatches_Level(1);
  if (Input::Get_State(INPUT_FUNCTION_PROFILE_ENTER_CHILD2))
    WW3D::Set_NPatches_Level(2);
  if (Input::Get_State(INPUT_FUNCTION_PROFILE_ENTER_CHILD3))
    WW3D::Set_NPatches_Level(3);
  if (Input::Get_State(INPUT_FUNCTION_PROFILE_ENTER_CHILD4))
    WW3D::Set_NPatches_Level(4);
  if (Input::Get_State(INPUT_FUNCTION_PROFILE_ENTER_CHILD5))
    WW3D::Set_NPatches_Level(5);
  if (Input::Get_State(INPUT_FUNCTION_PROFILE_ENTER_CHILD6))
    WW3D::Set_NPatches_Level(6);
  if (Input::Get_State(INPUT_FUNCTION_PROFILE_ENTER_CHILD7))
    WW3D::Set_NPatches_Level(7);
  if (Input::Get_State(INPUT_FUNCTION_PROFILE_ENTER_CHILD8))
  ...
  if (ProfileIterator) {
    ...
    */
    ...
    working_string.Format("   Name                      %%parent  %%total  ms/f   ms/call   calls/f\n\n");
    ...
    if ((total_frames > 0) && (total_time > 0.0f)) {
      ...
      */
      if (parent_time > 0.0f) {
        ...
        working_string.Format("   %-25s %-6.2f   %-6.2f  %-6.2f\n", "MISSING TIME", percent_of_parent, percent_of_total,
                              ms_per_frame);
        profile_string += working_string;
      }
    }
...
ProfileLogNodeClass::ProfileLogNodeClass(unsigned count_) : count(count_), succ(NULL) {
  if (!profile_log_head) {
    ...
    */
    ...
    if ((total_frames > 0) && (total_time > 0.0f)) {
      ...
      */
      ...
      for (ProfileIterator->First(); !ProfileIterator->Is_Done(); ProfileIterator->Next()) {
        ...
        */
        ...
        //				if (parent_time != 0.0f) {
        //					percent_of_parent = 100.0f * time / parent_time;
        //					working_string.Format(" %-6.2f
        //%-6.2f",percent_of_parent,percent_of_total); 				} else { 					percent_of_parent = 100.0f; 					working_string.Format("   --
        //%-6.2f",percent_of_total);
        //				}
        //				profile_string += working_string;
        ...
        */
        ...
        //					calls_per_frame = calls / total_frames;
        //					working_string.Format(" %-6.2f    %-4d (%4d)\n",ms_per_call,
        //calls_per_frame, calls); 				} else { 					working_string.Format("   --       -- \n");
        //				}
        //				profile_string += working_string;
      ...
      */
      ...
      //		Vector2 pos = (Render2DClass::Get_Screen_Resolution().Upper_Left() +
      //Render2DClass::Get_Screen_Resolution().Lower_Left()) * 0.5f; 		StatisticsDisplayManager::Set_Stat( "profile",
      //profile_string, 0xffffffff, pos );
    }
  }
}

void ConsoleGameModeClass::Update_Memory_Log(void) {
  if (!StatisticsDisplayManager::Is_Current_Display("memory"))
    return;
