 ...
 *   8/7/2001 7:45PM ST : Created                                                              *
 *=============================================================================================*/
WOLNATInterfaceClass::~WOLNATInterfaceClass(void) { CloseHandle(GameOptionsMutex); }
...
void WOLNATInterfaceClass::Init(void) {
  ...
  */
  ...
  if (reg.Is_Valid()) {
    ...
    */
    FirewallHelper.Set_Firewall_Info((unsigned long)last_behavior, last_source_port_allocation_delta,
                                     (unsigned short)source_port_pool, send_delay, confidence);
  ...
  */
  ...
  if (reg.Is_Valid()) {
    ...
    */
    ...
    FirewallHelper.Get_Firewall_Info(last_behavior, last_source_port_allocation_delta, source_port_pool, send_delay,
                                     confidence);
...
void WOLNATInterfaceClass::HandleNotification(WWOnline::UserEvent &event) {
  ...
  */
  ...
  /*
  ** If someone is leaving the channel, and we are a server they are waiting to negotiate with, then we need to remove
  *them
  ...
  if (ipevent.GetEvent() == WWOnline::UserIPEvent::GotIP) {
    ...
    if (Get_My_Name(namebuf)) {
      if (stricmp((char *)user.name, namebuf) == 0) {
        ...
        ChatExternalIP = ip;
        if (ForcePort || !FirewallHelper.Get_External_Address().Is_Valid() ||
            FirewallHelper.Get_External_Address().Get_Address() == 0) {
...
void WOLNATInterfaceClass::Send_Private_Game_Options(WOL::User *user, char *options) {
  /*
  ** This can be called from the firewall thread so all it does is add the options to a queue. We can't send them
  *directly
 ...
 *=============================================================================================*/
bool WOLNATInterfaceClass::Send_Game_Format_Packet_To(IPAddressClass *address, char *payload, int payload_size,
                                                      SocketHandlerClass *socket_handler) {
  ThreadLockClass locker(this);
  ...
  */
  if (Am_I_Server()) {
    ...
    ** Send it.
    */
    PacketManager.Take_Packet((unsigned char *)full_packet.Get_Data(), full_packet.Get_Compressed_Size_Bytes(),
                              (unsigned char *)&sock_address.sin_addr.s_addr, sock_address.sin_port,
    ...
    int result = sendto(socket_handler->Get_Socket(), full_packet.Get_Data(), full_packet.Get_Compressed_Size_Bytes(),
                        0, (LPSOCKADDR)&sock_address, sizeof(SOCKADDR_IN));
...
void WOLNATInterfaceClass::Intercept_Game_Packet(cPacket &packet) {
  ...
  */
  ...
  my_address.Set_Address((unsigned char *)&(from_address->FromAddress.sin_addr.s_addr),
                         (unsigned short)ntohs(from_address->FromAddress.sin_port));
...
void WOLNATInterfaceClass::Service_Receive_Queue(SocketHandlerClass *socket) {
  ...
  */
  if (!Am_I_Server()) {
    ...
    unsigned short port;
    int bytes = PacketManager.Get_Packet(socket->Get_Socket(), (unsigned char *)full_packet.Get_Data(),
                                         full_packet.Get_Max_Size(), ip_address, port);
    if (bytes > 0) {
    ...
    */
    if (bytes != 0) {
      ...
      */
      ...
      result = recvfrom(socket->Get_Socket(), full_packet.Get_Data(), full_packet.Get_Max_Size(), 0,
                        (LPSOCKADDR)&full_packet.Get_From_Address_Wrapper()->FromAddress, &addr_len);
...
void WOLNATInterfaceClass::Get_Compact_Log(StringClass &log_string) {
  ...
  */
  sprintf(temp, "%d\t%d\t%d\t%08X\t%08X\t%02X\t%d\t%d\t%08X\t%d\t", PortBase, ForcePort,
          FirewallHelper.Get_Send_Delay(), ntohl(FirewallHelper.Get_Local_Address()),
          (unsigned long)FirewallHelper.Get_External_Address().Get_Address(),
          (int)FirewallHelper.Get_Raw_Firewall_Behavior(), (int)FirewallHelper.Get_Source_Port_Allocation_Delta(),
          FirewallHelper.Get_Source_Port_Pool(), (unsigned long)ServerNegotiatedAddress.Get_Address(),
 ...
 *=============================================================================================*/
bool WOLNATInterfaceClass::Is_NAT_Thread_Busy(void) { return (FirewallHelper.Is_Busy()); }
