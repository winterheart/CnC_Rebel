...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::On_Init_Dialog(void) {
  //
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::On_ListCtrl_Column_Click(ListCtrlClass *list_ctrl, int ctrl_id, int col_index) {
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::On_ListCtrl_Delete_Entry(ListCtrlClass *list_ctrl, int ctrl_id, int item_index) {
  if (ctrl_id == IDC_LOAD_GAME_LIST_CTRL) {
...
//
////////////////////////////////////////////////////////////////
int CALLBACK SaveGameMenuClass::LoadListSortCallback(ListCtrlClass *list_ctrl, int item_index1, int item_index2,
                                                     uint32 user_param) {
  int retval = 0;
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::On_Command(int ctrl_id, int message_id, DWORD param) {
  switch (ctrl_id) {
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::Save_Game(bool prompt) {
  //
  //	Get a pointer to the list control
  //
  ListCtrlClass *list_ctrl = (ListCtrlClass *)Get_Dlg_Item(IDC_LOAD_GAME_LIST_CTRL);
  ...
  if (item_index != -1) {
    ...
    if (list_ctrl->Get_Entry_Data(item_index, 0) == NULL) {
      ...
      //
      if (prompt) {
        DlgMsgBox::DoDialog(IDS_MENU_SAVE_OVERWRITE_TITLE, IDS_MENU_SAVE_OVERWRITE_MSG, DlgMsgBox::YesNo, this,
                            MBEVENT_OVERWRITE_PROMPT);
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::On_ListCtrl_DblClk(ListCtrlClass *list_ctrl, int ctrl_id, int item_index) { return; }
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::Get_Unique_Save_Filename(StringClass &filename) {
  int slot = 1;
  bool done = false;
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::Update_Text_Field(void) {
  //
  //	Get a pointer to the list control
  //
  ListCtrlClass *list_ctrl = (ListCtrlClass *)Get_Dlg_Item(IDC_LOAD_GAME_LIST_CTRL);
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::On_ListCtrl_Sel_Change(ListCtrlClass *list_ctrl, int ctrl_id, int old_index, int new_index) {
  Update_Text_Field();
  Update_Button_State();
  return;
}
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::Delete_Game(bool prompt) {
  //
  //	Get a pointer to the list control
  //
  ListCtrlClass *list_ctrl = (ListCtrlClass *)Get_Dlg_Item(IDC_LOAD_GAME_LIST_CTRL);
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::Reload_List(const char *current_filename) {
  //
  //	Get a pointer to the list control
  //
  ListCtrlClass *list_ctrl = (ListCtrlClass *)Get_Dlg_Item(IDC_LOAD_GAME_LIST_CTRL);
  ...
  //
  int index = 1;
  for (file_find = ::FindFirstFile("data\\save\\*.sav", &find_info); (file_find != INVALID_HANDLE_VALUE) && keep_going;
    ...
    if (item_index >= 0) {
      ...
      //
      if (current_filename != NULL && ::lstrcmpi(current_filename, find_info.cFileName) == 0) {
        list_ctrl->Set_Curr_Sel(item_index);
      }
    }
  }
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::HandleNotification(DlgMsgBoxEvent &event) {
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::Update_Button_State(void) {
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::On_EditCtrl_Change(EditCtrlClass *edit_ctrl, int ctrl_id) {
...
//
////////////////////////////////////////////////////////////////
void SaveGameMenuClass::On_EditCtrl_Enter_Pressed(EditCtrlClass *edit_ctrl, int ctrl_id) {
  if (ctrl_id == IDC_FILENAME_EDIT) {
    Save_Game(true);
  }
...
//
////////////////////////////////////////////////////////////////
bool SaveGameMenuClass::Check_HD_Space(void) {
  bool retval = true;
  ...
  // NOTE IML: For Win'95, must query for support for GetDiskFreeSpaceEx before using it - otherwise use
  // GetDiskFreeSpace().
  GetSystemDirectory(kernelpathname.Get_Buffer(_MAX_PATH), _MAX_PATH);
  kernelpathname += "\\";
  kernelpathname += "Kernel32.dll";
  getfreediskspaceex = (int(_stdcall *)(LPCTSTR, PULARGE_INTEGER, PULARGE_INTEGER, PULARGE_INTEGER))GetProcAddress(
      GetModuleHandle(kernelpathname.Peek_Buffer()), "GetDiskFreeSpaceExA");
