...
**
** I need to make this stuff static so that I can templatize the derived class and have all expansions use the same
*data.
*/
...
**
** For each type, we have to declare more than just the class, we have to declare static data too. This is taken care of
*by
** the DECLARE_DATA_SAFE macro.
**
** All data safe type MUST be declared here so that they are constructed after the ::TypeList and ::TypeListCount
*statics above.
**
...
int GenericDataSafeClass::Get_Random_List_For_Insertion(int type) {
  ...
  */
  for (int i = 0; i < NumLists; i++) {
    ds_assert(Safe[i] != NULL);
    if (Safe[i] != NULL && (Safe[i]->EntryType == type || Safe[i]->EntryType == -1) &&
        Safe[i]->EntryCount < MAX_ENTRIES_PER_LIST) {
  ...
  */
  ...
  if (forced || time < ShuffleDelay || (time | ShuffleDelay) == 0 || (time - ShuffleDelay) > SHUFFLE_TIME) {
    ...
    */
    ...
    /*
    ** Now, mod_key contains the "modifier key" used to rekey items. We will use it after we have swapped some stuff
    *around.
    */
 ...
 *=============================================================================================*/
inline void GenericDataSafeClass::Unlock(void) { ReleaseMutex(GenericDataSafeClass::SafeMutex); }
