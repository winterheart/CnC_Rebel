...
MainMenuDialogClass::MainMenuDialogClass(void)
    : MenuDialogClass(IDD_MENU_MAIN), TitleTransModel(NULL), LogoModel(NULL), GizmoModel(NULL),
...
DialogTransitionClass *MainMenuDialogClass::Get_Transition_In(DialogBaseClass *prev_dlg) {
  ...
  //
  if (prev_dlg == NULL ||
      (prev_dlg != QuitVerificationDialogClass::Get_Instance() && prev_dlg != DlgWOLWait::Get_Instance())) {
...
DialogTransitionClass *MainMenuDialogClass::Get_Transition_Out(DialogBaseClass *next_dlg) {
  ...
  //	We only want to transition between menu dialogs
  //
  if (IsStartingPractice == false && (next_dlg == NULL || next_dlg->As_MenuDialogClass() != NULL)) {
    transition = new MainMenuTransitionClass;
    transition->Set_Model(TitleTransModel);
    transition->Set_Camera(Get_BackDrop()->Peek_Camera());
...
//
////////////////////////////////////////////////////////////////
void MainMenuDialogClass::On_Command(int ctrl_id, int message_id, DWORD param) {
  ...
  case IDC_MENU_MP_LAN_GAME_BUTTON:
    ...
    if (cNicEnum::Get_Num_Nics() > 0) {
      ...
      DlgMsgBox::DoDialog(TRANSLATE(IDS_MP_UNABLE_INITIALIZE_LAN), TRANSLATE(IDS_MP_NO_LAN_IP_ADDRESSES_FOUND));
      allow_default = false;
    }
    break;
