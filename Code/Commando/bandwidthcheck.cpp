...
*/
const char *BandwidthCheckerClass::ErrorList[13] = {"BANDTEST_OK",
...
unsigned long BandwidthCheckerClass::Bandwidths[NUM_BANDS * 2] = {
    12000,  14400,  28000,  28800,  29999,  33600,  53000,  57600,   62000,   67200,   105000,  115200,
    125000, 128000, 250000, 256000, 500000, 512000, 999999, 1024000, 1999999, 2048000, 3999999, 4096000};

/*
** Human readable names for each bandwidth level.
*/
const wchar_t *BandwidthCheckerClass::BandwidthNames[NUM_BANDS + 1] = {
    L"14400", L"28800", L"33600", L"57600", L"67200", L"115200", L"128k",
    L"256k",  L"512k",  L"1M",    L"2M",    L"4M",    L"> 4M"};
 ...
 *=============================================================================================*/
RefPtr<WaitCondition> BandwidthCheckerClass::Detect(void) { return (BandwidthDetectWait::Create()); }
...
void BandwidthCheckerClass::Check(void) {
  ...
  */
  ...
  if (host == NULL) {
    ...
    */
    UpstreamBandwidth = Detect_Bandwidth(ntohl(address.sin_addr.s_addr), 0, 2, failure_code, DownstreamBandwidth,
                                         BANDTEST_API_VERSION, &settings, APPLICATION_SUB_KEY_NAME_BANDTEST);
    ...
    */
    if (UpstreamBandwidth == 0) {
      if (failure_code == BANDTEST_NO_FINAL_PING_TIME ||
        ...
        UpstreamBandwidth = Detect_Bandwidth(ntohl(address.sin_addr.s_addr), 0, 0, failure_code, DownstreamBandwidth,
                                             BANDTEST_API_VERSION, &settings);
      }
    }
 ...
 *   11/21/2001 2:54PM ST : Created                                                            *
 *=============================================================================================*/
unsigned long BandwidthCheckerClass::Get_Upstream_Bandwidth(void) { return (UpstreamBandwidth); }
 ...
 *=============================================================================================*/
unsigned long BandwidthCheckerClass::Get_Reported_Upstream_Bandwidth(void) { return (ReportedUpstreamBandwidth); }
 ...
 *=============================================================================================*/
const wchar_t *BandwidthCheckerClass::Get_Upstream_Bandwidth_As_String(void) { return (UpstreamBandwidthString); }
 ...
 *   11/21/2001 2:56PM ST : Created                                                            *
 *=============================================================================================*/
unsigned long BandwidthCheckerClass::Get_Downstream_Bandwidth(void) { return (DownstreamBandwidth); }
 ...
 *=============================================================================================*/
unsigned long BandwidthCheckerClass::Get_Reported_Downstream_Bandwidth(void) { return (ReportedDownstreamBandwidth); }
 ...
 *   11/21/2001 2:57PM ST : Created                                                            *
 *=============================================================================================*/
const wchar_t *BandwidthCheckerClass::Get_Downstream_Bandwidth_As_String(void) { return (DownstreamBandwidthString); }
...
wchar_t *BandwidthCheckerClass::Get_Bandwidth_As_String(PackedBandwidthType bandwidth) {
  ...
  swprintf(_build_string, array_size, L"%s,%s", BandwidthNames[bandwidth.Bandwidth.Down],
           BandwidthNames[bandwidth.Bandwidth.Up]);
  return (_build_string);
}
 ...
 *=============================================================================================*/
RefPtr<BandwidthDetectWait> BandwidthDetectWait::Create(void) { return (new BandwidthDetectWait()); }
...
BandwidthDetectWait::BandwidthDetectWait(void)
    : SingleWait(TRANSLATE(IDS_MENU_TESTING_BANDWIDTH), 60000), mEvent(NULL), mPingsRemaining(0xffffffff) {
  if (!cGameSpyAdmin::Is_Gamespy_Game()) {
...
void BandwidthDetectWait::WaitBeginning(void) {
  ...
  if (mEvent == NULL) {
    ...
    if (WOLSession.IsValid())
      WOLSession->EnablePinging(false);
    mTimeout = 15000;
  }
}
