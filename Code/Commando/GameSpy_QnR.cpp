...
CGameSpyQnR::~CGameSpyQnR() { Shutdown(); }
...
void CGameSpyQnR::LaunchArcade(void) {
  ...
  if (result == ERROR_SUCCESS) {
    ...
    if (result == ERROR_SUCCESS && type == REG_SZ) {
      ...
      //
      ::RegQueryValueEx((HKEY)key, "InstDir", NULL, &type, (LPBYTE)value.Get_Buffer(data_size), &data_size);
    }
    if (!value.Is_Empty()) {
...
void CGameSpyQnR::Init(void) {
  ...
  if (m_GSEnabled && !m_GSInit && The_Game() && The_Game()->Get_Game_Type() == cGameData::GAME_TYPE_CNC) {
    ...
    test = qr_init(&query_reporting_rec, ip, cUserOptions::GameSpyQueryPort.Get(), gamename, secret_key,
                   c_basic_callback, c_info_callback, c_rules_callback, c_players_callback, this);
    WWASSERT(!test);
...
void CGameSpyQnR::info_callback(char *outbuf, int maxlen) {
  ...
  while (1) {
    ...
    if (!Append_InfoKey_Pair(outbuf, maxlen, "hostport", value))
      break;
    ...
    char *t = strrchr(s, '.');
    if (t)
      value.Erase(t - s, s + strlen(s) - t);
    if (!Append_InfoKey_Pair(outbuf, maxlen, "mapname", value))
      break;
    ...
    if (!value.Is_Empty()) {
      ...
      if (t)
        value.Erase(t - s, s + strlen(s) - t);
      if (!Append_InfoKey_Pair(outbuf, maxlen, "gametype", value))
        break;
    } else {
      if (!Append_InfoKey_Pair(outbuf, maxlen, "gametype", "C&C"))
        break;
    }
    ...
    if (!Append_InfoKey_Pair(outbuf, maxlen, "numplayers", value))
      break;
    value.Format("%d", The_Game()->Get_Max_Players());
    if (!Append_InfoKey_Pair(outbuf, maxlen, "maxplayers", value))
      break;
...
void CGameSpyQnR::rules_callback(char *outbuf, int maxlen) {
  ...
  while (1) {
    ...
    if (!Append_InfoKey_Pair(outbuf, maxlen, "CSVR", ConsoleBox.Is_Exclusive() ? one : zero))
      break;
    if (!Append_InfoKey_Pair(outbuf, maxlen, "DED", The_Game()->IsDedicated.Get() ? one : zero))
      break;
    if (!Append_InfoKey_Pair(outbuf, maxlen, "DG", The_Game()->DriverIsAlwaysGunner.Get() ? one : zero))
      break;
    if (!Append_InfoKey_Pair(outbuf, maxlen, "password", The_Game()->IsPassworded.Get() ? one : zero))
      break;
    if (!Append_InfoKey_Pair(outbuf, maxlen, "TC", The_Game()->IsTeamChangingAllowed.Get() ? one : zero))
      break;
    //		if (!Append_InfoKey_Pair(outbuf, maxlen, "WeaponSpawning", The_Game()->SpawnWeapons.Get() ? one : zero))
    //break; 		if (!Append_InfoKey_Pair(outbuf, maxlen, "TeamRemix", The_Game()->RemixTeams.Get() ? one : zero)) break; 		if
    //(!Append_InfoKey_Pair(outbuf, maxlen, "BuildingRepair", The_Game()->CanRepairBuildings.Get() ? one : zero)) break;
    if (!Append_InfoKey_Pair(outbuf, maxlen, "FF", The_Game()->IsFriendlyFirePermitted.Get() ? one : zero))
      break;
    //		if (!Append_InfoKey_Pair(outbuf, maxlen, "BaseDestructionEndsGame",
    //The_Game()->As_Cnc()->BaseDestructionEndsGame.Get() ? one : zero)) break; 		if (!Append_InfoKey_Pair(outbuf, maxlen,
    //"BeaconPlacementEndsGame", The_Game()->As_Cnc()->BeaconPlacementEndsGame.Get() ? one : zero)) break;
    ...
    if (!Append_InfoKey_Pair(outbuf, maxlen, "SC", value))
      break;
    ...
    //		if (!Append_InfoKey_Pair(outbuf, maxlen, "QuickMatch", The_Game()->Is_QuickMatch_Server() ? one : zero))
    //break;
...
BOOL CGameSpyQnR::Append_InfoKey_Pair(char *outbuf, int maxlen, const char *key, const char *value) {
  ...
  if (clen + strlen(key) + strlen(value) + 3 > (unsigned int)maxlen)
    return FALSE;
...
void CGameSpyQnR::players_callback(char *outbuf, int maxlen) {
  ...
  for (SLNode<cPlayer> *player_node = cPlayerManager::Get_Player_Object_List()->Head(); player_node != NULL;
    ...
    if (!Append_InfoKey_Pair(outbuf, maxlen, keyval.Peek_Buffer(), value))
      break;
