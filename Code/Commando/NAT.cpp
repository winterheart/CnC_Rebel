 ...
 *=============================================================================================*/
void FirewallHelperClass::Set_Firewall_Info(unsigned long last_behavior, int last_delta, unsigned short port_pool,
                                            bool send_delay, int confidence) {
  if (port_pool) {
 ...
 *   8/20/2001 12:16PM ST : Created                                                            *
 *=============================================================================================*/
void FirewallHelperClass::Get_Firewall_Info(unsigned long &last_behavior, int &last_delta, unsigned short &port_pool,
                                            bool &send_delay, int &confidence) const {
 ...
 *=============================================================================================*/
bool FirewallHelperClass::Send_To_Mangler(IPAddressClass *address, SocketHandlerClass *socket_handler,
                                          unsigned long packet_id, bool blitzme) {
  /*
 ...
 *=============================================================================================*/
unsigned short FirewallHelperClass::Get_Mangler_Response(unsigned long packet_id, SocketHandlerClass *socket_handler,
                                                         int time, bool all_service) {
  /*
  ** Locals.
  */
  int peek_packet = 0;
        ...
        */
        ...
        if (packet_id == id) {
          ...
          */
          ...
          WWDEBUG_SAY(
              ("FirewallHelper: Mangler is seeing our packets coming from address %s\n", ExternalAddress.As_String()));
...
FirewallHelperClass::FirewallBehaviorType FirewallHelperClass::Detect_Firewall_Behavior(void) {
  ...
  */
  static char _manglername[5][80] = {"mangler1.westwood.com", "mangler2.westwood.com", "mangler3.westwood.com",
    ...
    */
    if (!found) {
      ...
      WWDEBUG_SAY(("FirewallHelper: Found mangler address at %d.%d.%d.%d\n", mangler_addresses[m][0],
                   mangler_addresses[m][1], mangler_addresses[m][2], mangler_addresses[m][3]));
    }
  ...
  */
  ...
  if (mangled_port == 0) {
    ...
    ** We already tried the first one so skip that. Don't use the last one either since that will be needed later to
    *detect
    ...
    for (i = 0; i < NUM_TEST_PORTS; i++) {
      ...
      if (!port_sockets[i]->Open(source_ports[i], 4321)) {
        ...
        */
        ...
        WWDEBUG_SAY(("FirewallHelper: Unable to open all source ports for allocation pattern test - returning default "
                     "behavior\n"));
    ...
    if (num_responses < 4) {
      if (LastSourcePortAllocationDelta != 0 && (int)LastBehavior > (int)FIREWALL_TYPE_SIMPLE) {
        ...
        */
        SourcePortAllocationDelta = LastSourcePortAllocationDelta;
        WWDEBUG_SAY(
            ("FirewallHelper: Unable to verify responses from 4 mangler servers - returning default behavior\n"));
        return (behavior);
      }
      /*
      ** Try again.
  ...
  */
  if ((behavior & FIREWALL_TYPE_SMART_MANGLING) != 0) {
    ...
    if ((behavior & FIREWALL_TYPE_SIMPLE_PORT_ALLOCATION) != 0) {
      ...
      WWDEBUG_SAY(
          ("FirewallHelper: Testing to see if the NAT mangles differently per destination port at the same IP\n"));
 ...
 *=============================================================================================*/
int FirewallHelperClass::Get_NAT_Port_Allocation_Scheme(int num_ports, unsigned short *original_ports,
                                                        unsigned short *mangled_ports, bool &relative_delta,
                                                        bool &looks_good) {
...
unsigned short FirewallHelperClass::Get_Next_Mangled_Source_Port(unsigned short source_port) {
  ...
  */
  ...
  /*
  ** If our NAT uses the same mangled source port regardless of the dest IP then we can use any previous connection to a
  *different IP.
  */
  ...
  */
  ...
  if (NumManglerServers > 0) {
    ...
    if (got_name) {
      ...
      WWDEBUG_SAY(
          ("FirewallHelper - Using mangler server from servserv - address is %s ; %d\n", mangler_name, mangler_port));
    } else {
      WWDEBUG_SAY(("FirewallHelper - Using default mangler name for mangler %d\n", CurrentManglerServer));
    }
  }
 ...
 *=============================================================================================*/
int FirewallHelperClass::Build_Mangler_Packet(unsigned char *buffer, unsigned short port, unsigned long packet_id,
                                              bool blitzme) {
  /*
  ** Asserts.
  */
...
void FirewallHelperClass::Process_Game_Options(void) {
  ...
  */
  if (WOLNATInterface.Get_Private_Game_Options(&user, options_buffer, OPTIONS_STAGING_BUFFER_SIZE)) {
    ...
    */
    WOLNATInterfaceClass::PrivateGameOptionsStruct *options =
        (WOLNATInterfaceClass::PrivateGameOptionsStruct *)options_buffer;
    ...
    case WOLNATInterfaceClass::OPTION_INVITE_PORT_NEGOTIATION:
      ...
      if (!WOLNATInterface.Am_I_Server()) {
        ...
        */
        ...
        WWDEBUG_SAY(("external addr = %s, firewall = %08x\n", PlayersExternalAddress.As_String(),
                     (unsigned long)PlayersFirewallType));
        ...
        */
        ...
        sprintf(send_options.OptionData.Accept.LocalIP, "%08x,",
                (unsigned long)LocalChatConnectionAddress.Get_Address());
    ...
    case WOLNATInterfaceClass::OPTION_ACCEPT_PORT_NEGOTIATION_INVITATION:
      ...
      if (WOLNATInterface.Am_I_Server()) {
        ...
        */
        ...
        WWDEBUG_SAY(("FirewallHelper - Got client accept from %s. Local addr = %s,", client->Name,
                     client->LocalAddress.As_String()));
        WWDEBUG_SAY(
            (" external addr = %s, firewall = %08x\n", client->ExternalAddress.As_String(), (unsigned long)firewall));
    ...
    */
    case WOLNATInterfaceClass::OPTION_CONNECTION_RESULT: {
      ...
      WWDEBUG_SAY(("FirewallHelper - Got OPTION_CONNECTION_RESULT %d from %s\n", result,
                   options->OptionData.ConnectionResult.Name));
 ...
 *=============================================================================================*/
void FirewallHelperClass::Set_Client_Connect_Event(HANDLE thread_event, HANDLE cancel_event, int *flag_ptr,
                                                   int *queue_ptr) {
  ThreadLockClass locker(this);
...
int FirewallHelperClass::Negotiate_Port(void) {
  ...
  */
  ...
  WWDEBUG_SAY(("FirewallHelper - Negotiating port with player %s, local address = %s,", PlayersName,
               PlayersLocalAddress.As_String()));
  ...
  */
  ...
  do {
      ...
      */
      if (server && trying < 2) {
        if ((Behavior & FirewallHelperClass::FIREWALL_TYPE_DESTINATION_PORT_DELTA) == 0) {
          WWDEBUG_SAY(
              ("FirewallHelper: No destination port delta - looking for earlier connections at the same address\n"));
          ...
          for (int r = 0; r < ConnectionHistory.Count(); r++) {
            if (ConnectionHistory[r].Is_IP_Equal(PlayersExternalAddress)) {
              ...
              used_old_port = true;
              WWDEBUG_SAY(
                  ("FirewallHelper - Found old connection port number to use - port number %d\n", mangled_port));
              break;
            }
          }
        }
      ...
      /*
      ** If this isn't the first try then we might already know what the port will be since it can't change between
      *tries.
      */
      if (mangled_port == 0) {
        if ((Behavior & FirewallHelperClass::FIREWALL_TYPE_DESTINATION_PORT_DELTA) == 0) {
    ...
    */
    ...
    while (TIMEGETTIME() < timeout && ((mangling == true && PlayersMangledPort == last_send_port) ||
                                       (mangling == false && PlayersMangledPort == 0))) {
      Process_Game_Options();
      Sleep(5);
  ...
  if (gotit && PlayersConnectionResult == CONNRESULT_CONNECTED) {
    WWDEBUG_SAY(
        ("FirewallHelper: Port negotiation successful! Correct address is %s\n", PlayersFirewallAddress.As_String()));
...
*/
BOOL(__stdcall *SnmpExtensionInitPtr)(IN DWORD dwUpTimeReference, OUT HANDLE *phSubagentTrapEvent,
                                      OUT AsnObjectIdentifier *pFirstSupportedRegion);
BOOL(__stdcall *SnmpExtensionQueryPtr)(IN BYTE bPduType, IN OUT RFC1157VarBindList *pVarBindList,
                                       OUT AsnInteger32 *pErrorStatus, OUT AsnInteger32 *pErrorIndex);
...
bool FirewallHelperClass::Get_Local_Chat_Connection_Address(void) {
  ...
  */
  ...
  enum { tcpConnState = 1, tcpConnLocalAddress, tcpConnLocalPort, tcpConnRemAddress, tcpConnRemPort };
  ...
  ** Statics.
  */
  static char _conn_state[][32] = {"?",           "CLOSED",    "LISTENING", "SYN_SENT",   "SEN_RECEIVED",
  ...
  */
  SnmpExtensionInitPtr =
      (int(__stdcall *)(unsigned long, void **, AsnObjectIdentifier *))GetProcAddress(mib_ii_dll, "SnmpExtensionInit");
  SnmpExtensionQueryPtr = (int(__stdcall *)(unsigned char, SnmpVarBindList *, long *, long *))GetProcAddress(
      mib_ii_dll, "SnmpExtensionQuery");
  ...
  */
  ...
  SnmpUtilMemFreePtr(bind_list_ptr);
  SnmpUtilMemFreePtr(bind_ptr);
  // SnmpUtilMemFreePtr(mib_ii_name_ptr);		// Don't free this - the SnmpExtensionQueryPtr call frees it
  // apparently
...
unsigned short FirewallHelperClass::Get_Raw_Firewall_Behavior(void) {
  ...
  /*
  ** If we are forcing a particular port to be used then set the behavior type to be as if there was no firewall there
  *at all.
  */
