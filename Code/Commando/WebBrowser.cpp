...
bool WebBrowser::InstallPrerequisites(void) {
  ...
  HRESULT hr = CoGetClassObject(CLSID_WOLBrowser, CLSCTX_INPROC_SERVER, NULL, IID_IClassFactory, (void **)&factory);
  ...
  if (FAILED(hr)) {
    ...
    if (!success) {
      ...
      ::MessageBox(NULL, "WOLBrowser.dll not registered!\n\nDefaulting to external browser.", "Renegade Warning!",
  ...
  if (ERROR_SUCCESS != result) {
    ...
    ::MessageBox(NULL, "Embedded Browser prerequisite error!\n\nURL key not found.", "Renegade Warning!",
                 MB_ICONWARNING | MB_OK);

    // Attempt to create the key.
    LONG result = RegCreateKeyEx(HKEY_LOCAL_MACHINE, APPLICATION_SUB_KEY_NAME_URL, 0, NULL, REG_OPTION_NON_VOLATILE,
    ...
    static URLEntry urls[] = {
        {"BattleClans",
         "http://renchat2.westwood.com/cgi-bin/"
         "cgiclient?ren_clan_manager&request=expand_template&Template=index.html&SKU=3072&LANGCODE=0&embedded=1"},
        {"BattleClansX", "http://renchat2.westwood.com/cgi-bin/"
                         "cgiclient?ren_clan_manager&request=expand_template&Template=index.html&SKU=3072&LANGCODE=0"},
        {"Ladder", "http://renchat2.westwood.com/renegade_embedded/index.html"},
        {"LadderX", "http://renchat2.westwood.com/renegade/index.html"},
        {"NetStatus", "http://battleclans.westwood.com/cgi-bin/"
                      "cgiclient?rosetta&request=do_netstatus&LANGCODE=0&SKU=3072&embedded=1"},
        {"NetStatusX",
         "http://battleclans.westwood.com/cgi-bin/cgiclient?rosetta&request=do_netstatus&LANGCODE=0&SKU=3072"},
        {"News",
         "http://battleclans.westwood.com/cgi-bin/cgiclient?rosetta&request=do_news&LANGCODE=0&SKU=3072&embedded=1"},
        ...
        {"Signup",
         ...
         "cgiclient?ren_reg2&request=expand_template&Template=newreg_menu.html&LANGCODE=0&embedded=1&SKU=3072"},
        {"SignupX", "http://games2.westwood.com/cgi-bin/"
    ...
    while (valueName) {
      ...
      if (ERROR_SUCCESS != result || (strcmp(valueData, data) != 0)) {
        result = RegSetValueEx(key, valueName, NULL, REG_SZ, (CONST BYTE *)valueData, (strlen(valueData) + 1));
...
WebBrowser::WebBrowser() : mRefCount(1), mVisible(false), mSwitchedMode(false) {
...
bool WebBrowser::FinalizeCreate(HWND window) {
  if (window) {
    ...
    if ((windowWidth >= 800) && (windowHeight >= 600)) {
      ...
      HRESULT hr =
          CoCreateInstance(CLSID_WOLBrowser, NULL, CLSCTX_INPROC_SERVER, IID_IWOLBrowser, (void **)&mWOLBrowser);
...
STDMETHODIMP WebBrowser::OnBeforeNavigate(const wchar_t * /* url */, const wchar_t * /* targetFrame */) { return S_OK; }
...
STDMETHODIMP WebBrowser::OnDownloadBegin(void) { return S_OK; }
...
STDMETHODIMP WebBrowser::OnProgressChange(long /* progress */, long /* progressMax */) { return S_OK; }
...
STDMETHODIMP WebBrowser::OnDownloadComplete(void) { return S_OK; }
...
STDMETHODIMP WebBrowser::OnNavigateComplete(const wchar_t * /* url */) { return S_OK; }
...
STDMETHODIMP WebBrowser::OnStatusTextChange(const wchar_t * /* statusText */) { return S_OK; }
...
STDMETHODIMP WebBrowser::OnTitleChange(const wchar_t * /* title */) { return S_OK; }
...
STDMETHODIMP WebBrowser::OnNewWindow(void) { return E_FAIL; }
...
STDMETHODIMP WebBrowser::OnErrorMsg(const wchar_t * /* error */) { return S_OK; }
...
bool WebBrowser::LaunchExternal(const char *url) {
  ...
  BOOL createSuccess =
      CreateProcess(exeName, commandLine, NULL, NULL, FALSE, 0, NULL, NULL, &startupInfo, &mProcessInfo);
