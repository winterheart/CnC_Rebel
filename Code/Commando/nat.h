...
public:
  ...
  */
  int Build_Mangler_Packet(unsigned char *buffer, unsigned short port, unsigned long packet_id = 0,
                           bool blitzme = false);
  ...
  */
  void Set_Firewall_Info(unsigned long last_behavior, int last_delta, unsigned short port_pool, bool send_delay,
                         int confidence);
  void Get_Firewall_Info(unsigned long &last_behavior, int &last_delta, unsigned short &port_pool, bool &send_delay,
  ...
  */
  bool Send_To_Mangler(IPAddressClass *address, SocketHandlerClass *socket_handler, unsigned long packet_id,
                       bool blitzme = false);
  unsigned short Get_Mangler_Response(unsigned long packet_id, SocketHandlerClass *socket_handler, int time = 0,
                                      bool all_service = false);
  ...
  */
  enum { FW_RESULT_UNKNOWN, FW_RESULT_FAILED, FW_RESULT_SUCCEEDED, FW_RESULT_CANCELLED };
...
private:
  ...
  */
  ...
  int Get_NAT_Port_Allocation_Scheme(int num_ports, unsigned short *original_ports, unsigned short *mangled_ports,
                                     bool &relative_delta, bool &looks_good);
  ...
  ** Threading.
  **
  **		This class runs in it's own thread so that firewall negotiation doesn't stall the server when a new
  *client joins. *		It also means that the clients join dialog is serviced while connecting, allowing him to
  *cancel at any time.
  **
  */
  HANDLE ThreadHandle;
  ...
  */
  ...
  public:
    inline bool operator==(ThreadActionClass const &data) { return (memcmp((void *)this, &data, sizeof(*this)) == 0); };
    inline bool operator!=(ThreadActionClass const &data) { return (memcmp((void *)this, &data, sizeof(*this)) != 0); };
  ...
  */
  ...
  public:
    ...
    */
    inline ~ThreadLockClass(void) { ReleaseMutex(FWPtr->NATDataMutex); };
  };
...
#define SERIAL_MAX 23
  ...
  */
  struct GlobalHeaderType {
    ...
    unsigned short
        ProductID; // in, out = COMMAND_AND_CONQUER_RA2 = 0xaa03. Just return the value from the received packet
  };
