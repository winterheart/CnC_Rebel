...
void LoginProfile::EnableSaving(bool allowed) { _mSaveAllowed = allowed; }
...
LoginProfile::LoginProfile() : mLocale(WOL::LOC_UNKNOWN), mSidePref(-1), mGamesPlayed(0) {
...
LoginProfile::~LoginProfile() { WWDEBUG_SAY(("LoginProfile destroyed (%S)\n", (const WCHAR *)mName)); }
...
const wchar_t *LoginProfile::GetName(void) const { return mName; }
...
void LoginProfile::SetPreferredServer(const char *name) { mServer = name; }
...
void LoginProfile::SetSidePreference(int side) { mSidePref = side; }
...
void LoginProfile::SetGamesPlayed(unsigned long played) { mGamesPlayed = played; }
...
void LoginProfile::SetLocale(WOL::Locale locale) { mLocale = max<WOL::Locale>(locale, WOL::LOC_UNKNOWN); }
...
void LoginProfile::LoadRank(const char *valueName, LoginProfile::Ranking &rank) {
  ...
  if (registry.Is_Valid()) {
    ...
    sscanf(rankData, "%d,%d,%d,%d,%d,%d", &rank.Wins, &rank.Losses, &rank.Deaths, &rank.Kills, &rank.Points,
           &rank.Rank);
  } else {
...
void LoginProfile::SaveRank(const char *valueName, const LoginProfile::Ranking &rank) {
  ...
  if (registry.Is_Valid()) {
    char rankData[255];
    sprintf(rankData, "%d,%d,%d,%d,%d,%d", rank.Wins, rank.Losses, rank.Deaths, rank.Kills, rank.Points, rank.Rank);
...
void ShowProfileRanking(DialogBaseClass *dialog, const LoginProfile *profile) {
  ...
  if (list) {
    ...
    struct {
      const wchar_t *name;
      WWOnline::LadderType type;
    } _ladders[] = {{TRANSLATE(IDS_MENU_INDIVIDUAL), WWOnline::LadderType_Team},
    ...
    if (profile) {
      WideStringClass profileName(128u, true);
