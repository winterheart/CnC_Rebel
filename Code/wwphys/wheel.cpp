...
SuspensionElementClass::SuspensionElementClass(void)
    : Parent(NULL), Flags(DEFAULT_FLAGS), PositionBone(-1), AxisBone(-1), ForkBone(-1), ObjWheelTM(1),
      SpringConstant(DEFAULT_SPRING_CONSTANT), DampingCoefficient(DEFAULT_DAMPING_COEFFICIENT),
      SpringLength(DEFAULT_SPRING_LENGTH), SteeringAngle(0.0f), SuspensionForce(0, 0, 0), WheelTM(1), WheelP0(0, 0, 0),
      Contact(0, 0, 0), Normal(0, 0, 0), ContactSurface(0), LastPoint(0, 0, 0), TranslationScale(1.0f),
      ObjAxis(0, 0, -1), ObjForkTM(1), ForkLength(0.0f), ForkZ(0.0f), ForkSin0(0.0f), ForkCos0(0.0f), ForkA(0.0f),
      ForkB(0.0f), SpringEndP0(0.0f, 0.0f, 0.0f), SpringEndP1(0.0f, 0.0f, 0.0f) {}
...
void SuspensionElementClass::Intersect_Spring(void) {
  ...
  if (p1 == SpringEndP1 && WheelP0 == SpringEndP0)
    return;
  SpringEndP1 = p1;
  SpringEndP0 = WheelP0;
  ...
  PhysRayCollisionTestClass raytest(line, &result, Parent->Get_Collision_Group(),
...
WheelClass::WheelClass(void)
    : Radius(1.0f), RotationBone(-1), Rotation(0.0f), RotationDelta(0.0f), AxleTorque(0.0f),
      TractiveFrictionForce(0, 0, 0), LateralFrictionForce(0, 0, 0), SlipFactor(1.0f), IdealAngularVelocity(0.0f) {}
 ...
 *   12/18/2000 gth : Created.                                                                 *
 *=============================================================================================*/
void WheelClass::Init(VehiclePhysClass *obj, int position_bone, int rotation_bone, int fork_bone, int axis_bone) {
...
*************************************************************************************/
void WVWheelClass::Compute_Traction_Forces(const Vector3 &local_pdot, float wheel_normal_force,
                                           float *set_lateral_force, float *set_tractive_force) {
  ...
  if (wheel_normal_force > 0.0f) {
    ...
    float drag_coefficient =
        PhysicsConstants::Get_Contact_Drag_Coefficient(PhysicsConstants::DYNAMIC_OBJ_TYPE_TIRE, ContactSurface);

    float friction_coefficient =
        PhysicsConstants::Get_Contact_Friction_Coefficient(PhysicsConstants::DYNAMIC_OBJ_TYPE_TIRE, ContactSurface);
    ...
    float tractive_force = WWMath::Min((penalty_k * wheel_normal_force * WWMath::Fabs(local_pdot.X)),
                                       (wheel_normal_force * tractive_friction_coefficient));

    float lateral_force = WWMath::Min((penalty_k * wheel_normal_force * WWMath::Fabs(local_pdot.Y)),
...
void WVWheelClass::Roll_Wheel(void) {
  ...
  model->Control_Bone(RotationBone, rotation_tm);
  if (Rotation > 2.0f * (float)WWMATH_PI)
    Rotation -= 2.0f * (float)WWMATH_PI;
...
void TrackWheelClass::Compute_Traction_Forces(const Vector3 &local_pdot, float wheel_normal_force,
                                              float *set_lateral_force, float *set_tractive_force) {
...
void VTOLWheelClass::Compute_Traction_Forces(const Vector3 &local_pdot, float normal_force, float *set_lateral_force,
