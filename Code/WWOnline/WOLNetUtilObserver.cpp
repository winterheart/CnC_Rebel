...
NetUtilObserver::NetUtilObserver() : mRefCount(1), mOuter(NULL) {
...
NetUtilObserver::~NetUtilObserver() { WWDEBUG_SAY(("WOL: NetUtilObserver Destroyed\n")); }
...
void NetUtilObserver::Init(Session &outer) { mOuter = &outer; }
...
STDMETHODIMP NetUtilObserver::OnLadderList(HRESULT result, WOL::Ladder *list, int rungCount, long timeStamp,
                                           int keyRung) {
  if (mOuter == NULL) {
    WWDEBUG_SAY(("WOLERROR: Session not initialized\n"));
    WWASSERT(mOuter && "Session not initialized");
...
STDMETHODIMP NetUtilObserver::OnHighscore(HRESULT result, WOL::Highscore *list, int count, long time, int keyRung) {
