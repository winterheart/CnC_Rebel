...
Session::Session()
    : mChatCookie(0), mNetUtilCookie(0), mCurrentConnectionStatus(ConnectionDisconnected), mAutoRequestFlags(0),
      mLadderPending(0), mPingsPending(0), mPingEnable(1), mLastUserDataRequestTime(0), mRequestedChannelType(-1),
      mRequestingServerList(false), mIgnoreServerLists(false), mIsInsider(false), mServerTime(0) {
...
bool Session::FinalizeCreate(void) {
  ...
  HRESULT hr = CoCreateInstance(WOL::CLSID_Chat, NULL, CLSCTX_INPROC_SERVER, WOL::IID_IChat, (void **)&chatObject);
  ...
  hr = CoCreateInstance(WOL::CLSID_NetUtil, NULL, CLSCTX_INPROC_SERVER, WOL::IID_INetUtil, (void **)&utilObject);
...
RefPtr<WaitCondition> Session::GetNewServerList(void) { return ServerListWait::Create(this); }
...
RefPtr<WaitCondition> Session::LoginServer(const RefPtr<IRCServerData> &server, const RefPtr<LoginInfo> &login) {
  if (server.IsValid()) {
  ...
  if (mCurrentServer == server) {
    if (server.IsValid()) {
      ...
      if (ConnectionConnected == mCurrentConnectionStatus) {
        ...
        return EventValueWait<ConnectionStatus>::CreateAndObserve(*this, ConnectionConnected,
                                                                  WOLSTRING("WOL_CONNECTING"));
      }
    } else {
      ...
      if (ConnectionDisconnected == mCurrentConnectionStatus) {
        ...
        return EventValueWait<ConnectionStatus>::CreateAndObserve(*this, ConnectionDisconnected,
                                                                  WOLSTRING("WOL_DISCONNECTING"));
      }
    }
  }
  ...
  if (ConnectionDisconnecting == mCurrentConnectionStatus) {
    ...
    RefPtr<EventValueWait<ConnectionStatus>> finish = EventValueWait<ConnectionStatus>::CreateAndObserve(
        *this, ConnectionDisconnected, WOLSTRING("WOL_DISCONNECTING"));
    serverWait->Add((const RefPtr<WaitCondition>)finish);
  } else if (ConnectionConnecting == mCurrentConnectionStatus) {
    WWDEBUG_SAY(("WOL: LoginServer wait for connect to finish.\n"));
    RefPtr<EventValueWait<ConnectionStatus>> finish =
        EventValueWait<ConnectionStatus>::CreateAndObserve(*this, ConnectionConnected, WOLSTRING("WOL_DISCONNECTING"));
    serverWait->Add((const RefPtr<WaitCondition>)finish);
  }
...
RefPtr<WaitCondition> Session::JoinChannel(const RefPtr<ChannelData> &channel, const wchar_t *password) {
...
RefPtr<WaitCondition> Session::GetNewChatChannelList(void) { return ChannelListWait::Create(this, 0); }
...
RefPtr<WaitCondition> Session::CreateChannel(const RefPtr<ChannelData> &channel, const wchar_t *password) {
...
bool Session::IsCurrentUser(const RefPtr<UserData> &user) const {
  return user.IsValid() && mCurrentUser.IsValid() && user->GetName() == mCurrentUser->GetName();
}
...
void Session::AutoRequestUserDetails(const RefPtr<UserData> &user) { RequestUserDetails(user, mAutoRequestFlags); }
...
void Session::EnablePinging(bool enable) { mPingEnable += ((enable == true) ? 1 : -1); }
...
const CComPtr<WOL::IIGROptions> &Session::GetIGRObject(void) {
  if (mIGRObject == NULL) {
    ...
    HRESULT hr =
        CoCreateInstance(WOL::CLSID_IGROptions, NULL, CLSCTX_INPROC_SERVER, WOL::IID_IIGROptions, (void **)&igrObject);
...
RefPtr<ChatAdvisement> ChatAdvisement::Create(const CComPtr<WOL::IChat> &chat, const CComPtr<WOL::IChatEvent> &sink) {
