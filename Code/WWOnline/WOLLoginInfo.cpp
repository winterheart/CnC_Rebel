...
void LoginInfo::ClearList(void) { _mLoginList.clear(); }
...
LoginInfo::LoginInfo(const wchar_t *nickname, const wchar_t *password, bool isEncrypted)
    : mNickname(nickname), mPassword(password), mLocale(WOL::LOC_UNKNOWN), mIsPasswordEncrypted(isEncrypted),
      mIsStored(false) {
...
LoginInfo::~LoginInfo() { WWDEBUG_SAY(("WOL: Destructing LoginInfo %S\n", (const WCHAR *)mNickname)); }
...
int LoginInfo::IndexOf(void) const { return IndexOf(mNickname); }
...
void LoginInfo::StoreLogin(const char *nickname, const char *password, bool isPasswordEncrypted, WOL::Locale locale) {
  RefPtr<Session> session = Session::GetInstance(false);
