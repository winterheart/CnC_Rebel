...
ChatObserver::ChatObserver() : mRefCount(1), mOuter(NULL) { WWDEBUG_SAY(("WOL: ChatObserver Instantiated\n")); }
...
ChatObserver::~ChatObserver() { WWDEBUG_SAY(("WOL: ChatObserver Destroyed\n")); }
...
void ChatObserver::Init(Session &outer) { mOuter = &outer; }
...
STDMETHODIMP ChatObserver::OnServerList(HRESULT result, WOL::Server *servers) {
  ...
  while (curServer) {
    ...
    if ((strcmp((char *)curServer->connlabel, "IRC") == 0) || (strcmp((char *)curServer->connlabel, "IGS") == 0)) {
...
STDMETHODIMP ChatObserver::OnChannelJoin(HRESULT result, WOL::Channel *inChannel, WOL::User *inUser) {
  if (mOuter == NULL) {
    WWDEBUG_SAY(("WOLERROR: Session not initialized\n"));
    WWASSERT(mOuter && "Session not initialized");
    return S_OK;
...
STDMETHODIMP ChatObserver::OnChannelTopic(HRESULT result, WOL::Channel *inChannel, LPCSTR topic) {
  ...
  WWDEBUG_SAY(("WOL: Channel '%s' Topic changed.\nExInfo: %s\nTopic: %s\n", (const char *)inChannel->name,
               (const char *)inChannel->exInfo, topic));
...
STDMETHODIMP ChatObserver::OnPublicGameOptions(HRESULT result, WOL::Channel *inChannel, WOL::User *inUser,
                                               LPCSTR options) {
  if (mOuter == NULL) {
    WWDEBUG_SAY(("WOLERROR: Session not initialized\n"));
    WWASSERT(mOuter && "Session not initialized");
...
STDMETHODIMP ChatObserver::OnGameStart(HRESULT result, WOL::Channel *inChannel, WOL::User *inUsers, int gameID) {
  if (mOuter == NULL) {
    WWDEBUG_SAY(("WOLERROR: Session not initialized\n"));
    WWASSERT(mOuter && "Session not initialized");
    return S_OK;
...
STDMETHODIMP ChatObserver::OnUserKick(HRESULT result, WOL::Channel *inChannel, WOL::User *inUser, WOL::User *kicker) {
  if (mOuter == NULL) {
    WWDEBUG_SAY(("WOLERROR: Session not initialized\n"));
    WWASSERT(mOuter && "Session not initialized");
    return S_OK;
    ...
    if (squad.IsValid()) {
      ...
      if (iswdigit(firstChar)) {
        ...
        if (squad->GetID() == pendingID) {
          ...
          WWDEBUG_SAY(
              ("WOLWARNING: OnSquadInfo PendingID '%u' doesn't match CurrentID '%u'\n", pendingID, squad->GetID()));
        }
      } else {
...
STDMETHODIMP ChatObserver::OnPublicUnicodeMessage(HRESULT result, WOL::Channel *, WOL::User *user,
...
STDMETHODIMP ChatObserver::OnPrivateUnicodeMessage(HRESULT result, WOL::User *user, const wchar_t *message) {
  if (mOuter == NULL) {
    WWDEBUG_SAY(("WOLERROR: Session not initialized\n"));
    WWASSERT(mOuter && "Session not initialized");
    return S_OK;
...
STDMETHODIMP ChatObserver::OnPrivateUnicodeAction(HRESULT result, WOL::User *user, const wchar_t *message) {
  if (mOuter == NULL) {
    WWDEBUG_SAY(("WOLERROR: Session not initialized\n"));
    WWASSERT(mOuter && "Session not initialized");
    return S_OK;
