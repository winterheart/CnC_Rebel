...
RefPtr<UserData> UserData::Create(const WOL::User &user) { return new UserData(user); }
...
UserData::UserData(const WOL::User &user) : mUserName((char *)user.name), mLocation(USERLOCATION_UNKNOWN) {
...
UserData::~UserData() { WWDEBUG_SAY(("WOL: Destructing UserData '%S'\n", (const WCHAR *)mUserName)); }
...
void UserData::Squelch(bool onoff) {
  mData.flags = (onoff == true) ? (mData.flags | CHAT_USER_SQUELCHED) : (mData.flags ^ CHAT_USER_SQUELCHED);
}
...
void UserData::SetLocation(UserLocation location) { mLocation = location; }
...
void UserData::SetChannel(const RefPtr<ChannelData> &channel) { mChannel = channel; }
...
void UserData::SetTeam(int team) { mData.team = team; }
...
void UserData::SetLadder(const RefPtr<LadderData> &ladder) { mUserLadder = ladder; }
...
void UserData::SetTeamLadder(const RefPtr<LadderData> &ladder) { mTeamLadder = ladder; }
...
NativeWOLUserList::NativeWOLUserList(const UserList &users) : mNativeList(NULL) {
