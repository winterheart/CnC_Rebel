...
void BINKMovie::Init() { BinkSoundUseDirectSound(0); }
...
BINKMovieClass::BINKMovieClass(const char *filename, const char *subtitlename, FontCharsClass *font)
    : Filename(filename), Bink(0), FrameChanged(true), TicksPerFrame(0), SubTitleManager(NULL) {
  ...
  for (y = 0; y < Bink->Height;
       y += max_height - 2) { // Two pixels are lost due to duplicated edges to prevent bilinear artifacts
  ...
  for (y = 0; y < Bink->Height; y += max_height - 1) {
    for (x = 0; x < Bink->Width; x += max_width - 1) {
      TextureInfos[cnt].Texture = new TextureClass(max_width, max_height, D3DFormat_To_WW3DFormat(D3DFMT_R5G6B5),
      ...
      TextureInfos[cnt].Rect.Right =
          float(TextureInfos[cnt].TextureLocX + TextureInfos[cnt].TextureWidth) / float(Bink->Width);
      TextureInfos[cnt].Rect.Bottom =
          float(TextureInfos[cnt].TextureLocY + TextureInfos[cnt].TextureHeight) / float(Bink->Height);
...
void BINKMovieClass::Render() {
  ...
  if (FrameChanged) {
    ...
    for (unsigned t = 0; t < TextureCount; ++t) {
      DX_IDirect3DTexture *d3d_texture = TextureInfos[t].Texture->Peek_DX8_Texture();
  ...
  for (unsigned t = 0; t < TextureCount; ++t) {
    ...
    RectClass rect(TextureInfos[t].TextureLocX, TextureInfos[t].TextureLocY, TextureInfos[t].TextureWidth,
                   TextureInfos[t].TextureHeight);
